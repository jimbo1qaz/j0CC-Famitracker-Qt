...
void qtMfcInitMenuResource_IDR_MAINFRAME(CMenu *parent) {
  ...
  QObject::connect(subMenu->toQMenu(), SIGNAL(aboutToShow()), subMenu,
                   SLOT(menu_aboutToShow()));
  ...
  QObject::connect(subMenu->toQMenu(), SIGNAL(aboutToShow()), subMenu,
                   SLOT(menu_aboutToShow()));
  subMenuTree[0]->AppendMenu(MF_POPUP | MF_STRING, (UINT_PTR)subMenu->m_hMenu,
                             "Transpose");
  //        BEGIN
  //            MENUITEM "Decrease Note\tCtrl+F1", ID_TRANSPOSE_DECREASENOTE
  subMenu->AppendMenu(MF_STRING, ID_TRANSPOSE_DECREASENOTE,
                      "Decrease Note\tCtrl+F1");
  //            MENUITEM "Increase Note\tCtrl+F2", ID_TRANSPOSE_INCREASENOTE
  subMenu->AppendMenu(MF_STRING, ID_TRANSPOSE_INCREASENOTE,
                      "Increase Note\tCtrl+F2");
  //            MENUITEM "Decrease Octave\tCtrl+F3", ID_TRANSPOSE_DECREASEOCTAVE
  subMenu->AppendMenu(MF_STRING, ID_TRANSPOSE_DECREASEOCTAVE,
                      "Decrease Octave\tCtrl+F3");
  //            MENUITEM "Increase Octave\tCtrl+F4", ID_TRANSPOSE_INCREASEOCTAVE
  subMenu->AppendMenu(MF_STRING, ID_TRANSPOSE_INCREASEOCTAVE,
                      "Increase Octave\tCtrl+F4");
  //        END
  subMenu = subMenuTree[0];
  ...
  subMenu->AppendMenu(MF_STRING, ID_EDIT_REPLACEINSTRUMENT,
                      "R&eplace instrument\tAlt+S");
  //        MENUITEM SEPARATOR
  subMenu->AppendMenu(MF_SEPARATOR);
  ...
  subMenuTree[0]->AppendMenu(MF_POPUP | MF_STRING, (UINT_PTR)subMenu->m_hMenu,
                             "Clean up");
  //        BEGIN
  //            MENUITEM "Remove unused instruments",
  //            ID_CLEANUP_REMOVEUNUSEDINSTRUMENTS
  subMenu->AppendMenu(MF_STRING, ID_CLEANUP_REMOVEUNUSEDINSTRUMENTS,
                      "Remove unused instruments");
  //            MENUITEM "Remove unused patterns",
  //            ID_CLEANUP_REMOVEUNUSEDPATTERNS
  subMenu->AppendMenu(MF_STRING, ID_CLEANUP_REMOVEUNUSEDPATTERNS,
                      "Remove unused patterns");
  //   MENUITEM "Merge duplicated patterns", ID_CLEANUP_MERGEDUPLICATEDPATTERNS
  subMenu->AppendMenu(MF_STRING, ID_CLEANUP_MERGEDUPLICATEDPATTERNS,
  ...
  subMenu->AppendMenu(MF_STRING, ID_EDIT_INSTRUMENTMASK,
                      "Instrument Mask\tAlt+T");
  ...
  QObject::connect(subMenu->toQMenu(), SIGNAL(aboutToShow()), subMenu,
                   SLOT(menu_aboutToShow()));
  parent->AppendMenu(MF_POPUP | MF_STRING, (UINT_PTR)subMenu->m_hMenu,
                     "&Module");
  //    BEGIN
  ...
  subMenu->AppendMenu(MF_STRING, ID_MODULE_DUPLICATEFRAMEPATTERNS,
                      "&Duplicate patterns");
  //        MENUITEM SEPARATOR
  subMenu->AppendMenu(MF_SEPARATOR);
  ...
  subMenu->AppendMenu(MF_STRING, ID_MODULE_MODULEPROPERTIES,
  ...
  subMenu->AppendMenu(MF_STRING, ID_INSTRUMENT_DEEPCLONE,
  ...
  subMenu->AppendMenu(MF_STRING, ID_TRACKER_PLAYPATTERN,
                      "P&lay Pattern\tShift+Return");
  ...
  subMenu->AppendMenu(MF_STRING, ID_TRACKER_TOGGLECHANNEL,
                      "Toggle channel\tAlt+F9");
  //        MENUITEM "Solo channel\tAlt+F10",       ID_TRACKER_SOLOCHANNEL
  subMenu->AppendMenu(MF_STRING, ID_TRACKER_SOLOCHANNEL,
                      "Solo channel\tAlt+F10");
  //        MENUITEM SEPARATOR
  subMenu->AppendMenu(MF_SEPARATOR);
  //        MENUITEM "Switch to track instrument",
  //        ID_TRACKER_SWITCHTOTRACKINSTRUMENT
  subMenu->AppendMenu(MF_STRING, ID_TRACKER_SWITCHTOTRACKINSTRUMENT,
                      "Switch to track instrument");
  //   MENUITEM SEPARATOR
  subMenu->AppendMenu(MF_SEPARATOR);
  //   MENUITEM "Display register state",      ID_TRACKER_DISPLAYREGISTERSTATE
  subMenu->AppendMenu(MF_STRING, ID_TRACKER_DISPLAYREGISTERSTATE,
...
void qtMfcInitMenuResource_IDR_FRAME_POPUP(CMenu *parent) {
  ...
  subMenu->AppendMenu(MF_STRING, ID_FRAME_PASTENEWPATTERNS,
                      "Paste && duplicate");
...
void qtMfcInitMenuResource_IDR_SAMPLE_WND_POPUP(CMenu *parent) {
  ...
  subMenu->AppendMenu(MF_STRING, ID_POPUP_SPECTRUMANALYZER,
                      "Spectrum analyzer");
...
void qtMfcInitMenuResource_IDR_PATTERN_HEADER_POPUP(CMenu *parent) {
  ...
  subMenu->AppendMenu(MF_STRING, ID_POPUP_TOGGLECHANNEL,
                      "&Toggle channel\tAlt+F9");
  //           MENUITEM "&Solo channel\tAlt+F10",      ID_POPUP_SOLOCHANNEL
  subMenu->AppendMenu(MF_STRING, ID_POPUP_SOLOCHANNEL,
                      "&Solo channel\tAlt+F10");
  //           MENUITEM "&Unmute all channels", ID_POPUP_UNMUTEALLCHANNELS
  subMenu->AppendMenu(MF_STRING, ID_POPUP_UNMUTEALLCHANNELS,
                      "&Unmute all channels");
  //       END
  //   END
}
...
void qtMfcInitStringResources() {
  ...
  qtMfcStringResources.insert(
      IDR_MAINFRAME, "FamiTracker\n\nFamiTracker\nFamiTracker files "
                     "(*.ftm)\n.ftm\nFamiTracker.Module\nFamiTracker.Module");
  qtMfcStringResources.insert(IDS_FRAME_DROP_MOVE,
                              "Drop selection to move frames");
  qtMfcStringResources.insert(IDS_DPCM_IMPORT_QUALITY_FORMAT, "Quality: %1");
  qtMfcStringResources.insert(IDS_CLIPBOARD_ERROR,
                              "Could not register clipboard format");
  qtMfcStringResources.insert(IDS_FILE_OPEN_ERROR, "Could not open the file!");
  qtMfcStringResources.insert(IDS_FILE_VALID_ERROR,
                              "File is not a valid FamiTracker module");
  qtMfcStringResources.insert(IDS_FILE_VERSION_ERROR,
                              "File version is not supported");
  qtMfcStringResources.insert(IDS_INST_LIMIT,
                              "You cannot add more instruments");
  qtMfcStringResources.insert(IDS_CLIPBOARD_OPEN_ERROR,
                              "Cannot open clipboard.");
  qtMfcStringResources.insert(IDS_CLIPBOARD_NOT_AVALIABLE,
                              "Clipboard data is not available.");
  qtMfcStringResources.insert(ID_TRACKER_PLAY_START, "Play module from start");
  qtMfcStringResources.insert(IDS_INSTRUMENT_FILE_FAIL,
                              "Instrument file was not vaild");
  qtMfcStringResources.insert(
      IDS_CLIPPING_MESSAGE,
      "Warning: Audio is clipping, decrease mixing levels!");
  ...
  qtMfcStringResources.insert(
      IDS_START_ERROR, "Program could not start properly, default settings has "
                       "been restored. Please restart the program.");
  //   END
  ...
  qtMfcStringResources.insert(AFX_IDS_HELPMODEMESSAGE,
  ...
  qtMfcStringResources.insert(ID_FILE_CLOSE,
  ...
  qtMfcStringResources.insert(ID_HELP,
  ...
  //   STRINGTABLE
  //   BEGIN
  qtMfcStringResources.insert(ID_NEXT_PANE,
  ...
  //   STRINGTABLE
  //   BEGIN
  qtMfcStringResources.insert(ID_WINDOW_SPLIT,
  ...
  qtMfcStringResources.insert(ID_EDIT_PASTE,
                              "Insert Clipboard contents\nPaste");
  qtMfcStringResources.insert(ID_EDIT_REPEAT, "Repeat the last action\nRepeat");
  qtMfcStringResources.insert(
      ID_EDIT_REPLACE, "Replace specific text with different text\nReplace");
  qtMfcStringResources.insert(ID_EDIT_SELECT_ALL,
                              "Select the entire document\nSelect All");
  qtMfcStringResources.insert(ID_EDIT_UNDO, "Undo the last action\nUndo");
  qtMfcStringResources.insert(ID_EDIT_REDO,
                              "Redo the previously undone action\nRedo");
  //   END

  //   STRINGTABLE
  //   BEGIN
  qtMfcStringResources.insert(ID_VIEW_TOOLBAR,
                              "Show or hide the toolbar\nToggle ToolBar");
  qtMfcStringResources.insert(ID_VIEW_STATUS_BAR,
  ...
  qtMfcStringResources.insert(AFX_IDS_SCMINIMIZE,
                              "Reduce the window to an icon");
  qtMfcStringResources.insert(AFX_IDS_SCMAXIMIZE,
                              "Enlarge the window to full size");
  qtMfcStringResources.insert(AFX_IDS_SCNEXTWINDOW,
                              "Switch to the next document window");
  qtMfcStringResources.insert(AFX_IDS_SCPREVWINDOW,
  ...
  //   STRINGTABLE
  //   BEGIN
  qtMfcStringResources.insert(AFX_IDS_SCRESTORE,
  ...
  qtMfcStringResources.insert(ID_TRACKER_PLAYPATTERN,
                              "Play and loop current pattern\nPlay and loop");
  qtMfcStringResources.insert(ID_TRACKER_STOP, "Stop playing\nStop");
  qtMfcStringResources.insert(ID_TRACKER_EDIT,
                              "Enable/disable edit mode\nToggle edit mode");
  qtMfcStringResources.insert(ID_TRACKER_KILLSOUND,
                              "Promptly stop all channels");
  //   END

  //   STRINGTABLE
  //   BEGIN
  qtMfcStringResources.insert(
      ID_EDIT_DELETE, "Delete selection, or note if there is no selection.");
  qtMfcStringResources.insert(
      ID_EDIT_UNLOCKCURSOR,
      "Makes it possible to edit without scrolling the view");
  ...
  qtMfcStringResources.insert(
      ID_SPEED_DEFALUT,
      "Default playback rate (60 Hz for NTSC, 50 Hz for PAL)");
  qtMfcStringResources.insert(ID_SPEED_CUSTOM,
                              "Set the playback rate manually");
  qtMfcStringResources.insert(ID_HELP_PERFORMANCE,
                              "Display application performance");
  qtMfcStringResources.insert(ID_EDIT_PASTEOVERWRITE,
                              "Pasted notes will overwrite existing notes");
  //   END

  //   STRINGTABLE
  //   BEGIN
  qtMfcStringResources.insert(ID_FILE_CREATE_NSF,
                              "Brings up the create NSF dialog\nCreate NSF");
  //   END

  //   STRINGTABLE
  //   BEGIN
  qtMfcStringResources.insert(ID_TRANSPOSE_INCREASENOTE,
                              "Increase selected note/notes by one step");
  qtMfcStringResources.insert(ID_TRANSPOSE_DECREASENOTE,
                              "Decrease selected note/notes by one step");
  qtMfcStringResources.insert(ID_TRANSPOSE_INCREASEOCTAVE,
                              "Increase selected octave/octaves by one step");
  qtMfcStringResources.insert(ID_TRANSPOSE_DECREASEOCTAVE,
                              "Decrease selected octave/octaves by one step");
  qtMfcStringResources.insert(ID_EDIT_SELECTALL,
                              "Select pattern or all patterns\nSelect All");
  //   END

  //   STRINGTABLE
  //   BEGIN
  qtMfcStringResources.insert(ID_FILE_GENERALSETTINGS,
                              "Opens FamiTracker settings\nSettings");
  qtMfcStringResources.insert(
      ID_EDIT_ENABLEMIDI,
      "Temporarly enable / disable commands from the MIDI device");
  qtMfcStringResources.insert(
      ID_FRAME_INSERT, "Insert a new frame on current location\nAdd frame");
  qtMfcStringResources.insert(ID_FRAME_REMOVE,
  ...
  qtMfcStringResources.insert(
      ID_SPEED_DEFAULT,
      "Chooses default playback rate corresponding to selected video standard");
  qtMfcStringResources.insert(
      ID_MODULE_MODULEPROPERTIES,
      "Opens the module properties window\nModule properties");
  //   END

  //   STRINGTABLE
  //   BEGIN
  qtMfcStringResources.insert(IDS_FILE_LOAD_ERROR,
                              "Couldn't load file properly. Make sure you have "
                              "the latest version of FamiTracker.");
  qtMfcStringResources.insert(IDS_CONFIG_WINDOW, "FamiTracker configuration");
  //   END

  //   STRINGTABLE
  //   BEGIN
  qtMfcStringResources.insert(ID_EDIT_PASTEMIX,
                              "Pasted notes will mix with existing notes");
  qtMfcStringResources.insert(
      ID_MODULE_MOVEFRAMEDOWN,
      "Move selected frame one step down\nMove frame down");
  qtMfcStringResources.insert(ID_MODULE_MOVEFRAMEUP,
                              "Move selected frame one step up\nMove frame up");
  qtMfcStringResources.insert(ID_MODULE_SAVEINSTRUMENT,
                              "Save instrument to file\nSave instrument");
  qtMfcStringResources.insert(ID_MODULE_LOADINSTRUMENT,
  ...
  //   BEGIN
  qtMfcStringResources.insert(
      ID_MODULE_ADDINSTRUMENT,
      "Add a new instrument to the module\nAdd instrument");
  qtMfcStringResources.insert(
      ID_MODULE_REMOVEINSTRUMENT,
  ...
  qtMfcStringResources.insert(ID_PREV_SONG,
                              "Select previous song\nPrevious song");
  qtMfcStringResources.insert(
      ID_EDIT_INSTRUMENTMASK,
      "Disables insertion of values in the instrument column");
  qtMfcStringResources.insert(ID_TRACKER_SWITCHTOTRACKINSTRUMENT,
                              "Switch to channel instruments when playing");
  qtMfcStringResources.insert(ID_FRAME_INSERT_UNIQUE,
                              "Insert new frame with empty patterns");
  qtMfcStringResources.insert(ID_FILE_CREATEWAV,
                              "Saves the file as a WAV-file\nCreate WAV");
  qtMfcStringResources.insert(ID_VIEW_CONTROLPANEL,
                              "Show or hide the control panel");
  qtMfcStringResources.insert(ID_EDIT_CLEARPATTERNS,
                              "Resets all patterns and frames");
  qtMfcStringResources.insert(
      ID_EDIT_INTERPOLATE,
      "Interpolates a selection between two values\nInterpolate");
  //   END

  //   STRINGTABLE
  //   BEGIN
  qtMfcStringResources.insert(ID_TRACKER_SOLOCHANNEL,
                              "Mutes/unmutes all but the selected channel");
  qtMfcStringResources.insert(ID_TRACKER_TOGGLECHANNEL,
                              "Mute/unmute selected channel");
  qtMfcStringResources.insert(ID_EDIT_GRADIENT,
                              "Interpolates a selection of values");
  qtMfcStringResources.insert(ID_MODULE_EDITINSTRUMENT,
  ...
  qtMfcStringResources.insert(
      IDS_IMPORT_CHIP_MISMATCH,
      "Imported file must be of the same expansion chip type as current file.");
  qtMfcStringResources.insert(IDS_IMPORT_INSTRUMENT_COUNT,
                              "Can't import file, out of instrument slots!");
  qtMfcStringResources.insert(
      IDS_IMPORT_SAMPLE_SLOTS,
      "Could not import all samples, out of sample slots!");
  qtMfcStringResources.insert(IDS_DPCM_IMPORT_TITLE_FORMAT, "PCM Import (%1)");
  qtMfcStringResources.insert(IDS_DPCM_IMPORT_WAVE_FORMAT,
                              "%1 Hz, %2 bits, %3");
  qtMfcStringResources.insert(IDS_DPCM_IMPORT_TARGET_FORMAT,
                              "Target sample rate: %1 Hz");
  qtMfcStringResources.insert(IDS_PERFORMANCE_FRAMERATE_FORMAT,
                              "Frame rate: %1 Hz");
  ...
  qtMfcStringResources.insert(ID_TRACKER_DPCM,
                              "Display information about the DPCM channel");
  //   END
  ...
  qtMfcStringResources.insert(ID_INSTRUMENT_EDIT,
                              "Open instrument edtor\nEdit instrument");
  qtMfcStringResources.insert(
      ID_INSTRUMENT_CLONE,
      "Create a copy of selected instrument\nClone instrument");
  qtMfcStringResources.insert(ID_EDIT_REPLACEINSTRUMENT,
                              "Replaces all instruments in a selection with "
  ...
  qtMfcStringResources.insert(ID_HELP_EFFECTTABLE,
                              "Open help window with effect table");
  qtMfcStringResources.insert(
      ID_CLEANUP_REMOVEUNUSEDINSTRUMENTS,
  ...
  //   STRINGTABLE
  //   BEGIN
  qtMfcStringResources.insert(ID_MODULE_FRAME_INSERT,
                              "Insert a new frame on current location");
  qtMfcStringResources.insert(ID_MODULE_FRAME_REMOVE, "Removes selected frame");
  qtMfcStringResources.insert(ID_MODULE_REMOVEFRAME,
                              "Removes selected frame\nRemove frame");
  qtMfcStringResources.insert(ID_MODULE_COMMENTS,
                              "Open module comments dialog\nComments");
  qtMfcStringResources.insert(ID_MODULE_INSERTFRAME,
                              "Inserts a new frame\nInsert frame");
  qtMfcStringResources.insert(
      ID_INSTRUMENT_NEW, "Add a new instrument to the module\nAdd instrument");
  qtMfcStringResources.insert(ID_INSTRUMENT_LOAD,
                              "Load instrument from file\nLoad instrument");
  qtMfcStringResources.insert(ID_INSTRUMENT_SAVE,
                              "Save instrument to file\nSave instrument");
  //   END
  ...
  qtMfcStringResources.insert(
      IDS_SAVE_ERROR,
      "Couldn't save file, check that it isn't write protected or shared with "
      "other applications and enough disk space is available.");
  //   END

  //   STRINGTABLE
  //   BEGIN
  qtMfcStringResources.insert(IDC_OPT_WRAPCURSOR,
                              "Wrap around the cursor when reaching top or "
                              "bottom in the pattern editor.");
  qtMfcStringResources.insert(
      IDC_OPT_FREECURSOR,
      "Unlocks the cursor from the center of the pattern editor.");
  qtMfcStringResources.insert(IDC_OPT_WAVEPREVIEW,
                              "Preview wave and DPCM files in the open file "
                              "dialog when loading samples to the module.");
  qtMfcStringResources.insert(IDC_OPT_KEYREPEAT,
                              "Enable key repetition in the pattern editor.");
  qtMfcStringResources.insert(IDC_OPT_HEXROW,
                              "Display row numbers in hexadecimal.");
  qtMfcStringResources.insert(IDC_OPT_WRAPFRAMES,
                              "Move to next or previous frame when reaching "
                              "top or bottom in the pattern editor.");
  qtMfcStringResources.insert(
      IDC_OPT_FRAMEPREVIEW,
      "Preview next and previous frame in the pattern editor.");
  qtMfcStringResources.insert(
      IDC_OPT_NODPCMRESET,
      "Prevents resetting the DPCM channel on note stop commands.");
  qtMfcStringResources.insert(IDC_OPT_NOSTEPMOVE,
                              "Ignore the pattern step setting when moving the "
                              "cursor, only use it when inserting notes.");
  //   END

  //   STRINGTABLE
  //   BEGIN
  qtMfcStringResources.insert(
      IDC_OPT_PULLUPDELETE,
      "Makes delete key pull up rows rather than only deleting the value.");
  qtMfcStringResources.insert(
      IDC_OPT_BACKUPS,
      "Creates a backup copy of the existing file when saving a module.");
  qtMfcStringResources.insert(
      IDC_OPT_SINGLEINSTANCE,
      "Only allow one single instance of the FamiTracker application.");
  qtMfcStringResources.insert(
      IDC_OPT_PREVIEWFULLROW,
      "Preview all channels when inserting notes in the pattern editor.");
  //   END

  //   STRINGTABLE
  //   BEGIN
  qtMfcStringResources.insert(ID_MODULE_DUPLICATEFRAMEPATTERNS,
                              "Inserts a copy of current frame including all "
                              "patterns\nDuplicate patterns");
  qtMfcStringResources.insert(ID_EDIT_EXPANDPATTERNS,
                              "Expands selected pattern\nExpand pattern");
  qtMfcStringResources.insert(ID_EDIT_SHRINKPATTERNS,
                              "Shrinks selected pattern\nShrink pattern");
  qtMfcStringResources.insert(
      ID_FRAMEEDITOR_TOP,
      "Moves the frame editor to the top of the window (default)\nTop");
  qtMfcStringResources.insert(
      ID_FRAMEEDITOR_LEFT,
      "Moves the frame editor to the left of the window\nLeft");
  //   END

  //   STRINGTABLE
  //   BEGIN
  qtMfcStringResources.insert(ID_INSTRUMENT_DEEPCLONE,
                              "Creates a copy of selected instrument including "
                              "all sequences\nDeep clone instrument");
  //   END

  //   STRINGTABLE
  //   BEGIN
  qtMfcStringResources.insert(ID_TRACKER_PLAY_CURSOR,
                              "Play pattern from cursor");
  //   END

  //   STRINGTABLE
  //   BEGIN
  qtMfcStringResources.insert(IDS_CLEARPATTERN,
                              "Do you want to reset all patterns and frames? "
                              "There is no undo for this command.");
  qtMfcStringResources.insert(
      IDS_CLIPBOARD_COPY_ERROR,
      "An error occured while trying to copy data to the clipboard.");
  qtMfcStringResources.insert(
      IDS_CLIPBOARD_PASTE_ERROR,
      "An error occured while trying to paste data from the clipboard.");
  qtMfcStringResources.insert(IDS_DPCM_SPACE_FORMAT,
                              "Space used %1 kB, left %2 kB (%3 kB available)");
  qtMfcStringResources.insert(
      IDS_FILE_VERSION_TOO_NEW,
      "This file was created in a newer version of FamiTracker. Please "
      "download the most recent version to open this file.");
  qtMfcStringResources.insert(IDS_INSTRUMENT_EDITOR_TITLE,
                              "Instrument editor - %1");
  qtMfcStringResources.insert(IDS_INST_VERSION_UNSUPPORTED,
                              "File version not supported!");
  qtMfcStringResources.insert(IDS_FILE_SAVED, "File saved (%1 bytes)");
  qtMfcStringResources.insert(IDS_MIDI_ERR_INPUT,
                              "MIDI Error: Could not open MIDI input device!");
  qtMfcStringResources.insert(IDS_MIDI_ERR_OUTPUT,
                              "MIDI Error: Could not open MIDI output device!");
  qtMfcStringResources.insert(IDS_NEW_SOUND_CONFIG,
                              "New sound configuration has been loaded");
  //   END

  //   STRINGTABLE
  //   BEGIN
  qtMfcStringResources.insert(IDS_REMOVE_INSTRUMENTS,
                              "Do you want to remove all unused instruments? "
                              "There is no undo for this action.");
  qtMfcStringResources.insert(IDS_REMOVE_PATTERNS,
                              "Do you want to remove all unused patterns? "
                              "There is no undo for this action.");
  qtMfcStringResources.insert(
      IDS_SONG_DELETE,
      "Do you want to delete this song? There is no undo for this action.");
  qtMfcStringResources.insert(IDS_SOUND_FAIL,
                              "It appears the current sound settings aren't "
                              "working, change settings and try again.");
  qtMfcStringResources.insert(
      IDS_UNDERRUN_MESSAGE,
      "Warning: Audio buffer underrun, increase the audio buffer size!");
  qtMfcStringResources.insert(IDS_WELCOME_VER_FORMAT,
                              "Welcome to FamiTracker %s, press F1 for help");
  qtMfcStringResources.insert(
      IDS_DPCM_IMPORT_INVALID_WAVEFILE,
      "Cannot load this sample, file is invalid or incompatible. (Only "
      "uncompressed PCM is supported.)");
  qtMfcStringResources.insert(IDS_INST_FILE_ERROR,
                              "Could not load instrument file! Check that you "
                              "have the most recent version of FamiTracker.");
  ...
  qtMfcStringResources.insert(
      IDS_FRAME_DROP_COPY_NEW,
      "Drop selection to copy frames to new pattern numbers");
  qtMfcStringResources.insert(IDS_FRAME_DROP_COPY,
                              "Drop selection to copy frames");
  //   END
  ...
  qtMfcStringResources.insert(
      ID_TRACKER_DISPLAYREGISTERSTATE,
  ...
  qtMfcStringResources.insert(IDS_WAVE_PROGRESS_FRAME_FORMAT,
                              "Frame: %1 (%2 done)");
  qtMfcStringResources.insert(IDS_WAVE_PROGRESS_TIME_FORMAT,
                              "Time: %1 (%2 done)");
  qtMfcStringResources.insert(IDS_WAVE_PROGRESS_ELAPSED_FORMAT,
                              "Elapsed time: %1");
  qtMfcStringResources.insert(IDS_DSOUND_ERROR, "DirectX: DirectSound error!");
  qtMfcStringResources.insert(
      IDS_DSOUND_BUFFER_ERROR,
      "DirectX: DirectSound error: Could not create buffer!");
  ...
  //   STRINGTABLE
  //   BEGIN
  qtMfcStringResources.insert(IDC_OPT_DOUBLECLICK,
                              "Don't select the whole channel when "
                              "double-clicking in the pattern editor.");
  //   END

  //   STRINGTABLE
  //   BEGIN
  qtMfcStringResources.insert(IDS_LOADING_FILE, "Loading file...");
  qtMfcStringResources.insert(IDS_FILTER_DMC,
                              "Delta modulated samples (*.dmc)");
  qtMfcStringResources.insert(IDS_FILTER_WAV,
                              "Waveform Audio File Format (*.wav)");
  qtMfcStringResources.insert(IDS_FILTER_TXT, "Text files (*.txt)");
  qtMfcStringResources.insert(IDS_DPCM_EDIT_INFO_FORMAT,
                              "Wave end: %1, Size: %2 bytes ");
  qtMfcStringResources.insert(IDS_WAVE_EXPORT_DONE, "Done");
  qtMfcStringResources.insert(IDS_WAVE_EXPORT_FINISHED, "finished");
  qtMfcStringResources.insert(IDS_ABOUT_TOOLTIP_MAIL,
                              "Send mail to jsr@famitracker.com");
  qtMfcStringResources.insert(IDS_ABOUT_TOOLTIP_WEB,
                              "Go to http://www.famitracker.com");
  qtMfcStringResources.insert(IDS_ABOUT_VERSION_FORMAT,
                              "FamiTracker version %1");
  qtMfcStringResources.insert(IDS_PROPERTIES_CHANNELS, "Channels:");
  qtMfcStringResources.insert(IDS_LOADING_DONE, "File loaded");
  qtMfcStringResources.insert(IDS_DPCM_IMPORT_SIZE_FORMAT,
                              "Converted size: %1 bytes");
  qtMfcStringResources.insert(
      IDS_INSTRUMENT_FOLDER, "Choose a folder where you have instrument files");
  //   END
  ...
  qtMfcStringResources.insert(AFX_IDP_FAILED_TO_CREATE_DOC,
...
void qtMfcInitBitmapResources() {
  ...
  qtMfcBitmapResources.insert(
      IDB_KEY_BLACK, new CBitmap(":/resources/key_black_unpressed.bmp"));
  //   IDR_MAINFRAME           BITMAP                  "res\\MainToolbar-16.bmp"
  qtMfcBitmapResources.insert(IDR_MAINFRAME,
                              new CBitmap(":/resources/MainToolbar-16.bmp"));
  //   IDT_INSTRUMENT          BITMAP "res\\InstrumentToolbar-16.bmp"
  qtMfcBitmapResources.insert(
      IDT_INSTRUMENT, new CBitmap(":/resources/InstrumentToolbar-16.bmp"));
  //   IDB_VISUALIZER          BITMAP                  "res\\VisualizerBg.bmp"
  qtMfcBitmapResources.insert(IDB_VISUALIZER,
                              new CBitmap(":/resources/VisualizerBg.bmp"));
  //   IDB_KEY_BLACK_MARK      BITMAP "res\\key_black_pressed.bmp"
  qtMfcBitmapResources.insert(IDB_KEY_BLACK_MARK,
                              new CBitmap(":/resources/key_black_pressed.bmp"));
  //   IDB_KEY_WHITE           BITMAP "res\\key_white_unpressed.bmp"
  qtMfcBitmapResources.insert(
      IDB_KEY_WHITE, new CBitmap(":/resources/key_white_unpressed.bmp"));
  //   IDB_KEY_WHITE_MARK      BITMAP "res\\key_white_pressed.bmp"
  qtMfcBitmapResources.insert(IDB_KEY_WHITE_MARK,
                              new CBitmap(":/resources/key_white_pressed.bmp"));
  //   IDB_TOOLBAR_256         BITMAP "res\\MainToolbar-256.bmp"
  qtMfcBitmapResources.insert(IDB_TOOLBAR_256,
                              new CBitmap(":/resources/MainToolbar-256.bmp"));
  //   IDB_TOOLBAR_INST_256    BITMAP "res\\InstrumentToolbar-256.bmp"
  qtMfcBitmapResources.insert(
      IDB_TOOLBAR_INST_256,
      new CBitmap(":/resources/InstrumentToolbar-256.bmp"));
}
...
void qtMfcInitDialogResource_IDD_ABOUTBOX(CDialog *parent1) {
  ...
  // WS_SYSMENU
  parent->toQWidget()->setWindowFlags(
      parent->toQWidget()->windowFlags() | Qt::CustomizeWindowHint |
  ...
  // BEGIN
  //      LTEXT           "Copyright (C) 2005 - 2015
  //      jsr",IDC_ABOUT3,54,92,167,11
  CStatic *mfc2 = new CStatic(parent);
  CRect r2(CPoint(54, 92), CSize(167, 11));
  parent->MapDialogRect(&r2);
  mfc2->Create(_T("Copyright (C) 2005 - 2015 jsr"), SS_NOPREFIX | WS_VISIBLE,
               r2, parent, IDC_ABOUT3);
  ...
  mfc3->Create(_T("OK"), BS_DEFPUSHBUTTON | WS_GROUP | WS_VISIBLE, r3, parent,
               IDOK);
  mfcToQtWidget->insert(IDOK, mfc3);
  //      LTEXT           "? FDS sound emulator from nezplug (including a fix by
  //      rainwarrior)",IDC_STATIC,54,313,224,8
  ...
  mfc4->Create(
      ...
      WS_VISIBLE, r4, parent, IDC_STATIC);
  ...
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //      LTEXT           "This software is licensed under the GNU General
  //      Public\n License. Source is available on the
  //      webpage.",IDC_STATIC,54,167,224,17
  ...
  mfc6->Create(_T("This software is licensed under the GNU General Public\n ")
               _T("License. Source is available on the webpage."),
               WS_VISIBLE, r6, parent, IDC_STATIC);
  // IDC_STATIC do not get added to MFC-to-Qt map.
  ...
  //      everyone at the 2A03.org forum and everyone who has contributed with
  //      bug reports and suggestions!",IDC_STATIC,54,137,224,26
  ...
  mfc9->Create(_T("All comments and feedback are welcome\nGreets to everyone ")
               _T("at the 2A03.org forum and everyone who has contributed ")
               _T("with bug reports and suggestions!"),
               WS_VISIBLE, r9, parent, IDC_STATIC);
  // IDC_STATIC do not get added to MFC-to-Qt map.
  ...
  mfc10->Create(_T("jsr@famitracker.com"), SS_NOTIFY | WS_VISIBLE, r10, parent,
                IDC_MAIL);
  ...
  mfc11->Create(_T("http://www.famitracker.com"), SS_NOTIFY | WS_VISIBLE, r11,
                parent, IDC_WEBPAGE);
  ...
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //      LTEXT           "* Blip_buffer 0.4.0 is Copyright (C) blargg
  ...
  mfc14->Create(_T("* Blip_buffer 0.4.0 is Copyright (C) blargg ")
                _T("(http://www.slack.net/~ant/nes-emu/)\n"),
                WS_VISIBLE, r14, parent, IDC_STATIC);
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //      LTEXT           "* Fast Fourier Transform code is (C) Reliable
  //      Software",IDC_STATIC,54,292,224,8
  CStatic *mfc15 = new CStatic(parent);
  CRect r15(CPoint(54, 292), CSize(224, 8));
  parent->MapDialogRect(&r15);
  mfc15->Create(_T("* Fast Fourier Transform code is (C) Reliable Software"),
                WS_VISIBLE, r15, parent, IDC_STATIC);
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //      LTEXT           "* YM2413 && YM2149 emulators are written by Mitsutaka
  //      Okazaki",IDC_STATIC,54,303,224,8
  ...
  mfc16->Create(
      ...
      WS_VISIBLE, r16, parent, IDC_STATIC);
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //      CONTROL         "",IDC_STATIC,"Static",SS_ETCHEDHORZ,54,271,215,1
  //      LTEXT           "* Export plugin support by
  //      Gradualore",IDC_STATIC,54,204,224,9
  CStatic *mfc18 = new CStatic(parent);
  CRect r18(CPoint(54, 204), CSize(224, 9));
  parent->MapDialogRect(&r18);
  mfc18->Create(_T("* Export plugin support by Gradualore"), WS_VISIBLE, r18,
                parent, IDC_STATIC);
  // IDC_STATIC do not get added to MFC-to-Qt map.
  ...
  mfc19->Create(_T("* Icon is made by Kuhneghetz"), WS_VISIBLE, r19, parent,
                IDC_STATIC);
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //      LTEXT           "* Toolbar icons are made by
  //      ilkke",IDC_STATIC,54,223,224,9
  CStatic *mfc20 = new CStatic(parent);
  CRect r20(CPoint(54, 223), CSize(224, 9));
  parent->MapDialogRect(&r20);
  mfc20->Create(_T("* Toolbar icons are made by ilkke"), WS_VISIBLE, r20,
                parent, IDC_STATIC);
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //      CONTROL         "",IDC_STATIC,"Static",SS_ETCHEDHORZ,54,201,211,1
  //      LTEXT           "* DPCM import resampler by
  //      Jarhmander",IDC_STATIC,54,233,224,9
  CStatic *mfc22 = new CStatic(parent);
  CRect r22(CPoint(54, 233), CSize(224, 9));
  parent->MapDialogRect(&r22);
  mfc22->Create(_T("* DPCM import resampler by Jarhmander"), WS_VISIBLE, r22,
                parent, IDC_STATIC);
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //      LTEXT           "A Famicom/NES music
  //      tracker",IDC_ABOUT2,54,76,167,11,SS_NOPREFIX
  CStatic *mfc23 = new CStatic(parent);
  CRect r23(CPoint(54, 76), CSize(167, 11));
  parent->MapDialogRect(&r23);
  mfc23->Create(_T("A Famicom/NES music tracker"), SS_NOPREFIX | WS_VISIBLE,
                r23, parent, IDC_ABOUT2);
  mfcToQtWidget->insert(IDC_ABOUT2, mfc23);
  //      LTEXT           "* Module text import/export by
  //      rainwarrior",IDC_STATIC,54,244,224,9
  CStatic *mfc24 = new CStatic(parent);
  CRect r24(CPoint(54, 244), CSize(224, 9));
  parent->MapDialogRect(&r24);
  mfc24->Create(_T("* Module text import/export by rainwarrior"), WS_VISIBLE,
                r24, parent, IDC_STATIC);
  // IDC_STATIC do not get added to MFC-to-Qt map.
  ...
  //      LTEXT           "A Famicom/NES music
  //      tracker",IDC_ABOUT1,54,58,167,11,SS_NOPREFIX
  CStatic *mfc26 = new CStatic(parent);
  CRect r26(CPoint(54, 58), CSize(167, 11));
  parent->MapDialogRect(&r26);
  mfc26->Create(_T("A Famicom/NES music tracker"), SS_NOPREFIX | WS_VISIBLE,
                r26, parent, IDC_ABOUT1);
...
void qtMfcInitDialogResource_IDD_MAINFRAME(CDialog *parent1) {
  ...
  mfc1->Create(_T("Song settings"), BS_GROUPBOX | WS_VISIBLE, r1, parent,
               IDC_STATIC);
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //       GROUPBOX        "Edit
  //       settings",IDC_STATIC,7,78,78,43,0,WS_EX_TRANSPARENT
  CButton *mfc14 = new CButton(parent);
  CRect r14(CPoint(7, 78), CSize(78, 43));
  parent->MapDialogRect(&r14);
  mfc14->Create(_T("Edit settings"), BS_GROUPBOX | WS_VISIBLE, r14, parent,
                IDC_STATIC);
  // IDC_STATIC do not get added to MFC-to-Qt map.
  ...
  parent->MapDialogRect(&r19);
  mfc19->Create(_T("Song information"), BS_GROUPBOX | WS_VISIBLE, r19, parent,
                IDC_STATIC);
  // IDC_STATIC do not get added to MFC-to-Qt map.
  ...
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //       EDITTEXT        IDC_SPEED,43,15,38,12,ES_AUTOHSCROLL | ES_READONLY |
  //       ES_NUMBER
  CEdit *mfc3 = new CEdit(parent);
  CRect r3(CPoint(43, 15), CSize(38, 12));
  parent->MapDialogRect(&r3);
  mfc3->Create(ES_AUTOHSCROLL | ES_READONLY | ES_NUMBER | WS_VISIBLE, r3,
               parent, IDC_SPEED);
  mfcToQtWidget->insert(IDC_SPEED, mfc3);
  //       CONTROL         "",IDC_SPEED_SPIN,"msctls_updown32",UDS_ALIGNRIGHT |
  //       UDS_AUTOBUDDY | UDS_ARROWKEYS,81,15,11,13
  CSpinButtonCtrl *mfc4 = new CSpinButtonCtrl(parent);
  CRect r4(CPoint(81, 15), CSize(11, 13));
  parent->MapDialogRect(&r4);
  mfc4->Create(UDS_ALIGNRIGHT | UDS_AUTOBUDDY | UDS_ARROWKEYS | WS_VISIBLE, r4,
               parent, IDC_SPEED_SPIN);
  ...
  parent->MapDialogRect(&r6);
  mfc6->Create(ES_AUTOHSCROLL | ES_READONLY | WS_VISIBLE, r6, parent,
               IDC_TEMPO);
  mfcToQtWidget->insert(IDC_TEMPO, mfc6);
  //       CONTROL         "",IDC_TEMPO_SPIN,"msctls_updown32",UDS_ALIGNRIGHT |
  //       UDS_AUTOBUDDY | UDS_ARROWKEYS,81,28,11,13
  CSpinButtonCtrl *mfc7 = new CSpinButtonCtrl(parent);
  CRect r7(CPoint(81, 28), CSize(11, 13));
  parent->MapDialogRect(&r7);
  mfc7->Create(UDS_ALIGNRIGHT | UDS_AUTOBUDDY | UDS_ARROWKEYS | WS_VISIBLE, r7,
               parent, IDC_TEMPO_SPIN);
  ...
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //       EDITTEXT        IDC_ROWS,43,43,38,12,ES_AUTOHSCROLL | ES_READONLY |
  //       ES_WANTRETURN
  CEdit *mfc9 = new CEdit(parent);
  CRect r9(CPoint(43, 43), CSize(38, 12));
  parent->MapDialogRect(&r9);
  mfc9->Create(ES_AUTOHSCROLL | ES_READONLY | ES_WANTRETURN | WS_VISIBLE, r9,
               parent, IDC_ROWS);
  mfcToQtWidget->insert(IDC_ROWS, mfc9);
  //       CONTROL         "",IDC_ROWS_SPIN,"msctls_updown32",UDS_ALIGNRIGHT |
  //       UDS_AUTOBUDDY | UDS_ARROWKEYS,81,43,11,13
  CSpinButtonCtrl *mfc10 = new CSpinButtonCtrl(parent);
  CRect r10(CPoint(81, 43), CSize(11, 13));
  parent->MapDialogRect(&r10);
  mfc10->Create(UDS_ALIGNRIGHT | UDS_AUTOBUDDY | UDS_ARROWKEYS | WS_VISIBLE,
                r10, parent, IDC_ROWS_SPIN);
  ...
  parent->MapDialogRect(&r12);
  mfc12->Create(ES_AUTOHSCROLL | ES_READONLY | WS_VISIBLE, r12, parent,
                IDC_FRAMES);
  mfcToQtWidget->insert(IDC_FRAMES, mfc12);
  //       CONTROL         "",IDC_FRAME_SPIN,"msctls_updown32",UDS_ALIGNRIGHT |
  //       UDS_AUTOBUDDY | UDS_ARROWKEYS,81,58,11,13
  CSpinButtonCtrl *mfc13 = new CSpinButtonCtrl(parent);
  CRect r13(CPoint(81, 58), CSize(11, 13));
  parent->MapDialogRect(&r13);
  mfc13->Create(UDS_ALIGNRIGHT | UDS_AUTOBUDDY | UDS_ARROWKEYS | WS_VISIBLE,
                r13, parent, IDC_FRAME_SPIN);
  ...
  parent->MapDialogRect(&r16);
  mfc16->Create(ES_AUTOHSCROLL | ES_READONLY | WS_VISIBLE, r16, parent,
                IDC_KEYSTEP);
  mfcToQtWidget->insert(IDC_KEYSTEP, mfc16);
  //       CONTROL         "",IDC_KEYSTEP_SPIN,"msctls_updown32",UDS_ALIGNRIGHT
  //       | UDS_AUTOBUDDY | UDS_ARROWKEYS,81,88,11,13
  CSpinButtonCtrl *mfc17 = new CSpinButtonCtrl(parent);
  CRect r17(CPoint(81, 88), CSize(11, 13));
  parent->MapDialogRect(&r17);
  mfc17->Create(UDS_ALIGNRIGHT | UDS_AUTOBUDDY | UDS_ARROWKEYS | WS_VISIBLE,
                r17, parent, IDC_KEYSTEP_SPIN);
  mfcToQtWidget->insert(IDC_KEYSTEP_SPIN, mfc17);
  //       CONTROL         "Key
  //       repetition",IDC_KEYREPEAT,"Button",BS_AUTOCHECKBOX |
  //       WS_TABSTOP,13,105,68,9,WS_EX_TRANSPARENT
  CButton *mfc18 = new CButton(parent);
  CRect r18(CPoint(13, 105), CSize(68, 9));
  parent->MapDialogRect(&r18);
  mfc18->Create(_T("Key repetition"), BS_AUTOCHECKBOX | WS_TABSTOP | WS_VISIBLE,
                r18, parent, IDC_KEYREPEAT);
  ...
  //       COMBOBOX        IDC_SUBTUNE,97,104,84,85,CBS_DROPDOWNLIST |
  //       WS_VSCROLL | WS_TABSTOP
  CComboBox *mfc24 = new CComboBox(parent);
  CRect r24(CPoint(97, 104), CSize(84, 85));
  parent->MapDialogRect(&r24);
  mfc24->Create(CBS_DROPDOWNLIST | WS_VSCROLL | WS_TABSTOP | WS_VISIBLE, r24,
                parent, IDC_SUBTUNE);
  mfcToQtWidget->insert(IDC_SUBTUNE, mfc24);
  //   CONTROL         "",IDC_INSTRUMENTS,"SysListView32",LVS_LIST |
  //   LVS_SINGLESEL | LVS_SHOWSELALWAYS | LVS_EDITLABELS | LVS_ALIGNLEFT |
  //   LVS_NOCOLUMNHEADER | LVS_NOSORTHEADER | WS_BORDER |
  //   WS_TABSTOP,193,5,202,103
  CListCtrl *mfc25 = new CListCtrl(parent);
  CRect r25(CPoint(193, 5), CSize(202, 103));
  parent->MapDialogRect(&r25);
  mfc25->Create(LVS_LIST | LVS_SINGLESEL | LVS_SHOWSELALWAYS | LVS_EDITLABELS |
                    LVS_ALIGNLEFT | LVS_NOCOLUMNHEADER | LVS_NOSORTHEADER |
                    WS_BORDER | WS_TABSTOP | WS_VISIBLE,
                r25, parent, IDC_INSTRUMENTS);
...
void qtMfcInitDialogResource_IDD_PERFORMANCE(CDialog *parent) {
  ...
  //   WS_CAPTION | WS_SYSMENU CAPTION "Performance" FONT 8, "MS Shell Dlg",
  //   400, 0, 0x1 BEGIN
  //       DEFPUSHBUTTON   "Close",IDOK,58,74,60,14
  //       GROUPBOX        "CPU usage",IDC_STATIC,7,7,68,53
  //       CTEXT           "--%",IDC_CPU,43,30,29,10
  //       CONTROL         "",IDC_CPU_BAR,"msctls_progress32",PBS_SMOOTH |
  //       PBS_VERTICAL | WS_BORDER,18,19,18,34 LTEXT           "Frame rate: 0
  //       Hz",IDC_FRAMERATE,89,18,72,8 LTEXT           "Underruns:
  //       0",IDC_UNDERRUN,89,45,66,8 CONTROL
  //       "",IDC_STATIC,"Static",SS_ETCHEDHORZ,7,67,162,1 GROUPBOX
  //       "Other",IDC_STATIC,81,7,88,26 GROUPBOX "Audio",IDC_STATIC,81,34,88,26
  //   END
}
...
void qtMfcInitDialogResource_IDD_PCMIMPORT(CDialog *parent1) {
  ...
  parent->MapDialogRect(&r1);
  mfc1->Create(_T("Quality: 0"), BS_GROUPBOX | WS_VISIBLE, r1, parent,
               IDC_QUALITY_FRM);
  ...
  //   CONTROL         "",IDC_QUALITY,"msctls_trackbar32",TBS_AUTOTICKS |
  //   WS_TABSTOP,30,19,90,15
  CSliderCtrl *mfc3 = new CSliderCtrl(parent);
  CRect r3(CPoint(30, 19), CSize(90, 15));
  parent->MapDialogRect(&r3);
  mfc3->Create(TBS_AUTOTICKS | WS_TABSTOP | WS_VISIBLE, r3, parent,
               IDC_QUALITY);
  ...
  parent->MapDialogRect(&r5);
  mfc5->Create(_T("Gain: +0dB"), BS_GROUPBOX | WS_VISIBLE, r5, parent,
               IDC_VOLUME_FRM);
  ...
  //   CONTROL         "",IDC_VOLUME,"msctls_trackbar32",TBS_AUTOTICKS |
  //   WS_TABSTOP,30,59,90,15
  ...
  parent->MapDialogRect(&r9);
  mfc9->Create(_T("File info"), BS_GROUPBOX | WS_VISIBLE, r9, parent,
               IDC_STATIC);
...
void qtMfcInitDialogResource_IDD_INSTRUMENT_INTERNAL(CDialog *parent1) {
  CInstrumentEditor2A03 *parent2A03 =
      dynamic_cast<CInstrumentEditor2A03 *>(parent1);
  CInstrumentEditorN163 *parentN163 =
      dynamic_cast<CInstrumentEditorN163 *>(parent1);
  CInstrumentEditorS5B *parentS5B =
      dynamic_cast<CInstrumentEditorS5B *>(parent1);
  CInstrumentEditorVRC6 *parentVRC6 =
      dynamic_cast<CInstrumentEditorVRC6 *>(parent1);
  CDialog *parent = NULL;
  if (parent2A03)
    parent = parent2A03;
  else if (parentN163)
  ...
  parent->MapDialogRect(&r5);
  mfc5->Create(_T("Sequence editor"), BS_GROUPBOX | WS_VISIBLE, r5, parent,
               IDC_STATIC);
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //   GROUPBOX        "Instrument
  //   settings",IDC_STATIC,7,7,107,158,0,WS_EX_TRANSPARENT
  CButton *mfc6 = new CButton(parent);
  CRect r6(CPoint(7, 7), CSize(107, 158));
  parent->MapDialogRect(&r6);
  mfc6->Create(_T("Instrument settings"), BS_GROUPBOX | WS_VISIBLE, r6, parent,
               IDC_STATIC);
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //   CONTROL         "",IDC_INSTSETTINGS,"SysListView32",LVS_REPORT |
  //   LVS_SINGLESEL | LVS_SHOWSELALWAYS | LVS_ALIGNLEFT | LVS_NOSORTHEADER |
  //   WS_BORDER | WS_TABSTOP,12,18,96,109,WS_EX_TRANSPARENT
  CListCtrl *mfc1 = new CListCtrl(parent);
  CRect r1(CPoint(12, 18), CSize(96, 109));
  parent->MapDialogRect(&r1);
  mfc1->Create(LVS_REPORT | LVS_SINGLESEL | LVS_SHOWSELALWAYS | LVS_ALIGNLEFT |
                   LVS_NOSORTHEADER | WS_BORDER | WS_TABSTOP | WS_VISIBLE,
               r1, parent, IDC_INSTSETTINGS);
  mfcToQtWidget->insert(IDC_INSTSETTINGS, mfc1);
  //   CONTROL         "Sequence #",IDC_STATIC,"Static",SS_LEFTNOWORDWRAP |
  //   SS_CENTERIMAGE | WS_GROUP,12,149,53,10,WS_EX_TRANSPARENT
  CStatic *mfc2 = new CStatic(parent);
  CRect r2(CPoint(12, 149), CSize(53, 10));
  parent->MapDialogRect(&r2);
  mfc2->Create(_T("Sequence #"),
               SS_LEFTNOWORDWRAP | SS_CENTERIMAGE | WS_GROUP | WS_VISIBLE, r2,
               parent, IDC_STATIC);
  // IDC_STATIC do not get added to MFC-to-Qt map.
  ...
  parent->MapDialogRect(&r3);
  mfc3->Create(ES_AUTOHSCROLL | ES_NUMBER | WS_VISIBLE, r3, parent,
               IDC_SEQ_INDEX);
  mfcToQtWidget->insert(IDC_SEQ_INDEX, mfc3);
  //   CONTROL         "",IDC_SEQUENCE_SPIN,"msctls_updown32",UDS_SETBUDDYINT |
  //   UDS_ALIGNRIGHT | UDS_AUTOBUDDY | UDS_ARROWKEYS,66,153,11,9
  CSpinButtonCtrl *mfc4 = new CSpinButtonCtrl(parent);
  CRect r4(CPoint(66, 153), CSize(11, 9));
  parent->MapDialogRect(&r4);
  mfc4->Create(UDS_SETBUDDYINT | UDS_ALIGNRIGHT | UDS_AUTOBUDDY |
                   UDS_ARROWKEYS | WS_VISIBLE,
               r4, parent, IDC_SEQUENCE_SPIN);
  ...
  parent->MapDialogRect(&r7);
  mfc7->Create(_T("Select next empty slot"), WS_VISIBLE, r7, parent,
               IDC_FREE_SEQ);
...
void qtMfcInitDialogResource_IDD_INSTRUMENT_DPCM(CDialog *parent1) {
  CInstrumentEditorDPCM *parent =
      dynamic_cast<CInstrumentEditorDPCM *>(parent1);
  QHash<int, CWnd *> *mfcToQtWidget = parent->mfcToQtWidgetMap();
  ...
  parent->MapDialogRect(&r1);
  mfc1->Create(_T("Assigned samples"), BS_GROUPBOX | WS_VISIBLE, r1, parent,
               IDC_STATIC);
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //   CONTROL         "",IDC_TABLE,"SysListView32",LVS_REPORT |
  //   LVS_SHOWSELALWAYS | LVS_ALIGNLEFT | LVS_NOSORTHEADER | WS_BORDER |
  //   WS_TABSTOP,15,19,117,125
  CListCtrl *mfc2 = new CListCtrl(parent);
  CRect r2(CPoint(15, 19), CSize(117, 125));
  parent->MapDialogRect(&r2);
  mfc2->Create(LVS_REPORT | LVS_SHOWSELALWAYS | LVS_ALIGNLEFT |
                   LVS_NOSORTHEADER | WS_BORDER | WS_TABSTOP | WS_VISIBLE,
               r2, parent, IDC_TABLE);
  mfcToQtWidget->insert(IDC_TABLE, mfc2);
  //   COMBOBOX        IDC_SAMPLES,15,148,117,125,CBS_DROPDOWNLIST | CBS_SORT |
  //   WS_VSCROLL | WS_TABSTOP
  CComboBox *mfc3 = new CComboBox(parent);
  CRect r3(CPoint(15, 148), CSize(117, 125));
  parent->MapDialogRect(&r3);
  mfc3->Create(CBS_DROPDOWNLIST | CBS_SORT | WS_VSCROLL | WS_TABSTOP |
                   WS_VISIBLE,
               r3, parent, IDC_SAMPLES);
  ...
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //   COMBOBOX        IDC_OCTAVE,138,29,42,53,CBS_DROPDOWNLIST | WS_VSCROLL |
  //   WS_TABSTOP
  CComboBox *mfc5 = new CComboBox(parent);
  CRect r5(CPoint(138, 29), CSize(42, 53));
  parent->MapDialogRect(&r5);
  mfc5->Create(CBS_DROPDOWNLIST | WS_VSCROLL | WS_TABSTOP | WS_VISIBLE, r5,
               parent, IDC_OCTAVE);
  ...
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //   COMBOBOX        IDC_PITCH,138,54,42,53,CBS_DROPDOWNLIST | WS_VSCROLL |
  //   WS_TABSTOP
  CComboBox *mfc7 = new CComboBox(parent);
  CRect r7(CPoint(138, 54), CSize(42, 53));
  parent->MapDialogRect(&r7);
  mfc7->Create(CBS_DROPDOWNLIST | WS_VSCROLL | WS_TABSTOP | WS_VISIBLE, r7,
               parent, IDC_PITCH);
  mfcToQtWidget->insert(IDC_PITCH, mfc7);
  //   CONTROL         "Loop",IDC_LOOP,"Button",BS_AUTOCHECKBOX |
  //   WS_TABSTOP,138,71,42,9
  CButton *mfc8 = new CButton(parent);
  CRect r8(CPoint(138, 71), CSize(42, 9));
  parent->MapDialogRect(&r8);
  mfc8->Create(_T("Loop"), BS_AUTOCHECKBOX | WS_TABSTOP | WS_VISIBLE, r8,
               parent, IDC_LOOP);
  ...
  //   CONTROL         "",IDC_DELTA_SPIN,"msctls_updown32",UDS_ALIGNRIGHT |
  //   UDS_AUTOBUDDY | UDS_ARROWKEYS,170,106,11,14
  CSpinButtonCtrl *mfc10 = new CSpinButtonCtrl(parent);
  CRect r10(CPoint(170, 106), CSize(11, 14));
  parent->MapDialogRect(&r10);
  mfc10->Create(UDS_ALIGNRIGHT | UDS_AUTOBUDDY | UDS_ARROWKEYS | WS_VISIBLE,
                r10, parent, IDC_DELTA_SPIN);
  ...
  parent->MapDialogRect(&r13);
  mfc13->Create(_T("Loaded samples"), BS_GROUPBOX | WS_VISIBLE, r13, parent,
                IDC_STATIC);
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //   CONTROL         "",IDC_SAMPLE_LIST,"SysListView32",LVS_REPORT |
  //   LVS_SHOWSELALWAYS | LVS_ALIGNLEFT | LVS_NOSORTHEADER | WS_BORDER |
  //   WS_TABSTOP,198,19,108,125
  CListCtrl *mfc14 = new CListCtrl(parent);
  CRect r14(CPoint(198, 19), CSize(108, 125));
  parent->MapDialogRect(&r14);
  mfc14->Create(LVS_REPORT | LVS_SHOWSELALWAYS | LVS_ALIGNLEFT |
                    LVS_NOSORTHEADER | WS_BORDER | WS_TABSTOP | WS_VISIBLE,
                r14, parent, IDC_SAMPLE_LIST);
  ...
  parent->MapDialogRect(&r15);
  mfc15->Create(_T("Space used 16 / 16 kb"), WS_VISIBLE, r15, parent,
                IDC_SPACE);
  ...
  parent->MapDialogRect(&r16);
  mfc16->Create(_T("Load"), BS_DEFPUSHBUTTON | WS_VISIBLE, r16, parent,
                IDC_LOAD);
...
void qtMfcInitDialogResource_IDD_INSTRUMENT(CDialog *parent1) {
  ...
  parent->toQWidget()->setWindowFlags(parent->toQWidget()->windowFlags() |
  ...
  //   CONTROL         "",IDC_KEYBOARD,"Static",SS_OWNERDRAW | SS_REALSIZEIMAGE
  //   | SS_SUNKEN,7,198,375,37
  CStatic *mfc2 = new CStatic(parent);
  CRect r2(CPoint(7, 198), CSize(375, 37));
  parent->MapDialogRect(&r2);
  mfc2->Create(_T(""), SS_OWNERDRAW | SS_REALSIZEIMAGE | SS_SUNKEN | WS_VISIBLE,
               r2, parent, IDC_KEYBOARD);
...
void qtMfcInitDialogResource_IDD_CONFIG_APPEARANCE(CDialog *parent1) {
  ...
  parent->MapDialogRect(&r1);
  mfc1->Create(_T("Color schemes"), BS_GROUPBOX | WS_VISIBLE, r1, parent,
               IDC_STATIC);
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //   COMBOBOX        IDC_SCHEME,15,22,135,126,CBS_DROPDOWNLIST | CBS_SORT |
  //   WS_VSCROLL | WS_TABSTOP
  CComboBox *mfc2 = new CComboBox(parent);
  CRect r2(CPoint(15, 22), CSize(135, 126));
  parent->MapDialogRect(&r2);
  mfc2->Create(CBS_DROPDOWNLIST | CBS_SORT | WS_VSCROLL | WS_TABSTOP |
                   WS_VISIBLE,
               r2, parent, IDC_SCHEME);
  ...
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //   COMBOBOX        IDC_COL_ITEM,35,66,115,172,CBS_DROPDOWNLIST | WS_VSCROLL
  //   | WS_TABSTOP
  CComboBox *mfc5 = new CComboBox(parent);
  ...
  //   CONTROL         "Pattern
  //   colors",IDC_PATTERNCOLORS,"Button",BS_AUTOCHECKBOX |
  //   WS_TABSTOP,15,105,58,9
  CButton *mfc8 = new CButton(parent);
  CRect r8(CPoint(15, 105), CSize(58, 9));
  parent->MapDialogRect(&r8);
  mfc8->Create(_T("Pattern colors"), BS_AUTOCHECKBOX | WS_TABSTOP | WS_VISIBLE,
               r8, parent, IDC_PATTERNCOLORS);
  ...
  parent->MapDialogRect(&r9);
  mfc9->Create(_T("Pattern font and size"), BS_GROUPBOX | WS_VISIBLE, r9,
               parent, IDC_STATIC);
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //   COMBOBOX        IDC_FONT,15,139,102,93,CBS_DROPDOWNLIST | CBS_SORT |
  //   WS_VSCROLL | WS_TABSTOP
  CComboBox *mfc10 = new CComboBox(parent);
  CRect r10(CPoint(15, 139), CSize(102, 93));
  parent->MapDialogRect(&r10);
  mfc10->Create(CBS_DROPDOWNLIST | CBS_SORT | WS_VSCROLL | WS_TABSTOP |
                    WS_VISIBLE,
                r10, parent, IDC_FONT);
  mfcToQtWidget->insert(IDC_FONT, mfc10);
  //   COMBOBOX        IDC_FONT_SIZE,122,139,28,30,CBS_DROPDOWNLIST | WS_VSCROLL
  //   | WS_TABSTOP
  CComboBox *mfc11 = new CComboBox(parent);
  CRect r11(CPoint(122, 139), CSize(28, 30));
  parent->MapDialogRect(&r11);
  mfc11->Create(CBS_DROPDOWNLIST | WS_VSCROLL | WS_TABSTOP | WS_VISIBLE, r11,
                parent, IDC_FONT_SIZE);
  mfcToQtWidget->insert(IDC_FONT_SIZE, mfc11);
  //   GROUPBOX        "Preview",IDC_STATIC,166,7,107,153
  CButton *mfc12 = new CButton(parent);
  CRect r12(CPoint(166, 7), CSize(107, 153));
  parent->MapDialogRect(&r12);
  mfc12->Create(_T("Preview"), BS_GROUPBOX | WS_VISIBLE, r12, parent,
                IDC_STATIC);
  // IDC_STATIC do not get added to MFC-to-Qt map.
  ...
  //   CONTROL         "Display flats",IDC_DISPLAYFLATS,"Button",BS_AUTOCHECKBOX
  //   | WS_TABSTOP,83,105,58,9
  CButton *mfc14 = new CButton(parent);
  CRect r14(CPoint(83, 105), CSize(58, 9));
  parent->MapDialogRect(&r14);
  mfc14->Create(_T("Display flats"), BS_AUTOCHECKBOX | WS_TABSTOP | WS_VISIBLE,
                r14, parent, IDC_DISPLAYFLATS);
...
void qtMfcInitDialogResource_IDD_CONFIG_GENERAL(CDialog *parent1) {
  ...
  parent->MapDialogRect(&r1);
  mfc1->Create(_T("General settings"), BS_GROUPBOX | WS_VISIBLE, r1, parent,
               IDC_STATIC);
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //       CONTROL         "Wrap
  //       cursor",IDC_OPT_WRAPCURSOR,"Button",BS_AUTOCHECKBOX |
  //       WS_TABSTOP,13,18,113,9
  CButton *mfc2 = new CButton(parent);
  CRect r2(CPoint(13, 18), CSize(113, 9));
  parent->MapDialogRect(&r2);
  mfc2->Create(_T("Wrap cursor"), BS_AUTOCHECKBOX | WS_TABSTOP | WS_VISIBLE, r2,
               parent, IDC_OPT_WRAPCURSOR);
  mfcToQtWidget->insert(IDC_OPT_WRAPCURSOR, mfc2);
  //   CONTROL         "Wrap across
  //   frames",IDC_OPT_WRAPFRAMES,"Button",BS_AUTOCHECKBOX |
  //   WS_TABSTOP,13,28,113,9
  CButton *mfc3 = new CButton(parent);
  CRect r3(CPoint(13, 28), CSize(113, 9));
  parent->MapDialogRect(&r3);
  mfc3->Create(_T("Wrap across frames"),
               BS_AUTOCHECKBOX | WS_TABSTOP | WS_VISIBLE, r3, parent,
               IDC_OPT_WRAPFRAMES);
  mfcToQtWidget->insert(IDC_OPT_WRAPFRAMES, mfc3);
  //   CONTROL         "Free cursor
  //   edit",IDC_OPT_FREECURSOR,"Button",BS_AUTOCHECKBOX |
  //   WS_TABSTOP,13,38,113,9
  CButton *mfc4 = new CButton(parent);
  CRect r4(CPoint(13, 38), CSize(113, 9));
  parent->MapDialogRect(&r4);
  mfc4->Create(_T("Free cursor edit"),
               BS_AUTOCHECKBOX | WS_TABSTOP | WS_VISIBLE, r4, parent,
               IDC_OPT_FREECURSOR);
  mfcToQtWidget->insert(IDC_OPT_FREECURSOR, mfc4);
  //   CONTROL         "Preview
  //   wave-files",IDC_OPT_WAVEPREVIEW,"Button",BS_AUTOCHECKBOX |
  //   WS_TABSTOP,13,48,113,9
  CButton *mfc5 = new CButton(parent);
  CRect r5(CPoint(13, 48), CSize(113, 9));
  parent->MapDialogRect(&r5);
  mfc5->Create(_T("Preview wave-files"),
               BS_AUTOCHECKBOX | WS_TABSTOP | WS_VISIBLE, r5, parent,
               IDC_OPT_WAVEPREVIEW);
  mfcToQtWidget->insert(IDC_OPT_WAVEPREVIEW, mfc5);
  //   CONTROL         "Key repeat",IDC_OPT_KEYREPEAT,"Button",BS_AUTOCHECKBOX |
  //   WS_TABSTOP,13,58,113,9
  CButton *mfc6 = new CButton(parent);
  CRect r6(CPoint(13, 58), CSize(113, 9));
  parent->MapDialogRect(&r6);
  mfc6->Create(_T("Key repeat"), BS_AUTOCHECKBOX | WS_TABSTOP | WS_VISIBLE, r6,
               parent, IDC_OPT_KEYREPEAT);
  mfcToQtWidget->insert(IDC_OPT_KEYREPEAT, mfc6);
  //   CONTROL         "Show row numbers in
  //   hex",IDC_OPT_HEXROW,"Button",BS_AUTOCHECKBOX | WS_TABSTOP,13,68,113,9
  CButton *mfc7 = new CButton(parent);
  CRect r7(CPoint(13, 68), CSize(113, 9));
  parent->MapDialogRect(&r7);
  mfc7->Create(_T("Show row numbers in hex"),
               BS_AUTOCHECKBOX | WS_TABSTOP | WS_VISIBLE, r7, parent,
               IDC_OPT_HEXROW);
  ...
  parent->MapDialogRect(&r8);
  mfc8->Create(_T("Preview next/previous frame"),
               BS_AUTOCHECKBOX | WS_TABSTOP | WS_VISIBLE, r8, parent,
               IDC_OPT_FRAMEPREVIEW);
  ...
  parent->MapDialogRect(&r9);
  mfc9->Create(_T("Don't reset DPCM on note stop"),
               BS_AUTOCHECKBOX | WS_TABSTOP | WS_VISIBLE, r9, parent,
               IDC_OPT_NODPCMRESET);
  mfcToQtWidget->insert(IDC_OPT_NODPCMRESET, mfc9);
  // CONTROL         "Ignore Step when
  // moving",IDC_OPT_NOSTEPMOVE,"Button",BS_AUTOCHECKBOX |
  // WS_TABSTOP,13,98,113,9
  CButton *mfc10 = new CButton(parent);
  CRect r10(CPoint(13, 98), CSize(113, 9));
  parent->MapDialogRect(&r10);
  mfc10->Create(_T("Ignore Step when moving"),
                BS_AUTOCHECKBOX | WS_TABSTOP | WS_VISIBLE, r10, parent,
                IDC_OPT_NOSTEPMOVE);
  mfcToQtWidget->insert(IDC_OPT_NOSTEPMOVE, mfc10);
  // CONTROL         "Delete-key pulls up
  // rows",IDC_OPT_PULLUPDELETE,"Button",BS_AUTOCHECKBOX |
  // WS_TABSTOP,13,108,113,9
  CButton *mfc11 = new CButton(parent);
  CRect r11(CPoint(13, 108), CSize(113, 9));
  parent->MapDialogRect(&r11);
  mfc11->Create(_T("Delete-key pulls up rows"),
                BS_AUTOCHECKBOX | WS_TABSTOP | WS_VISIBLE, r11, parent,
                IDC_OPT_PULLUPDELETE);
  mfcToQtWidget->insert(IDC_OPT_PULLUPDELETE, mfc11);
  // CONTROL         "Backup files",IDC_OPT_BACKUPS,"Button",BS_AUTOCHECKBOX |
  // WS_TABSTOP,13,118,113,9
  CButton *mfc12 = new CButton(parent);
  CRect r12(CPoint(13, 118), CSize(113, 9));
  parent->MapDialogRect(&r12);
  mfc12->Create(_T("Backup files"), BS_AUTOCHECKBOX | WS_TABSTOP | WS_VISIBLE,
                r12, parent, IDC_OPT_BACKUPS);
  mfcToQtWidget->insert(IDC_OPT_BACKUPS, mfc12);
  // CONTROL         "Single
  // instance",IDC_OPT_SINGLEINSTANCE,"Button",BS_AUTOCHECKBOX |
  // WS_TABSTOP,13,128,113,9
  CButton *mfc13 = new CButton(parent);
  CRect r13(CPoint(13, 128), CSize(113, 9));
  parent->MapDialogRect(&r13);
  mfc13->Create(_T("Single instance"),
                BS_AUTOCHECKBOX | WS_TABSTOP | WS_VISIBLE, r13, parent,
                IDC_OPT_SINGLEINSTANCE);
  ...
  parent->MapDialogRect(&r14);
  mfc14->Create(_T("Pattern edit style"), BS_GROUPBOX | WS_VISIBLE, r14, parent,
                IDC_STATIC);
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //       CONTROL         "FastTracker
  //       2",IDC_STYLE1,"Button",BS_AUTORADIOBUTTON,144,18,120,8
  CButton *mfc15 = new CButton(parent);
  CRect r15(CPoint(144, 18), CSize(120, 8));
  parent->MapDialogRect(&r15);
  mfc15->Create(_T("FastTracker 2"), BS_AUTORADIOBUTTON | WS_VISIBLE, r15,
                parent, IDC_STYLE1);
  mfcToQtWidget->insert(IDC_STYLE1, mfc15);
  //       CONTROL         "ModPlug
  //       tracker",IDC_STYLE2,"Button",BS_AUTORADIOBUTTON,144,30,120,8
  CButton *mfc16 = new CButton(parent);
  CRect r16(CPoint(144, 30), CSize(120, 8));
  parent->MapDialogRect(&r16);
  mfc16->Create(_T("ModPlug tracker"), BS_AUTORADIOBUTTON | WS_VISIBLE, r16,
                parent, IDC_STYLE2);
  mfcToQtWidget->insert(IDC_STYLE2, mfc16);
  //       CONTROL         "Impulse
  //       Tracker",IDC_STYLE3,"Button",BS_AUTORADIOBUTTON,144,42,120,8
  CButton *mfc17 = new CButton(parent);
  CRect r17(CPoint(144, 42), CSize(120, 8));
  parent->MapDialogRect(&r17);
  mfc17->Create(_T("Impulse Tracker"), BS_AUTORADIOBUTTON | WS_VISIBLE, r17,
                parent, IDC_STYLE3);
  ...
  parent->MapDialogRect(&r18);
  mfc18->Create(_T("Edit settings"), BS_GROUPBOX | WS_VISIBLE, r18, parent,
                IDC_STATIC);
  // IDC_STATIC do not get added to MFC-to-Qt map.
  ...
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //       COMBOBOX        IDC_PAGELENGTH,215,68,48,65,CBS_DROPDOWN | WS_VSCROLL
  //       | WS_TABSTOP
  CComboBox *mfc20 = new CComboBox(parent);
  CRect r20(CPoint(215, 68), CSize(48, 65));
  parent->MapDialogRect(&r20);
  mfc20->Create(CBS_DROPDOWN | WS_VSCROLL | WS_TABSTOP | WS_VISIBLE, r20,
                parent, IDC_PAGELENGTH);
  ...
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //       EDITTEXT        IDC_KEY_NOTE_CUT,204,99,60,13,ES_AUTOHSCROLL |
  //       ES_READONLY
  CEdit *mfc23 = new CEdit(parent);
  CRect r23(CPoint(204, 99), CSize(60, 13));
  parent->MapDialogRect(&r23);
  mfc23->Create(ES_AUTOHSCROLL | ES_READONLY | WS_VISIBLE, r23, parent,
                IDC_KEY_NOTE_CUT);
  ...
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //       EDITTEXT        IDC_KEY_CLEAR,204,114,60,13,ES_AUTOHSCROLL |
  //       ES_READONLY
  CEdit *mfc25 = new CEdit(parent);
  CRect r25(CPoint(204, 114), CSize(60, 13));
  parent->MapDialogRect(&r25);
  mfc25->Create(ES_AUTOHSCROLL | ES_READONLY | WS_VISIBLE, r25, parent,
                IDC_KEY_CLEAR);
  ...
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //       EDITTEXT        IDC_KEY_REPEAT,204,128,60,13,ES_AUTOHSCROLL |
  //       ES_READONLY
  CEdit *mfc27 = new CEdit(parent);
  CRect r27(CPoint(204, 128), CSize(60, 13));
  parent->MapDialogRect(&r27);
  mfc27->Create(ES_AUTOHSCROLL | ES_READONLY | WS_VISIBLE, r27, parent,
                IDC_KEY_REPEAT);
  ...
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //       EDITTEXT        IDC_KEY_NOTE_RELEASE,204,142,60,13,ES_AUTOHSCROLL |
  //       ES_READONLY
  CEdit *mfc29 = new CEdit(parent);
  CRect r29(CPoint(204, 142), CSize(60, 13));
  parent->MapDialogRect(&r29);
  mfc29->Create(ES_AUTOHSCROLL | ES_READONLY | WS_VISIBLE, r29, parent,
                IDC_KEY_NOTE_RELEASE);
  mfcToQtWidget->insert(IDC_KEY_NOTE_RELEASE, mfc29);
  //   CONTROL         "Preview full
  //   row",IDC_OPT_PREVIEWFULLROW,"Button",BS_AUTOCHECKBOX |
  //   WS_TABSTOP,13,138,113,9
  CButton *mfc30 = new CButton(parent);
  CRect r30(CPoint(13, 138), CSize(113, 9));
  parent->MapDialogRect(&r30);
  mfc30->Create(_T("Preview full row"),
                BS_AUTOCHECKBOX | WS_TABSTOP | WS_VISIBLE, r30, parent,
                IDC_OPT_PREVIEWFULLROW);
  ...
  mfc31->Create(_T("Don't select on double-click"),
                BS_AUTOCHECKBOX | WS_TABSTOP | WS_VISIBLE, r31, parent,
                IDC_OPT_DOUBLECLICK);
...
void qtMfcInitDialogResource_IDD_PROPERTIES(CDialog *parent1) {
  ...
  parent->MapDialogRect(&r1);
  mfc1->Create(_T("Song editor"), BS_GROUPBOX | WS_VISIBLE, r1, parent,
               IDC_STATIC);
  // IDC_STATIC do not get added to MFC-to-Qt map.
  ...
  parent->MapDialogRect(&r10);
  mfc10->Create(_T("Expansion sound"), BS_GROUPBOX | WS_VISIBLE, r10, parent,
                IDC_STATIC);
  // IDC_STATIC do not get added to MFC-to-Qt map.
  ...
  parent->MapDialogRect(&r12);
  mfc12->Create(_T("Vibrato"), BS_GROUPBOX | WS_VISIBLE, r12, parent,
                IDC_STATIC);
  // IDC_STATIC do not get added to MFC-to-Qt map.
  ...
  parent->MapDialogRect(&r17);
  mfc17->Create(_T("Channels: 0"), BS_GROUPBOX | WS_VISIBLE, r17, parent,
                IDC_CHANNELS_NR);
  mfcToQtWidget->insert(IDC_CHANNELS_NR, mfc17);
  //       CONTROL         "",IDC_SONGLIST,"SysListView32",LVS_REPORT |
  //       LVS_SINGLESEL | LVS_SHOWSELALWAYS | LVS_ALIGNLEFT |
  //       LVS_NOCOLUMNHEADER | WS_BORDER | WS_TABSTOP,14,18,120,114
  CListCtrl *mfc2 = new CListCtrl(parent);
  CRect r2(CPoint(14, 18), CSize(120, 114));
  parent->MapDialogRect(&r2);
  mfc2->Create(LVS_REPORT | LVS_SINGLESEL | LVS_SHOWSELALWAYS | LVS_ALIGNLEFT |
                   LVS_NOCOLUMNHEADER | WS_BORDER | WS_TABSTOP | WS_VISIBLE,
               r2, parent, IDC_SONGLIST);
  ...
  parent->MapDialogRect(&r8);
  mfc8->Create(_T(" Title"), SS_CENTERIMAGE | WS_VISIBLE, r8, parent,
               IDC_STATIC);
  // IDC_STATIC do not get added to MFC-to-Qt map.
  ...
  //       COMBOBOX        IDC_EXPANSION,14,173,113,61,CBS_DROPDOWNLIST |
  //       WS_VSCROLL | WS_TABSTOP
  CComboBox *mfc11 = new CComboBox(parent);
  CRect r11(CPoint(14, 173), CSize(113, 61));
  parent->MapDialogRect(&r11);
  mfc11->Create(CBS_DROPDOWNLIST | WS_VSCROLL | WS_TABSTOP | WS_VISIBLE, r11,
                parent, IDC_EXPANSION);
  mfcToQtWidget->insert(IDC_EXPANSION, mfc11);
  //       COMBOBOX        IDC_VIBRATO,14,204,184,61,CBS_DROPDOWNLIST |
  //       WS_VSCROLL | WS_TABSTOP
  CComboBox *mfc13 = new CComboBox(parent);
  CRect r13(CPoint(14, 204), CSize(184, 61));
  parent->MapDialogRect(&r13);
  mfc13->Create(CBS_DROPDOWNLIST | WS_VSCROLL | WS_TABSTOP | WS_VISIBLE, r13,
                parent, IDC_VIBRATO);
  ...
  //       CONTROL         "",IDC_CHANNELS,"msctls_trackbar32",TBS_AUTOTICKS |
  //       WS_TABSTOP,147,173,51,13
  CSliderCtrl *mfc18 = new CSliderCtrl(parent);
  CRect r18(CPoint(147, 173), CSize(51, 13));
  parent->MapDialogRect(&r18);
  mfc18->Create(TBS_AUTOTICKS | WS_TABSTOP | WS_VISIBLE, r18, parent,
                IDC_CHANNELS);
...
void qtMfcInitDialogResource_IDD_CONFIG_MIDI(CDialog *parent1) {
  ...
  parent->MapDialogRect(&r10);
  mfc10->Create(_T("Options"), BS_GROUPBOX | WS_VISIBLE, r10, parent,
                IDC_STATIC);
  // IDC_STATIC do not get added to MFC-to-Qt map.
  ...
  parent->MapDialogRect(&r1);
  mfc1->Create(_T("Input Device"), BS_GROUPBOX | WS_VISIBLE, r1, parent,
               IDC_STATIC);
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //       COMBOBOX        IDC_INDEVICES,13,20,253,50,CBS_DROPDOWNLIST |
  //       WS_VSCROLL | WS_TABSTOP
  CComboBox *mfc2 = new CComboBox(parent);
  CRect r2(CPoint(13, 20), CSize(253, 50));
  parent->MapDialogRect(&r2);
  mfc2->Create(CBS_DROPDOWNLIST | WS_VSCROLL | WS_TABSTOP | WS_VISIBLE, r2,
               parent, IDC_INDEVICES);
  mfcToQtWidget->insert(IDC_INDEVICES, mfc2);
  //       GROUPBOX        "Output Device",IDC_STATIC,7,48,266,35
  CButton *mfc3 = new CButton(parent);
  CRect r3(CPoint(7, 48), CSize(266, 35));
  parent->MapDialogRect(&r3);
  mfc3->Create(_T("Output Device"), BS_GROUPBOX | WS_VISIBLE, r3, parent,
               IDC_STATIC);
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //       COMBOBOX        IDC_OUTDEVICES,13,61,253,50,CBS_DROPDOWNLIST |
  //       WS_VSCROLL | WS_TABSTOP
  CComboBox *mfc4 = new CComboBox(parent);
  CRect r4(CPoint(13, 61), CSize(253, 50));
  parent->MapDialogRect(&r4);
  mfc4->Create(CBS_DROPDOWNLIST | WS_VSCROLL | WS_TABSTOP | WS_VISIBLE, r4,
               parent, IDC_OUTDEVICES);
  mfcToQtWidget->insert(IDC_OUTDEVICES, mfc4);
  //       CONTROL         "Receive sync
  //       ticks",IDC_MASTER_SYNC,"Button",BS_AUTOCHECKBOX |
  //       WS_TABSTOP,13,102,173,10
  CButton *mfc5 = new CButton(parent);
  CRect r5(CPoint(13, 102), CSize(173, 10));
  parent->MapDialogRect(&r5);
  mfc5->Create(_T("Receive sync ticks"),
               BS_AUTOCHECKBOX | WS_TABSTOP | WS_VISIBLE, r5, parent,
               IDC_MASTER_SYNC);
  mfcToQtWidget->insert(IDC_MASTER_SYNC, mfc5);
  //       CONTROL         "Skip key
  //       releases",IDC_KEY_RELEASE,"Button",BS_AUTOCHECKBOX |
  //       WS_TABSTOP,13,112,173,10
  CButton *mfc6 = new CButton(parent);
  CRect r6(CPoint(13, 112), CSize(173, 10));
  parent->MapDialogRect(&r6);
  mfc6->Create(_T("Skip key releases"),
               BS_AUTOCHECKBOX | WS_TABSTOP | WS_VISIBLE, r6, parent,
               IDC_KEY_RELEASE);
  mfcToQtWidget->insert(IDC_KEY_RELEASE, mfc6);
  //       CONTROL         "Map MIDI channels to NES
  //       channels",IDC_CHANMAP,"Button",BS_AUTOCHECKBOX |
  //       WS_TABSTOP,13,122,173,10
  CButton *mfc7 = new CButton(parent);
  CRect r7(CPoint(13, 122), CSize(173, 10));
  parent->MapDialogRect(&r7);
  mfc7->Create(_T("Map MIDI channels to NES channels"),
               BS_AUTOCHECKBOX | WS_TABSTOP | WS_VISIBLE, r7, parent,
               IDC_CHANMAP);
  mfcToQtWidget->insert(IDC_CHANMAP, mfc7);
  //       CONTROL         "Record
  //       velocities",IDC_VELOCITY,"Button",BS_AUTOCHECKBOX |
  //       WS_TABSTOP,13,132,173,10
  CButton *mfc8 = new CButton(parent);
  CRect r8(CPoint(13, 132), CSize(173, 10));
  parent->MapDialogRect(&r8);
  mfc8->Create(_T("Record velocities"),
               BS_AUTOCHECKBOX | WS_TABSTOP | WS_VISIBLE, r8, parent,
               IDC_VELOCITY);
  mfcToQtWidget->insert(IDC_VELOCITY, mfc8);
  //       CONTROL         "Auto arpeggiate
  //       chords",IDC_ARPEGGIATE,"Button",BS_AUTOCHECKBOX |
  //       WS_TABSTOP,13,142,173,10
  CButton *mfc9 = new CButton(parent);
  CRect r9(CPoint(13, 142), CSize(173, 10));
  parent->MapDialogRect(&r9);
  mfc9->Create(_T("Auto arpeggiate chords"),
               BS_AUTOCHECKBOX | WS_TABSTOP | WS_VISIBLE, r9, parent,
               IDC_ARPEGGIATE);
...
void qtMfcInitDialogResource_IDD_CONFIG_SOUND(CDialog *parent1) {
  ...
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //       COMBOBOX        IDC_DEVICES,13,20,253,12,CBS_DROPDOWNLIST |
  //       WS_VSCROLL | WS_TABSTOP
  CComboBox *mfc2 = new CComboBox(parent);
  ...
  parent->MapDialogRect(&r3);
  mfc3->Create(_T("Sample rate"), BS_GROUPBOX | WS_VISIBLE, r3, parent,
               IDC_STATIC);
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //       COMBOBOX        IDC_SAMPLE_RATE,13,61,101,62,CBS_DROPDOWNLIST |
  //       CBS_SORT | WS_VSCROLL | WS_TABSTOP
  CComboBox *mfc4 = new CComboBox(parent);
  CRect r4(CPoint(13, 61), CSize(101, 62));
  parent->MapDialogRect(&r4);
  mfc4->Create(CBS_DROPDOWNLIST | CBS_SORT | WS_VSCROLL | WS_TABSTOP |
                   WS_VISIBLE,
               r4, parent, IDC_SAMPLE_RATE);
  mfcToQtWidget->insert(IDC_SAMPLE_RATE, mfc4);
  //       GROUPBOX        "Sample size",IDC_STATIC,7,90,113,33
  CButton *mfc5 = new CButton(parent);
  CRect r5(CPoint(7, 90), CSize(113, 33));
  parent->MapDialogRect(&r5);
  mfc5->Create(_T("Sample size"), BS_GROUPBOX | WS_VISIBLE, r5, parent,
               IDC_STATIC);
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //       COMBOBOX        IDC_SAMPLE_SIZE,13,102,101,62,CBS_DROPDOWNLIST |
  //       CBS_SORT | WS_VSCROLL | WS_TABSTOP
  CComboBox *mfc6 = new CComboBox(parent);
  CRect r6(CPoint(13, 102), CSize(101, 62));
  parent->MapDialogRect(&r6);
  mfc6->Create(CBS_DROPDOWNLIST | CBS_SORT | WS_VSCROLL | WS_TABSTOP |
                   WS_VISIBLE,
               r6, parent, IDC_SAMPLE_SIZE);
  ...
  parent->MapDialogRect(&r7);
  mfc7->Create(_T("Buffer length"), BS_GROUPBOX | WS_VISIBLE, r7, parent,
               IDC_STATIC);
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //       CONTROL         "",IDC_BUF_LENGTH,"msctls_trackbar32",TBS_BOTH |
  //       TBS_NOTICKS | WS_TABSTOP,13,141,69,12
  CSliderCtrl *mfc8 = new CSliderCtrl(parent);
  CRect r8(CPoint(13, 141), CSize(69, 12));
  parent->MapDialogRect(&r8);
  mfc8->Create(TBS_BOTH | TBS_NOTICKS | WS_TABSTOP | WS_VISIBLE, r8, parent,
               IDC_BUF_LENGTH);
  ...
  parent->MapDialogRect(&r10);
  mfc10->Create(_T("Bass filtering"), BS_GROUPBOX | WS_VISIBLE, r10, parent,
                IDC_STATIC);
  // IDC_STATIC do not get added to MFC-to-Qt map.
  ...
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //       CONTROL         "",IDC_BASS_FREQ,"msctls_trackbar32",TBS_BOTH |
  //       TBS_NOTICKS | WS_TABSTOP,174,63,55,12
  CSliderCtrl *mfc12 = new CSliderCtrl(parent);
  CRect r12(CPoint(174, 63), CSize(55, 12));
  parent->MapDialogRect(&r12);
  mfc12->Create(TBS_BOTH | TBS_NOTICKS | WS_TABSTOP | WS_VISIBLE, r12, parent,
                IDC_BASS_FREQ);
  ...
  parent->MapDialogRect(&r13);
  mfc13->Create(_T("16 Hz"), SS_CENTER | WS_VISIBLE, r13, parent,
                IDC_BASS_FREQ_T);
  ...
  parent->MapDialogRect(&r14);
  mfc14->Create(_T("Treble filtering"), BS_GROUPBOX | WS_VISIBLE, r14, parent,
                IDC_STATIC);
  // IDC_STATIC do not get added to MFC-to-Qt map.
  ...
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //       CONTROL         "",IDC_TREBLE_DAMP,"msctls_trackbar32",TBS_BOTH |
  //       TBS_NOTICKS | WS_TABSTOP,174,99,55,12
  CSliderCtrl *mfc16 = new CSliderCtrl(parent);
  CRect r16(CPoint(174, 99), CSize(55, 12));
  parent->MapDialogRect(&r16);
  mfc16->Create(TBS_BOTH | TBS_NOTICKS | WS_TABSTOP | WS_VISIBLE, r16, parent,
                IDC_TREBLE_DAMP);
  ...
  parent->MapDialogRect(&r17);
  mfc17->Create(_T("-24 dB"), SS_CENTER | WS_VISIBLE, r17, parent,
                IDC_TREBLE_DAMP_T);
  ...
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //       CONTROL         "",IDC_TREBLE_FREQ,"msctls_trackbar32",TBS_BOTH |
  //       TBS_NOTICKS | WS_TABSTOP,174,112,55,12
  CSliderCtrl *mfc19 = new CSliderCtrl(parent);
  CRect r19(CPoint(174, 112), CSize(55, 12));
  parent->MapDialogRect(&r19);
  mfc19->Create(TBS_BOTH | TBS_NOTICKS | WS_TABSTOP | WS_VISIBLE, r19, parent,
                IDC_TREBLE_FREQ);
  ...
  parent->MapDialogRect(&r20);
  mfc20->Create(_T("12000 Hz"), SS_CENTER | WS_VISIBLE, r20, parent,
                IDC_TREBLE_FREQ_T);
  ...
  parent->MapDialogRect(&r21);
  mfc21->Create(_T("Volume"), BS_GROUPBOX | WS_VISIBLE, r21, parent,
                IDC_STATIC);
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //       CONTROL         "",IDC_VOLUME,"msctls_trackbar32",TBS_BOTH |
  //       TBS_NOTICKS | WS_TABSTOP,132,144,96,12
  CSliderCtrl *mfc22 = new CSliderCtrl(parent);
  CRect r22(CPoint(132, 144), CSize(96, 12));
  parent->MapDialogRect(&r22);
  mfc22->Create(TBS_BOTH | TBS_NOTICKS | WS_TABSTOP | WS_VISIBLE, r22, parent,
                IDC_VOLUME);
...
void qtMfcInitDialogResource_IDD_CONFIG_SHORTCUTS(CDialog *parent1) {
  ...
  //   FONT 8, "MS Shell Dlg", 400, 0, 0x1
  //   BEGIN
  //       CONTROL         "",IDC_SHORTCUTS,"SysListView32",LVS_REPORT |
  //       LVS_SINGLESEL | LVS_SHOWSELALWAYS | LVS_ALIGNLEFT | WS_BORDER |
  //       WS_TABSTOP,7,7,266,136
  CListCtrl *mfc1 = new CListCtrl(parent);
  CRect r1(CPoint(7, 7), CSize(266, 136));
  parent->MapDialogRect(&r1);
  mfc1->Create(LVS_REPORT | LVS_SINGLESEL | LVS_SHOWSELALWAYS | LVS_ALIGNLEFT |
                   WS_BORDER | WS_TABSTOP | WS_VISIBLE,
               r1, parent, IDC_SHORTCUTS);
...
void qtMfcInitDialogResource_IDD_OCTAVE(CDialog *parent) {
  ...
  //   FONT 8, "MS Shell Dlg", 400, 0, 0x1
  //   BEGIN
  //       COMBOBOX        IDC_OCTAVE,36,0,30,74,CBS_DROPDOWNLIST | CBS_SORT |
  //       WS_VSCROLL | WS_TABSTOP
  CComboBox *mfc1 = new CComboBox(parent);
  CRect r1(CPoint(36, 0), CSize(30, 74));
  parent->MapDialogRect(&r1);
  mfc1->Create(CBS_DROPDOWNLIST | CBS_SORT | WS_VSCROLL | WS_TABSTOP |
                   WS_VISIBLE,
               r1, parent, IDC_OCTAVE);
  ...
  //       0
  //   END
  //       CONTROL         "Octave",IDC_STATIC,"Static",SS_LEFTNOWORDWRAP |
  //       WS_GROUP,6,1,24,8
  CStatic *mfc2 = new CStatic(parent);
  CRect r2(CPoint(6, 1), CSize(24, 8));
  parent->MapDialogRect(&r2);
  mfc2->Create(_T("Octave"), SS_LEFTNOWORDWRAP | WS_GROUP | WS_VISIBLE, r2,
               parent, IDC_STATIC);
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //       CONTROL         "Follow-mode",IDC_FOLLOW,"Button",BS_AUTOCHECKBOX |
  //       BS_PUSHLIKE | WS_TABSTOP,72,0,52,13
  CButton *mfc3 = new CButton(parent);
  CRect r3(CPoint(72, 0), CSize(52, 13));
  parent->MapDialogRect(&r3);
  mfc3->Create(_T("Follow-mode"),
               BS_AUTOCHECKBOX | BS_PUSHLIKE | WS_TABSTOP | WS_VISIBLE, r3,
               parent, IDC_FOLLOW);
  mfcToQtWidget->insert(IDC_FOLLOW, mfc3);
  //   CONTROL         "Row highlight,
  //   1:st",IDC_STATIC,"Static",SS_LEFTNOWORDWRAP | WS_GROUP,129,1,58,8
  CStatic *mfc4 = new CStatic(parent);
  CRect r4(CPoint(129, 1), CSize(58, 8));
  parent->MapDialogRect(&r4);
  mfc4->Create(_T("Row highlight, 1:st"),
               SS_LEFTNOWORDWRAP | WS_GROUP | WS_VISIBLE, r4, parent,
               IDC_STATIC);
  // IDC_STATIC do not get added to MFC-to-Qt map.
  ...
  parent->MapDialogRect(&r5);
  mfc5->Create(ES_AUTOHSCROLL | ES_NUMBER | WS_VISIBLE, r5, parent,
               IDC_HIGHLIGHT1);
  mfcToQtWidget->insert(IDC_HIGHLIGHT1, mfc5);
  //       CONTROL "",IDC_HIGHLIGHTSPIN1,"msctls_updown32",UDS_SETBUDDYINT |
  //       UDS_ALIGNRIGHT | UDS_AUTOBUDDY | UDS_ARROWKEYS |
  //       UDS_HOTTRACK,213,2,11,6
  CSpinButtonCtrl *mfc6 = new CSpinButtonCtrl(parent);
  CRect r6(CPoint(213, 2), CSize(11, 6));
  parent->MapDialogRect(&r6);
  mfc6->Create(UDS_SETBUDDYINT | UDS_ALIGNRIGHT | UDS_AUTOBUDDY |
                   UDS_ARROWKEYS | UDS_HOTTRACK | WS_VISIBLE,
               r6, parent, IDC_HIGHLIGHTSPIN1);
  mfcToQtWidget->insert(IDC_HIGHLIGHTSPIN1, mfc6);
  //       CONTROL         "2:nd",IDC_STATIC,"Static",SS_LEFTNOWORDWRAP |
  //       WS_GROUP,225,1,13,8
  CStatic *mfc7 = new CStatic(parent);
  CRect r7(CPoint(225, 1), CSize(13, 8));
  parent->MapDialogRect(&r7);
  mfc7->Create(_T("2:nd"), SS_LEFTNOWORDWRAP | WS_GROUP | WS_VISIBLE, r7,
               parent, IDC_STATIC);
  // IDC_STATIC do not get added to MFC-to-Qt map.
  ...
  parent->MapDialogRect(&r8);
  mfc8->Create(ES_AUTOHSCROLL | ES_NUMBER | WS_VISIBLE, r8, parent,
               IDC_HIGHLIGHT2);
  mfcToQtWidget->insert(IDC_HIGHLIGHT2, mfc8);
  //       CONTROL "",IDC_HIGHLIGHTSPIN2,"msctls_updown32",UDS_SETBUDDYINT |
  //       UDS_ALIGNRIGHT | UDS_AUTOBUDDY | UDS_ARROWKEYS |
  //       UDS_HOTTRACK,263,2,11,6
  CSpinButtonCtrl *mfc9 = new CSpinButtonCtrl(parent);
  CRect r9(CPoint(263, 2), CSize(11, 6));
  parent->MapDialogRect(&r9);
  mfc9->Create(UDS_SETBUDDYINT | UDS_ALIGNRIGHT | UDS_AUTOBUDDY |
                   UDS_ARROWKEYS | UDS_HOTTRACK | WS_VISIBLE,
               r9, parent, IDC_HIGHLIGHTSPIN2);
...
void qtMfcInitDialogResource_IDD_EXPORT(CDialog *parent1) {
  ...
  parent->MapDialogRect(&r10);
  mfc10->Create(_T("NSF file options"), BS_GROUPBOX | WS_VISIBLE, r10, parent,
                IDC_STATIC);
  // IDC_STATIC do not get added to MFC-to-Qt map.
  ...
  parent->MapDialogRect(&r14);
  mfc14->Create(_T("Progress"), BS_GROUPBOX | WS_VISIBLE, r14, parent,
                IDC_STATIC);
  // IDC_STATIC do not get added to MFC-to-Qt map.
  ...
  parent->MapDialogRect(&r16);
  mfc16->Create(_T("Type of file"), BS_GROUPBOX | WS_VISIBLE, r16, parent,
                IDC_STATIC);
  // IDC_STATIC do not get added to MFC-to-Qt map.
  ...
  parent->MapDialogRect(&r1);
  mfc1->Create(_T("Export"), BS_DEFPUSHBUTTON | WS_VISIBLE, r1, parent,
               IDC_EXPORT);
  mfcToQtWidget->insert(IDOK, mfc1);
  ...
  parent->MapDialogRect(&r6);
  mfc6->Create(_T("NTSC"), BS_AUTORADIOBUTTON | WS_VISIBLE, r6, parent,
               IDC_NTSC);
  ...
  parent->MapDialogRect(&r8);
  mfc8->Create(_T("Dual"), BS_AUTORADIOBUTTON | WS_VISIBLE, r8, parent,
               IDC_DUAL);
  mfcToQtWidget->insert(IDC_DUAL, mfc8);
  //       COMBOBOX        IDC_TYPE,16,97,218,100,CBS_DROPDOWNLIST | WS_VSCROLL
  //       | WS_TABSTOP
  CComboBox *mfc9 = new CComboBox(parent);
  CRect r9(CPoint(16, 97), CSize(218, 100));
  parent->MapDialogRect(&r9);
  mfc9->Create(CBS_DROPDOWNLIST | WS_VSCROLL | WS_TABSTOP | WS_VISIBLE, r9,
               parent, IDC_TYPE);
  ...
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //       EDITTEXT        IDC_OUTPUT,16,132,218,139,ES_MULTILINE |
  //       ES_AUTOHSCROLL | ES_READONLY | NOT WS_BORDER | WS_VSCROLL
  CEdit *mfc15 = new CEdit(parent);
  CRect r15(CPoint(16, 132), CSize(218, 139));
  parent->MapDialogRect(&r15);
  mfc15->Create(ES_MULTILINE | ES_AUTOHSCROLL | ES_READONLY | WS_VSCROLL |
                    WS_VISIBLE,
                r15, parent, IDC_COPYRIGHT);
...
void qtMfcInitDialogResource_IDD_INSTRUMENT_VRC7(CDialog *parent1) {
  CInstrumentEditorVRC7 *parent =
      dynamic_cast<CInstrumentEditorVRC7 *>(parent1);
  QHash<int, CWnd *> *mfcToQtWidget = parent->mfcToQtWidgetMap();
  ...
  parent->MapDialogRect(&r5);
  mfc5->Create(_T("Modulator settings"), BS_GROUPBOX | WS_VISIBLE, r5, parent,
               IDC_STATIC);
  // IDC_STATIC do not get added to MFC-to-Qt map.
  ...
  parent->MapDialogRect(&r23);
  mfc23->Create(_T("Carrier settings"), BS_GROUPBOX | WS_VISIBLE, r23, parent,
                IDC_STATIC);
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //       COMBOBOX        IDC_PATCH,15,18,295,128,CBS_DROPDOWNLIST | WS_VSCROLL
  //       | WS_TABSTOP
  CComboBox *mfc2 = new CComboBox(parent);
  ...
  //       CONTROL         "Amplitude
  //       modulation",IDC_M_AM,"Button",BS_AUTOCHECKBOX |
  //       WS_TABSTOP,14,54,84,10,WS_EX_TRANSPARENT
  CButton *mfc6 = new CButton(parent);
  CRect r6(CPoint(14, 54), CSize(84, 10));
  parent->MapDialogRect(&r6);
  mfc6->Create(_T("Amplitude modulation"),
               BS_AUTOCHECKBOX | WS_TABSTOP | WS_VISIBLE, r6, parent, IDC_M_AM);
  mfcToQtWidget->insert(IDC_M_AM, mfc6);
  //   CONTROL         "Vibrato",IDC_M_VIB,"Button",BS_AUTOCHECKBOX |
  //   WS_TABSTOP,109,54,39,10,WS_EX_TRANSPARENT
  CButton *mfc7 = new CButton(parent);
  CRect r7(CPoint(109, 54), CSize(39, 10));
  parent->MapDialogRect(&r7);
  mfc7->Create(_T("Vibrato"), BS_AUTOCHECKBOX | WS_TABSTOP | WS_VISIBLE, r7,
               parent, IDC_M_VIB);
  mfcToQtWidget->insert(IDC_M_VIB, mfc7);
  //       CONTROL         "Sustained",IDC_M_EG,"Button",BS_AUTOCHECKBOX |
  //       WS_TABSTOP,14,70,47,10,WS_EX_TRANSPARENT
  CButton *mfc8 = new CButton(parent);
  CRect r8(CPoint(14, 70), CSize(47, 10));
  parent->MapDialogRect(&r8);
  mfc8->Create(_T("Sustained"), BS_AUTOCHECKBOX | WS_TABSTOP | WS_VISIBLE, r8,
               parent, IDC_M_EG);
  mfcToQtWidget->insert(IDC_M_EG, mfc8);
  //   CONTROL         "Key scaling",IDC_M_KSR2,"Button",BS_AUTOCHECKBOX |
  //   WS_TABSTOP,14,86,51,10,WS_EX_TRANSPARENT
  CButton *mfc9 = new CButton(parent);
  CRect r9(CPoint(14, 86), CSize(51, 10));
  parent->MapDialogRect(&r9);
  mfc9->Create(_T("Key scaling"), BS_AUTOCHECKBOX | WS_TABSTOP | WS_VISIBLE, r9,
               parent, IDC_M_KSR2);
  mfcToQtWidget->insert(IDC_M_KSR2, mfc9);
  //   CONTROL         "Wave rectification",IDC_M_DM,"Button",BS_AUTOCHECKBOX |
  //   WS_TABSTOP,74,70,74,10,WS_EX_TRANSPARENT
  CButton *mfc10 = new CButton(parent);
  CRect r10(CPoint(74, 70), CSize(74, 10));
  parent->MapDialogRect(&r10);
  mfc10->Create(_T("Wave rectification"),
                BS_AUTOCHECKBOX | WS_TABSTOP | WS_VISIBLE, r10, parent,
                IDC_M_DM);
  ...
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //   CONTROL         "",IDC_M_KSL,"msctls_trackbar32",TBS_AUTOTICKS |
  //   WS_TABSTOP,90,86,58,11,WS_EX_TRANSPARENT
  CSliderCtrl *mfc12 = new CSliderCtrl(parent);
  CRect r12(CPoint(90, 86), CSize(58, 11));
  parent->MapDialogRect(&r12);
  mfc12->Create(TBS_AUTOTICKS | WS_TABSTOP | WS_VISIBLE, r12, parent,
                IDC_M_KSL);
  ...
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //       CONTROL         "",IDC_M_MUL,"msctls_trackbar32",TBS_AUTOTICKS |
  //       WS_TABSTOP,51,107,105,11,WS_EX_TRANSPARENT
  CSliderCtrl *mfc14 = new CSliderCtrl(parent);
  CRect r14(CPoint(51, 107), CSize(105, 11));
  parent->MapDialogRect(&r14);
  mfc14->Create(TBS_AUTOTICKS | WS_TABSTOP | WS_VISIBLE, r14, parent,
                IDC_M_MUL);
  ...
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //       CONTROL         "",IDC_M_AR,"msctls_trackbar32",TBS_AUTOTICKS |
  //       WS_TABSTOP,40,126,42,11
  CSliderCtrl *mfc16 = new CSliderCtrl(parent);
  ...
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //       CONTROL         "",IDC_M_DR,"msctls_trackbar32",TBS_AUTOTICKS |
  //       WS_TABSTOP,114,126,42,11
  CSliderCtrl *mfc18 = new CSliderCtrl(parent);
  ...
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //       CONTROL         "",IDC_M_SL,"msctls_trackbar32",TBS_AUTOTICKS |
  //       WS_TABSTOP,39,143,42,11
  ...
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //       CONTROL         "",IDC_M_RR,"msctls_trackbar32",TBS_AUTOTICKS |
  //       WS_TABSTOP,114,143,42,11
  CSliderCtrl *mfc22 = new CSliderCtrl(parent);
  ...
  //       CONTROL         "Amplitude
  //       modulation",IDC_C_AM,"Button",BS_AUTOCHECKBOX |
  //       WS_TABSTOP,171,54,84,10
  CButton *mfc24 = new CButton(parent);
  CRect r24(CPoint(171, 54), CSize(84, 10));
  parent->MapDialogRect(&r24);
  mfc24->Create(_T("Amplitude modulation"),
                BS_AUTOCHECKBOX | WS_TABSTOP | WS_VISIBLE, r24, parent,
                IDC_C_AM);
  mfcToQtWidget->insert(IDC_C_AM, mfc24);
  //   CONTROL         "Vibrato",IDC_C_VIB,"Button",BS_AUTOCHECKBOX |
  //   WS_TABSTOP,267,54,39,10
  CButton *mfc25 = new CButton(parent);
  CRect r25(CPoint(267, 54), CSize(39, 10));
  parent->MapDialogRect(&r25);
  mfc25->Create(_T("Vibrato"), BS_AUTOCHECKBOX | WS_TABSTOP | WS_VISIBLE, r25,
                parent, IDC_C_VIB);
  mfcToQtWidget->insert(IDC_C_VIB, mfc25);
  //       CONTROL         "Sustained",IDC_C_EG,"Button",BS_AUTOCHECKBOX |
  //       WS_TABSTOP,171,70,47,10
  CButton *mfc26 = new CButton(parent);
  CRect r26(CPoint(171, 70), CSize(47, 10));
  parent->MapDialogRect(&r26);
  mfc26->Create(_T("Sustained"), BS_AUTOCHECKBOX | WS_TABSTOP | WS_VISIBLE, r26,
                parent, IDC_C_EG);
  mfcToQtWidget->insert(IDC_C_EG, mfc26);
  //   CONTROL         "Key scaling",IDC_C_KSR,"Button",BS_AUTOCHECKBOX |
  //   WS_TABSTOP,171,86,51,10
  CButton *mfc27 = new CButton(parent);
  CRect r27(CPoint(171, 86), CSize(51, 10));
  parent->MapDialogRect(&r27);
  mfc27->Create(_T("Key scaling"), BS_AUTOCHECKBOX | WS_TABSTOP | WS_VISIBLE,
                r27, parent, IDC_C_KSR);
  mfcToQtWidget->insert(IDC_C_KSR, mfc27);
  //   CONTROL         "Wave rectification",IDC_C_DM,"Button",BS_AUTOCHECKBOX |
  //   WS_TABSTOP,232,70,74,10
  CButton *mfc28 = new CButton(parent);
  CRect r28(CPoint(232, 70), CSize(74, 10));
  parent->MapDialogRect(&r28);
  mfc28->Create(_T("Wave rectification"),
                BS_AUTOCHECKBOX | WS_TABSTOP | WS_VISIBLE, r28, parent,
                IDC_C_DM);
  ...
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //   CONTROL         "",IDC_C_KSL,"msctls_trackbar32",TBS_AUTOTICKS |
  //   WS_TABSTOP,247,86,59,11
  CSliderCtrl *mfc30 = new CSliderCtrl(parent);
  CRect r30(CPoint(247, 86), CSize(59, 11));
  parent->MapDialogRect(&r30);
  mfc30->Create(TBS_AUTOTICKS | WS_TABSTOP | WS_VISIBLE, r30, parent,
                IDC_C_KSL);
  ...
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //       CONTROL         "",IDC_C_MUL,"msctls_trackbar32",TBS_AUTOTICKS |
  //       WS_TABSTOP,208,107,106,11
  CSliderCtrl *mfc32 = new CSliderCtrl(parent);
  CRect r32(CPoint(208, 107), CSize(106, 11));
  parent->MapDialogRect(&r32);
  mfc32->Create(TBS_AUTOTICKS | WS_TABSTOP | WS_VISIBLE, r32, parent,
                IDC_C_MUL);
  ...
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //       CONTROL         "",IDC_C_AR,"msctls_trackbar32",TBS_AUTOTICKS |
  //       WS_TABSTOP,199,126,42,11
  ...
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //       CONTROL         "",IDC_C_DR,"msctls_trackbar32",TBS_AUTOTICKS |
  //       WS_TABSTOP,271,126,43,11
  ...
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //       CONTROL         "",IDC_C_SL,"msctls_trackbar32",TBS_AUTOTICKS |
  //       WS_TABSTOP,199,143,42,11
  ...
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //       CONTROL         "",IDC_C_RR,"msctls_trackbar32",TBS_AUTOTICKS |
  //       WS_TABSTOP,271,143,43,11
  ...
  mfc41->Create(_T("Modulator\nlevel"), SS_CENTER | WS_VISIBLE, r41, parent,
                IDC_STATIC);
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //       CONTROL         "",IDC_TL,"msctls_trackbar32",TBS_VERT | TBS_BOTH |
  //       TBS_NOTICKS | WS_TABSTOP,329,59,16,53
  CSliderCtrl *mfc42 = new CSliderCtrl(parent);
  CRect r42(CPoint(329, 59), CSize(16, 53));
  parent->MapDialogRect(&r42);
  mfc42->Create(TBS_VERT | TBS_BOTH | TBS_NOTICKS | WS_TABSTOP | WS_VISIBLE,
                r42, parent, IDC_TL);
  ...
  mfc43->Create(_T("Feedback"), SS_CENTER | WS_VISIBLE, r43, parent,
                IDC_STATIC);
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //       CONTROL         "",IDC_FB,"msctls_trackbar32",TBS_AUTOTICKS |
  //       TBS_VERT | TBS_BOTH | WS_TABSTOP,325,124,25,41
  CSliderCtrl *mfc44 = new CSliderCtrl(parent);
  CRect r44(CPoint(325, 124), CSize(25, 41));
  parent->MapDialogRect(&r44);
  mfc44->Create(TBS_AUTOTICKS | TBS_VERT | TBS_BOTH | WS_TABSTOP | WS_VISIBLE,
                r44, parent, IDC_FB);
...
void qtMfcInitDialogResource_IDD_CREATEWAV(CDialog *parent1) {
  ...
  parent->MapDialogRect(&r1);
  mfc1->Create(_T("Begin"), BS_DEFPUSHBUTTON | WS_VISIBLE, r1, parent,
               IDC_BEGIN);
  ...
  parent->MapDialogRect(&r3);
  mfc3->Create(_T("Song length"), BS_GROUPBOX | WS_VISIBLE, r3, parent,
               IDC_STATIC);
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //       CONTROL         "Play the
  //       song",IDC_RADIO_LOOP,"Button",BS_AUTORADIOBUTTON,14,20,59,10
  CButton *mfc4 = new CButton(parent);
  CRect r4(CPoint(14, 20), CSize(59, 10));
  parent->MapDialogRect(&r4);
  mfc4->Create(_T("Play the song"), BS_AUTORADIOBUTTON | WS_VISIBLE, r4, parent,
               IDC_RADIO_LOOP);
  mfcToQtWidget->insert(IDC_RADIO_LOOP, mfc4);
  //       CONTROL         "Play
  //       for",IDC_RADIO_TIME,"Button",BS_AUTORADIOBUTTON,14,38,41,10
  CButton *mfc5 = new CButton(parent);
  CRect r5(CPoint(14, 38), CSize(41, 10));
  parent->MapDialogRect(&r5);
  mfc5->Create(_T("Play for"), BS_AUTORADIOBUTTON | WS_VISIBLE, r5, parent,
               IDC_RADIO_TIME);
  ...
  //       CONTROL         "",IDC_SPIN_LOOP,"msctls_updown32",UDS_ALIGNRIGHT |
  //       UDS_AUTOBUDDY | UDS_ARROWKEYS,105,17,11,17
  CSpinButtonCtrl *mfc7 = new CSpinButtonCtrl(parent);
  CRect r7(CPoint(105, 17), CSize(11, 17));
  parent->MapDialogRect(&r7);
  mfc7->Create(UDS_ALIGNRIGHT | UDS_AUTOBUDDY | UDS_ARROWKEYS | WS_VISIBLE, r7,
               parent, IDC_SPIN_LOOP);
  ...
  //       CONTROL         "",IDC_SPIN_TIME,"msctls_updown32",UDS_ALIGNRIGHT |
  //       UDS_AUTOBUDDY | UDS_ARROWKEYS,93,36,11,14
  CSpinButtonCtrl *mfc10 = new CSpinButtonCtrl(parent);
  CRect r10(CPoint(93, 36), CSize(11, 14));
  parent->MapDialogRect(&r10);
  mfc10->Create(UDS_ALIGNRIGHT | UDS_AUTOBUDDY | UDS_ARROWKEYS | WS_VISIBLE,
                r10, parent, IDC_SPIN_TIME);
  ...
  parent->MapDialogRect(&r12);
  mfc12->Create(_T("Channels"), BS_GROUPBOX | WS_VISIBLE, r12, parent,
                IDC_STATIC);
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //       LISTBOX         IDC_CHANNELS,14,107,124,70,LBS_OWNERDRAWFIXED |
  ...
  parent->MapDialogRect(&r13);
  mfc13->Create(LBS_OWNERDRAWFIXED | LBS_HASSTRINGS | LBS_NOINTEGRALHEIGHT |
                    WS_VSCROLL | WS_TABSTOP | WS_VISIBLE,
                r13, parent, IDC_CHANNELS);
  ...
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //   COMBOBOX        IDC_TRACKS,14,72,124,30,CBS_DROPDOWNLIST | CBS_SORT |
  //   WS_VSCROLL | WS_TABSTOP
  CComboBox *mfc15 = new CComboBox(parent);
  CRect r15(CPoint(14, 72), CSize(124, 30));
  parent->MapDialogRect(&r15);
  mfc15->Create(CBS_DROPDOWNLIST | CBS_SORT | WS_VSCROLL | WS_TABSTOP |
                    WS_VISIBLE,
                r15, parent, IDC_TRACKS);
...
void qtMfcInitDialogResource_IDD_WAVE_PROGRESS(CDialog *parent1) {
  ...
  parent->MapDialogRect(&r3);
  mfc3->Create(_T("Progress"), SS_CENTER | WS_VISIBLE, r3, parent,
               IDC_PROGRESS_LBL);
  ...
  parent->MapDialogRect(&r5);
  mfc5->Create(_T("File"), SS_CENTER | SS_CENTERIMAGE | WS_VISIBLE, r5, parent,
               IDC_PROGRESS_FILE);
...
void qtMfcInitDialogResource_IDD_INSTRUMENT_FDS(CDialog *parent1) {
  ...
  parent->MapDialogRect(&r1);
  mfc1->Create(_T("Wave editor"), BS_GROUPBOX | WS_VISIBLE, r1, parent,
               IDC_STATIC);
  // IDC_STATIC do not get added to MFC-to-Qt map.
  ...
  parent->MapDialogRect(&r16);
  mfc16->Create(_T("Frequency modulation"), BS_GROUPBOX | WS_VISIBLE, r16,
                parent, IDC_STATIC);
  // IDC_STATIC do not get added to MFC-to-Qt map.
  ...
  parent->MapDialogRect(&r6);
  mfc6->Create(ES_AUTOHSCROLL | ES_NUMBER | WS_VISIBLE, r6, parent,
               IDC_MOD_RATE);
  mfcToQtWidget->insert(IDC_MOD_RATE, mfc6);
  //       CONTROL "",IDC_MOD_RATE_SPIN,"msctls_updown32",UDS_SETBUDDYINT |
  //       UDS_ALIGNRIGHT | UDS_AUTOBUDDY | UDS_ARROWKEYS |
  //       UDS_NOTHOUSANDS,344,25,11,14
  CSpinButtonCtrl *mfc7 = new CSpinButtonCtrl(parent);
  CRect r7(CPoint(344, 25), CSize(11, 14));
  parent->MapDialogRect(&r7);
  mfc7->Create(UDS_SETBUDDYINT | UDS_ALIGNRIGHT | UDS_AUTOBUDDY |
                   UDS_ARROWKEYS | UDS_NOTHOUSANDS | WS_VISIBLE,
               r7, parent, IDC_MOD_RATE_SPIN);
  ...
  parent->MapDialogRect(&r9);
  mfc9->Create(ES_AUTOHSCROLL | ES_NUMBER | WS_VISIBLE, r9, parent,
               IDC_MOD_DEPTH);
  mfcToQtWidget->insert(IDC_MOD_DEPTH, mfc9);
  //       CONTROL "",IDC_MOD_DEPTH_SPIN,"msctls_updown32",UDS_SETBUDDYINT |
  //       UDS_ALIGNRIGHT | UDS_AUTOBUDDY | UDS_ARROWKEYS,344,43,11,14
  CSpinButtonCtrl *mfc10 = new CSpinButtonCtrl(parent);
  CRect r10(CPoint(344, 43), CSize(11, 14));
  parent->MapDialogRect(&r10);
  mfc10->Create(UDS_SETBUDDYINT | UDS_ALIGNRIGHT | UDS_AUTOBUDDY |
                    UDS_ARROWKEYS | WS_VISIBLE,
                r10, parent, IDC_MOD_DEPTH_SPIN);
  ...
  parent->MapDialogRect(&r12);
  mfc12->Create(ES_AUTOHSCROLL | ES_NUMBER | WS_VISIBLE, r12, parent,
                IDC_MOD_DELAY);
  mfcToQtWidget->insert(IDC_MOD_DELAY, mfc12);
  //       CONTROL "",IDC_MOD_DELAY_SPIN,"msctls_updown32",UDS_SETBUDDYINT |
  //       UDS_ALIGNRIGHT | UDS_AUTOBUDDY | UDS_ARROWKEYS,344,63,11,14
  CSpinButtonCtrl *mfc13 = new CSpinButtonCtrl(parent);
  CRect r13(CPoint(344, 63), CSize(11, 14));
  parent->MapDialogRect(&r13);
  mfc13->Create(UDS_SETBUDDYINT | UDS_ALIGNRIGHT | UDS_AUTOBUDDY |
                    UDS_ARROWKEYS | WS_VISIBLE,
                r13, parent, IDC_MOD_DELAY_SPIN);
...
void qtMfcInitDialogResource_IDD_FRAMECONTROLS(CDialog *parent1) {
  ...
  //       CONTROL         "Change all",IDC_CHANGE_ALL,"Button",BS_AUTOCHECKBOX
  //       | WS_TABSTOP,51,2,56,10
  CButton *mfc3 = new CButton(parent);
  CRect r3(CPoint(51, 2), CSize(56, 10));
  parent->MapDialogRect(&r3);
  mfc3->Create(_T("Change all"), BS_AUTOCHECKBOX | WS_TABSTOP | WS_VISIBLE, r3,
               parent, IDC_CHANGE_ALL);
...
void qtMfcInitDialogResource_IDD_SAMPLE_EDITOR(CDialog *parent1) {
  ...
  mfc2->Create(_T("Cancel"), WS_VISIBLE, r2, parent, IDCANCEL);
  mfcToQtWidget->insert(IDCANCEL, mfc2);
  //   CONTROL         "",IDC_SAMPLE,"Static",SS_WHITERECT | SS_NOTIFY |
  //   SS_SUNKEN,7,7,467,204
  CStatic *mfc3 = new CStatic(parent);
  CRect r3(CPoint(7, 7), CSize(467, 204));
  parent->MapDialogRect(&r3);
  mfc3->Create(_T(""), SS_WHITERECT | SS_NOTIFY | SS_SUNKEN | WS_VISIBLE, r3,
               parent, IDC_SAMPLE);
  ...
  parent->MapDialogRect(&r5);
  mfc5->Create(_T("Offset: 0, Pos: 0"), SS_SUNKEN | WS_VISIBLE, r5, parent,
               IDC_POS);
  ...
  //   CONTROL         "",IDC_PITCH,"msctls_trackbar32",TBS_AUTOTICKS |
  //   WS_TABSTOP,110,236,79,12
  CSliderCtrl *mfc7 = new CSliderCtrl(parent);
  ...
  //   CONTROL         "Center wave",IDC_DELTASTART,"Button",BS_AUTOCHECKBOX |
  //   WS_TABSTOP,314,238,57,10
  CButton *mfc11 = new CButton(parent);
  CRect r11(CPoint(314, 238), CSize(57, 10));
  parent->MapDialogRect(&r11);
  mfc11->Create(_T("Center wave"), BS_AUTOCHECKBOX | WS_TABSTOP | WS_VISIBLE,
                r11, parent, IDC_DELTASTART);
  mfcToQtWidget->insert(IDC_DELTASTART, mfc11);
  //   CONTROL         "",IDC_ZOOM,"msctls_trackbar32",TBS_AUTOTICKS |
  //   WS_TABSTOP,360,217,61,12
  CSliderCtrl *mfc12 = new CSliderCtrl(parent);
...
void qtMfcInitDialogResource_IDD_INSTRUMENT_FDS_ENVELOPE(CDialog *parent1) {
  CInstrumentEditorFDSEnvelope *parent =
      dynamic_cast<CInstrumentEditorFDSEnvelope *>(parent1);
  QHash<int, CWnd *> *mfcToQtWidget = parent->mfcToQtWidgetMap();
  ...
  //       COMBOBOX        IDC_TYPE,8,151,63,30,CBS_DROPDOWNLIST | WS_VSCROLL |
  //       WS_TABSTOP
  CComboBox *mfc2 = new CComboBox(parent);
...
void qtMfcInitDialogResource_IDD_CHANNELS(CDialog *parent1) {
  ...
  parent->MapDialogRect(&r6);
  mfc6->Create(_T("Available channels"), BS_GROUPBOX | WS_VISIBLE, r6, parent,
               IDC_STATIC);
  // IDC_STATIC do not get added to MFC-to-Qt map.
  ...
  parent->MapDialogRect(&r7);
  mfc7->Create(_T("Added channels"), BS_GROUPBOX | WS_VISIBLE, r7, parent,
               IDC_STATIC);
  // IDC_STATIC do not get added to MFC-to-Qt map.
  ...
  //       CONTROL         "",IDC_ADDED_LIST,"SysListView32",LVS_REPORT |
  //       LVS_SINGLESEL | LVS_SHOWSELALWAYS | LVS_ALIGNLEFT |
  //       LVS_NOCOLUMNHEADER | WS_BORDER | WS_TABSTOP,159,17,129,181
  CListCtrl *mfc5 = new CListCtrl(parent);
  CRect r5(CPoint(159, 17), CSize(129, 181));
  parent->MapDialogRect(&r5);
  mfc5->Create(LVS_REPORT | LVS_SINGLESEL | LVS_SHOWSELALWAYS | LVS_ALIGNLEFT |
                   LVS_NOCOLUMNHEADER | WS_BORDER | WS_TABSTOP | WS_VISIBLE,
               r5, parent, IDC_ADDED_LIST);
  ...
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //       CONTROL         "",IDC_AVAILABLE_TREE,"SysTreeView32",TVS_HASBUTTONS
  //       | TVS_HASLINES | TVS_LINESATROOT | WS_BORDER | WS_HSCROLL |
  //       WS_TABSTOP,14,17,122,181
  CTreeCtrl *mfc9 = new CTreeCtrl(parent);
  CRect r9(CPoint(14, 17), CSize(122, 181));
  parent->MapDialogRect(&r9);
  mfc9->Create(TVS_HASBUTTONS | TVS_HASLINES | TVS_LINESATROOT | WS_BORDER |
                   WS_HSCROLL | WS_TABSTOP | WS_VISIBLE,
               r9, parent, IDC_AVAILABLE_TREE);
...
void qtMfcInitDialogResource_IDD_COMMENTS(CDialog *parent1) {
  ...
  //   EDITTEXT        IDC_COMMENTS,0,0,358,206,ES_MULTILINE | ES_AUTOVSCROLL |
  //   ES_AUTOHSCROLL | ES_WANTRETURN | WS_VSCROLL | WS_HSCROLL
  CEdit *mfc2 = new CEdit(parent);
  CRect r2(CPoint(0, 0), CSize(358, 206));
  parent->MapDialogRect(&r2);
  mfc2->Create(ES_MULTILINE | ES_AUTOHSCROLL | ES_WANTRETURN | WS_VSCROLL |
                   WS_VISIBLE,
               r2, parent, IDC_COMMENTS);
  mfcToQtWidget->insert(IDC_COMMENTS, mfc2);
  //   CONTROL         "Show when loading
  //   file",IDC_SHOWONOPEN,"Button",BS_AUTOCHECKBOX | WS_TABSTOP,5,215,89,10
  CButton *mfc3 = new CButton(parent);
  CRect r3(CPoint(5, 215), CSize(89, 10));
  parent->MapDialogRect(&r3);
  mfc3->Create(_T("Show when loading file"),
               BS_AUTOCHECKBOX | WS_TABSTOP | WS_VISIBLE, r3, parent,
               IDC_SHOWONOPEN);
...
void qtMfcInitDialogResource_IDD_CONFIG_MIXER(CDialog *parent1) {
  ...
  // STYLE DS_SETFONT | DS_MODALFRAME | DS_FIXEDSYS | WS_POPUP | WS_CAPTION |
  // WS_SYSMENU EXSTYLE WS_EX_CONTEXTHELP CAPTION "Mixer"
  parent->SetWindowText("Mixer");
  // FONT 8, "MS Shell Dlg", 400, 0, 0x1
  // BEGIN
  //    CONTROL         "",IDC_SLIDER_APU1,"msctls_trackbar32",TBS_AUTOTICKS |
  //    TBS_VERT | TBS_BOTH | WS_TABSTOP,7,30,26,114
  CSliderCtrl *mfc1 = new CSliderCtrl(parent);
  CRect r1(CPoint(7, 30), CSize(26, 114));
  parent->MapDialogRect(&r1);
  mfc1->Create(TBS_AUTOTICKS | TBS_VERT | TBS_BOTH | WS_TABSTOP | WS_VISIBLE,
               r1, parent, IDC_SLIDER_APU1);
  ...
  //    CONTROL         "",IDC_SLIDER_APU2,"msctls_trackbar32",TBS_AUTOTICKS |
  //    TBS_VERT | TBS_BOTH | WS_TABSTOP,41,30,26,114
  CSliderCtrl *mfc3 = new CSliderCtrl(parent);
  CRect r3(CPoint(41, 30), CSize(26, 114));
  parent->MapDialogRect(&r3);
  mfc3->Create(TBS_AUTOTICKS | TBS_VERT | TBS_BOTH | WS_TABSTOP | WS_VISIBLE,
               r3, parent, IDC_SLIDER_APU2);
  ...
  // IDC_STATIC do not get added to widget list
  //    CONTROL         "",IDC_SLIDER_VRC6,"msctls_trackbar32",TBS_AUTOTICKS |
  //    TBS_VERT | TBS_BOTH | WS_TABSTOP,75,30,26,114
  CSliderCtrl *mfc5 = new CSliderCtrl(parent);
  CRect r5(CPoint(75, 30), CSize(26, 114));
  parent->MapDialogRect(&r5);
  mfc5->Create(TBS_AUTOTICKS | TBS_VERT | TBS_BOTH | WS_TABSTOP | WS_VISIBLE,
               r5, parent, IDC_SLIDER_VRC6);
  ...
  // IDC_STATIC do not get added to widget list
  //    CONTROL         "",IDC_SLIDER_VRC7,"msctls_trackbar32",TBS_AUTOTICKS |
  //    TBS_VERT | TBS_BOTH | WS_TABSTOP,109,30,26,114
  CSliderCtrl *mfc7 = new CSliderCtrl(parent);
  CRect r7(CPoint(109, 30), CSize(26, 114));
  parent->MapDialogRect(&r7);
  mfc7->Create(TBS_AUTOTICKS | TBS_VERT | TBS_BOTH | WS_TABSTOP | WS_VISIBLE,
               r7, parent, IDC_SLIDER_VRC7);
  ...
  // IDC_STATIC do not get added to widget list
  //    CONTROL         "",IDC_SLIDER_MMC5,"msctls_trackbar32",TBS_AUTOTICKS |
  //    TBS_VERT | TBS_BOTH | WS_TABSTOP,143,30,26,114
  CSliderCtrl *mfc9 = new CSliderCtrl(parent);
  CRect r9(CPoint(143, 30), CSize(26, 114));
  parent->MapDialogRect(&r9);
  mfc9->Create(TBS_AUTOTICKS | TBS_VERT | TBS_BOTH | WS_TABSTOP | WS_VISIBLE,
               r9, parent, IDC_SLIDER_MMC5);
  ...
  // IDC_STATIC do not get added to widget list
  //    CONTROL         "",IDC_SLIDER_FDS,"msctls_trackbar32",TBS_AUTOTICKS |
  //    TBS_VERT | TBS_BOTH | WS_TABSTOP,177,30,26,114
  CSliderCtrl *mfc11 = new CSliderCtrl(parent);
  CRect r11(CPoint(177, 30), CSize(26, 114));
  parent->MapDialogRect(&r11);
  mfc11->Create(TBS_AUTOTICKS | TBS_VERT | TBS_BOTH | WS_TABSTOP | WS_VISIBLE,
                r11, parent, IDC_SLIDER_FDS);
  ...
  // IDC_STATIC do not get added to widget list
  //    CONTROL         "",IDC_SLIDER_N163,"msctls_trackbar32",TBS_AUTOTICKS |
  //    TBS_VERT | TBS_BOTH | WS_TABSTOP,211,30,26,114
  CSliderCtrl *mfc13 = new CSliderCtrl(parent);
  CRect r13(CPoint(211, 30), CSize(26, 114));
  parent->MapDialogRect(&r13);
  mfc13->Create(TBS_AUTOTICKS | TBS_VERT | TBS_BOTH | WS_TABSTOP | WS_VISIBLE,
                r13, parent, IDC_SLIDER_N163);
  ...
  // IDC_STATIC do not get added to widget list
  //    CONTROL         "",IDC_SLIDER_S5B,"msctls_trackbar32",TBS_AUTOTICKS |
  //    TBS_VERT | TBS_BOTH | WS_DISABLED | WS_TABSTOP,245,30,26,114
  CSliderCtrl *mfc15 = new CSliderCtrl(parent);
  CRect r15(CPoint(245, 30), CSize(26, 114));
  parent->MapDialogRect(&r15);
  mfc15->Create(TBS_AUTOTICKS | TBS_VERT | TBS_BOTH | WS_TABSTOP | WS_VISIBLE,
                r15, parent, IDC_SLIDER_S5B);
  ...
  parent->MapDialogRect(&r17);
  mfc17->Create(_T("0.0dB"), SS_CENTER | WS_VISIBLE, r17, parent,
                IDC_LEVEL_APU1);
  ...
  parent->MapDialogRect(&r18);
  mfc18->Create(_T("0.0dB"), SS_CENTER | WS_VISIBLE, r18, parent,
                IDC_LEVEL_APU2);
  ...
  parent->MapDialogRect(&r19);
  mfc19->Create(_T("0.0dB"), SS_CENTER | WS_VISIBLE, r19, parent,
                IDC_LEVEL_VRC6);
  ...
  parent->MapDialogRect(&r20);
  mfc20->Create(_T("0.0dB"), SS_CENTER | WS_VISIBLE, r20, parent,
                IDC_LEVEL_VRC7);
  ...
  parent->MapDialogRect(&r21);
  mfc21->Create(_T("0.0dB"), SS_CENTER | WS_VISIBLE, r21, parent,
                IDC_LEVEL_MMC5);
  ...
  parent->MapDialogRect(&r22);
  mfc22->Create(_T("0.0dB"), SS_CENTER | WS_VISIBLE, r22, parent,
                IDC_LEVEL_FDS);
  ...
  parent->MapDialogRect(&r23);
  mfc23->Create(_T("0.0dB"), SS_CENTER | WS_VISIBLE, r23, parent,
                IDC_LEVEL_N163);
  ...
  parent->MapDialogRect(&r24);
  mfc24->Create(_T("0.0dB"), SS_CENTER | WS_VISIBLE, r24, parent,
                IDC_LEVEL_S5B);
  ...
  parent->MapDialogRect(&r25);
  mfc25->Create(_T("Hardware mixing levels"), WS_VISIBLE, r25, parent,
                IDC_STATIC);
  // IDC_STATIC do not get added to MFC-to-Qt map.
  // END
}
...
void qtMfcInitDialogResource_IDD_INSTRUMENT_N163_WAVE(CDialog *parent1) {
  CInstrumentEditorN163Wave *parent =
      dynamic_cast<CInstrumentEditorN163Wave *>(parent1);
  QHash<int, CWnd *> *mfcToQtWidget = parent->mfcToQtWidgetMap();
  ...
  parent->MapDialogRect(&r21);
  mfc21->Create(_T("Wave #"), BS_GROUPBOX | WS_VISIBLE, r21, parent,
                IDC_STATIC);
  // IDC_STATIC do not get added to MFC-to-Qt map.
  ...
  parent->MapDialogRect(&r7);
  mfc7->Create(_T("MML string"), BS_GROUPBOX | WS_VISIBLE, r7, parent,
               IDC_STATIC);
  // IDC_STATIC do not get added to MFC-to-Qt map.
  ...
  parent->MapDialogRect(&r16);
  mfc16->Create(_T("Wave RAM settings"), BS_GROUPBOX | WS_VISIBLE, r16, parent,
                IDC_STATIC);
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //       PUSHBUTTON      "Sine",IDC_PRESET_SINE,14,101,41,12
  CButton *mfc2 = new CButton(parent);
  ...
  parent->MapDialogRect(&r11);
  mfc11->Create(ES_AUTOHSCROLL | ES_READONLY | WS_VISIBLE, r11, parent,
                IDC_INDEX);
  mfcToQtWidget->insert(IDC_INDEX, mfc11);
  //       CONTROL         "",IDC_INDEX_SPIN,"msctls_updown32",UDS_SETBUDDYINT |
  //       UDS_ALIGNRIGHT | UDS_AUTOBUDDY | UDS_ARROWKEYS,281,26,11,14
  CSpinButtonCtrl *mfc12 = new CSpinButtonCtrl(parent);
  CRect r12(CPoint(281, 26), CSize(11, 12));
  parent->MapDialogRect(&r12);
  mfc12->Create(UDS_SETBUDDYINT | UDS_ALIGNRIGHT | UDS_AUTOBUDDY |
                    UDS_ARROWKEYS | WS_VISIBLE,
                r12, parent, IDC_INDEX_SPIN);
  ...
  parent->MapDialogRect(&r14);
  mfc14->Create(ES_AUTOHSCROLL | ES_READONLY | WS_VISIBLE, r14, parent,
                IDC_WAVES);
  mfcToQtWidget->insert(IDC_WAVES, mfc14);
  //       CONTROL         "",IDC_WAVES_SPIN,"msctls_updown32",UDS_SETBUDDYINT |
  //       UDS_ALIGNRIGHT | UDS_AUTOBUDDY | UDS_ARROWKEYS,342,19,11,14
  CSpinButtonCtrl *mfc15 = new CSpinButtonCtrl(parent);
  CRect r15(CPoint(342, 19), CSize(11, 14));
  parent->MapDialogRect(&r15);
  mfc15->Create(UDS_SETBUDDYINT | UDS_ALIGNRIGHT | UDS_AUTOBUDDY |
                    UDS_ARROWKEYS | WS_VISIBLE,
                r15, parent, IDC_WAVES_SPIN);
  ...
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //       COMBOBOX        IDC_WAVE_SIZE,305,57,48,30,CBS_DROPDOWNLIST |
  //       WS_VSCROLL | WS_TABSTOP
  CComboBox *mfc18 = new CComboBox(parent);
  CRect r18(CPoint(305, 57), CSize(48, 30));
  parent->MapDialogRect(&r18);
  mfc18->Create(CBS_DROPDOWNLIST | WS_VSCROLL | WS_TABSTOP | WS_VISIBLE, r18,
                parent, IDC_WAVE_SIZE);
  ...
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //       COMBOBOX        IDC_WAVE_POS,305,75,48,30,CBS_DROPDOWN | WS_VSCROLL |
  //       WS_TABSTOP
  CComboBox *mfc20 = new CComboBox(parent);
  CRect r20(CPoint(305, 75), CSize(48, 30));
  parent->MapDialogRect(&r20);
  mfc20->Create(CBS_DROPDOWNLIST | WS_VSCROLL | WS_TABSTOP | WS_VISIBLE, r20,
                parent, IDC_WAVE_POS);
...
void qtMfcInitDialogResource_IDD_IMPORT(CDialog *parent1) {
  ...
  mfcToQtWidget->insert(IDCANCEL, mfc2);
  //       CONTROL         "Include
  //       instruments",IDC_INSTRUMENTS,"Button",BS_AUTOCHECKBOX |
  //       WS_TABSTOP,14,161,116,10
  CButton *mfc4 = new CButton(parent);
  CRect r4(CPoint(14, 161), CSize(116, 10));
  parent->MapDialogRect(&r4);
  mfc4->Create(_T("Include instruments"),
               BS_AUTOCHECKBOX | WS_TABSTOP | WS_VISIBLE, r4, parent,
               IDC_INSTRUMENTS);
  ...
  // IDC_STATIC do not get added to MFC-to-Qt map.
  //   LISTBOX         IDC_TRACKS,14,18,133,120,LBS_OWNERDRAWFIXED |
  //   LBS_HASSTRINGS | LBS_NOINTEGRALHEIGHT | WS_VSCROLL | WS_TABSTOP
  CListBox *mfc7 = new CListBox(parent);
  CRect r7(CPoint(14, 18), CSize(133, 120));
  parent->MapDialogRect(&r7);
  mfc7->Create(LBS_OWNERDRAWFIXED | LBS_HASSTRINGS | LBS_NOINTEGRALHEIGHT |
                   WS_VSCROLL | WS_TABSTOP | WS_VISIBLE,
               r7, parent, IDC_TRACKS);
...
void qtMfcInitToolBarResource_IDR_MAINFRAME(UINT dlgID, CToolBar *parent) {
  ...
  QObject::connect(toolBarAction, SIGNAL(triggered()), parent,
                   SLOT(toolBarAction_triggered()));
  ...
  for (x = 0; x < toolBar->actions().count(); x++) {
    CString toolTipCString =
        qtMfcStringResource(toolBar->actions().at(x)->data().toInt());
    QString toolTip = toolTipCString;
    if (toolTip.indexOf('\n') >= 0) {
      toolBar->actions().value(x)->setToolTip(
          toolTip.right(toolTip.length() - toolTip.indexOf('\n') - 1));
    }
  }
}

void qtMfcInitToolBarResource_IDR_INSTRUMENT_TOOLBAR(UINT dlgID,
                                                     CToolBar *parent) {
