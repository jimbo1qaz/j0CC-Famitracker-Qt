...
CPatternCompiler::CPatternCompiler(CFamiTrackerDoc *pDoc,
                                   unsigned int *pInstList,
                                   DPCM_List_t *pDPCMList,
                                   CCompilerLog *pLogger)
    : m_pDocument(pDoc), m_pInstrumentList(pInstList), m_pDPCMList(pDPCMList),
      m_pLogger(pLogger) {
...
void CPatternCompiler::CompileData(int Track, int Pattern, int Channel) {
  ...
  for (unsigned int i = 0; i < iPatternLen; ++i) {
    ...
    if (ChanNote.Instrument != MAX_INSTRUMENTS &&
        ChanNote.Instrument != HOLD_INSTRUMENT && Note != HALT &&
        Note != NONE && Note != RELEASE) { // // //
      if (!pTrackerChannel->IsInstrumentCompatible(
              ChanNote.Instrument,
              m_pDocument->GetInstrumentType(ChanNote.Instrument))) { // // //
        CString str;
        str.Format(_T("Error: Missing or incompatible instrument (on row %i, ")
                   _T("channel %i, pattern %i)\n"),
                   i, Channel, Pattern);
        Print(str);
      }
    }
    ...
    if (SpaceInfo.SpaceCount > 2) {
      if (SpaceInfo.SpaceSize != m_iCurrentDefaultDuration &&
          SpaceInfo.SpaceCount != 0xFF) {
      ...
      if (m_iCurrentDefaultDuration != 0xFF &&
        ...
        WriteData(Command(CMD_RESET_DURATION));
                                           ...
                                           /*
                                                           if (SpaceInfo.SpaceCount > 2 && SpaceInfo.SpaceSize !=
          CurrentDefaultDuration) {    CurrentDefaultDuration =
          SpaceInfo.SpaceSize;    WriteData(CMD_SET_DURATION);
                                              CurrentDefaultDuration) {                                        CurrentDefaultDuration = SpaceInfo.SpaceSize;
                                                                   WriteData(CMD_SET_DURATION);
                                                                   WriteData(CurrentDefaultDuration);
                                                           }
                                                           else if (SpaceInfo.SpaceCount < 2 && SpaceInfo.SpaceSize ==
    ...
    if (Note != HALT && Note != RELEASE) { // // //
      ...
      if (Instrument == HOLD_INSTRUMENT &&
          ChanID != CHANID_DPCM) { // // // 050B
        WriteDuration();
    ...
    if (Note == 0) {
      ...
      if (ChanID == CHANID_DPCM) {
        ...
        if (LookUp > 0) {
          NESNote = LookUp - 1;
          if (auto pInstrument = std::dynamic_pointer_cast<CInstrument2A03>(
                  m_pDocument->GetInstrument(DPCMInst)))
            m_bDSamplesAccessed[pInstrument->GetSampleIndex(Octave, Note - 1) -
                                1] = true;
          // TODO: Print errors if incompatible or non-existing instrument is
          // found
        } else {
          NESNote = 0xFF; // Invalid sample, skip
          CString str;
          str.Format(_T("Error: Missing DPCM sample (on row %i, channel %i, ")
                     _T("pattern %i)\n"),
                     i, Channel, Pattern);
          Print(str);
        }
    ...
    for (int j = 0; j < EffColumns; ++j) {
      ...
      switch (Effect) {
      case EF_SPEED:
        if (EffParam >= m_pDocument->GetSpeedSplitPoint() &&
            m_pDocument->GetSongTempo(Track)) // // //
...
case EF_PORTAOFF:
      ...
      case EF_DUTY_CYCLE:
        ...
        } else if (ChanID != CHANID_TRIANGLE &&
                   ChanID != CHANID_DPCM) { // Not triangle and dpcm
          WriteData(Command(CMD_EFF_DUTY));
          WriteData(EffParam);
        }
        break;
    ...
    if (NESNote == 0xFF) {
      if (Action) {
        // A instrument/effect command was issued but no new note, write rest
        // command
        WriteData(0);
      }
      AccumulateDuration();
...
unsigned char CPatternCompiler::Command(int cmd) const {
  ...
  if (!bMultichip) { // // // truncate values if some chips do not exist
    if (!m_pDocument->ExpansionEnabled(SNDCHIP_N163) &&
        cmd > CMD_EFF_N163_WAVE_BUFFER)
      cmd -= sizeof(N163_EFFECTS);
    // MMC5
    if (!m_pDocument->ExpansionEnabled(SNDCHIP_FDS) &&
        cmd > CMD_EFF_FDS_MOD_BIAS)
      cmd -= sizeof(FDS_EFFECTS);
    if (!m_pDocument->ExpansionEnabled(SNDCHIP_VRC7) &&
        cmd > CMD_EFF_VRC7_WRITE)
      cmd -= sizeof(VRC7_EFFECTS) + 1;
    // VRC6
  }
...
unsigned int CPatternCompiler::FindSample(int Instrument, int Octave,
                                          int Key) const {
  return (*m_pDPCMList)[Instrument][Octave][Key - 1];
}

void CPatternCompiler::ScanNoteLengths(stSpacingInfo &Info, int Track,
                                       unsigned int StartRow, int Pattern,
                                       int Channel) {
  stChanNote NoteData;
  ...
  for (unsigned i = StartRow; i < m_pDocument->GetPatternLength(Track); ++i) {
    ...
    if (NoteData.Note > 0)
      NoteUsed = true;
    else if (NoteData.Instrument < MAX_INSTRUMENTS ||
             NoteData.Instrument == HOLD_INSTRUMENT) // // //
      NoteUsed = true;
    else if (NoteData.Vol < MAX_VOLUME)
      NoteUsed = true;
    else
      for (unsigned j = 0, Count = m_pDocument->GetEffColumns(Track, Channel);
           j <= Count; ++j)
...
void CPatternCompiler::AccumulateDuration() { ++m_iDuration; }
...
// Returns the size of the block at 'position' in the data array. A block is
// terminated by a note
...
void CPatternCompiler::OptimizeString() {
  ...
  */
  ...
  for (i = 0; i < m_vData.size(); /*i += 2*/) {
    ...
    // Compress
    if ((best_matches > 1 && best_length > 4) ||
        best_matches > 2 /*&& (best_length > 2 && best_matches > 1)*/) {
      ...
      //
      std::copy(m_vData.begin() + i, m_vData.begin() + i + best_length,
                m_vCompressedData.end());
      // Define a loop point: 0xFF (number of loops) (number of bytes)
      m_vCompressedData.push_back(Command(CMD_LOOP_POINT));
      m_vCompressedData.push_back(best_matches -
                                  1); // the nsf code sees one less
      m_vCompressedData.push_back(best_length);
      i += size;
    } else {
      // No loop
      int size = GetBlockSize(i);
      std::copy(m_vData.begin() + i, m_vData.begin() + i + size,
                m_vCompressedData.end());
      i += size;
    }
  }
...
unsigned int CPatternCompiler::GetHash() const { return m_iHash; }
...
const std::vector<char> &CPatternCompiler::GetData() const { return m_vData; }

const std::vector<char> &CPatternCompiler::GetCompressedData() const {
  return m_vCompressedData;
}

unsigned int CPatternCompiler::GetDataSize() const { return m_vData.size(); }
