...
CCursorPos::CCursorPos()
    : m_iRow(0), m_iChannel(0), m_iColumn(C_NOTE), m_iFrame(0) // // //
{}
...
bool CCursorPos::operator<(const CCursorPos &other) const // // //
{
  return m_iFrame < other.m_iFrame ||
         (m_iFrame == other.m_iFrame && m_iRow < other.m_iRow);
}
...
int CSelection::GetRowStart() const {
  ...
  return (m_cpEnd.m_iRow > m_cpStart.m_iRow ? m_cpStart.m_iRow
                                            : m_cpEnd.m_iRow);
}
...
int CSelection::GetRowEnd() const {
  ...
  return (m_cpEnd.m_iRow > m_cpStart.m_iRow ? m_cpEnd.m_iRow
                                            : m_cpStart.m_iRow);
}
...
cursor_column_t CSelection::GetColStart() const {
  ...
  if (m_cpStart.m_iChannel == m_cpEnd.m_iChannel)
    Col = (m_cpEnd.m_iColumn > m_cpStart.m_iColumn ? m_cpStart.m_iColumn
                                                   : m_cpEnd.m_iColumn);
...
cursor_column_t CSelection::GetColEnd() const {
  cursor_column_t Col = C_NOTE;
  if (m_cpStart.m_iChannel == m_cpEnd.m_iChannel)
    Col = (m_cpEnd.m_iColumn > m_cpStart.m_iColumn ? m_cpEnd.m_iColumn
                                                   : m_cpStart.m_iColumn);
  else if (m_cpEnd.m_iChannel > m_cpStart.m_iChannel)
    Col = m_cpEnd.m_iColumn;
  else
...
int CSelection::GetChanStart() const {
  return (m_cpEnd.m_iChannel > m_cpStart.m_iChannel) ? m_cpStart.m_iChannel
                                                     : m_cpEnd.m_iChannel;
}

int CSelection::GetChanEnd() const {
  return (m_cpEnd.m_iChannel > m_cpStart.m_iChannel) ? m_cpEnd.m_iChannel
                                                     : m_cpStart.m_iChannel;
}

int CSelection::GetFrameStart() const // // //
{
  return (m_cpEnd.m_iFrame > m_cpStart.m_iFrame) ? m_cpStart.m_iFrame
                                                 : m_cpEnd.m_iFrame;
}

int CSelection::GetFrameEnd() const // // //
{
  return (m_cpEnd.m_iFrame > m_cpStart.m_iFrame) ? m_cpEnd.m_iFrame
                                                 : m_cpStart.m_iFrame;
}
...
bool CSelection::IsColumnSelected(column_t Column, int Channel) const {
  ...
  return (Channel > GetChanStart() ||
          (Channel == GetChanStart() && Column >= SelStart)) // // //
         && (Channel < GetChanEnd() ||
             (Channel == GetChanEnd() && Column <= SelEnd));
}

void CSelection::Normalize(CCursorPos &Begin, CCursorPos &End) const // // //
{
  CCursorPos Temp{GetRowStart(), GetChanStart(), GetColStart(),
                  GetFrameStart()};
  std::swap(End,
            CCursorPos{GetRowEnd(), GetChanEnd(), GetColEnd(), GetFrameEnd()});
  std::swap(Begin, Temp);
}
...
CPatternClipData::CPatternClipData() { memset(&ClipInfo, 0, sizeof(ClipInfo)); }
...
CPatternClipData::~CPatternClipData() { SAFE_RELEASE_ARRAY(pPattern); }
...
CPatternIterator::CPatternIterator(const CPatternIterator &it)
    : m_iTrack(it.m_iTrack), m_pDocument(it.m_pDocument),
      CCursorPos(static_cast<const CCursorPos &>(it)) {}

CPatternIterator::CPatternIterator(CFamiTrackerDoc *pDoc, int Track,
...
CPatternIterator::FromCursor(const CCursorPos &Pos, CFamiTrackerDoc *const pDoc,
                             int Track) {
  return std::make_pair(CPatternIterator{pDoc, Track, Pos},
                        CPatternIterator{pDoc, Track, Pos});
}

std::pair<CPatternIterator, CPatternIterator>
CPatternIterator::FromSelection(const CSelection &Sel,
                                CFamiTrackerDoc *const pDoc, int Track) {
  CCursorPos it, end;
  Sel.Normalize(it, end);
  return std::make_pair(CPatternIterator{pDoc, Track, it},
...
void CPatternIterator::Get(int Channel, stChanNote *pNote) const {
  ...
  if (Frame < 0)
    Frame += m_pDocument->GetFrameCount(m_iTrack);
...
CPatternIterator &CPatternIterator::operator++() { return operator+=(1); }
...
{
        // Calculate pattern layout. Must be called when layout or window size
has changed
