...
CPatternEditorState::CPatternEditorState(const CPatternEditor *pEditor,
                                         int Track)
    : Track(Track), Cursor(pEditor->GetCursor()),
...
void CPatternEditorState::ApplyState(CPatternEditor *pEditor) const {
  ...
  IsSelecting ? pEditor->SetSelection(OriginalSelection)
...
// // // for note writes
#define STATE_EXPAND(st)                                                       \
  (st)->Track, (st)->Cursor.m_iFrame, (st)->Cursor.m_iChannel,                 \
      (st)->Cursor.m_iRow

CPatternAction::CPatternAction(int iAction)
    : CAction(iAction), m_pUndoState(nullptr), m_pRedoState(nullptr),
      m_pClipData(NULL), m_pUndoClipData(NULL),
...
void CPatternAction::SetDragAndDrop(const CPatternClipData *pClipData,
                                    bool bDelete, bool bMix,
                                        ...
                                        CSelection &Sel) // // //
{
  ...
  if ((m_iPastePos == PASTE_SELECTION || m_iPastePos == PASTE_FILL) &&
      !m_bSelecting)
  ...
  case PASTE_FILL:
    ...
    End.m_iColumn = GetCursorEndColumn(
        !((End.m_iChannel - Start.m_iChannel + 1) %
          m_pClipData->ClipInfo.Channels)
            ? m_pClipData->ClipInfo.EndColumn
            : static_cast<column_t>(COLUMN_EFF1 +
                                    CFamiTrackerDoc::GetDoc()->GetEffColumns(
                                        m_pUndoState->Track, End.m_iChannel)));
    break;
  case PASTE_DRAG:
  ...
  int OFFS = 3 * (GetSelectColumn(m_pUndoState->Cursor.m_iColumn) -
                  m_pClipData->ClipInfo.StartColumn);
  if (static_cast<int>(EFBEGIN - Start.m_iColumn) > OFFS)
    OFFS = EFBEGIN - Start.m_iColumn;
  if (Start.m_iChannel == End.m_iChannel && Start.m_iColumn >= EFBEGIN &&
      End.m_iColumn >= EFBEGIN) {
  ...
  if (Cond == SEL_CLEAN) {
    ...
    case SEL_REPEATED_ROW:
      Confirm = AfxMessageBox(IDS_PASTE_REPEATED_ROW,
    ...
    case SEL_TERMINAL_SKIP:
      if (!bOverflow)
        break;
      Confirm = AfxMessageBox(IDS_PASTE_NONTERMINAL,
                              MB_YESNO | MB_ICONEXCLAMATION | MB_DEFBUTTON2);
      break;
    }
    if (Confirm == IDYES) {
...
void CPatternAction::DeleteSelection(CMainFrame *pMainFrm,
                                     const CSelection &Sel) const // // //
{
  auto it = CPatternIterator::FromSelection(
      Sel,
...
bool CPatternAction::ValidateSelection(
    const CMainFrame *pMainFrm) const // // //
{
  if (!m_pUndoState->IsSelecting)
    return false;
  switch (static_cast<CFamiTrackerView *>(pMainFrm->GetActiveView())
              ->GetPatternEditor()
  ...
  case SEL_TERMINAL_SKIP:
    static_cast<CFrameWnd *>(AfxGetMainWnd())
        ->SetMessageText(IDS_SEL_TERMINAL_SKIP);
    break;
  }
...
void CPatternAction::UpdateView(CFamiTrackerDoc *pDoc) const // // //
{
  ...
  pDoc->UpdateAllViews(
      NULL, UPDATE_FRAME); // cursor might have moved to different channel
}

std::pair<CPatternIterator, CPatternIterator>
CPatternAction::GetIterators(const CMainFrame *pMainFrm) const {
  auto pDoc =
      static_cast<CFamiTrackerView *>(pMainFrm->GetActiveView())->GetDocument();
  return m_pUndoState->IsSelecting
             ? CPatternIterator::FromSelection(m_pUndoState->Selection, pDoc,
                                               m_pUndoState->Track)
             : CPatternIterator::FromCursor(m_pUndoState->Cursor, pDoc,
                                            m_pUndoState->Track);
}
...
bool CPatternAction::SaveState(const CMainFrame *pMainFrm) {
  CFamiTrackerView *pView =
      static_cast<CFamiTrackerView *>(pMainFrm->GetActiveView());
...
void CPatternAction::SaveUndoState(const CMainFrame *pMainFrm) // // //
{
  ...
  CFamiTrackerView *pView =
      static_cast<CFamiTrackerView *>(pMainFrm->GetActiveView());
  const CPatternEditor *pPatternEditor =
      pView->GetPatternEditor(); // TODO: remove

  SAFE_RELEASE(m_pUndoState);
  m_pUndoState =
      new CPatternEditorState{pPatternEditor, pMainFrm->GetSelectedTrack()};
...
void CPatternAction::SaveRedoState(const CMainFrame *pMainFrm) // // //
{
  CFamiTrackerView *pView =
      static_cast<CFamiTrackerView *>(pMainFrm->GetActiveView());
  SAFE_RELEASE(m_pRedoState);
  m_pRedoState = new CPatternEditorState{pView->GetPatternEditor(),
                                         pMainFrm->GetSelectedTrack()};
  ...
  case ACT_DRAG_AND_DROP:
    ...
    if (m_bDragDelete)
      pPatternEditor->PasteRaw(m_pAuxiliaryClipData,
                               m_selection.GetNormalized().m_cpStart);
    break;
#ifdef _DEBUG
  default:
...
CPSelectionAction::~CPSelectionAction() { SAFE_RELEASE(m_pUndoClipData); }
...
CPActionReplaceNote::CPActionReplaceNote(const stChanNote &Note, int Frame,
                                         int Row, int Channel)
    : CPatternAction(ACT_REPLACE_NOTE), m_NewNote(Note), m_iFrame(Frame),
      m_iRow(Row), m_iChannel(Channel) {}

bool CPActionReplaceNote::SaveState(const CMainFrame *pMainFrm) {
  const CFamiTrackerDoc *pDoc =
      static_cast<CFamiTrackerView *>(pMainFrm->GetActiveView())->GetDocument();
  pDoc->GetNoteData(m_pUndoState->Track, m_iFrame, m_iChannel, m_iRow,
                    &m_OldNote);
  return true;
}
...
void CPActionReplaceNote::Undo(CMainFrame *pMainFrm) const {
  CFamiTrackerDoc *pDoc =
      static_cast<CFamiTrackerView *>(pMainFrm->GetActiveView())->GetDocument();
  pDoc->SetNoteData(m_pUndoState->Track, m_iFrame, m_iChannel, m_iRow,
                    &m_OldNote);
}
...
CPActionInsertRow::CPActionInsertRow() : CPatternAction(ACT_INSERT_ROW) {}

bool CPActionInsertRow::SaveState(const CMainFrame *pMainFrm) {
  CFamiTrackerDoc *pDoc =
      static_cast<CFamiTrackerView *>(pMainFrm->GetActiveView())->GetDocument();
  pDoc->GetNoteData(m_pUndoState->Track, m_pUndoState->Cursor.m_iFrame,
                    m_pUndoState->Cursor.m_iChannel,
...
void CPActionInsertRow::Undo(CMainFrame *pMainFrm) const {
  ...
  pDoc->SetNoteData(m_pUndoState->Track, m_pUndoState->Cursor.m_iFrame,
                    m_pUndoState->Cursor.m_iChannel,
                    pDoc->GetPatternLength(m_pUndoState->Track) - 1,
                    &m_OldNote);
}
...
bool CPActionDeleteRow::SaveState(const CMainFrame *pMainFrm) {
  ...
  const CFamiTrackerDoc *pDoc =
      static_cast<CFamiTrackerView *>(pMainFrm->GetActiveView())->GetDocument();
  pDoc->GetNoteData(STATE_EXPAND(m_pUndoState) - (m_bBack ? 1 : 0),
                    &m_OldNote); // bad
  return true;
}
...
void CPActionDeleteRow::Redo(CMainFrame *pMainFrm) const {
  CFamiTrackerDoc *pDoc =
      static_cast<CFamiTrackerView *>(pMainFrm->GetActiveView())->GetDocument();
  pDoc->ClearRowField(STATE_EXPAND(m_pUndoState) - (m_bBack ? 1 : 0),
                      m_pUndoState->Cursor.m_iColumn);
  if (m_bPullUp)
...
bool CPActionScrollField::SaveState(const CMainFrame *pMainFrm) {
  ...
  case C_INSTRUMENT2:
    return m_OldNote.Instrument < MAX_INSTRUMENTS &&
           m_OldNote.Instrument != HOLD_INSTRUMENT; // // // 050B
  case C_VOLUME:
...
void CPActionScrollField::Redo(CMainFrame *pMainFrm) const {
  ...
  case C_INSTRUMENT1:
  case C_INSTRUMENT2:
    ScrollFunc(Note.Instrument, MAX_INSTRUMENTS);
    break;
  case C_VOLUME:
    ScrollFunc(Note.Vol, MAX_VOLUME);
    break;
  case C_EFF1_NUM:
  case C_EFF1_PARAM1:
  case C_EFF1_PARAM2:
    ScrollFunc(Note.EffParam[0], 0x100);
    break;
  case C_EFF2_NUM:
  case C_EFF2_PARAM1:
  case C_EFF2_PARAM2:
    ScrollFunc(Note.EffParam[1], 0x100);
    break;
  case C_EFF3_NUM:
  case C_EFF3_PARAM1:
  case C_EFF3_PARAM2:
    ScrollFunc(Note.EffParam[2], 0x100);
    break;
  case C_EFF4_NUM:
  case C_EFF4_PARAM1:
  case C_EFF4_PARAM2:
    ScrollFunc(Note.EffParam[3], 0x100);
    break;
  }
...
CPActionClearSel::CPActionClearSel() : CPSelectionAction(ACT_EDIT_DELETE) {}
...
CPActionDeleteAtSel::CPActionDeleteAtSel()
    : CPatternAction(ACT_EDIT_DELETE_ROWS), m_pUndoHead(nullptr),
      m_pUndoTail(nullptr) {}
...
bool CPActionDeleteAtSel::SaveState(const CMainFrame *pMainFrm) {
  ...
  const CPatternEditor *pPatternEditor =
      static_cast<CFamiTrackerView *>(pMainFrm->GetActiveView())
          ->GetPatternEditor();
  m_cpTailPos = CCursorPos{m_pUndoState->Selection.m_cpEnd.m_iRow + 1,
  ...
  int Length =
      pPatternEditor->GetCurrentPatternLength(m_cpTailPos.m_iFrame) - 1;
  if (m_cpTailPos.m_iRow <= Length)
    m_pUndoTail = pPatternEditor->CopyRaw(CSelection{
        m_cpTailPos,
        CCursorPos{Length, m_pUndoState->Selection.m_cpEnd.m_iChannel,
...
void CPActionDeleteAtSel::Redo(CMainFrame *pMainFrm) const {
  ...
  Sel.m_cpEnd.m_iRow =
      pPatternEditor->GetCurrentPatternLength(Sel.m_cpEnd.m_iFrame) - 1;
...
CPActionInsertAtSel::CPActionInsertAtSel()
    : CPatternAction(ACT_INSERT_SEL_ROWS), m_pUndoHead(nullptr),
      m_pUndoTail(nullptr) {}
...
bool CPActionInsertAtSel::SaveState(const CMainFrame *pMainFrm) {
  ...
  const CPatternEditor *pPatternEditor =
      static_cast<CFamiTrackerView *>(pMainFrm->GetActiveView())
          ->GetPatternEditor();
  m_cpTailPos = CCursorPos{pPatternEditor->GetCurrentPatternLength(
                               m_pUndoState->Selection.m_cpEnd.m_iFrame) -
                               1,
                           m_pUndoState->Selection.m_cpStart.m_iChannel,
                           m_pUndoState->Selection.m_cpStart.m_iColumn,
                           m_pUndoState->Selection.m_cpEnd.m_iFrame};
  CCursorPos HeadEnd{
      m_cpTailPos.m_iRow, m_pUndoState->Selection.m_cpEnd.m_iChannel,
      m_pUndoState->Selection.m_cpEnd.m_iColumn, m_cpTailPos.m_iFrame};

  m_pUndoTail =
      pPatternEditor->CopyRaw(CSelection{m_cpTailPos, CCursorPos{HeadEnd}});
  if (--HeadEnd.m_iRow < 0) {
    --HeadEnd.m_iFrame;
    HeadEnd.m_iRow += pPatternEditor->GetCurrentPatternLength(HeadEnd.m_iFrame);
  }
  if (m_pUndoState->Selection.m_cpStart <= HeadEnd) {
    m_pUndoHead = pPatternEditor->CopyRaw(
        CSelection{m_pUndoState->Selection.m_cpStart, HeadEnd});
    m_cpHeadPos = m_pUndoState->Selection.m_cpStart;
    if (++m_cpHeadPos.m_iRow >=
        pPatternEditor->GetCurrentPatternLength(m_cpHeadPos.m_iFrame)) {
...
void CPActionTranspose::Redo(CMainFrame *pMainFrm) const {
  ...
  int ChanStart = (m_pUndoState->IsSelecting ? m_pUndoState->Selection.m_cpStart
                                             : m_pUndoState->Cursor)
                      .m_iChannel;
  int ChanEnd = (m_pUndoState->IsSelecting ? m_pUndoState->Selection.m_cpEnd
                                           : m_pUndoState->Cursor)
                    .m_iChannel;
  column_t ColStart = GetSelectColumn((m_pUndoState->IsSelecting
                                           ? m_pUndoState->Selection.m_cpStart
                                           : m_pUndoState->Cursor)
                                          .m_iColumn);
  column_t ColEnd = GetSelectColumn((m_pUndoState->IsSelecting
                                         ? m_pUndoState->Selection.m_cpEnd
                                         : m_pUndoState->Cursor)
                                        .m_iColumn);
  ...
  do {
    ...
    for (int i = ChanStart; i <= ChanEnd; ++i) {
      ...
      if (Note.Note == ECHO) {
        ...
        int NewNote =
            MIDI_NOTE(Note.Octave, Note.Note) + AMOUNT[m_iTransposeMode];
        if (NewNote < 0)
          NewNote = 0;
        if (NewNote >= NOTE_COUNT)
          NewNote = NOTE_COUNT - 1;
...
void CPActionInterpolate::Redo(CMainFrame *pMainFrm) const {
  ...
  for (int i = Sel.m_cpStart.m_iChannel; i <= Sel.m_cpEnd.m_iChannel; ++i) {
    const int Columns =
        pDoc->GetEffColumns(m_pUndoState->Track, i) + 4; // // //
    for (int j = 0; j < Columns; ++j) {
      if (!Sel.IsColumnSelected(static_cast<column_t>(j), i))
        continue;
      ...
      case COLUMN_NOTE:
        ...
        break;
      case COLUMN_INSTRUMENT:
        if (StartData.Instrument == MAX_INSTRUMENTS ||
            EndData.Instrument == MAX_INSTRUMENTS)
          continue;
        if (StartData.Instrument == HOLD_INSTRUMENT ||
            EndData.Instrument == HOLD_INSTRUMENT) // // // 050B
          continue;
      ...
      case COLUMN_EFF3:
      case COLUMN_EFF4:
        if (StartData.EffNumber[j - 3] == EF_NONE ||
            EndData.EffNumber[j - 3] == EF_NONE ||
...
CPActionReverse::CPActionReverse() : CPSelectionAction(ACT_REVERSE) {}
  ...
  while (it.first < it.second) {
    for (int c = Sel.m_cpStart.m_iChannel; c <= Sel.m_cpEnd.m_iChannel; ++c) {
      ...
      if (c == Sel.m_cpStart.m_iChannel && ColStart > 0) { // // //
        Temp = NoteEnd;
        CopyNoteSection(&NoteEnd, &NoteBegin, PASTE_DEFAULT, COLUMN_NOTE,
                        static_cast<column_t>(ColStart - 1));
        CopyNoteSection(&NoteBegin, &Temp, PASTE_DEFAULT, COLUMN_NOTE,
                        static_cast<column_t>(ColStart - 1));
      }
      if (c == Sel.m_cpEnd.m_iChannel && ColEnd < COLUMN_EFF4) {
        Temp = NoteEnd;
        CopyNoteSection(&NoteEnd, &NoteBegin, PASTE_DEFAULT,
                        static_cast<column_t>(ColEnd + 1), COLUMN_EFF4);
        CopyNoteSection(&NoteBegin, &Temp, PASTE_DEFAULT,
                        static_cast<column_t>(ColEnd + 1), COLUMN_EFF4);
      }
...
bool CPActionReplaceInst::SaveState(const CMainFrame *pMainFrm) {
  if (!m_pUndoState->IsSelecting ||
      m_iInstrumentIndex > static_cast<unsigned>(MAX_INSTRUMENTS))
    return false;
  return CPSelectionAction::SaveState(pMainFrm);
}
...
void CPActionReplaceInst::Redo(CMainFrame *pMainFrm) const {
  ...
  const int cBegin =
      Sel.GetChanStart() +
      (Sel.IsColumnSelected(COLUMN_INSTRUMENT, Sel.GetChanStart()) ? 0 : 1);
  const int cEnd =
      Sel.GetChanEnd() -
      (Sel.IsColumnSelected(COLUMN_INSTRUMENT, Sel.GetChanEnd()) ? 0 : 1);
...
/*
// // // TODO: move stuff in CPatternAction::SetTargetSelection to the redo
state

CPActionDragDrop::CPActionDragDrop(const CPatternClipData *pClipData, bool
bDelete, bool bMix, const CSelection &pDragTarget) :
        CPatternAction(ACT_DRAG_AND_DROP),
        m_pClipData(pClipData), m_bDragDelete(bDelete), m_bDragMix(bMix),
m_dragTarget(pDragTarget)
{
}
...
static_cast<CFamiTrackerView*>(pMainFrm->GetActiveView())->GetPatternEditor();
        if (m_bDragDelete)
                m_pAuxiliaryClipData = pPatternEditor->CopyRaw();
        if (!SetTargetSelection(pPatternEditor, m_newSelection))
// // // return false; m_pUndoClipData = pPatternEditor->CopyRaw(); return true;
}
...
{
  const CPActionPatternLen *pAction =
      dynamic_cast<const CPActionPatternLen *>(Other);
  if (!pAction)
    return false;
  ...
  do {
    for (int i = Sel.m_cpStart.m_iChannel; i <= Sel.m_cpEnd.m_iChannel; ++i) {
      if (Offset < m_pUndoClipData->ClipInfo.Rows && m_iStretchMap[Pos] > 0)
        Source = *(
            m_pUndoClipData->GetPattern(i - Sel.m_cpStart.m_iChannel, Offset));
      else
        Source = stChanNote{}; // // //
    ...
    for (int i = 0; i < dist; ++i) {
      ...
      if (s.m_iRow <= oldRow)
        Offset +=
            pDoc->GetPatternLength(m_pUndoState->Track) + s.m_iRow - oldRow - 1;
    }
...
CPActionEffColumn::CPActionEffColumn(int Channel, int Count)
    : // // //
      CPatternAction(ACT_EFFECT_COLUMNS), m_iChannel(Channel),
      m_iNewColumns(Count) {}
