...
CDMCFileSoundDialog::CDMCFileSoundDialog(BOOL bOpenFileDialog,
                                         LPCTSTR lpszDefExt,
  ...
  QObject::connect(_qtd, SIGNAL(fileSelected(QString)), this,
                   SLOT(fileSelected(QString)));
}
...
void CDMCFileSoundDialog::OnFileNameChange() {
  // Preview DMC file
  if (!GetFileExt().CompareNoCase(_T("dmc")) &&
      theApp.GetSettings()->General.bWavePreview) {
    DWORD dwAttrib = GetFileAttributes(GetPathName());
    if (!(dwAttrib & FILE_ATTRIBUTE_DIRECTORY) &&
        GetPathName() != m_strLastFile) {
      ...
      theApp.GetSoundGenerator()->PreviewSample(pSample, 0,
...
CInstrumentEditorDPCM::CInstrumentEditorDPCM(CWnd *pParent)
    : CInstrumentEditPanel(CInstrumentEditorDPCM::IDD, pParent),
      m_pInstrument(NULL) {}
...
// ON_NOTIFY(NM_DBLCLK, IDC_SAMPLE_LIST,
// &CInstrumentEditorDPCM::OnNMDblclkSampleList) ON_BN_CLICKED(IDC_PREVIEW,
// &CInstrumentEditorDPCM::OnBnClickedPreview) ON_NOTIFY(NM_RCLICK,
// IDC_SAMPLE_LIST, &CInstrumentEditorDPCM::OnNMRClickSampleList)
...
// ON_EN_CHANGE(IDC_DELTA_COUNTER,
// &CInstrumentEditorDPCM::OnEnChangeDeltaCounter) ON_NOTIFY(UDN_DELTAPOS,
// IDC_DELTA_SPIN, &CInstrumentEditorDPCM::OnDeltaposDeltaSpin)
...
BOOL CInstrumentEditorDPCM::OnInitDialog() {
  ...
  pTableListCtrl->InsertColumn(0, _T("Key"), LVCFMT_LEFT,
                               static_cast<int>(.2 * Width));
  pTableListCtrl->InsertColumn(1, _T("Pitch"), LVCFMT_LEFT,
                               static_cast<int>(.23 * Width));
  pTableListCtrl->InsertColumn(2, _T("Sample"), LVCFMT_LEFT,
                               static_cast<int>(.57 * Width));
  pTableListCtrl->SendMessage(LVM_SETEXTENDEDLISTVIEWSTYLE, 0,
                              LVS_EX_FULLROWSELECT);

  CListCtrl *pSampleListCtrl =
      static_cast<CListCtrl *>(GetDlgItem(IDC_SAMPLE_LIST));
  pSampleListCtrl->GetClientRect(&r);
  Width = r.Width();
  pSampleListCtrl->DeleteAllItems();
  pSampleListCtrl->InsertColumn(0, _T("#"), LVCFMT_LEFT,
                                static_cast<int>(.2 * Width)); // // //
  pSampleListCtrl->InsertColumn(1, _T("Name"), LVCFMT_LEFT,
                                static_cast<int>(.55 * Width));
  pSampleListCtrl->InsertColumn(2, _T("Size"), LVCFMT_LEFT,
                                static_cast<int>(.25 * Width));
  pSampleListCtrl->SendMessage(LVM_SETEXTENDEDLISTVIEWSTYLE, 0,
                               LVS_EX_FULLROWSELECT);
  ...
  for (int i = 0; i < NOTE_COUNT; ++i) {
    text.Format(_T("%s%d"), stChanNote::NOTE_NAME[GET_NOTE(i) - 1],
                GET_OCTAVE(i));
  ...
  static_cast<CButton *>(GetDlgItem(IDC_ADD))
      ->SetIcon(::LoadIcon(AfxGetInstanceHandle(), MAKEINTRESOURCE(IDI_LEFT)));
  static_cast<CButton *>(GetDlgItem(IDC_REMOVE))
      ->SetIcon(::LoadIcon(AfxGetInstanceHandle(), MAKEINTRESOURCE(IDI_RIGHT)));
...
void CInstrumentEditorDPCM::BuildSampleList() {
  ...
  AfxFormatString3(Text, IDS_DPCM_SPACE_FORMAT, MakeIntString(Size / 0x400),
...
bool CInstrumentEditorDPCM::LoadSample(const CString &FilePath,
...
bool CInstrumentEditorDPCM::InsertSample(CDSample *pNewSample) {
  ...
  if ((Size + pNewSample->GetSize()) > MAX_SAMPLE_SPACE) {
    CString message;
    AfxFormatString1(message, IDS_OUT_OF_SAMPLEMEM_FORMAT,
                     MakeIntString(MAX_SAMPLE_SPACE / 1024));
...
void CInstrumentEditorDPCM::OnBnClickedLoad() {
  ...
  CDMCFileSoundDialog OpenFileDialog(TRUE, 0, 0,
                                     OFN_HIDEREADONLY | OFN_FILEMUSTEXIST |
                                         OFN_ALLOWMULTISELECT | OFN_EXPLORER,
                                     fileFilter);

  OpenFileDialog.m_pOFN->lpstrInitialDir =
      theApp.GetSettings()->GetPath(PATH_DMC);
  ...
  if (OpenFileDialog.GetFileName().GetLength() == 0) {
    theApp.GetSettings()->SetPath(OpenFileDialog.GetPathName() + _T("\\"),
                                  PATH_DMC); // // //
    // Multiple files
    ...
    while (Pos) {
      ...
      CString FileName =
          Path.Right(Path.GetLength() - Path.ReverseFind('\\') - 1);
...
void CInstrumentEditorDPCM::OnNMClickSampleList(NMHDR *pNMHDR,
                                                LRESULT *pResult) {
  CListCtrl *pSampleListCtrl =
      static_cast<CListCtrl *>(GetDlgItem(IDC_SAMPLE_LIST));
...
void CInstrumentEditorDPCM::OnNMClickTable(NMHDR *pNMHDR, LRESULT *pResult) {
  ...
  CSpinButtonCtrl *pSpinButton =
      static_cast<CSpinButtonCtrl *>(GetDlgItem(IDC_DELTA_SPIN));
  ...
  Text.Format(_T("%02i - %s"), Sample,
              (LPCTSTR)pTableListCtrl->GetItemText(
                  pTableListCtrl->GetSelectionMark(), 2));
...
void CInstrumentEditorDPCM::OnCbnSelchangeSamples() {
  ...
  if (Sample > 0) {
    ...
    if (PrevSample == 0)
      m_pInstrument->SetSamplePitch(m_iOctave, m_iSelectedKey,
                                    pPitchBox->GetCurSel());
  }
...
void CInstrumentEditorDPCM::OnBnClickedSave() {
  ...
  CString fileFilter = LoadDefaultFilter(IDS_FILTER_DMC, _T(".dmc"));
  CFileDialog SaveFileDialog(FALSE, _T("dmc"), (LPCSTR)pDSample->GetName(),
                             OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT,
                             fileFilter);

  SaveFileDialog.m_pOFN->lpstrInitialDir =
      theApp.GetSettings()->GetPath(PATH_DMC);
...
void CInstrumentEditorDPCM::SelectInstrument(
    std::shared_ptr<CInstrument> pInst) {
...
void CInstrumentEditorDPCM::OnBnClickedAdd() {
  ...
  if (GetDocument()->IsSampleUsed(m_iSelectedSample)) {
    m_pInstrument->SetSampleIndex(m_iOctave, m_iSelectedKey,
                                  m_iSelectedSample + 1);
    m_pInstrument->SetSamplePitch(m_iOctave, m_iSelectedKey, Pitch);
    UpdateCurrentKey();
  }
  ...
  if (m_iSelectedKey < NOTE_RANGE && m_iSelectedSample < MAX_DSAMPLES) {
    pSampleListCtrl->SetItemState(m_iSelectedSample, 0,
                                  LVIS_FOCUSED | LVIS_SELECTED);
    pTableListCtrl->SetItemState(m_iSelectedKey, 0,
                                 LVIS_FOCUSED | LVIS_SELECTED);
    ...
    pSampleListCtrl->SetItemState(m_iSelectedSample,
                                  LVIS_FOCUSED | LVIS_SELECTED,
                                  LVIS_FOCUSED | LVIS_SELECTED);
    pTableListCtrl->SetItemState(m_iSelectedKey, LVIS_FOCUSED | LVIS_SELECTED,
                                 LVIS_FOCUSED | LVIS_SELECTED);
  }
}
...
void CInstrumentEditorDPCM::OnEnChangeLoopPoint() {
  ...
  // CInstrumentEditPanel::OnInitDialog() function and call
...
void CInstrumentEditorDPCM::OnBnClickedEdit() {
  ...
  CSampleEditorDlg Editor(this,
                          new CDSample(*pSample)); // // // copy sample here
...
void CInstrumentEditorDPCM::OnNMDblclkSampleList(NMHDR *pNMHDR,
                                                 LRESULT *pResult) {
...
void CInstrumentEditorDPCM::OnNMRClickSampleList(NMHDR *pNMHDR,
                                                 LRESULT *pResult) {
...
void CInstrumentEditorDPCM::OnNMRClickTable(NMHDR *pNMHDR, LRESULT *pResult) {
  ...
  UINT Result = PopupMenu.TrackPopupMenu(TPM_RIGHTBUTTON | TPM_RETURNCMD,
...
void CInstrumentEditorDPCM::OnNMDblclkTable(NMHDR *pNMHDR, LRESULT *pResult) {
  ...
  if (int Sample =
          m_pInstrument->GetSampleIndex(m_iOctave, m_iSelectedKey)) { // // //
    ...
    if (pSample && pSample->GetSize() > 0U &&
        pTableListCtrl->GetItemText(MIDI_NOTE(m_iOctave, m_iSelectedKey + 1),
                                    2) != NO_SAMPLE_STR)
...
void CInstrumentEditorDPCM::OnDeltaposDeltaSpin(NMHDR *pNMHDR,
                                                LRESULT *pResult) {
