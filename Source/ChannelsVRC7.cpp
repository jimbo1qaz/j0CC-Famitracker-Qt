...
// True if custom instrument registers needs to be updated, shared among all
// channels
bool CChannelHandlerVRC7::m_bRegsDirty = false;
// Each bit represents that the custom patch register on that index has been
// updated
...
CChannelHandlerVRC7::CChannelHandlerVRC7()
    : CChannelHandlerInverted((1 << (VRC7_PITCH_RESOLUTION + 9)) - 1,
                              15), // // //
      m_iCommand(CMD_NONE), m_iTriggeredNote(0) {
...
void CChannelHandlerVRC7::HandleNoteData(stChanNote *pNoteData,
                                         int EffColumns) {
  CChannelHandlerInverted::HandleNoteData(pNoteData, EffColumns); // // //

  if (m_iCommand == CMD_NOTE_TRIGGER &&
      pNoteData->Instrument == HOLD_INSTRUMENT) // // // 050B
    m_iCommand = CMD_NOTE_ON;
}

bool CChannelHandlerVRC7::HandleEffect(effect_t EffNum,
  ...
  case EF_VRC7_WRITE: // // // 050B
    ...
    break;
  default:
    return CChannelHandlerInverted::HandleEffect(EffNum, EffParam);
  }
...
void CChannelHandlerVRC7::HandleNote(int Note, int Octave) {
  ...
  /*
          if ((m_iEffect != EF_PORTAMENTO || m_iPortaSpeed == 0) ||
                  m_iCommand == CMD_NOTE_HALT || m_iCommand == CMD_NOTE_RELEASE)
     // // // 050B m_iCommand = CMD_NOTE_TRIGGER;
  */
  if (m_iPortaSpeed > 0 && m_iEffect == EF_PORTAMENTO &&
      m_iCommand != CMD_NOTE_HALT &&
      m_iCommand != CMD_NOTE_RELEASE) // // // 050B
    CorrectOctave();
  else
...
int CChannelHandlerVRC7::TriggerNote(int Note) {
  ...
  return m_bLinearPitch ? (Note << LINEAR_PITCH_AMOUNT)
                        : GetFnum(Note); // // //
}

unsigned int CChannelHandlerVRC7::GetFnum(int Note) const {
  return m_pNoteLookupTable[Note % NOTE_RANGE]
         << VRC7_PITCH_RESOLUTION; // // //
}
...
int CChannelHandlerVRC7::CalculatePeriod() const {
  ...
  int Period =
      LimitPeriod(GetPeriod() + (Detune << VRC7_PITCH_RESOLUTION)); // // //
  if (m_bLinearPitch && m_pNoteLookupTable != nullptr) {
    ...
    int Offset = (GetFnum(Note + 1) << ((Note < NOTE_RANGE - 1) ? 0 : 1)) -
                 GetFnum(Note);
    Offset = Offset * Sub >> LINEAR_PITCH_AMOUNT;
    if (Sub && Offset < (1 << VRC7_PITCH_RESOLUTION))
      Offset = 1 << VRC7_PITCH_RESOLUTION;
...
void CVRC7Channel::RefreshChannel() {
  ...
  int Bnum =
      !m_bLinearPitch
          ? m_iOctave
          : ((GetPeriod() + GetVibrato() - GetFinePitch() - GetPitch()) >>
             LINEAR_PITCH_AMOUNT) /
                NOTE_RANGE;
  ...
  if (m_iCommand != CMD_NOTE_HALT) {
    // Select volume & patch
    RegWrite(0x30 + m_iChannel,
             (m_iDutyPeriod << 4) | (Volume ^ 0x0F)); // // //
  }
