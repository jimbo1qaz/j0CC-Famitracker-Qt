...
CChannelHandler::CChannelHandler(int MaxPeriod, int MaxVolume)
    : m_iChannelID(0), m_iInstTypeCurrent(INST_NONE), // // //
      m_iInstrument(0), m_pNoteLookupTable(NULL), m_pVibratoTable(NULL),
      m_pAPU(NULL), m_pInstHandler(), // // //
      m_iPitch(0), m_iNote(0), m_iInstVolume(0), m_iDefaultDuty(0),
      m_iDutyPeriod(0), m_iMaxPeriod(MaxPeriod), m_iMaxVolume(MaxVolume),
      m_bGate(false), m_bNewVibratoMode(false), m_bLinearPitch(false),
...
void CChannelHandler::InitChannel(CAPU *pAPU, int *pVibTable,
                                  CSoundGen *pSoundGen) {
...
void CChannelHandler::ResetChannel() {
  ...
  m_iDefaultVolume = (VOL_COLUMN_MAX >> VOL_COLUMN_SHIFT)
                     << VOL_COLUMN_SHIFT; // // //
...
CString CChannelHandler::GetStateString() // // //
{
  CString log = "";
  log.Format(_T("Inst.: "));
  if (m_iInstrument == MAX_INSTRUMENTS) // never happens because famitracker
                                        // will switch to selected inst
    log.Append("None");
  else
    log.AppendFormat(_T("%02X"), m_iInstrument);
  log.AppendFormat(_T("        Vol.: %X        Active effects:"),
                   m_iDefaultVolume >> VOL_COLUMN_SHIFT);
...
void CChannelHandler::ApplyChannelState(stChannelState *State) {
  ...
  m_iDefaultVolume = m_iVolume = (State->Volume == MAX_VOLUME)
                                     ? VOL_COLUMN_MAX
                                     : (State->Volume << VOL_COLUMN_SHIFT);
...
CString CChannelHandler::GetEffectString() const // // //
{
  ...
  if ((m_iDefaultDuty && m_iChannelID < CHANID_S5B_CH1) ||
      (m_iDefaultDuty != 0x40 && m_iChannelID >= CHANID_S5B_CH1))
  ...
  if (m_iNoteVolume > 0)
    str.AppendFormat(_T(" M%X%X"), m_iNoteVolume,
                     m_iNewVolume >> VOL_COLUMN_SHIFT);
  if (m_iNoteCut)
    str.AppendFormat(_T(" S%02X"), m_iNoteCut);
  if (m_iTranspose)
    str.AppendFormat(_T(" T%X%X"), m_iTranspose + (m_bTransposeDown ? 8 : 0),
                     m_iTransposeTarget);
...
CString CChannelHandler::GetSlideEffectString() const // // //
{
  ...
  case EF_ARPEGGIO:
    if (m_iEffectParam)
      str.AppendFormat(_T(" %c%02X"), EFF_CHAR[m_iEffect - 1], m_iEffectParam);
    break;
  case EF_PORTA_UP:
  case EF_PORTA_DOWN:
  case EF_PORTAMENTO:
    if (m_iPortaSpeed)
      str.AppendFormat(_T(" %c%02X"), EFF_CHAR[m_iEffect - 1], m_iPortaSpeed);
    break;
  }
...
void CChannelHandler::WriteEchoBuffer(stChanNote *NoteData, int Pos,
  ...
  case NONE:
    Value = ECHO_BUFFER_NONE;
    break;
  case HALT:
    Value = ECHO_BUFFER_HALT;
    break;
  case ECHO:
    Value = ECHO_BUFFER_ECHO + NoteData->Octave;
    break;
  default:
...
void CChannelHandler::HandleNoteData(stChanNote *pNoteData, int EffColumns) {
  ...
  bool Trigger = (pNoteData->Note != NONE) && (pNoteData->Note != HALT) &&
  ...
  if (pNoteData->Note == ECHO &&
      pNoteData->Octave <= ECHO_BUFFER_LENGTH) { // retrieve buffer
  ...
  for (int n = 0; n < EffColumns; n++) {
    ...
    if (EffNum == EF_VOLUME_SLIDE && !EffParam && Trigger &&
        m_iNoteVolume == 0) { // // //
      m_iVolume = m_iDefaultVolume;
      m_iNoteVolume = -1;
    }
  ...
  if (pNoteData->Note == HALT || pNoteData->Note == RELEASE) // // //
    Instrument =
        MAX_INSTRUMENTS; // Ignore instrument for release and halt commands
  ...
  bool NewInstrument =
      (m_iInstrument != LastInstrument && m_iInstrument != HOLD_INSTRUMENT) ||
  ...
  if (Trigger &&
      (m_iEffect == EF_SLIDE_DOWN || m_iEffect == EF_SLIDE_UP)) // // //
    SetupSlide();
...
bool CChannelHandler::HandleInstrument(bool Trigger,
                                       bool NewInstrument) // // //
{
  auto pDoc = m_pSoundGen->GetDocumentInterface();
  if (!pDoc)
    return false;
  std::shared_ptr<CInstrument> pInstrument =
      pDoc->GetInstrumentManager()->GetInstrument(m_iInstrument);
  if (!pInstrument)
    return false;
...
bool CChannelHandler::CreateInstHandler(inst_type_t Type) { return false; }
...
void CChannelHandler::ReleaseNote() {
  ...
  if (m_pInstHandler)
    m_pInstHandler->ReleaseInstrument(); // // //
  m_bRelease = true;
}
...
case EF_SLIDE_DOWN:
    ...
    */
  case EF_PORTA_DOWN:
    m_bLinearPitch ? PeriodRemove(m_iPortaSpeed)
                   : PeriodAdd(m_iPortaSpeed); // // //
    break;
  case EF_PORTA_UP:
    m_bLinearPitch ? PeriodAdd(m_iPortaSpeed)
                   : PeriodRemove(m_iPortaSpeed); // // //
    break;
  }
}
...
void CChannelHandler::ProcessChannel() {
  ...
  UpdateEffects();
  if (m_pInstHandler)
    m_pInstHandler->UpdateInstrument(); // // //
  // instruments are updated after running effects and before writing to sound
  // registers
}
...
int CChannelHandler::CalculatePeriod() const {
  ...
  if (m_bLinearPitch && m_pNoteLookupTable != nullptr) {
    ...
    int Offset = Note < NOTE_COUNT - 1
                     ? m_pNoteLookupTable[Note] - m_pNoteLookupTable[Note + 1]
                     : 0;
...
int CChannelHandler::CalculateVolume() const {
  // Volume calculation
  return LimitVolume((m_iInstVolume * (m_iVolume >> VOL_COLUMN_SHIFT)) / 15 -
                     GetTremolo()); // // //
}
...
int CChannelHandler::LimitVolume(int Volume) const // // //
{
  ...
  if (Volume == 0 && !theApp.GetSettings()->General.bCutVolume &&
      m_iInstVolume > 0 && m_iVolume > 0) // // //
    return 1;
  return Volume;
}

void CChannelHandler::AddCycles(int count) { m_pSoundGen->AddCycles(count); }
...
void CChannelHandler::SetPeriod(int Period) { m_iPeriod = LimitPeriod(Period); }

int CChannelHandler::GetPeriod() const { return m_iPeriod; }

void CChannelHandler::SetNote(int Note) { m_iNote = Note; }

int CChannelHandler::GetNote() const { return m_iNote; }

void CChannelHandler::SetVolume(int Volume) { m_iInstVolume = Volume; }

int CChannelHandler::GetVolume() const { return m_iInstVolume; }
...
int CChannelHandler::GetDutyPeriod() const { return m_iDutyPeriod; }

unsigned char CChannelHandler::GetArpParam() const {
  return m_iEffect == EF_ARPEGGIO ? m_iEffectParam : 0U;
}

bool CChannelHandler::IsActive() const { return m_bGate; }

bool CChannelHandler::IsReleasing() const { return m_bRelease; }
...
bool CChannelHandlerInverted::HandleEffect(effect_t EffNum,
                                           unsigned char EffParam) {
  if (!m_bLinearPitch)
    switch (EffNum) { // // //
    case EF_PORTA_UP:
      EffNum = EF_PORTA_DOWN;
      break;
    case EF_PORTA_DOWN:
      EffNum = EF_PORTA_UP;
      break;
...
int CChannelHandlerInverted::CalculatePeriod() const {
  int Period = LimitPeriod(GetPeriod() + GetVibrato() - GetFinePitch() -
                           GetPitch()); // // //
  if (m_bLinearPitch && m_pNoteLookupTable != nullptr) {
    int Note = Period >> LINEAR_PITCH_AMOUNT;
    int Sub = Period % (1 << LINEAR_PITCH_AMOUNT);
    int Offset = Note < NOTE_COUNT - 1
                     ? m_pNoteLookupTable[Note + 1] - m_pNoteLookupTable[Note]
                     : 0;
    Offset = Offset * Sub >> LINEAR_PITCH_AMOUNT;
    if (Sub && !Offset)
      Offset = 1;
...
CString CChannelHandlerInverted::GetSlideEffectString() const // // //
{
  ...
  case EF_PORTA_UP:
    if (m_iPortaSpeed)
      str.AppendFormat(_T(" %c%02X"), EFF_CHAR[EF_PORTA_DOWN - 1],
                       m_iPortaSpeed);
    break;
  case EF_PORTA_DOWN:
    if (m_iPortaSpeed)
      str.AppendFormat(_T(" %c%02X"), EFF_CHAR[EF_PORTA_UP - 1], m_iPortaSpeed);
    break;
  case EF_PORTAMENTO:
    if (m_iPortaSpeed)
      str.AppendFormat(_T(" %c%02X"), EFF_CHAR[m_iEffect - 1], m_iPortaSpeed);
