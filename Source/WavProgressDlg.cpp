...
CWavProgressDlg::CWavProgressDlg(CWnd *pParent /*=NULL*/)
    : CDialog(CWavProgressDlg::IDD, pParent), m_dwStartTime(0),
      m_iSongEndType(SONG_TIME_LIMIT), m_iSongEndParam(0), m_iTrack(0) {}
...
void CWavProgressDlg::BeginRender(const CString &File, render_end_t LengthType,
                                  int LengthParam, int Track) {
...
BOOL CWavProgressDlg::OnInitDialog() {
  ...
  CView *pView =
      static_cast<CFrameWnd *>(AfxGetMainWnd())->GetActiveView(); // // //
  CSoundGen *pSoundGen = theApp.GetSoundGenerator();
  ...
  if (!pSoundGen->RenderToFile(m_sFile.GetBuffer(), m_iSongEndType,
                               m_iSongEndParam, m_iTrack))
    EndDialog(0);
...
void CWavProgressDlg::OnTimer(UINT_PTR nIDEvent) {
  ...
  CProgressCtrl *pProgressBar =
      static_cast<CProgressCtrl *>(GetDlgItem(IDC_PROGRESS_BAR));
  CSoundGen *pSoundGen = theApp.GetSoundGenerator();
  ...
  bool Done;
  pSoundGen->GetRenderStat(Frame, RenderedTime, Done, FramesToRender, Row,
                           RowCount);
  ...
  case SONG_TIME_LIMIT:
    ...
    str1.Format(_T("%02i:%02i / %02i:%02i"), CurrMin, CurrSec, TotalMin,
                TotalSec);
