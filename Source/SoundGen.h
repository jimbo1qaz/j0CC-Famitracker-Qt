...
enum render_end_t { SONG_TIME_LIMIT, SONG_LOOP_LIMIT };
...
public:
  ...
  // Rendering
  bool RenderToFile(LPTSTR pFile, render_end_t SongEndType, int SongEndParam,
                    int Track);
  void StopRendering();
  void GetRenderStat(int &Frame, int &Time, bool &Done, int &FramesToRender,
                     int &Row, int &RowCount) const;
...
private:
  ...
  void FillBuffer(int16_t *pBuffer, uint32_t Size);
  bool PlayBuffer();
...
private:
  ...
  unsigned int
      m_iSpeedSplitPoint;    // Speed/tempo split point fetched from the module
