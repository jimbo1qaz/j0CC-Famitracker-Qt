...
extern void CopyNoteSection(stChanNote *Target, const stChanNote *Source,
                            paste_mode_t Mode, column_t Begin,
                            column_t End); // // //
...
public:
  ...
  bool IsInRange(const CSelection &sel, int Frame, int Row, int Channel,
  ...
  void DragPaste(const CPatternClipData *pClipData,
                 const CSelection *pDragTarget, bool bMix);

  // OLE support
  void BeginDrag(const CPatternClipData *pClipData);
  void EndDrag();
  bool PerformDrop(const CPatternClipData *pClipData, bool bCopy,
                   bool bCopyMix);
...
private:
  ...
  // // //
  void DrawCell(CDC *pDC, int PosX, cursor_column_t Column, int Channel,
                bool bInvert, stChanNote *pNoteData,
...
private:
  ...
  int m_iCenterRow; // The row in the middle of the editor, will always point to
                    // a valid row in current frame
  ...
  int m_iLinesVisible;     // Number of lines visible on screen (may include one
                           // incomplete line)
  int m_iLinesFullVisible; // Number of lines full visible on screen
  int m_iChannelsVisible;  // Number of channels visible on screen (may include
                           // one incomplete channel)
  ...
  cursor_column_t m_iColumns[MAX_CHANNELS]; // // // Cached *index of rightmost
                                            // column* in each channel
  ...
  // Mouse
  bool
      m_bMouseActive; // Indicates that mouse activity is in progess by the user
