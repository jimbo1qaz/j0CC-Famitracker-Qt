...
CConfigShortcuts::CConfigShortcuts()
    : CPropertyPage(CConfigShortcuts::IDD), m_bShift(false), m_bControl(false),
      m_bAlt(false), m_iSelectedItem(0),
...
BOOL CConfigShortcuts::OnInitDialog() {
  ...
  pListView->InsertColumn(0, _T("Action"), LVCFMT_LEFT,
                          static_cast<int>(.52 * w));
  pListView->InsertColumn(1, _T("Modifier"), LVCFMT_LEFT,
                          static_cast<int>(.23 * w));
...
void CConfigShortcuts::OnNMClickShortcuts(NMHDR *pNMHDR, LRESULT *pResult) {
  ...
  CString KeyString =
      AssembleKeyString(m_iMods[m_iSelectedItem], m_iKeys[m_iSelectedItem]);
...
BOOL CConfigShortcuts::OnApply() {
  ...
  for (int i = 0; i < CAccelerator::ACCEL_COUNT; ++i) {
    ...
    if (it == m.end())
      ...
      CString msg;
      msg.Format(_T("These two commands are assigned to the same shortcut ")
                 _T("(%s):\n- %s\n- %s"),
...
void CConfigShortcuts::SetupKey(int Key) {
  int Mod = (m_bShift ? MOD_SHIFT : 0) | (m_bControl ? MOD_CONTROL : 0) |
            (m_bAlt ? MOD_ALT : 0);
...
CString CConfigShortcuts::AssembleKeyString(int Mod, int Key) {
  ...
  if (Mod & MOD_CONTROL) {
    ...
    //		KeyStr.Append(_T(" + ")); CP: Name of Qt::ControlModifier
    // includes
    //'+'
  }
