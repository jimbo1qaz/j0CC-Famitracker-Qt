...
enum { TMR_PLAY_CURSOR, TMR_START_CURSOR };
...
BOOL CSampleEditorDlg::OnInitDialog() {
  ...
  if (!m_wndInfoStatusBar.Create(this) ||
      !m_wndInfoStatusBar.SetIndicators(
          indicators, sizeof(indicators) / sizeof(*indicators))) // // // 050B
    return FALSE;
...
void CSampleEditorDlg::OnBnClickedPlay() {
  ...
  m_pSoundGen->PreviewSample(m_pSample, m_pSampleEditorView->GetStartOffset(),
                             Pitch);
...
void CSampleEditorDlg::OnBnClickedDelete() {
  ...
  //	TRACE(_T("Removing selected part from sample, start: %i, end %i (diff:
  //%i)\n"), StartSample, EndSample, EndSample - StartSample);

  // Remove the selected part
  memcpy(m_pSample->GetData() + StartSample, m_pSample->GetData() + EndSample,
         m_pSample->GetSize() - EndSample);
...
void CSampleEditorDlg::OnBnClickedDeltastart() { UpdateSampleView(); }

void CSampleEditorDlg::UpdateSampleView() {
  m_pSampleEditorView->ExpandSample(
      m_pSample, IsDlgButtonChecked(IDC_DELTASTART) ? 64 : 0);
...
void CSampleEditorDlg::OnHScroll(UINT nSBCode, UINT nPos,
