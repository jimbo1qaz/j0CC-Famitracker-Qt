...
CSequenceEditor::CSequenceEditor()
    : CWnd(), // // //
      m_pGraphEditor(NULL), m_pSizeEditor(NULL), m_pSetting(NULL),
      m_pFont(NULL), m_iMaxVol(15), m_iMaxDuty(3), m_pParent(NULL),
      m_pSequence(NULL), m_iSelectedSetting(0), m_iInstrumentType(0) {}
...
BOOL CSequenceEditor::CreateEditor(CWnd *pParentWnd, const RECT &rect) {
  ...
  if (CWnd::CreateEx(WS_EX_STATICEDGE, NULL, _T(""), WS_CHILD | WS_VISIBLE,
                     rect, pParentWnd, 0) == -1)
    return -1;

  m_pFont = new CFont();
  m_pFont->CreateFont(-11, 0, 0, 0, FW_NORMAL, FALSE, FALSE, 0, DEFAULT_CHARSET,
                      OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY,
                      DEFAULT_PITCH, _T("Tahoma"));
  ...
  if (m_pSizeEditor->CreateEx(
          NULL, NULL, _T(""), WS_CHILD | WS_VISIBLE,
          CRect(34, GraphRect.bottom + 5, 94, GraphRect.bottom + 22), this,
          0) == -1)
    return -1;

  menuRect = CRect(GraphRect.right - 72, GraphRect.bottom + 5,
                   GraphRect.right - 2, GraphRect.bottom + 24);

  // Sequence settings editor
  m_pSetting = new CSequenceSetting(this);

  if (m_pSetting->CreateEx(NULL, NULL, _T(""), WS_CHILD | WS_VISIBLE, menuRect,
                           this, 0) == -1)
    return -1;
...
void CSequenceEditor::OnPaint() {
  ...
  float Rate; // // //
  Rate =
      static_cast<CInstrumentEditDlg *>(
          static_cast<CSequenceInstrumentEditPanel *>(m_pParent)->GetParent())
          ->GetRefreshRate();
  LengthStr.Format(_T("%.0f ms  "),
                   (1000.0f * m_pSizeEditor->GetValue()) / Rate);
...
LRESULT CSequenceEditor::OnCursorChange(WPARAM wParam, LPARAM lParam) {
  ...
  if (m_pConversion != nullptr) // // //
    Text.Format(_T("{%i, %s}        "), wParam,
                m_pConversion->ToString(static_cast<char>(lParam)).c_str());
  else
...
LRESULT CSequenceEditor::OnSettingChanged(WPARAM wParam, LPARAM lParam) // // //
{
  ...
  case SEQ_VOLUME: // // //
    if (m_iInstrumentType == INST_VRC6) {
      ...
      static_cast<CBarGraphEditor *>(m_pGraphEditor)
          ->SetMaxItems(
...
void CSequenceEditor::SequenceChangedMessage(bool Changed) {
  static_cast<CSequenceInstrumentEditPanel *>(m_pParent)->UpdateSequenceString(
      Changed); // // //
...
void CSequenceEditor::SelectSequence(CSequence *pSequence, int Type,
                                     int InstrumentType) {
  ...
  switch (Type) {
  case SEQ_VOLUME:
    if (m_iInstrumentType == INST_VRC6 && m_iSelectedSetting == SEQ_VOLUME &&
        pSequence->GetSetting() == SETTING_VOL_64_STEPS)
  ...
  if (m_pGraphEditor->CreateEx(NULL, NULL, _T(""), WS_CHILD, GraphRect, this,
                               0) == -1)
    return;
