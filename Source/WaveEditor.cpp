...
BOOL CWaveEditor::CreateEx(DWORD dwExStyle, LPCTSTR lpszClassName,
  ...
  if (CWnd::CreateEx(dwExStyle, lpszClassName, lpszWindowName, dwStyle, newRect,
                     pParentWnd, 0) == -1)
    return -1;
...
void CWaveEditor::OnPaint() {
  ...
  for (int i = 0; i < m_iLY; ++i) {
    dc.FillSolidRect(0, i * m_iSY, m_iLX * m_iSX, m_iSY,
                     (i & 1) ? GRAY1 : GRAY2);
  }
  ...
  if ((m_iLX * m_iSX) < rect.Width()) {
    dc.FillSolidRect(m_iLX * m_iSX, 0, rect.Width() - (m_iLX * m_iSX),
                     m_iLY * m_iSY, GRAY1);
  }
...
void CWaveEditor::EditWave(CPoint point) {
  ...
  if (!GetLineMode()) {
    ...
    if (pDC != NULL) {
      ...
      pDC->FillSolidRect(index * m_iSX, s * m_iSY, m_iSX, m_iSY,
                         (s & 1) ? 0xA0A0A0 : 0xB0B0B0);
...
int CWaveEditorFDS::GetMaxSamples() const { return CInstrumentFDS::WAVE_SIZE; }
...
void CWaveEditorN163::SetWave(int i) { m_iWaveIndex = i; }
