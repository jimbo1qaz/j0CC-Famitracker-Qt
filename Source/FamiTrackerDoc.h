...
public:
  ...
  //
  void ResetChannels();
  void RegisterChannel(CTrackerChannel *pChannel, int ChannelType,
                       int ChipType);
  ...
  unsigned int
  GetCurrentPatternLength(unsigned int Track,
                          int Frame) const; // // // moved from pattern editor

  unsigned int GetEffColumns(unsigned int Track, unsigned int Channel) const;
  void SetEffColumns(unsigned int Track, unsigned int Channel,
                     unsigned int Columns);

  unsigned int GetPatternAtFrame(unsigned int Track, unsigned int Frame,
                                 unsigned int Channel) const;
  void SetPatternAtFrame(unsigned int Track, unsigned int Frame,
                         unsigned int Channel, unsigned int Pattern);

  bool IsPatternEmpty(unsigned int Track, unsigned int Channel,
                      unsigned int Pattern) const;
  bool ArePatternsSame(unsigned int Track, unsigned int Channel,
  ...
  // Pattern editing
  void SetNoteData(unsigned int Track, unsigned int Frame, unsigned int Channel,
                   unsigned int Row, const stChanNote *pData);
  void GetNoteData(unsigned int Track, unsigned int Frame, unsigned int Channel,
                   unsigned int Row, stChanNote *pData) const;

  void SetDataAtPattern(unsigned int Track, unsigned int Pattern,
                        unsigned int Channel, unsigned int Row,
                        const stChanNote *pData);
  void GetDataAtPattern(unsigned int Track, unsigned int Pattern,
                        unsigned int Channel, unsigned int Row,
                        stChanNote *pData) const;

  void ClearPatterns(unsigned int Track);
  void ClearPattern(unsigned int Track, unsigned int Frame,
                    unsigned int Channel);

  void PopulateUniquePatterns(unsigned int Track); // // //

  bool InsertRow(unsigned int Track, unsigned int Frame, unsigned int Channel,
                 unsigned int Row);
  bool ClearRow(unsigned int Track, unsigned int Frame, unsigned int Channel,
                unsigned int Row);
  bool ClearRowField(unsigned int Track, unsigned int Frame,
                     unsigned int Channel, unsigned int Row,
                     cursor_column_t Column);
  bool RemoveNote(unsigned int Track, unsigned int Frame, unsigned int Channel,
                  unsigned int Row);
  bool PullUp(unsigned int Track, unsigned int Frame, unsigned int Channel,
              unsigned int Row);
  void CopyPattern(unsigned int Track, int Target, int Source, int Channel);

  void SwapChannels(unsigned int Track, unsigned int First,
                    unsigned int Second); // // //
  ...
  stHighlight GetHighlightAt(unsigned int Track, unsigned int Frame,
                             unsigned int Row) const; // // //
  unsigned int GetHighlightState(unsigned int Track, unsigned int Frame,
                                 unsigned int Row) const; // // //
  CBookmark *GetBookmarkAt(unsigned int Track, unsigned int Frame,
                           unsigned int Row) const; // // //
  ...
  void SetInstrumentName(unsigned int Index,
                         const char *pName); // Set the name of an instrument
  void GetInstrumentName(unsigned int Index,
                         char *pName) const; // Get the name of an instrument
  ...
  CSequence *GetSequence(inst_type_t InstType, unsigned int Index,
                         int Type) const; // // //
  unsigned int GetSequenceItemCount(inst_type_t InstType, unsigned int Index,
                                    int Type) const; // // //
  int GetFreeSequence(inst_type_t InstType, int Type,
                      CSeqInstrument *pInst = nullptr) const; // // //
  int GetSequenceCount(inst_type_t InstType, int Type) const; // // //

  // DPCM samples
  const CDSample *
  GetSample(unsigned int Index) const; // // // non-const getter removed
  ...
  // Other
  unsigned int ScanActualLength(unsigned int Track,
                                unsigned int Count) const;            // // //
  double GetStandardLength(int Track, unsigned int ExtraLoops) const; // // //
  unsigned int GetFirstFreePattern(unsigned int Track,
                                   unsigned int Channel) const; // // //
  ...
  stFullState *RetrieveSoundState(unsigned int Track, unsigned int Frame,
                                  unsigned int Row, int Channel); // // //

  // For file version compability
  static void ConvertSequence(stSequence *pOldSequence, CSequence *pNewSequence,
                              int Type);
...
private:
  ...
  bool WriteBlock_Parameters(CDocumentFile *pDocFile,
                             const int Version) const; // // // version
  ...
  bool WriteBlock_ChannelLayout(CDocumentFile *pDocFile,
                                const int Version) const;
  bool WriteBlock_SequencesVRC6(CDocumentFile *pDocFile,
                                const int Version) const;
  bool WriteBlock_SequencesN163(CDocumentFile *pDocFile,
                                const int Version) const;
  bool WriteBlock_SequencesS5B(CDocumentFile *pDocFile,
                               const int Version) const;
  // // //
  bool WriteBlock_ParamsExtra(CDocumentFile *pDocFile, const int Version) const;
  bool WriteBlock_DetuneTables(CDocumentFile *pDocFile,
                               const int Version) const;
  ...
  /*!	\brief Validates a given condition and throws an exception otherwise.
          \details This method replaces the previous ASSERT_FILE_DATA
     preprocessor macro. \param Cond The condition to check against. \param Msg
     The error message.
  */
  template <module_error_level_t l = MODULE_ERROR_DEFAULT>
  void AssertFileData(bool Cond, std::string Msg) const; // // //

  template <module_error_level_t l = MODULE_ERROR_DEFAULT, typename T,
...
template <class T> class CInstrumentContainer {
public:
  ...
  T *operator()() const { return dynamic_cast<T *>(m_pInstrument); }
