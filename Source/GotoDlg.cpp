...
CGotoDlg::CGotoDlg(CWnd *pParent /*=NULL*/) : CDialog(CGotoDlg::IDD, pParent) {}

CGotoDlg::~CGotoDlg() { SAFE_RELEASE(m_cChipEdit); }
...
BOOL CGotoDlg::OnInitDialog() {
  ...
  CFamiTrackerView *pView = static_cast<CFamiTrackerView *>(
...
void CGotoDlg::CheckDestination() const {
  ...
  if (m_iDestFrame >= pDoc->GetFrameCount(Track))
    Valid = false;
           ...
           static_cast<unsigned>(pDoc->GetFrameLength(Track, m_iDestFrame)))
    Valid = false;
