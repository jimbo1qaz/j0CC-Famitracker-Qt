...
const TCHAR *CInstrumentEditDlg::CHIP_NAMES[] = {
    _T(""),    _T("2A03"),  _T("VRC6"),   _T("VRC7"),
    _T("FDS"), _T("Namco"), _T("Sunsoft")};
...
CInstrumentEditDlg::CInstrumentEditDlg(CWnd *pParent /*=NULL*/)
    : CDialog(CInstrumentEditDlg::IDD, pParent), m_bOpened(false),
      m_fRefreshRate(60.0f),                     // // //
      m_iInstrument(-1), m_pInstManager(nullptr) // // //
{}
...
void CInstrumentEditDlg::InsertPane(CInstrumentEditPanel *pPanel, bool Show) {
  ...
  Rect.MoveToXY(ParentRect.left - Rect.left + DPI::SX(1),
                ParentRect.top - Rect.top + DPI::SY(21));
...
void CInstrumentEditDlg::SetCurrentInstrument(int Index) {
  ...
  if (InstType != m_iSelectedInstType) {
    ...
    case INST_2A03: {
      ...
      bool bShowDPCM = (Type == CHANID_DPCM) ||
                       (std::static_pointer_cast<CInstrument2A03>(pInstrument)
                            ->AssignedSamples());
      InsertPane(new CInstrumentEditorSeq(NULL, _T("2A03 settings"),
                                          CInstrument2A03::SEQUENCE_NAME, 15, 3,
                                          INST_2A03),
                 !bShowDPCM); // // //
      InsertPane(new CInstrumentEditorDPCM(), bShowDPCM);
    } break;
    case INST_VRC6:
      InsertPane(new CInstrumentEditorSeq(NULL, _T("Konami VRC6"),
                                          CInstrumentVRC6::SEQUENCE_NAME, 15, 7,
                                          INST_VRC6),
                 true);
      break;
    case INST_VRC7:
    ...
    case INST_N163:
      InsertPane(new CInstrumentEditorSeq(
                     NULL, _T("Envelopes"), CInstrumentN163::SEQUENCE_NAME, 15,
                     CInstrumentN163::MAX_WAVE_COUNT - 1, INST_N163),
                 true);
      InsertPane(new CInstrumentEditorN163Wave(), false);
      break;
    case INST_S5B:
      InsertPane(new CInstrumentEditorSeq(NULL, _T("Sunsoft 5B"),
                                          CInstrumentS5B::SEQUENCE_NAME, 15,
                                          255, INST_S5B),
                 true);
      break;
    }
...
void CInstrumentEditDlg::OnTcnSelchangeInstTab(NMHDR *pNMHDR,
                                               LRESULT *pResult) {
  CTabCtrl *pTabControl = static_cast<CTabCtrl *>(GetDlgItem(IDC_INST_TAB));
...
/*!
        \brief Helper class to automatically select the previous drawing object
...
struct CDCObjectContext // // // TODO: put it somewhere else, maybe Graphics.h
{
  ...
  ~CDCObjectContext() { _dc.SelectObject(_obj); }
...
void CInstrumentEditDlg::OnPaint() {
  ...
  for (int j = 0; j < 8; j++) {
    ...
    for (int i = 0; i < 2; i++) {
      bool Selected =
          (Note == BLACK_1[i]) && (Octave == j) && m_iActiveKey != -1;
    ...
    for (int i = 0; i < 3; i++) {
      bool Selected =
          (Note == BLACK_2[i]) && (Octave == j) && m_iActiveKey != -1;
      BlackKey.SelectObject(Selected ? BlackKeyMarkBmp : BlackKeyBmp);
  ...
  dc.BitBlt(m_KeyboardRect.left, m_KeyboardRect.top, KEYBOARD_WIDTH,
            KEYBOARD_HEIGHT, &BackDC, 0, 0, SRCCOPY); // // //
}
...
void CInstrumentEditDlg::SwitchOnNote(int x, int y) {
  ...
  CFamiTrackerDoc *pDoc = static_cast<CFamiTrackerDoc *>(
      static_cast<CFrameWnd *>(GetParent())->GetActiveDocument());
  ...
  // // // Send to respective channels whenever cursor is outside instrument
  // chip
  if (m_iSelectedInstType == INST_2A03) {
    ...
    case INST_VRC6:
      First = CHANID_VRC6_PULSE1;
      break;
    case INST_N163:
      First = CHANID_N163_CH1;
      break;
    case INST_FDS:
      First = CHANID_FDS;
      break;
    case INST_VRC7:
      First = CHANID_VRC7_CH1;
      break;
    case INST_S5B:
      First = CHANID_S5B_CH1;
      break;
    }
...
void CInstrumentEditDlg::OnCancel() { DestroyWindow(); }
...
bool CInstrumentEditDlg::IsOpened() const { return m_bOpened; }
