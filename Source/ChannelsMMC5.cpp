...
void CChannelHandlerMMC5::HandleNoteData(stChanNote *pNoteData,
                                         int EffColumns) {
  // // //
...
bool CChannelHandlerMMC5::HandleEffect(effect_t EffNum,
...
void CChannelHandlerMMC5::HandleCut() { CutNote(); }
...
void CChannelHandlerMMC5::RefreshChannel() // // //
{
  ...
  if (m_bGate) // // //
    WriteRegister(Offs, (DutyCycle << 6) | (m_bEnvelopeLoop << 5) |
...
void CChannelHandlerMMC5::ClearRegisters() {
  unsigned char Offs =
      0x5000 + 4 * (m_iChannelID - CHANID_MMC5_SQUARE1); // // //
  WriteRegister(Offs, 0x30);
