...
LPCTSTR CAccelerator::MOD_NAMES[] = {
    _T("None"),  _T("Alt"),       _T("Ctrl"),       _T("Ctrl+Alt"),
    _T("Shift"), _T("Shift+Alt"), _T("Shift+Ctrl"), _T("Shift+Ctrl+Alt")};
...
const std::vector<stAccelEntry> CAccelerator::DEFAULT_TABLE{
    ...
    {_T("Play from row marker"), MOD_CONTROL, VK_F7, ID_TRACKER_PLAY_MARKER,
     _T("Play from bookmark")}, // // // 050B
    ...
    {_T("Set row marker"), MOD_CONTROL, 'B', ID_TRACKER_SET_MARKER,
     _T("Set row marker")}, // // // 050B
    ...
    {_T("Toggle chip"), MOD_CONTROL | MOD_ALT, VK_F9,
     ID_TRACKER_TOGGLECHIP}, // // //
    {_T("Solo chip"), MOD_CONTROL | MOD_ALT, VK_F10,
     ID_TRACKER_SOLOCHIP},                                              // // //
    ...
    {_T("Previous bookmark"), MOD_CONTROL, VK_PRIOR,
     ID_BOOKMARKS_PREVIOUS}, // // //
    {_T("Transpose, decrease notes"), MOD_CONTROL, VK_F1,
     ID_TRANSPOSE_DECREASENOTE},
    {_T("Transpose, increase notes"), MOD_CONTROL, VK_F2,
     ID_TRANSPOSE_INCREASENOTE},
    {_T("Transpose, decrease octaves"), MOD_CONTROL, VK_F3,
     ID_TRANSPOSE_DECREASEOCTAVE},
    {_T("Transpose, increase octaves"), MOD_CONTROL, VK_F4,
     ID_TRANSPOSE_INCREASEOCTAVE},
    ...
    {_T("Decrease step size"), MOD_CONTROL, VK_SUBTRACT,
     ID_CMD_DECREASESTEPSIZE},
    ...
    {_T("Clone frame"), MOD_NONE, 0, ID_MODULE_DUPLICATEFRAMEPATTERNS,
     _T("Duplicate patterns")}, // // //
    {_T("Clone pattern"), MOD_ALT, 'D', ID_MODULE_DUPLICATECURRENTPATTERN,
     _T("Duplicate current pattern")}, // // //
    {_T("Decrease pattern values"), MOD_SHIFT, VK_F1, ID_DECREASEVALUES},
    {_T("Increase pattern values"), MOD_SHIFT, VK_F2, ID_INCREASEVALUES},
    {_T("Coarse decrease values"), MOD_SHIFT, VK_F3,
     ID_DECREASEVALUESCOARSE}, // // //
    {_T("Coarse increase values"), MOD_SHIFT, VK_F4,
     ID_INCREASEVALUESCOARSE}, // // //
    {_T("Toggle find / replace tab"), MOD_CONTROL, 'F',
     ID_EDIT_FIND_TOGGLE},                                              // // //
...
static BYTE GetMod(int Mod) {
  return ((Mod & MOD_CONTROL) ? FCONTROL : 0) |
         ((Mod & MOD_SHIFT) ? FSHIFT : 0) | ((Mod & MOD_ALT) ? FALT : 0);
}
...
CAccelerator::~CAccelerator() { ASSERT(m_hAccel == nullptr); }
  ...
  for (const auto &x : m_pEntriesTable) { // // //
    if (x.id == id) {
      ...
      if (KeyName.GetLength() > 1)
        KeyName = KeyName.Mid(0, 1).MakeUpper() +
                  KeyName.Mid(1, KeyName.GetLength() - 1).MakeLower();
      if (x.mod > 0)
        str.Format(_T("\t%s+%s"), MOD_NAMES[x.mod], (LPCTSTR)KeyName);
                   (LPCTSTR)KeyName);
      else
...
void CAccelerator::LoadShortcuts(CSettings *pSettings) {
  ...
  /*
          ...
          3. HKCU/SOFTWARE/FamiTracker, original key (using
     stAccelEntry::orig_name)
          4. default value
  */
