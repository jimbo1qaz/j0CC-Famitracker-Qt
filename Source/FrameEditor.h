...
public:
  CFrameEditorDropTarget(CFrameEditor *pParent)
      : m_iClipboard(0), m_nDropEffect(DROPEFFECT_NONE),
        m_bCopyNewPatterns(false), m_pParent(pParent){};
...
public:
  ...
  void MoveSelection(unsigned int Track, const CFrameSelection &Sel,
  ...
  void PasteAt(unsigned int Track, const CFrameClipData *pClipData,
               const CFrameCursorPos &Pos); // // //
  void PasteInsert(unsigned int Track, int Frame,
                   const CFrameClipData *pClipData); // // //
  void PasteNew(unsigned int Track, int Frame,
                const CFrameClipData *pClipData); // // //
