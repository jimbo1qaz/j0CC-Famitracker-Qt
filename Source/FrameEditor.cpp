...
CFrameEditor::CFrameEditor(CMainFrame *pMainFrm)
    : m_pMainFrame(pMainFrm), m_pDocument(NULL), m_pView(NULL), m_iClipboard(0),
      m_iWinWidth(0), m_iWinHeight(0), m_iHiglightLine(0), m_iFirstChannel(0),
      m_iCursorPos(0), m_iRowsVisible(0), m_iMiddleRow(0),
      m_bInputEnable(false), m_bCursor(false), m_bInvalidated(false),
      m_iLastCursorFrame(0), m_iLastCursorChannel(0), m_iLastPlayFrame(0),
      m_hAccel(0), m_bSelecting(false), m_bStartDrag(false),
      m_bDeletedRows(false), m_iDragRow(0), m_bLastRow(false), // // //
      m_iTopScrollArea(0), m_iBottomScrollArea(0), m_DropTarget(this),
      m_iDragThresholdX(0), m_iDragThresholdY(0), m_iUpdates(0), m_iPaints(0) {}
...
//	ON_UPDATE_COMMAND_UI(ID_FRAME_PASTEOVERWRITE,
// OnUpdateEditPasteOverwrite)
...
int CFrameEditor::OnCreate(LPCREATESTRUCT lpCreateStruct) {
  ...
  m_hAccel =
      LoadAccelerators(AfxGetInstanceHandle(), MAKEINTRESOURCE(IDR_FRAMEWND));

  m_Font.CreateFont(
      DPI::SY(14), 0, 0, 0, FW_DONTCARE, FALSE, FALSE, FALSE, ANSI_CHARSET,
...
void CFrameEditor::DrawFrameEditor(CDC *pDC) {
  ...
  const COLORREF ColDragCursor =
      INTENSITY(ColBackground) > 0x80 ? 0x000000 : 0xFFFFFF;
  const COLORREF ColSelectEdge = BLEND(ColSelect, 0xFFFFFF, 70);
  const COLORREF ColTextDimmed =
      DIM(theApp.GetSettings()->Appearance.iColPatternText, 90);
  ...
  GradientBar(
      &m_dcBack,
      DPI::Rect(0, m_iMiddleRow * ROW_HEIGHT + 3, m_iWinWidth, ROW_HEIGHT + 1),
      RowColor, ColBackground);
  ...
  if (FirstVisibleFrame + End >= FrameCount)
    End =
        Start + FrameCount - ((FirstVisibleFrame > 0) ? FirstVisibleFrame : 0);

  // Draw rows
  CBookmarkCollection *pCol =
      m_pDocument->GetBookmarkManager()->GetCollection(Track); // // //
  for (int i = Start; i <= End; ++i) {
    CRect RowRect =
        DPI::Rect(0, i * ROW_HEIGHT + 4, m_iWinWidth, ROW_HEIGHT - 1); // // //
      ...
      if (const unsigned Count = pCol->GetCount())
        for (unsigned j = 0; j < Count; ++j)
          if (pCol->GetBookmark(j)->m_iFrame == Frame) {
            GradientBar(&m_dcBack, RowRect,
                        theApp.GetSettings()->Appearance.iColBackgroundHilite,
                        ColBackground);
            break;
          }
    ...
    if (Frame == pView->GetMarkerFrame())
      GradientBar(&m_dcBack,
                  DPI::Rect(2, i * ROW_HEIGHT + 4, ROW_COLUMN_WIDTH - 5,
                            ROW_HEIGHT - 1),
                  ColCursor, DIM(ColCursor, 30));

    // Play cursor
    if (PlayFrame == Frame && !pView->GetFollowMode() && theApp.IsPlaying())
      GradientBar(&m_dcBack, RowRect,
                  theApp.GetSettings()->Appearance.iColCurrentRowPlaying,
                  ColBackground); // // //
    ...
    bool bSelectedRow =
        m_bSelecting && (Frame >= SelectStart) && (Frame <= SelectEnd);

    // Selection
    if (bSelectedRow) {
      CRect RowRect = DPI::Rect(
          ROW_COLUMN_WIDTH + FRAME_ITEM_WIDTH * CBegin, i * ROW_HEIGHT + 3,
      ...
      if (Frame == SelectStart)
        m_dcBack.FillSolidRect(RowRect.left, RowRect.top, RowRect.Width(), 1,
                               ColSelectEdge);
      if (Frame == SelectEnd)
        m_dcBack.FillSolidRect(RowRect.left, RowRect.bottom - 1,
                               RowRect.Width(), 1, ColSelectEdge);
      m_dcBack.FillSolidRect(RowRect.left, RowRect.top, 1, RowRect.Height(),
                             ColSelectEdge); // // //
      m_dcBack.FillSolidRect(RowRect.right - 1, RowRect.top, 1,
                             RowRect.Height(), ColSelectEdge); // // //
    }
    ...
    if (i == m_iMiddleRow) {
      ...
      GradientBar(&m_dcBack,
                  DPI::Rect(ROW_COLUMN_WIDTH + 2 + x, y, FRAME_ITEM_WIDTH,
                            ROW_HEIGHT + 1),
                  ColCursor, ColBackground);
      m_dcBack.Draw3dRect(DPI::Rect(ROW_COLUMN_WIDTH + 2 + x, y,
                                    FRAME_ITEM_WIDTH, ROW_HEIGHT + 1),
                          BLEND(ColCursor, 0xFFFFFF, 90),
                          BLEND(ColCursor, ColBackground, 60));

      if (m_bInputEnable && m_bCursor) {
        // Flashing black box indicating that input is active
        m_dcBack.FillSolidRect(
            DPI::Rect(ROW_COLUMN_WIDTH + 4 + x + CURSOR_WIDTH * m_iCursorPos,
                      y + 2, CURSOR_WIDTH, ROW_HEIGHT - 3),
            ColBackground);
      }
    }
    ...
    if (i == End) {
      ...
      m_dcBack.TextOut(DPI::SX(3 + FRAME_ITEM_WIDTH / 2),
                       DPI::SY(i * ROW_HEIGHT + 3), _T(">>"));

      COLORREF CurrentColor = (i == m_iHiglightLine || m_iHiglightLine == -1)
                                  ? ColText
                                  : ColTextDimmed;
      ...
      for (int j = 0; j < ChannelCount; ++j) {
        ...
        m_dcBack.DrawText(
            ...
            DPI::Rect(28 + j * FRAME_ITEM_WIDTH + FRAME_ITEM_WIDTH / 2,
                      i * ROW_HEIGHT + 3, FRAME_ITEM_WIDTH - 2, 20),
            DT_LEFT | DT_TOP | DT_NOCLIP);
      }
    } else {
      m_dcBack.SetTextColor(ColTextHilite);
      m_dcBack.TextOut(DPI::SX(3 + FRAME_ITEM_WIDTH / 2),
                       DPI::SY(i * ROW_HEIGHT + 3),
                       MakeIntString(Frame, ROW_FORMAT));
      ...
      for (int j = 0; j < ChannelCount; ++j) {
        ...
        if (!m_bLastRow &&
                pDoc->GetPatternAtFrame(Track, Frame, Chan) ==
                    pDoc->GetPatternAtFrame(Track, ActiveFrame, Chan) ||
            bSelectedRow)
          m_dcBack.SetTextColor(CurrentColor);
        else
          m_dcBack.SetTextColor(DIM(CurrentColor, 70));
        ...
        m_dcBack.DrawText(
            MakeIntString(pDoc->GetPatternAtFrame(Track, Frame, Chan),
                          _T("%02X")), // // //
            DPI::Rect(28 + j * FRAME_ITEM_WIDTH + FRAME_ITEM_WIDTH / 2,
                      i * ROW_HEIGHT + 3, FRAME_ITEM_WIDTH - 2, 20),
            DT_LEFT | DT_TOP | DT_NOCLIP);
  ...
  if (m_DropTarget.IsDragging()) {
    ...
    if (!m_bSelecting ||
        (m_iDragRow <= SelectStart || m_iDragRow >= (SelectEnd + 1))) {
      if (m_iDragRow >= FirstVisibleFrame &&
          m_iDragRow <= FirstVisibleFrame + m_iRowsVisible) {
        int x = ROW_COLUMN_WIDTH + CBegin * FRAME_ITEM_WIDTH + 2; // // //
        int y = m_iDragRow - FirstVisibleFrame;
        m_dcBack.FillSolidRect(
            DPI::Rect(x, y * ROW_HEIGHT + 3,
                      FRAME_ITEM_WIDTH * (CEnd - CBegin + 1) + 1, 2),
            ColDragCursor);
      }
    }
  }
  ...
  m_dcBack.FillSolidRect(DPI::SX(ROW_COLUMN_WIDTH - 1), 0, DPI::SY(1),
                         m_iWinHeight, colSeparator);
  ...
  if (m_bInputEnable) {
    ...
    case VK_END:
      if (bShift && !m_bLastRow) { // // //
        if (!m_bSelecting) {
          m_bSelecting = true;
          m_selection.m_cpStart.m_iFrame = m_selection.m_cpEnd.m_iFrame =
              Frame; // // //
    ...
    if (Num != -1) {
      if (IsSelecting()) { // // //
        ...
        if (m_iCursorPos == 0) {
          ...
          Pattern = m_pDocument->GetPatternAtFrame(
                        Track, GetSelection().GetFrameStart(),
                        GetSelection().GetChanStart()) |
                    Num;
          m_iCursorPos = 0;
        }
        ...
        int Pattern = m_bLastRow ? 0
                                 : m_pDocument->GetPatternAtFrame(
                                       Track, Frame, Channel); // // //
        if (m_iCursorPos == 0)
        ...
        if (m_bLastRow)
          m_pMainFrame->AddAction(
              new CFActionFrameCount{static_cast<int>(FrameCount) + 1});
        CAction *pAction = m_pMainFrame->ChangeAllPatterns()
                               ? (CAction *)new CFActionSetPatternAll{Pattern}
                               : new CFActionSetPattern{Pattern};
        m_pMainFrame->AddAction(pAction);
        m_pDocument->SetModifiedFlag();

        const int SelectedChannel =
            (m_pView->GetSelectedChannel() + 1) %
            m_pDocument->GetAvailableChannels(); // // //
...
int CFrameEditor::GetRowFromPoint(const CPoint &point, bool DropTarget) const {
  ...
  int Delta =
      ((point.y - TOP_OFFSET) / DPI::SY(ROW_HEIGHT)) - m_iMiddleRow; // // //
...
void CFrameEditor::OnLButtonDown(UINT nFlags, CPoint point) {
  ...
  if (m_bSelecting) {
    int SelectStart = m_selection.GetFrameStart(),
        SelectEnd = m_selection.GetFrameEnd(); // // //
    int ChanStart = m_selection.GetChanStart(),
        ChanEnd = m_selection.GetChanEnd(); // // //

    if (Row < SelectStart || Row > SelectEnd || Chan < ChanStart ||
        Chan > ChanEnd) { // // //
      if (nFlags & MK_SHIFT) {
...
void CFrameEditor::OnLButtonUp(UINT nFlags, CPoint point) {
  ...
  if (m_bSelecting) {
    ...
    if ((nFlags & MK_CONTROL) && theApp.IsPlaying()) {
      ...
      // Switch to frame
      SetEditFrame(GetRowFromPoint(
          point, GetFocus() == this)); // // // allow one-past-the-end
      if (Channel >= 0) {
        if (Channel >= m_pDocument->GetChannelCount())
          Channel = m_pDocument->GetChannelCount() - 1;
        m_pView->SelectChannel(Channel); // // //
      }
    }
  }
...
void CFrameEditor::OnMouseMove(UINT nFlags, CPoint point) {
  if (nFlags & MK_LBUTTON) {
    if (!m_bSelecting) {
      if (abs(m_ButtonPoint.x - point.x) > m_iDragThresholdX ||
          abs(m_ButtonPoint.y - point.y) > m_iDragThresholdY) {
        m_bSelecting = true;
        EnableInput(); // // //
      }
    }
    if (m_bStartDrag) {
      ...
      if (m_bFullFrameSelect) { // // //
        ...
        if (m_selection.m_cpEnd.m_iChannel < 0)
          m_selection.m_cpEnd.m_iChannel = 0;
      }
      InvalidateFrameData();
      Invalidate();
    }
...
void CFrameEditor::OnLButtonDblClk(UINT nFlags, CPoint point) {
  ...
  const int NewFrame =
      GetRowFromPoint(point, true); // // // allow one-past-the-end
...
void CFrameEditor::AutoScroll(const CPoint &point) {
  ...
  const int Rows =
      m_pDocument->GetFrameCount(m_pMainFrame->GetSelectedTrack()) + 1;
...
void CFrameEditor::OnContextMenu(CWnd * /*pWnd*/, CPoint point) {
  ...
  pPopupMenu->EnableMenuItem(
      ID_FRAME_PASTE,
      MF_BYCOMMAND | (ClipboardAvailable ? MF_ENABLED : MF_DISABLED));
  pPopupMenu->EnableMenuItem(
      ID_FRAME_PASTENEWPATTERNS,
      MF_BYCOMMAND | (ClipboardAvailable ? MF_ENABLED : MF_DISABLED));
...
void CFrameEditor::OnEditPaste() {
  ...
  m_pMainFrame->AddAction(
      new CFActionPaste{pClipData, GetEditFrame(), false}); // // //
}
...
void CFrameEditor::OnEditPasteNewPatterns() {
  ...
  m_pMainFrame->AddAction(
      new CFActionPaste{pClipData, GetEditFrame(), true}); // // //
}

void CFrameEditor::OnEditDelete() {
  if (!m_bSelecting) {
    m_selection.m_cpStart.m_iFrame = m_selection.m_cpEnd.m_iFrame =
        m_pView->GetSelectedFrame();      // // //
    m_selection.m_cpStart.m_iChannel = 0; // // //
    m_selection.m_cpEnd.m_iChannel =
        m_pDocument->GetChannelCount() - 1; // // //
  }
...
{
  return CFrameCursorPos{static_cast<int>(m_pView->GetSelectedFrame()),
                         static_cast<int>(m_pView->GetSelectedChannel())};
}

std::pair<CFrameIterator, CFrameIterator>
CFrameEditor::GetIterators() const // // //
{
  int Track = m_pMainFrame->GetSelectedTrack();
  return m_bSelecting
...
{
  return m_bSelecting ? Copy(m_selection)
                      : CopyFrame(m_pView->GetSelectedFrame());
}
...
void CFrameEditor::PasteInsert(unsigned int Track, int Frame,
                               const CFrameClipData *pClipData) // // //
{
  ...
  for (int f = 0; f < Frames; ++f) {
    ...
    for (int c = it.m_iChannel + Channels,
             Count = m_pDocument->GetChannelCount();
         c < Count; ++c)
      it.Set(c, 0);
    ++it;
  }
...
void CFrameEditor::PasteAt(unsigned int Track, const CFrameClipData *pClipData,
                           const CFrameCursorPos &Pos) // // //
{
  ...
  for (int f = 0; f < pClipData->ClipInfo.Frames; ++f) {
    for (int c = 0; c < pClipData->ClipInfo.Channels; ++c)
      it.Set(c + /*it.m_iChannel*/ pClipData->ClipInfo.FirstChannel,
             pClipData->GetFrame(f, c));
    ++it;
    if (it.m_iFrame == 0)
      break;
  }
...
void CFrameEditor::PasteNew(unsigned int Track, int Frame,
                            const CFrameClipData *pClipData) // // //
{
  ...
  Sel.m_cpStart.m_iChannel = pClipData->ClipInfo.FirstChannel;
  Sel.m_cpEnd.m_iChannel =
      Sel.m_cpStart.m_iChannel + pClipData->ClipInfo.Channels - 1;
...
void CFrameEditor::ClonePatterns(unsigned int Track,
                                 const CFrameSelection &_Sel) // // //
{
  ...
  while (true) {
    for (int c = it.first.m_iChannel; c <= it.second.m_iChannel; ++c) {
      ...
      m_pDocument->SetPatternAtFrame(Track, it.first.m_iFrame, c,
                                     NewPatterns[Index]);
    }
...
void CFrameEditor::ClearPatterns(unsigned int Track,
...
bool CFrameEditor::InputEnabled() const { return m_bInputEnable; }
...
{
  CFrameSelection Sel;
  Sel.m_cpEnd.m_iFrame =
      m_pDocument->GetFrameCount(m_pMainFrame->GetSelectedTrack()) - 1;
  Sel.m_cpStart.m_iChannel = Sel.m_cpEnd.m_iChannel =
      m_pView->GetSelectedChannel();
  SetSelection(Sel);
}
...
void CFrameEditor::InitiateDrag() {
  ...
  const int Channels = m_selection.GetChanEnd() - ChanStart +
                       1; // // // m_pDocument->GetChannelCount();
  ...
  for (int i = 0; i < Rows; ++i)
    for (int j = 0; j < Channels; ++j)
      ClipData.SetFrame(i, j,
                        m_pDocument->GetPatternAtFrame(Track, i + SelectStart,
                                                       j + ChanStart));
  ...
  if (hMem != NULL) {
    ...
    DROPEFFECT res =
        pSrc->DoDragDrop(DROPEFFECT_COPY | DROPEFFECT_MOVE); // calls DropData
...
bool CFrameEditor::IsCopyValid(COleDataObject *pDataObject) const {
  ...
  return (m_pDocument->GetFrameCount(m_pMainFrame->GetSelectedTrack()) +
          Frames) <= MAX_FRAMES;
}
...
BOOL CFrameEditor::DropData(COleDataObject *pDataObject,
  ...
  case DROPEFFECT_MOVE:
    ...
    if (m_bStartDrag) { // // // same window
      ...
      m_pMainFrame->AddAction(
          new CFActionDropMove{pClipData, m_iDragRow}); // // //
      break;
    }
    // [[fallthrough]]
  case DROPEFFECT_COPY:
    // Copy
    if (!m_pMainFrame->AddAction(
            new CFActionPaste{pClipData, m_iDragRow,
                              m_DropTarget.CopyToNewPatterns()})) { // // //
      SAFE_RELEASE(pClipData);
      return FALSE;
    }
    break;
...
void CFrameEditor::MoveSelection(unsigned int Track, const CFrameSelection &Sel,
                                 const CFrameCursorPos &Target) // // //
{
  if (Target.m_iFrame == Sel.GetFrameStart())
    return;
  ...
  if (Delta > 0) {
    ...
    Delta -= Frames;
    PasteAt(Track, pData.get(),
            {Normal.m_cpStart.m_iFrame + Delta, Normal.m_cpStart.m_iChannel});
  } else {
...
DROPEFFECT CFrameEditorDropTarget::OnDragEnter(CWnd *pWnd,
...
DROPEFFECT CFrameEditorDropTarget::OnDragOver(CWnd *pWnd,
                                              COleDataObject *pDataObject,
                                              DWORD dwKeyState, CPoint point) {
  m_pParent->UpdateDrag(point);
  return m_nDropEffect;
...
BOOL CFrameEditorDropTarget::OnDrop(CWnd *pWnd, COleDataObject *pDataObject,
