...
const CString MODULE_ERROR_DESC[] = {
    ...
    _T("Default: Perform the usual error checking according to the most ")
    _T("recent official stable build."),
    _T("Official: Perform extra bounds checking to ensure that modules are ")
    _T("openable in the official build."),
    _T("Strict: Validate all modules so that they do not contain any illegal "
       "data. "
...
CConfigVersion::CConfigVersion()
    : CPropertyPage(CConfigVersion::IDD), m_cComboVersion(nullptr),
      m_cSliderErrorLevel(nullptr) {}
...
void CConfigVersion::UpdateInfo() {
  GetDlgItem(IDC_STATIC_VERSION_ERROR)
      ->SetWindowText(MODULE_ERROR_DESC[m_iModuleErrorLevel]);
}

BEGIN_MESSAGE_MAP(CConfigVersion, CPropertyPage)
ON_WM_VSCROLL()
ON_WM_HSCROLL()
ON_NOTIFY(NM_CUSTOMDRAW, IDC_SLIDER_VERSION_ERRORLEVEL,
          OnNMCustomdrawSliderVersionErrorlevel)
END_MESSAGE_MAP()
...
BOOL CConfigVersion::OnInitDialog() {
  ...
  m_cComboVersion->SetCurSel(m_cComboVersion->GetCount() -
                             1); // TODO: add to registry
...
void CConfigVersion::OnVScroll(UINT nSBCode, UINT nPos,
                               CScrollBar *pScrollBar) {
  UpdateInfo();
  CPropertyPage::OnVScroll(nSBCode, nPos, pScrollBar);
}

void CConfigVersion::OnHScroll(UINT nSBCode, UINT nPos,
                               CScrollBar *pScrollBar) {
  UpdateInfo();
  CPropertyPage::OnHScroll(nSBCode, nPos, pScrollBar);
}

void CConfigVersion::OnNMCustomdrawSliderVersionErrorlevel(NMHDR *pNMHDR,
                                                           LRESULT *pResult) {
