...
#define ASSEMBLE_PARAM(Status, Byte1, Byte2)                                   \
  ((Status) | ((Byte1) << 8) | ((Byte2) << 16))

// Static stuff

CMIDI *CMIDI::m_pInstance = NULL;

// void CALLBACK CMIDI::MidiInProc(HMIDIIN hMidiIn, UINT wMsg, DWORD dwInstance,
...
void CALLBACK CMIDI::RtMidiInProc(double timeStamp,
...
CMIDI::CMIDI()
    : m_bInStarted(false), m_iInDevice(0), m_iOutDevice(0), m_iQueueHead(0),
      m_iQueueTail(0), m_hMIDIIn(NULL), m_hMIDIOut(NULL), m_iTimingCounter(0) {
...
bool CMIDI::OpenDevices(void) {
  ...
  if (m_iInDevice != 0) {
    ...
    //		Result = midiInOpen(&m_hMIDIIn, m_iInDevice - 1,
  ...
  if (m_iOutDevice != 0) {
    ...
    //		Result = midiOutOpen(&m_hMIDIOut, m_iOutDevice - 1, NULL, 0,
    // CALLBACK_NULL);
    ...
    //		midiOutShortMsg(m_hMIDIOut, (MIDI_MSG_PROGRAM_CHANGE << 4 |
    // 0x00) | (1
    //<< 8));
...
int CMIDI::GetNumInputDevices() const { return midiin->getPortCount(); }

int CMIDI::GetNumOutputDevices() const { return midiout->getPortCount(); }
...
void CMIDI::Enqueue(unsigned char MsgType, unsigned char MsgChannel,
...
void CMIDI::Event(unsigned char Status, unsigned char Data1,
                  unsigned char Data2) {
...
bool CMIDI::ReadMessage(unsigned char &Message, unsigned char &Channel,
                        unsigned char &Data1, unsigned char &Data2) {
  bool Result = false;
...
int CMIDI::GetQuantization() const { return m_iQuant; }
...
void CMIDI::WriteNote(unsigned char Channel, unsigned char Note,
                      unsigned char Octave, unsigned char Velocity) {
...
bool CMIDI::IsOpened() const { return m_bInStarted; }

bool CMIDI::IsAvailable() const { return m_hMIDIIn != NULL; }

int CMIDI::GetInputDevice() const { return m_iInDevice; }

int CMIDI::GetOutputDevice() const { return m_iOutDevice; }
