...
// The instrument file tree, used in the instrument toolbar to quickly load an
// instrument
...
CInstrumentFileTree::CInstrumentFileTree()
    : m_pRootMenu(NULL), m_iFileIndex(0), m_bShouldRebuild(true) {}

CInstrumentFileTree::~CInstrumentFileTree() { DeleteMenuObjects(); }
...
void CInstrumentFileTree::Changed() { m_bShouldRebuild = true; }
...
bool CInstrumentFileTree::BuildMenuTree(CString instrumentPath) {
  ...
  if (instrumentPath.GetLength() == 0) {
    ...
    m_pRootMenu->AppendMenu(MF_STRING | MF_DISABLED, MENU_BASE + 2,
                            _T("(select a directory)"));
  } else {
    m_iFileIndex = 2;
...
bool CInstrumentFileTree::ScanDirectory(CString path, CMenu *pMenu, int level) {
  ...
  while (working) {
    ...
    if (fileFinder.IsDirectory() && !fileFinder.IsHidden() &&
        !fileFinder.IsDots() && m_iTotalMenusAdded++ < MAX_MENUS) {
      CMenu *pSubMenu = new CMenu();
      m_menuArray.Add(pSubMenu);
      pSubMenu->CreatePopupMenu();
      // Recursive scan
      bool bDisabled = false;
      if (!ScanDirectory(path + _T("\\") + fileFinder.GetFileName(), pSubMenu,
                         level + 1))
        bDisabled = true;
      pMenu->AppendMenu(MF_STRING | MF_POPUP |
                            (bDisabled ? MF_DISABLED : MF_ENABLED),
                        (UINT_PTR)pSubMenu->m_hMenu, fileFinder.GetFileName());
      bNoFile = false;
    }
  }
  ...
  while (working) {
    working = fileFinder.FindNextFile();
    pMenu->AppendMenu(MF_STRING | MF_ENABLED, MENU_BASE + m_iFileIndex++,
...
CMenu *CInstrumentFileTree::GetMenu() const { return m_pRootMenu; }
