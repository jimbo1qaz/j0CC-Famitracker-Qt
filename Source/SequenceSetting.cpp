...
static LPCTSTR SEQ_SETTING_TEXT[][SEQ_COUNT] = {
    // // // 050B
...
void CSequenceSetting::Setup(CFont *pFont) { m_pFont = pFont; }
...
void CSequenceSetting::OnPaint() {
  ...
  if (mode > SEQ_SETTING_COUNT[m_iType] ||
      SEQ_SETTING_TEXT[mode][m_iType] == nullptr) {
...
void CSequenceSetting::OnLButtonDown(UINT nFlags, CPoint point) {
  ...
  if (SEQ_SETTING_COUNT[m_iType] < 2)
    return;

  m_menuPopup.CreatePopupMenu();
  for (unsigned i = 0; i < SEQ_SETTING_COUNT[m_iType]; ++i)
    m_menuPopup.AppendMenu(MF_STRING, MENU_ID_BASE + i,
                           SEQ_SETTING_TEXT[i][m_iType]);
  m_menuPopup.CheckMenuRadioItem(MENU_ID_BASE, MENU_ID_MAX,
                                 MENU_ID_BASE + Setting, MF_BYCOMMAND);
#ifndef _DEBUG
  if (m_iType == SEQ_VOLUME && m_iInstType != INST_VRC6)
    m_menuPopup.EnableMenuItem(MENU_ID_BASE + SETTING_VOL_64_STEPS,
                               MF_DISABLED); // // // 050B
//	else if (m_iType == SEQ_PITCH && m_iInstType != INST_2A03)
//		m_menuPopup.EnableMenuItem(MENU_ID_BASE + SETTING_PITCH_SWEEP,
// MF_DISABLED);
#endif
...
void CSequenceSetting::SelectSequence(CSequence *pSequence, int Type,
                                      int InstrumentType) {
...
void CSequenceSetting::OnMenuSettingChanged(UINT ID) // // //
{
  ...
  const auto MapFunc = [&](signed char (*f)(signed char)) {
    for (unsigned int i = 0, Count = m_pSequence->GetItemCount(); i < Count;
         ++i)
  ...
  if (New != Setting)
    ...
    case SEQ_VOLUME:
      ...
      case SETTING_VOL_16_STEPS:
        MapFunc([](signed char x) -> signed char { return x / 4; });
        break;
    ...
    case SEQ_ARPEGGIO:
      ...
      case SETTING_ARP_SCHEME:
        MapFunc([](signed char x) -> signed char {
      ...
      case SETTING_ARP_SCHEME:
        MapFunc([](signed char x) -> signed char {
          return (x > ARPSCHEME_MAX ? ARPSCHEME_MAX
                                    : (x < ARPSCHEME_MIN ? ARPSCHEME_MIN : x)) &
                 0x3F;
        });
        break;
      case SETTING_ARP_FIXED:
        MapFunc([](signed char x) -> signed char { return x < 0 ? 0 : x; });
        break;
      }
      break;
    }
