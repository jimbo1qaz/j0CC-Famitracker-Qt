...
CChannelHandler2A03::CChannelHandler2A03()
    : CChannelHandler(0x7FF, 0x0F), m_bHardwareEnvelope(false),
      m_bEnvelopeLoop(true), m_bResetEnvelope(false), m_iLengthCounter(1) {}

void CChannelHandler2A03::HandleNoteData(stChanNote *pNoteData,
                                         int EffColumns) {
  // // //
...
bool CChannelHandler2A03::HandleEffect(effect_t EffNum,
...
void CChannelHandler2A03::HandleCut() { CutNote(); }
  ...
  case INST_FDS:
    ...
    default:
      m_pInstHandler.reset(
          new CSeqInstHandler(this, 0x0F, Type == INST_S5B ? 0x40 : 0));
      return true;
    }
  }
  return false;
...
C2A03Square::C2A03Square()
    : CChannelHandler2A03(), m_cSweep(0), m_bSweeping(0), m_iSweep(0) {}
...
void C2A03Square::RefreshChannel() {
  ...
  if (m_bGate)                           // // //
    WriteRegister(Address, (DutyCycle << 6) | (m_bEnvelopeLoop << 5) |
                               (!m_bHardwareEnvelope << 4) | Volume); // // //
  else {
...
int C2A03Square::ConvertDuty(int Duty) const // // //
{
  switch (m_iInstTypeCurrent) {
  case INST_VRC6:
    return DUTY_2A03_FROM_VRC6[Duty & 0x07];
  case INST_S5B:
    return 0x02;
  default:
    return Duty;
...
bool C2A03Square::HandleEffect(effect_t EffNum, unsigned char EffParam) {
  ...
  case EF_SWEEPDOWN:
    ...
    break;
  default:
    return CChannelHandler2A03::HandleEffect(EffNum, EffParam);
  }
...
CTriangleChan::CTriangleChan()
    : // // //
      CChannelHandler2A03(), m_iLinearCounter(-1) {}
...
void CTriangleChan::RefreshChannel() {
  ...
  if (m_iInstVolume > 0 && m_iVolume > 0 && m_bGate) {
    WriteRegister(0x4008,
                  (m_bEnvelopeLoop << 7) | (m_iLinearCounter & 0x7F)); // // //
...
{
        return LimitPeriod(m_iPeriod - GetVibrato() + GetFinePitch() +
GetPitch());
}
*/

CNoiseChan::CNoiseChan()
    : CChannelHandler2A03() // // //
{}
...
void CNoiseChan::RefreshChannel() {
  ...
  if (m_bGate) // // //
    WriteRegister(0x400C, (m_bEnvelopeLoop << 5) | (!m_bHardwareEnvelope << 4) |
                              Volume); // // //
  else {
...
CDPCMChan::CDPCMChan()
    :                             // // //
      ...
      mEnabled(false), mTriggerSample(false), m_cDAC(255), mRetriggerPeriod(0),
      mRetriggerCtr(0) {}
...
bool CDPCMChan::HandleEffect(effect_t EffNum, unsigned char EffParam) {
  ...
  case EF_RETRIGGER:
    ...
    triggerSample() {
            ...
            queueSample()			// If mRetriggerPeriod != 0,
    this initializes retriggering.
                                                            // Otherwise reset
    mRetriggerCtr.
    }
    queueSample() {
    ...
    If new row:
            ...
            If Xxx: HandleEffect.EF_RETRIGGER:
                    mRetriggerPeriod = xx
                    if mRetriggerCtr == 0:	// Most recent row contains note
    without Xxx queueSample()
    ...
    Each frame: RefreshChannel():	// renders channel (pushes registers to
    2a03) If mRetriggerPeriod: mRetriggerCtr-- If zero: mRetriggerCtr =
    mRetriggerPeriod. mEnabled = true mTriggerSample = true
    ...
    In the absence of queueSample(), mRetriggerCtr is cycled within
    [1..mRetriggerPeriod] by RefreshChannel(). If a row-note plays on any
    non-Xxx row, mRetriggerCtr is reset to mRetriggerPeriod=0.
    */
  ...
  case EF_NOTE_RELEASE:
    return CChannelHandler::HandleEffect(EffNum, EffParam);
  default:
    return false; // unless WAVE_CHAN analog for CChannelHandler exists
  }
...
void CDPCMChan::HandleRelease() { m_bRelease = true; }
...
void CDPCMChan::triggerSample() {
  ...
  // If mRetriggerPeriod != 0, this initializes retriggering. Otherwise reset
  // mRetriggerCtr.
  queueSample();
}
...
void CDPCMChan::queueSample() {
  if (mRetriggerPeriod == 0) {
    ...
    // mRetriggerCtr gets decremented this frame, and reaches 0 in
    // mRetriggerPeriod frames.
...
int CDPCMChan::GetChannelVolume() const { return VOL_COLUMN_MAX; }
