...
CSplitKeyboardDlg::CSplitKeyboardDlg(CWnd *pParent /*=NULL*/)
    : CDialog(IDD_SPLIT_KEYBOARD, pParent), m_bSplitEnable(false),
      m_iSplitChannel(-1), m_iSplitNote(-1),
      m_iSplitInstrument(MAX_INSTRUMENTS), m_iSplitTranspose(0) {}
...
BOOL CSplitKeyboardDlg::OnInitDialog() {
  ...
  CheckDlgButton(IDC_CHECK_SPLIT_ENABLE,
                 m_bSplitEnable ? BST_CHECKED : BST_UNCHECKED);
...
void CSplitKeyboardDlg::OnBnClickedCheckSplitEnable() {
  if (m_bSplitEnable =
          (IsDlgButtonChecked(IDC_CHECK_SPLIT_ENABLE) == BST_CHECKED)) {
...
void CSplitKeyboardDlg::OnCbnSelchangeComboSplitNote() {
  m_iSplitNote = MIDI_NOTE(
      ...
      static_cast<CComboBox *>(GetDlgItem(IDC_COMBO_SPLIT_NOTE))->GetCurSel() +
          1);
}

void CSplitKeyboardDlg::OnCbnSelchangeComboSplitChan() {
  if (int Pos = static_cast<CComboBox *>(GetDlgItem(IDC_COMBO_SPLIT_CHAN))
                    ->GetCurSel())
...
void CSplitKeyboardDlg::OnCbnSelchangeComboSplitInst() {
  ...
  m_iSplitInstrument =
      str == KEEP_INST_STRING ? MAX_INSTRUMENTS : strtol(str, nullptr, 16);
}

void CSplitKeyboardDlg::OnCbnSelchangeComboSplitTrsp() {
  m_iSplitTranspose =
      static_cast<CComboBox *>(GetDlgItem(IDC_COMBO_SPLIT_TRSP))->GetCurSel() -
      MAX_TRANSPOSE;
}
