...
CSwapDlg::CSwapDlg(CWnd *pParent /*=NULL*/) : CDialog(IDD_SWAP, pParent) {}

void CSwapDlg::SetTrack(unsigned int Track) { m_iTrack = Track; }
...
void CSwapDlg::CheckDestination() const {
  GetDlgItem(IDOK)->EnableWindow(
      GetFinalChannel(m_iDestChannel1, m_iDestChip1) != -1 &&
...
void CSwapDlg::OnBnClickedOk() {
  ...
  if (IsDlgButtonChecked(IDC_CHECK_SWAP_ALL) == BST_CHECKED)
    ...
    pDoc->SwapChannels(m_iTrack, GetFinalChannel(m_iDestChannel1, m_iDestChip1),
                       GetFinalChannel(m_iDestChannel2, m_iDestChip2));
