...
enum { WC_NOTE = 0, WC_OCT, WC_INST, WC_VOL, WC_EFF, WC_PARAM };

#pragma warning(disable : 4351) // "new behaviour: elements of array [...] will
                                // be default initialized"

searchTerm::searchTerm()
    : NoiseChan(false), Note(new CharRange()), Oct(new CharRange()),
      Inst(new CharRange(0, MAX_INSTRUMENTS)),
      Vol(new CharRange(0, MAX_VOLUME)), EffParam(new CharRange()), Definite(),
      EffNumber() {}

searchTerm::searchTerm(searchTerm &&other)
    : NoiseChan(other.NoiseChan), Note(std::move(other.Note)),
      Oct(std::move(other.Oct)), Inst(std::move(other.Inst)),
      Vol(std::move(other.Vol)), EffParam(std::move(other.EffParam)) {
...
CFindCursor::CFindCursor(CFamiTrackerDoc *pDoc, int Track,
                         const CCursorPos &Pos, const CSelection &Scope)
    : CPatternIterator(pDoc, Track, Pos),
      m_Scope(Scope.GetNormalized()), m_cpBeginPos{Pos} {}
...
void CFindCursor::Move(direction_t Dir) {
  ...
  case direction_t::RIGHT:
    m_iChannel = m_Scope.m_cpStart.m_iChannel;
    if (++m_iRow >= static_cast<int>(m_pDocument->GetCurrentPatternLength(
                        m_iTrack, m_iFrame))) {
      ++m_iFrame;
      m_iRow = 0;
    }
...
void CFindCursor::ResetPosition(direction_t Dir) {
  ...
  case direction_t::DOWN:
  case direction_t::RIGHT:
    m_cpBeginPos = m_Scope.m_cpEnd;
    Source = &m_Scope.m_cpStart;
    break;
  case direction_t::UP:
  case direction_t::LEFT:
    m_cpBeginPos = m_Scope.m_cpStart;
    Source = &m_Scope.m_cpEnd;
    break;
  }
...
bool CFindCursor::Contains() const {
  ...
  bool InStart =
      Frame > fStart || (Frame == fStart && m_iRow >= m_Scope.m_cpStart.m_iRow);
  bool InEnd =
      Frame < fEnd || (Frame == fEnd && m_iRow <= m_Scope.m_cpEnd.m_iRow);
  if (fStart > fEnd ||
      (fStart == fEnd && m_Scope.m_cpStart.m_iRow > m_Scope.m_cpEnd.m_iRow))
...
CFindResultsBox::CFindResultsBox(CWnd *pParent)
    : CDialog(IDD_FINDRESULTS, pParent) {
  m_iLastsortColumn = ID;
  m_bLastSortDescending = false;
  m_iChannelPositionCache.clear();
}

CFindResultsBox::~CFindResultsBox() { SAFE_RELEASE(m_cListResults); }

void CFindResultsBox::DoDataExchange(CDataExchange *pDX) {
  CDialog::DoDataExchange(pDX);
}

void CFindResultsBox::AddResult(const stChanNote *pNote,
                                const CFindCursor *pCursor, bool Noise) const {
  int Pos = m_cListResults->GetItemCount();
  CString str;
  str.Format(_T("%d"), Pos + 1);
  m_cListResults->InsertItem(Pos, str);

  const auto pDoc = static_cast<CFamiTrackerDoc *>(
      ((CFrameWnd *)AfxGetMainWnd())->GetActiveDocument());
  m_cListResults->SetItemText(
      Pos, CHANNEL, pDoc->GetChannel(pCursor->m_iChannel)->GetChannelName());
  str.Format(_T("%02X"),
             pDoc->GetPatternAtFrame(pCursor->m_iTrack, pCursor->m_iFrame,
                                     pCursor->m_iChannel));
  ...
  case NONE:
    break;
  case HALT:
    m_cListResults->SetItemText(Pos, NOTE, _T("---"));
    break;
  case RELEASE:
    m_cListResults->SetItemText(Pos, NOTE, _T("==="));
    break;
  ...
  for (int i = 0; i < MAX_EFFECT_COLUMNS; ++i)
    if (pNote->EffNumber[i] != EF_NONE) {
      str.Format(_T("%c%02X"), EFF_CHAR[pNote->EffNumber[i] - 1],
                 pNote->EffParam[i]);
...
void CFindResultsBox::SelectItem(int Index) {
  ...
  const auto ToChannelIndex = [](const std::string &_x) {
    ...
    static const CString HEADER_STR[] = {
        ...
        _T("VRC6 Pulse "), _T("Sawtooth"),    _T("MMC5 Pulse "), _T("Namco "),
        _T("FDS"),         _T("FM Channel "), _T("5B Square ")};
    static const int HEADER_ID[] = {
        CHANID_SQUARE1,      CHANID_TRIANGLE,    CHANID_NOISE,
        CHANID_DPCM,         CHANID_VRC6_PULSE1, CHANID_VRC6_SAWTOOTH,
        CHANID_MMC5_SQUARE1, CHANID_N163_CH1,    CHANID_FDS,
        CHANID_VRC7_CH1,     CHANID_S5B_CH1,
    };
  ...
  const auto Cache = [&](const std::string &x) {
    ...
    if (it == m_iChannelPositionCache.end())
      return m_iChannelPositionCache[x] =
                 pDoc->GetChannelIndex(ToChannelIndex(x));
    return it->second;
  };

  auto pView = static_cast<CFamiTrackerView *>(
  ...
  pView->SelectFrame(
      strtol(m_cListResults->GetItemText(Index, FRAME), nullptr, 16));
  pView->SelectRow(
      strtol(m_cListResults->GetItemText(Index, ROW), nullptr, 16));
...
void CFindResultsBox::UpdateCount() const {
  ...
  AfxFormatString2(str, IDS_FINDRESULT_COUNT, str,
                   Count == 1 ? _T("result") : _T("results"));
...
BOOL CFindResultsBox::OnInitDialog() {
  ...
  m_cListResults->InsertColumn(ID, _T("ID"), LVCFMT_LEFT,
                               static_cast<int>(.085 * w));
  m_cListResults->InsertColumn(CHANNEL, _T("Channel"), LVCFMT_LEFT,
                               static_cast<int>(.19 * w));
  m_cListResults->InsertColumn(PATTERN, _T("Pa."), LVCFMT_LEFT,
                               static_cast<int>(.065 * w));
  m_cListResults->InsertColumn(FRAME, _T("Fr."), LVCFMT_LEFT,
                               static_cast<int>(.065 * w));
  m_cListResults->InsertColumn(ROW, _T("Ro."), LVCFMT_LEFT,
                               static_cast<int>(.065 * w));
  m_cListResults->InsertColumn(NOTE, _T("Note"), LVCFMT_LEFT,
                               static_cast<int>(.08 * w));
  m_cListResults->InsertColumn(INST, _T("In."), LVCFMT_LEFT,
                               static_cast<int>(.065 * w));
  m_cListResults->InsertColumn(VOL, _T("Vo."), LVCFMT_LEFT,
                               static_cast<int>(.065 * w));
  for (int i = MAX_EFFECT_COLUMNS; i > 0; --i) {
    CString str;
    str.Format(_T("fx%d"), i);
    m_cListResults->InsertColumn(EFFECT, str, LVCFMT_LEFT,
                                 static_cast<int>(.08 * w));
  }
...
void CFindResultsBox::OnNMDblclkListFindresults(NMHDR *pNMHDR,
                                                LRESULT *pResult) {
  ...
  if (lvhti.iItem == -1)
    return;
  m_cListResults->SetItemState(lvhti.iItem, LVIS_SELECTED | LVIS_FOCUSED,
                               LVIS_SELECTED | LVIS_FOCUSED);
  SelectItem(lvhti.iItem);
}

void CFindResultsBox::OnLvnColumnClickFindResults(NMHDR *pNMHDR,
                                                  LRESULT *pResult) {
  ...
  case ID:
    m_cListResults->SortItemsEx(IntCompareFunc, (LPARAM)m_cListResults);
    break;
  case CHANNEL:
    m_cListResults->SortItemsEx(ChannelCompareFunc, (LPARAM)m_cListResults);
    break;
  case NOTE:
...
int CFindResultsBox::IntCompareFunc(LPARAM lParam1, LPARAM lParam2,
                                    LPARAM lParamSort) {
...
int CFindResultsBox::HexCompareFunc(LPARAM lParam1, LPARAM lParam2,
                                    LPARAM lParamSort) {
  CListCtrl *pList = reinterpret_cast<CListCtrl *>(lParamSort);
...
int CFindResultsBox::StringCompareFunc(LPARAM lParam1, LPARAM lParam2,
                                       LPARAM lParamSort) {
  CListCtrl *pList = reinterpret_cast<CListCtrl *>(lParamSort);
...
int CFindResultsBox::ChannelCompareFunc(LPARAM lParam1, LPARAM lParam2,
                                        LPARAM lParamSort) {
  CListCtrl *pList = reinterpret_cast<CListCtrl *>(lParamSort);
  CString x = pList->GetItemText(lParam1, m_iLastsortColumn);
  CString y = pList->GetItemText(lParam2, m_iLastsortColumn);
...
int CFindResultsBox::NoteCompareFunc(LPARAM lParam1, LPARAM lParam2,
                                     LPARAM lParamSort) {
  CListCtrl *pList = reinterpret_cast<CListCtrl *>(lParamSort);
  CString x = pList->GetItemText(lParam1, m_iLastsortColumn);
  CString y = pList->GetItemText(lParam2, m_iLastsortColumn);
...
CFindDlg::CFindDlg(CWnd *pParent /*=NULL*/)
    : CDialog(CFindDlg::IDD, pParent), m_bFound(false), m_bSkipFirst(true),
...
ON_CONTROL_RANGE(BN_CLICKED, IDC_CHECK_FIND_NOTE, IDC_CHECK_FIND_EFF,
                 OnUpdateFields)
ON_CONTROL_RANGE(BN_CLICKED, IDC_CHECK_REPLACE_NOTE, IDC_CHECK_REPLACE_EFF,
                 OnUpdateFields)
ON_CONTROL_RANGE(EN_CHANGE, IDC_EDIT_FIND_NOTE, IDC_EDIT_FIND_EFF,
                 OnUpdateFields)
ON_CONTROL_RANGE(EN_CHANGE, IDC_EDIT_REPLACE_NOTE, IDC_EDIT_REPLACE_EFF,
                 OnUpdateFields)
...
const CString CFindDlg::m_pNoteName[7] = {_T("C"), _T("D"), _T("E"), _T("F"),
                                          _T("G"), _T("A"), _T("B")};
const CString CFindDlg::m_pNoteSign[3] = {_T("b"), _T("-"), _T("#")};
const int CFindDlg::m_iNoteOffset[7] = {NOTE_C, NOTE_D, NOTE_E, NOTE_F,
...
void CFindDlg::OnUpdateFields(UINT nID) { UpdateFields(); }

void CFindDlg::ParseNote(searchTerm &Term, CString str, bool Half) {
  if (!Half)
    Term.Definite[WC_NOTE] = Term.Definite[WC_OCT] = false;
  ...
  if (str.GetAt(0) == _T('^')) {
    RaiseIf(Half && !Term.Definite[WC_OCT],
            _T("Cannot use wildcards in a range search query."));
    Term.Definite[WC_NOTE] = true;
    Term.Definite[WC_OCT] = true;
    Term.Note->Set(ECHO);
    if (str.Delete(0)) {
      ...
      RaiseIf(atoi(str) > ECHO_BUFFER_LENGTH,
              _T("Echo buffer access \"^%s\" is out of range, maximum is %d."),
              str, ECHO_BUFFER_LENGTH);
      Term.Oct->Set(atoi(str), Half);
    } else {
      Term.Oct->Min = 0;
      Term.Oct->Max = ECHO_BUFFER_LENGTH;
    }
    return;
  }
  ...
  if (str.Mid(1, 2) != _T("-#"))
    for (int i = 0; i < 7; i++) {
      if (str.Left(1).MakeUpper() == m_pNoteName[i]) {
        ...
        if (str.Delete(0)) {
          Term.Definite[WC_OCT] = true;
          RaiseIf(str.SpanIncluding("0123456789") != str,
                  _T("Unknown note octave."));
          Oct = atoi(str);
          RaiseIf(Oct >= OCTAVE_RANGE || Oct < 0,
                  _T("Note octave \"%s\" is out of range, maximum is %d."), str,
                  OCTAVE_RANGE - 1);
          Term.Oct->Set(Oct, Half);
        } else
          RaiseIf(Half, _T("Cannot use wildcards in a range search query."));
        while (Note > NOTE_RANGE) {
          Note -= NOTE_RANGE;
          if (Term.Definite[WC_OCT])
            Term.Oct->Set(++Oct, Half);
        }
        while (Note < NOTE_C) {
          Note += NOTE_RANGE;
        ...
        RaiseIf(Term.Definite[WC_OCT] && (Oct >= OCTAVE_RANGE || Oct < 0),
                _T("Note octave \"%s\" is out of range, check if the note ")
                _T("contains Cb or B#."),
                str);
        return;
      }
  ...
  if (str.SpanIncluding("0123456789") == str) {
    ...
    RaiseIf(NoteValue == 0 && str.GetAt(0) != _T('0'),
            _T("Invalid note \"%s\"."), str);
    RaiseIf(NoteValue >= NOTE_COUNT || NoteValue < 0,
            _T("Note value \"%s\" is out of range, maximum is %d."), str,
            NOTE_COUNT - 1);
    Term.Definite[WC_NOTE] = true;
    Term.Definite[WC_OCT] = true;
    Term.Note->Set(NoteValue % NOTE_RANGE + 1, Half);
...
void CFindDlg::ParseInst(searchTerm &Term, CString str, bool Half) {
  ...
  RaiseIf(Half && !Term.Inst->IsSingle(),
          _T("Cannot use wildcards in a range search query."));
  ...
  if (str == _T(".")) {
    ...
    RaiseIf(Val >= MAX_INSTRUMENTS,
            _T("Instrument \"%s\" is out of range, maximum is %X."), str,
            MAX_INSTRUMENTS - 1);
...
void CFindDlg::ParseVol(searchTerm &Term, CString str, bool Half) {
  ...
  RaiseIf(Half && !Term.Vol->IsSingle(),
          _T("Cannot use wildcards in a range search query."));
  ...
  if (str == _T(".")) {
    ...
    RaiseIf(Val >= MAX_VOLUME,
            _T("Channel volume \"%s\" is out of range, maximum is %X."), str,
            MAX_VOLUME - 1);
...
void CFindDlg::ParseEff(searchTerm &Term, CString str, bool Half) {
  ...
  if (str.IsEmpty()) {
    ...
    RaiseIf(Term.EffNumber[EF_NONE],
            _T("Unknown effect \"%s\" found in search query."), str.Left(1));
  }
  if (str.GetLength() > 1) {
    Term.Definite[WC_PARAM] = true;
    Term.EffParam->Set(
        static_cast<unsigned char>(strtol(str.Right(2), NULL, 16)));
  }
}

void CFindDlg::GetFindTerm() {
  RaiseIf(m_cSearchArea->GetCurSel() == 4 &&
              !m_pView->GetPatternEditor()->IsSelecting(),
          _T("Cannot use \"Selection\" as the search scope if there is no ")
          _T("active pattern selection."));
  ...
  if (IsDlgButtonChecked(IDC_CHECK_FIND_NOTE)) {
    ...
    RaiseIf(
        (newTerm.Note->Min == ECHO && newTerm.Note->Max >= NOTE_C &&
             newTerm.Note->Max <= NOTE_B ||
         newTerm.Note->Max == ECHO && newTerm.Note->Min >= NOTE_C &&
             newTerm.Note->Min <= NOTE_B) &&
...
void CFindDlg::GetReplaceTerm() {
  ...
  if ((newTerm.Note->Min == HALT || newTerm.Note->Min == RELEASE) &&
      newTerm.Note->Min == newTerm.Note->Max)
  ...
  if (IsDlgButtonChecked(IDC_CHECK_FIND_REMOVE)) {
    RaiseIf(newTerm.Definite[WC_NOTE] && !newTerm.Definite[WC_OCT],
            _T("Replacement query cannot contain a note with an unspecified ")
            _T("octave if ")
            _T("the option \"Remove original data\" is enabled."));
    RaiseIf(newTerm.Definite[WC_EFF] && !newTerm.Definite[WC_PARAM],
            _T("Replacement query cannot contain an effect with an ")
            _T("unspecified parameter if ")
            _T("the option \"Remove original data\" is enabled."));
  }
...
bool CFindDlg::CompareFields(const stChanNote Target, bool Noise,
                             int EffCount) {
  int EffColumn = m_cEffectColumn->GetCurSel();
  if (EffColumn > EffCount && EffColumn != 4)
    EffColumn = EffCount;
  bool Negate = IsDlgButtonChecked(IDC_CHECK_FIND_NEGATE) == BST_CHECKED;
  bool EffectMatch = false;

  bool Melodic = m_searchTerm.Note->Min >= NOTE_C &&
                 m_searchTerm.Note->Min <= NOTE_B && // ||
                 m_searchTerm.Note->Max >= NOTE_C &&
                 m_searchTerm.Note->Max <= NOTE_B &&
  ...
  if (m_searchTerm.Definite[WC_NOTE]) {
    if (m_searchTerm.NoiseChan) {
      ...
      if (m_searchTerm.Note->Min < NOTE_C || m_searchTerm.Note->Min > NOTE_B ||
        ...
        if (!m_searchTerm.Note->IsMatch(Target.Note))
          return Negate;
      } else {
        int NoiseNote = MIDI_NOTE(Target.Octave, Target.Note) % 16;
        int Low = MIDI_NOTE(m_searchTerm.Oct->Min, m_searchTerm.Note->Min) % 16;
        int High =
            MIDI_NOTE(m_searchTerm.Oct->Max, m_searchTerm.Note->Max) % 16;
        if ((NoiseNote < Low && NoiseNote < High) ||
            (NoiseNote > Low && NoiseNote > High))
          return Negate;
      }
    } else {
      if (Noise && Melodic)
      ...
      if (Melodic) {
        ...
        if ((NoteValue < Low && NoteValue < High) ||
            (NoteValue > Low && NoteValue > High))
          return Negate;
      } else {
        if (!m_searchTerm.Note->IsMatch(Target.Note))
          return Negate;
        if (m_searchTerm.Definite[WC_OCT] &&
            !m_searchTerm.Oct->IsMatch(Target.Octave))
          return Negate;
      }
    }
  }
  if (m_searchTerm.Definite[WC_INST] &&
      !m_searchTerm.Inst->IsMatch(Target.Instrument))
    return Negate;
  if (m_searchTerm.Definite[WC_VOL] && !m_searchTerm.Vol->IsMatch(Target.Vol))
    return Negate;
  ...
  for (int i = EffColumn % MAX_EFFECT_COLUMNS; i <= Limit; i++) {
    if ((!m_searchTerm.Definite[WC_EFF] ||
         m_searchTerm.EffNumber[Target.EffNumber[i]]) &&
        (!m_searchTerm.Definite[WC_PARAM] ||
         m_searchTerm.EffParam->IsMatch(Target.EffParam[i])))
      EffectMatch = true;
  }
  if (!EffectMatch)
    return Negate;
...
bool CFindDlg::Find(bool ShowEnd) {
  const int Track =
      static_cast<CMainFrame *>(AfxGetMainWnd())->GetSelectedTrack();
  ...
  do {
    ...
    if (CompareFields(
            Target, m_pFindCursor->m_iChannel == CHANID_NOISE,
  ...
  if (m_bFound) {
    ...
    if (m_replaceTerm.Definite[WC_EFF] || m_replaceTerm.Definite[WC_PARAM]) {
      ...
      if (m_cEffectColumn->GetCurSel() < MAX_EFFECT_COLUMNS)
        ...
        const int c = m_pDocument->GetEffColumns(m_pFindCursor->m_iTrack,
                                                 m_pFindCursor->m_iChannel);
        for (int i = 0; i <= c; ++i)
          if ((!m_searchTerm.Definite[WC_EFF] ||
               m_searchTerm.EffNumber[Target.EffNumber[i]]) &&
              (!m_searchTerm.Definite[WC_PARAM] ||
      ...
      if (m_replaceTerm.Definite[WC_EFF]) {
        effect_t fx = GetEffectFromChar(
            EFF_CHAR[m_replaceTerm.Note.EffNumber[0] - 1],
    ...
    if (pAction)
      pAction->JoinAction(new CPActionReplaceNote(
          Target, m_pFindCursor->m_iFrame, m_pFindCursor->m_iRow,
          m_pFindCursor->m_iChannel));
    else
...
bool CFindDlg::PrepareFind() {
  m_pDocument = static_cast<CFamiTrackerDoc *>(
      ((CFrameWnd *)AfxGetMainWnd())->GetActiveDocument());
  m_pView = static_cast<CFamiTrackerView *>(
      ((CFrameWnd *)AfxGetMainWnd())->GetActiveView());
  if (!m_pDocument || !m_pView) {
    AfxMessageBox(_T("Unknown error."), MB_ICONERROR);
    return false;
  }
...
void CFindDlg::PrepareCursor(bool ReplaceAll) {
  ...
  if (m_cSearchArea->GetCurSel() == 4) { // Selection
    ...
    case 0:
    case 1: // Track, Channel
      Scope.m_cpStart.m_iFrame = 0;
      Scope.m_cpEnd.m_iFrame = Frames - 1;
      break;
    case 2:
    case 3: // Frame, Pattern
      Scope.m_cpStart.m_iFrame = Scope.m_cpEnd.m_iFrame = Cursor.m_iFrame;
      break;
    }
    ...
    case 2: // Track, Frame
      ...
      Scope.m_cpEnd.m_iChannel = m_pDocument->GetChannelCount() - 1;
      break;
    case 1:
    case 3: // Channel, Pattern
      Scope.m_cpStart.m_iChannel = Scope.m_cpEnd.m_iChannel = Cursor.m_iChannel;
      break;
    }

    Scope.m_cpStart.m_iRow = 0;
    Scope.m_cpEnd.m_iRow =
        pEditor->GetCurrentPatternLength(Scope.m_cpEnd.m_iFrame) - 1;
  }
  m_pFindCursor = new CFindCursor{m_pDocument, Track,
...
void CFindDlg::OnBnClickedButtonFindNext() {
  ...
  m_iSearchDirection = IsDlgButtonChecked(IDC_CHECK_VERTICAL_SEARCH)
                           ? CFindCursor::direction_t::DOWN
                           : CFindCursor::direction_t::RIGHT;
  Find(true);
  m_pView->SetFocus();
}
...
void CFindDlg::OnBnClickedButtonFindPrevious() {
  ...
  m_iSearchDirection = IsDlgButtonChecked(IDC_CHECK_VERTICAL_SEARCH)
                           ? CFindCursor::direction_t::UP
                           : CFindCursor::direction_t::LEFT;
  Find(true);
  m_pView->SetFocus();
}
...
void CFindDlg::OnBnClickedButtonFindAll() {
  ...
  do {
    ...
    if (CompareFields(
            Target, m_pFindCursor->m_iChannel == CHANID_NOISE,
            m_pDocument->GetEffColumns(Track, m_pFindCursor->m_iChannel)))
      m_cResultsBox->AddResult(&Target, m_pFindCursor,
                               m_pFindCursor->m_iChannel == CHANID_NOISE);
    m_pFindCursor->Move(m_iSearchDirection);
  } while (!m_pFindCursor->AtStart());
