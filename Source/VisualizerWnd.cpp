...
CVisualizerWnd::CVisualizerWnd()
    : m_iCurrentState(0), m_bThreadRunning(false), m_pWorkerThread(NULL),
      m_iBufferSize(0), m_pBuffer1(NULL), m_pBuffer2(NULL), m_pFillBuffer(NULL),
      m_hNewSamples(NULL), m_bNoAudio(false) {
...
UINT CVisualizerWnd::ThreadProc() {
  ...
  while (::WaitForSingleObject(m_hNewSamples, INFINITE) == WAIT_OBJECT_0 &&
         m_bThreadRunning) {
...
BOOL CVisualizerWnd::CreateEx(DWORD dwExStyle, LPCTSTR lpszClassName,
  ...
  BOOL Result = CWnd::CreateEx(dwExStyle, lpszClassName, lpszWindowName,
  ...
  if (Result) {
    ...
    m_pWorkerThread = AfxBeginThread(&ThreadProcFunc, (LPVOID)this,
...
void CVisualizerWnd::OnRButtonUp(UINT nFlags, CPoint point) {
  ...
  static const UINT menuIds[] = {ID_POPUP_SAMPLESCOPE1, ID_POPUP_SAMPLESCOPE2,
                                 ID_POPUP_SPECTRUMANALYZER,
                                 ID_POPUP_SPECTRUMANALYZER2, // // //
                                 ID_POPUP_NOTHING};

  pPopupMenu->CheckMenuItem(menuIds[m_iCurrentState],
                            MF_BYCOMMAND | MF_CHECKED);

  UINT Result =
      pPopupMenu->TrackPopupMenu(TPM_RETURNCMD, menuPoint.x, menuPoint.y, this);
