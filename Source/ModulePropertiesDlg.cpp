...
CModulePropertiesDlg::CModulePropertiesDlg(CWnd *pParent /*=NULL*/)
    : CDialog(CModulePropertiesDlg::IDD, pParent), m_iSelectedSong(0),
      m_pDocument(NULL) {}
...
// ON_CBN_SELCHANGE(IDC_EXPANSION,
// &CModulePropertiesDlg::OnCbnSelchangeExpansion)
...
ON_WM_HSCROLL()
// ON_NOTIFY(LVN_ITEMCHANGED, IDC_SONGLIST,
// &CModulePropertiesDlg::OnLvnItemchangedSonglist)
...
ON_EN_CHANGE(IDC_N163_OFFSET_EDIT, OnEnChangeEditN163Offset)
END_MESSAGE_MAP()

const int LEVEL_RANGE = 12; // +/- 12 dB range

const int FINE_DELTA = 10;  // you better hope this matches
                            // CConfigMixer::LEVEL_SCALE and SoundGen.cpp.
...
BOOL CModulePropertiesDlg::OnInitDialog() {
  ...
  ((CButton *)GetDlgItem(IDC_EXPANSION_VRC6))
      ->SetCheck((m_iExpansions & SNDCHIP_VRC6) != 0);
  ((CButton *)GetDlgItem(IDC_EXPANSION_VRC7))
      ->SetCheck((m_iExpansions & SNDCHIP_VRC7) != 0);
  ((CButton *)GetDlgItem(IDC_EXPANSION_FDS))
      ->SetCheck((m_iExpansions & SNDCHIP_FDS) != 0);
  ((CButton *)GetDlgItem(IDC_EXPANSION_MMC5))
      ->SetCheck((m_iExpansions & SNDCHIP_MMC5) != 0);
  ((CButton *)GetDlgItem(IDC_EXPANSION_N163))
      ->SetCheck((m_iExpansions & SNDCHIP_N163) != 0);
  ((CButton *)GetDlgItem(IDC_EXPANSION_S5B))
      ->SetCheck((m_iExpansions & SNDCHIP_S5B) != 0);

  // Namco channel count
  CSliderCtrl *pChanSlider =
      static_cast<CSliderCtrl *>(GetDlgItem(IDC_CHANNELS));
  ...
  pVibratoBox->SetCurSel((m_pDocument->GetVibratoStyle() == VIBRATO_NEW) ? 0
                                                                         : 1);

  // Pitch
  CComboBox *pPitchBox =
      static_cast<CComboBox *>(GetDlgItem(IDC_COMBO_LINEARPITCH)); // // //
  ...
  // (Slider)
  CSliderCtrl *pSlider =
      static_cast<CSliderCtrl *>(GetDlgItem(IDC_N163_OFFSET_SLIDER));
...
void CModulePropertiesDlg::OnBnClickedOk() {
      ...
      m_pDocument->GetExpansionChip() != m_iExpansions) // // //
  {
    ...
    str = "You are going to remove channels from the following expansion "
          "chips:\n" +
          str;
  ...
  // Vibrato
  CComboBox *pVibratoBox = static_cast<CComboBox *>(GetDlgItem(IDC_VIBRATO));
  m_pDocument->SetVibratoStyle((pVibratoBox->GetCurSel() == 0) ? VIBRATO_NEW
                                                               : VIBRATO_OLD);
...
// **** events ****
void CModulePropertiesDlg::OnHScroll(UINT nSBCode, UINT nPos,
...
void CModulePropertiesDlg::OnBnClickedSongAdd() {
  ...
  TrackTitle.Format(TRACK_FORMAT, NewTrack + 1,
                    m_pDocument->GetTrackTitle(NewTrack));
  static_cast<CListCtrl *>(GetDlgItem(IDC_SONGLIST))
      ->InsertItem(NewTrack, TrackTitle);
...
void CModulePropertiesDlg::OnBnClickedSongInsert() // // //
{
  ...
  TrackTitle.Format(TRACK_FORMAT, NewTrack,
                    m_pDocument->GetTrackTitle(NewTrack));
...
void CModulePropertiesDlg::OnBnClickedSongRemove() {
  ...
  if (AfxMessageBox(IDS_SONG_DELETE,
...
void CModulePropertiesDlg::SelectSong(int Song) {
  ...
  for (int i = pSongList->GetItemCount() - 1; i >= 0; --i) // // //
    pSongList->SetItemState(i, i == Song ? (LVIS_SELECTED | LVIS_FOCUSED) : 0,
...
void CModulePropertiesDlg::UpdateSongButtons() {
  ...
  bool Empty =
      static_cast<CListCtrl *>(GetDlgItem(IDC_SONGLIST))->GetSelectedCount() ==
      0;

  GetDlgItem(IDC_SONG_ADD)
      ->EnableWindow((TrackCount == MAX_TRACKS) ? FALSE : TRUE);
  GetDlgItem(IDC_SONG_INSERT)
      ->EnableWindow((TrackCount == MAX_TRACKS || !m_bSingleSelection || Empty)
                         ? FALSE
                         : TRUE);
  GetDlgItem(IDC_SONG_REMOVE)
      ->EnableWindow((TrackCount == 1 || Empty) ? FALSE : TRUE);
  GetDlgItem(IDC_SONG_UP)
      ->EnableWindow((m_iSelectedSong == 0 || !m_bSingleSelection || Empty)
                         ? FALSE
                         : TRUE);
  GetDlgItem(IDC_SONG_DOWN)
      ->EnableWindow(
          (m_iSelectedSong == TrackCount - 1 || !m_bSingleSelection || Empty)
              ? FALSE
              : TRUE);
  GetDlgItem(IDC_SONG_IMPORT)
      ->EnableWindow((TrackCount == MAX_TRACKS) ? FALSE : TRUE);
}
...
void CModulePropertiesDlg::OnBnClickedSongImport() {
  ...
  CFileDialog OpenFileDlg(TRUE, _T("0cc"), 0, OFN_HIDEREADONLY,
                          _T("0CC-FamiTracker modules (*.0cc;*.ftm)|*.0cc; ")
                          _T("*.ftm|All files (*.*)|*.*||"), // // //
                          theApp.GetMainWnd(), 0);
...
void CModulePropertiesDlg::OnLvnItemchangedSonglist(NMHDR *pNMHDR,
                                                    LRESULT *pResult) {
  ...
  if (pNMLV->uChanged & LVIF_STATE) {
    if (pNMLV->uNewState & LVNI_SELECTED) { // // //
      ...
      GetDlgItem(IDC_SONGNAME)
          ->SetWindowText(m_pDocument->GetTrackTitle(m_iSelectedSong));
    }
    CListCtrl *pSongList = static_cast<CListCtrl *>(GetDlgItem(IDC_SONGLIST));
...
void CModulePropertiesDlg::FillSongList() {
  ...
  for (int i = 0; i < Songs; ++i) {
    Text.Format(TRACK_FORMAT, i + 1,
                m_pDocument->GetTrackTitle(i)
                    .GetString()); // start counting songs from 1
...
void CModulePropertiesDlg::OnCbnSelchangeComboLinearpitch() {
  ...
  if (First) {
    First = false;
    AfxMessageBox(_T("Because linear pitch mode is a planned feature in the "
                     "official build, "
                     "changes to this setting might not be reflected when the "
                     "current module is loaded from "
...
void CModulePropertiesDlg::updateN163GUI(bool renderText) {
  ...
  // FIXME
  // https://web.archive.org/web/20080218162024/https://www.microsoft.com/msj/1297/c1297.aspx
  // look up, look down, everything is wrong, this file must be burnt to the
  // ground
  ...
  CWnd *N163Enable[]{pChanSlider, pChannelsLabel, levelSlider, levelEdit,
                     levelText};
  ...
  // Enable/disable UI.
  for (CWnd *widget : N163Enable)
    widget->EnableWindow(N163Enabled);

  // Redraw UI.
  pChanSlider->SetPos(m_iN163Channels);
  levelSlider->SetPos(
      (int)std::round(1.0 * N163LevelOffset * COARSE_DELTA / FINE_DELTA));
  if (renderText) {
