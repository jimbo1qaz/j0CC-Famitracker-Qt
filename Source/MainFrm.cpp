...
static UINT indicators[] = {
    ...
    ID_INDICATOR_CHIP, ID_INDICATOR_INSTRUMENT, ID_INDICATOR_OCTAVE,
    ID_INDICATOR_RATE, ID_INDICATOR_TEMPO,      ID_INDICATOR_TIME,
    ID_INDICATOR_POS,  ID_INDICATOR_CAPS,       ID_INDICATOR_NUM,
    ID_INDICATOR_SCRL,
};

// Timers
enum { TMR_WELCOME, TMR_AUDIO_CHECK, TMR_AUTOSAVE };
...
CMainFrame::CMainFrame()
    : m_pVisualizerWnd(NULL), m_pFrameEditor(NULL), m_pGrooveDlg(NULL), // // //
      m_pFindDlg(NULL),                                                 // // //
      m_pBookmarkDlg(NULL),                                             // // //
      m_pPerformanceDlg(NULL),                                          // // //
      m_pImageList(NULL), m_pLockedEditSpeed(NULL), m_pLockedEditTempo(NULL),
      m_pLockedEditLength(NULL), m_pLockedEditFrames(NULL),
      m_pLockedEditStep(NULL), m_pLockedEditHighlight1(NULL), // // //
      m_pLockedEditHighlight2(NULL),                          // // //
      m_pButtonGroove(NULL),                                  // // //
      m_pButtonFixTempo(NULL),                                // // //
      m_pBannerEditName(NULL), m_pBannerEditArtist(NULL),
      m_pBannerEditCopyright(NULL), m_pInstrumentList(NULL),
      m_pActionHandler(NULL), m_iFrameEditorPos(FRAME_EDIT_POS_TOP),
      m_pInstrumentFileTree(NULL), m_iInstrument(0), m_iTrack(0), m_iOctave(3),
      m_iInstNumDigit(0),                 // // //
...
ON_UPDATE_COMMAND_UI(ID_TRACKER_SWITCHTOTRACKINSTRUMENT,
                     OnUpdateTrackerSwitchToInstrument)
...
ON_UPDATE_COMMAND_UI(ID_TRACKER_PLAY_MARKER,
                     OnUpdateTrackerPlayMarker) // // // 050B
ON_UPDATE_COMMAND_UI(ID_VIEW_AVERAGEBPM,
                     OnUpdateDisplayAverageBPM) // // // 050B
ON_UPDATE_COMMAND_UI(ID_VIEW_CHANNELSTATE,
                     OnUpdateDisplayChannelState) // // // 050B
ON_UPDATE_COMMAND_UI(ID_TRACKER_DISPLAYREGISTERSTATE,
                     OnUpdateDisplayRegisterState)
...
BOOL CMainFrame::Create(LPCTSTR lpszClassName, LPCTSTR lpszWindowName,
  ...
  return CFrameWnd::Create(lpszClassName, lpszWindowName, dwStyle, newrect,
...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct) {
  // Get the DPI setting
  if (CDC *pDC = GetDC()) { // // //
    DPI::SetScale(pDC->GetDeviceCaps(LOGPIXELSX),
  ...
  m_wndStatusBar.SetPaneInfo(2, ID_INDICATOR_INSTRUMENT, SBPS_NORMAL,
                             100); // // //
  ...
  */
  ...
  m_wndOctaveBar.SetDlgItemInt(
      IDC_HIGHLIGHT1, CPatternData::DEFAULT_HIGHLIGHT.First, 0); // // //
  m_wndOctaveBar.SetDlgItemInt(IDC_HIGHLIGHT2,
                               CPatternData::DEFAULT_HIGHLIGHT.Second, 0);
...
bool CMainFrame::CreateToolbars() {
  ...
  m_wndToolBar.SetBarStyle(CBRS_ALIGN_TOP | CBRS_SIZE_DYNAMIC | CBRS_TOOLTIPS |
                           CBRS_FLYBY);

  if (!m_wndOctaveBar.Create(this, (UINT)IDD_OCTAVE, CBRS_TOOLTIPS | CBRS_FLYBY,
                             IDD_OCTAVE)) {
  ...
  // MAKEINTRESOURCE(IDB_TOOLBAR_256), IMAGE_BITMAP, 0,0, LR_CREATEDIBSECTION
  // /*| LR_LOADMAP3DCOLORS*/); 	m_bmToolbar.Attach(hbm);
...
bool CMainFrame::CreateDialogPanels() {
  ...
  // Top area
  if (!m_wndControlBar.Create(this, IDD_MAINBAR,
  ...
  /////////
  if (!m_wndVerticalControlBar.Create(this, IDD_MAINBAR,
  ...
  if (!m_pFrameEditor->CreateEx(WS_EX_STATICEDGE, NULL, _T(""),
                                WS_CHILD | WS_VISIBLE | WS_VSCROLL | WS_HSCROLL,
                                rect, (CWnd *)&m_wndControlBar, 0)) {
  ...
  if (!m_wndFindControlBar.Create(this, IDD_MAINBAR,
  ...
  m_pLockedEditHighlight1->SubclassDlgItem(IDC_HIGHLIGHT1,
                                           &m_wndOctaveBar); // // //
  m_pLockedEditHighlight2->SubclassDlgItem(IDC_HIGHLIGHT2,
                                           &m_wndOctaveBar);            // // //
  m_pButtonGroove->SubclassDlgItem(IDC_BUTTON_GROOVE, &m_wndDialogBar); // // //
  m_pButtonFixTempo->SubclassDlgItem(IDC_BUTTON_FIXTEMPO,
                                     &m_wndDialogBar); // // //
  ...
  m_pInstrumentList->SendMessage(LVM_SETEXTENDEDLISTVIEWSTYLE, 0,
  ...
  CEdit *pInstName =
      static_cast<CEdit *>(m_wndDialogBar.GetDlgItem(IDC_INSTNAME));
...
/*
        if (!m_wndInstrumentBar.Create(this, IDD_INSTRUMENTPANEL, CBRS_RIGHT |
   CBRS_TOOLTIPS | CBRS_FLYBY, IDD_INSTRUMENTPANEL)) { TRACE("Failed to create
   frame instrument bar\n");
        }
...
*/
  ...
  // Frame bar
  /*
          if (!m_wndFrameBar.Create(this, IDD_FRAMEBAR, CBRS_LEFT |
     CBRS_TOOLTIPS | CBRS_FLYBY, IDD_FRAMEBAR)) { TRACE("Failed to create frame
     bar\n");
          }
...
bool CMainFrame::CreateVisualizerWindow() {
  ...
  if (!m_pVisualizerWnd->CreateEx(WS_EX_STATICEDGE, NULL, _T("Samples"),
...
bool CMainFrame::CreateInstrumentToolbar() {
  ...
  if (!m_wndInstToolBarWnd.CreateEx(0, NULL, _T(""), WS_CHILD | WS_VISIBLE,
                                    DPI::Rect(310, 173, 184, 26),
                                    (CWnd *)&m_wndDialogBar, 0))
    return false;

  if (!m_wndInstToolReBar.Create(WS_CHILD | WS_VISIBLE, CRect(0, 0, 0, 0),
                                 &m_wndInstToolBarWnd, AFX_IDW_REBAR))
    return false;

  if (!m_wndInstToolBar.CreateEx(
          &m_wndInstToolReBar, TBSTYLE_FLAT | TBSTYLE_TRANSPARENT,
  ...
  //	// Set 24-bit icons
  //	HBITMAP hbm = (HBITMAP)::LoadImage(AfxGetInstanceHandle(),
  // MAKEINTRESOURCE(IDB_TOOLBAR_INST_256), IMAGE_BITMAP, 0, 0,
  // LR_CREATEDIBSECTION); 	m_bmInstToolbar.Attach(hbm);
...
void CMainFrame::ResizeFrameWindow() {
  ...
  CFamiTrackerDoc *pDocument =
      static_cast<CFamiTrackerDoc *>(GetActiveDocument());
  ...
  if (pDocument != NULL) {
    ...
    if (m_iFrameEditorPos == FRAME_EDIT_POS_TOP) {
      ...
      // Move frame controls
      m_wndFrameControls.MoveWindow(
          DPI::Rect(10, Height + 21, 150, 26)); // // //
    }
    // Located to the left
    else {
      // Frame editor window
      ...
      Height =
          rect.Height() - DPI::SY(CPatternEditor::HEADER_HEIGHT - 2); // // //
      Width = m_pFrameEditor->CalcWidth(Channels);

      m_pFrameEditor->MoveWindow(DPI::SX(2),
                                 DPI::SY(CPatternEditor::HEADER_HEIGHT + 1),
                                 DPI::SX(Width), Height); // // //
  ...
  m_wndDialogBar.MoveWindow(DialogStartPos, DPI::SY(2),
                            ParentRect.Width() - DialogStartPos,
                            ParentRect.Height() - DPI::SY(4)); // // //

  CRect ControlRect; // // //
  m_wndDialogBar.GetDlgItem(IDC_MAINFRAME_INST_TOOLBAR)
...
void CMainFrame::SetupColors() {
  ...
  m_pInstrumentList->SetBkColor(
      theApp.GetSettings()->Appearance.iColBackground);
  m_pInstrumentList->SetTextBkColor(
      theApp.GetSettings()->Appearance.iColBackground);
  m_pInstrumentList->SetTextColor(
      theApp.GetSettings()->Appearance.iColPatternText);
...
void CMainFrame::SetSpeed(int Speed) {
  CFamiTrackerDoc *pDoc = static_cast<CFamiTrackerDoc *>(GetActiveDocument());
  int MaxSpeed =
      pDoc->GetSongTempo(m_iTrack) ? pDoc->GetSpeedSplitPoint() - 1 : 0xFF;
...
void CMainFrame::SetRowCount(int Count) {
  ...
  AddAction(new CPActionPatternLen{
      std::min(std::max(Count, 1), MAX_PATTERN_LENGTH)}); // // //
...
void CMainFrame::SetFrameCount(int Count) {
  ...
  AddAction(new CFActionFrameCount{
      std::min(std::max(Count, 1), MAX_FRAMES)}); // // //
...
void CMainFrame::DisplayOctave() {
  CComboBox *pOctaveList =
      static_cast<CComboBox *>(m_wndOctaveBar.GetDlgItem(IDC_OCTAVE));
...
void CMainFrame::SelectInstrument(int Index) {
  ...
  if (pDoc->IsInstrumentUsed(Index)) {
    ...
    m_pInstrumentList->SetItemState(m_iInstrument, 0,
...
void CMainFrame::ShowInstrumentNumberText() {
  ...
  for (int i = 0; i < INST_DIGITS; ++i)
    if (i >= m_iInstNumDigit - 1)
      ...
      digit.AppendFormat(
          "%X", (m_iInstNumCurrent >> (4 * (INST_DIGITS - i - 1))) & 0xF);
  AfxFormatString1(msg, IDS_TYPE_INST_NUM, digit);
  SetMessageText(msg);
}

void CMainFrame::GetInstrumentName(char *pText) const {
  m_wndDialogBar.GetDlgItem(IDC_INSTNAME)
      ->GetWindowText(pText, CInstrument::INST_NAME_MAX);
}
...
void CMainFrame::OnInstNameChange() {
  ...
  if (SelInstIndex !=
      m_iInstrument) // Instrument selection out of sync, ignore name change
    return;
...
void CMainFrame::OnAddInstrument2A03() { NewInstrument(SNDCHIP_NONE); }

void CMainFrame::OnAddInstrumentVRC6() { NewInstrument(SNDCHIP_VRC6); }

void CMainFrame::OnAddInstrumentVRC7() { NewInstrument(SNDCHIP_VRC7); }

void CMainFrame::OnAddInstrumentFDS() { NewInstrument(SNDCHIP_FDS); }

void CMainFrame::OnAddInstrumentMMC5() { NewInstrument(SNDCHIP_MMC5); }

void CMainFrame::OnAddInstrumentN163() { NewInstrument(SNDCHIP_N163); }

void CMainFrame::OnAddInstrumentS5B() { NewInstrument(SNDCHIP_S5B); }
...
void CMainFrame::OnBnClickedEditInst() { OpenInstrumentEditor(); }

void CMainFrame::OnEditInstrument() { OpenInstrumentEditor(); }
...
void CMainFrame::OnLoadInstrument() {
  ...
  CFamiTrackerDoc *pDoc = static_cast<CFamiTrackerDoc *>(GetActiveDocument());
  CFileDialog FileDialog(TRUE, _T("fti"), 0,
  ...
  if (FileDialog.GetFileName().GetLength() == 0) // // //
    theApp.GetSettings()->SetPath(FileDialog.GetPathName() + _T("\\"),
                                  PATH_FTI);
  else
...
void CMainFrame::OnSaveInstrument() {
  ...
  CString filter = LoadDefaultFilter(IDS_FILTER_FTI, _T(".fti"));
  CFileDialog FileDialog(FALSE, _T("fti"), Name,
...
void CMainFrame::OnDeltaposTempoSpin(NMHDR *pNMHDR, LRESULT *pResult) {
  int NewTempo = CFamiTrackerDoc::GetDoc()->GetSongTempo(m_iTrack) -
                 ((NMUPDOWN *)pNMHDR)->iDelta;
  SetTempo(NewTempo);
}

void CMainFrame::OnDeltaposRowsSpin(NMHDR *pNMHDR, LRESULT *pResult) {
  int NewRows = CFamiTrackerDoc::GetDoc()->GetPatternLength(m_iTrack) -
                ((NMUPDOWN *)pNMHDR)->iDelta;
  SetRowCount(NewRows);
}

void CMainFrame::OnDeltaposFrameSpin(NMHDR *pNMHDR, LRESULT *pResult) {
  int NewFrames = CFamiTrackerDoc::GetDoc()->GetFrameCount(m_iTrack) -
                  ((NMUPDOWN *)pNMHDR)->iDelta;
...
{
  pCmdUI->Enable(
      static_cast<CFamiTrackerView *>(GetActiveView())->IsMarkerValid()
          ? TRUE
          : FALSE);
}
...
void CMainFrame::OnBnClickedIncFrame() {
  if (static_cast<CFamiTrackerView *>(GetActiveView())->GetSelectedFrame() !=
      GetFrameEditor()->GetEditFrame()) // // //
    return;
...
bool CMainFrame::ChangeAllPatterns() const {
  return m_wndFrameControls.IsDlgButtonChecked(IDC_CHANGE_ALL) != 0 &&
         !GetFrameEditor()->IsSelecting(); // // //
}

void CMainFrame::OnKeyRepeat() {
  theApp.GetSettings()->General.bKeyRepeat =
      (m_wndDialogBar.IsDlgButtonChecked(IDC_KEYREPEAT) == 1);
}

void CMainFrame::OnDeltaposKeyStepSpin(NMHDR *pNMHDR, LRESULT *pResult) {
  int Pos =
      m_wndDialogBar.GetDlgItemInt(IDC_KEYSTEP) - ((NMUPDOWN *)pNMHDR)->iDelta;
  Pos = std::max(Pos, 0);
...
void CMainFrame::OnUpdateSBInstrument(CCmdUI *pCmdUI) {
  ...
  unsigned int Split =
      static_cast<CFamiTrackerView *>(GetActiveView())->GetSplitInstrument();
...
void CMainFrame::OnUpdateSBOctave(CCmdUI *pCmdUI) {
  CString String;
  AfxFormatString1(String, ID_INDICATOR_OCTAVE,
                   MakeIntString(GetSelectedOctave())); // // //
  pCmdUI->Enable();
...
void CMainFrame::OnUpdateSBFrequency(CCmdUI *pCmdUI) {
  ...
  if (EngineSpeed == 0)
    EngineSpeed =
        (Machine == NTSC) ? CAPU::FRAME_RATE_NTSC : CAPU::FRAME_RATE_PAL;
...
void CMainFrame::OnUpdateModuleMoveframedown(CCmdUI *pCmdUI) {
  ...
  pCmdUI->Enable(
      !(pView->GetSelectedFrame() == (pDoc->GetFrameCount(m_iTrack) - 1)));
}
...
void CMainFrame::OnUpdateInstrumentClone(CCmdUI *pCmdUI) {
  pCmdUI->Enable(m_pInstrumentList->GetItemCount() > 0 &&
                 m_pInstrumentList->GetItemCount() < MAX_INSTRUMENTS);
}
...
void CMainFrame::OnTimer(UINT_PTR nIDEvent) {
  ...
  case TMR_AUTOSAVE: {
    /*
       ...
       dynamic_cast<CFamiTrackerDoc*>(GetActiveDocument()); if (pDoc != NULL)
                    pDoc->AutoSave();
                    */
  } break;
#endif
...
void CMainFrame::OnUpdateKeyStepEdit(CCmdUI *pCmdUI) {
  pCmdUI->SetText(MakeIntString(
      static_cast<CFamiTrackerView *>(GetActiveView())->GetStepping()));
}
...
void CMainFrame::OnUpdateSpeedEdit(CCmdUI *pCmdUI) {
  if (!m_pLockedEditSpeed->IsEditable()) {
    if (m_pLockedEditSpeed->Update())
      ...
      pCmdUI->SetText(
          MakeIntString(static_cast<CFamiTrackerDoc *>(GetActiveDocument())
                            ->GetSongSpeed(m_iTrack)));
    }
  }
}
  ...
  if (!m_pLockedEditHighlight1->IsEditable()) {
    ...
    if (m_pLockedEditHighlight1->Update())
      AddAction(new CPActionHighlight{stHighlight{
          std::max(0, std::min(MAX_PATTERN_LENGTH,
                               m_pLockedEditHighlight1->GetValue())),
          pDoc->GetHighlight().Second, 0}});
    else
  ...
  if (!m_pLockedEditHighlight2->IsEditable()) {
    ...
    if (m_pLockedEditHighlight2->Update())
      AddAction(new CPActionHighlight{stHighlight{
          ...
          std::max(0, std::min(MAX_PATTERN_LENGTH,
                               m_pLockedEditHighlight2->GetValue())),
          0}});
    else
...
void CMainFrame::SetSongInfo(const char *pName, const char *pArtist,
...
void CMainFrame::OnFileImportText() {
  ...
  CString sResult; // // //
  do
    sResult = Exporter.ImportFile(FileDialog.GetPathName(), pDoc);
  while (sResult == _T("Retry"));
  if (sResult.GetLength() > 0) {
    AfxMessageBox(sResult, MB_OK | MB_ICONEXCLAMATION);
  }

  SetSongInfo(pDoc->GetSongName(), pDoc->GetSongArtist(),
...
void CMainFrame::OnFileExportText() {
  ...
  CString fileFilter = LoadDefaultFilter(IDS_FILTER_TXT, _T(".txt"));
  HistoryFileDlg FileDialog(PATH_EXPORT, FALSE, _T(".txt"), DefFileName,
                            OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, fileFilter);
  // FileDialog.m_pOFN->lpstrInitialDir =
  // theApp.GetSettings()->GetPath(PATH_NSF);
...
void CMainFrame::OnFileExportRows() // // //
{
  ...
  HistoryFileDlg FileDialog(
      PATH_EXPORT, FALSE, _T(".csv"), DefFileName,
      OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT,
      _T("Comma-separated values (*.csv)|*.csv|All files|*.*||"));
  // FileDialog.m_pOFN->lpstrInitialDir =
  // theApp.GetSettings()->GetPath(PATH_NSF);
...
BOOL CMainFrame::DestroyWindow() {
  ...
  theApp.GetSettings()->SetWindowPos(WinRect.left, WinRect.top, WinRect.right,
                                     WinRect.bottom, State);

  return CFrameWnd::DestroyWindow();
}

BOOL CMainFrame::OnEraseBkgnd(CDC *pDC) { return FALSE; }
...
{
  theApp.GetSettings()->Display.bAverageBPM =
      !theApp.GetSettings()->Display.bAverageBPM;
}

void CMainFrame::OnTrackerDisplayChannelState() // // // 050B
{
  theApp.GetSettings()->Display.bChannelState =
      !theApp.GetSettings()->Display.bChannelState;
}

void CMainFrame::OnTrackerDisplayRegisterState() {
  theApp.GetSettings()->Display.bRegisterState =
      !theApp.GetSettings()->Display.bRegisterState; // // //
}

void CMainFrame::OnUpdateDisplayAverageBPM(CCmdUI *pCmdUI) // // // 050B
{
  pCmdUI->SetCheck(theApp.GetSettings()->Display.bAverageBPM ? MF_CHECKED
                                                             : MF_UNCHECKED);
}

void CMainFrame::OnUpdateDisplayChannelState(CCmdUI *pCmdUI) // // // 050B
{
  pCmdUI->SetCheck(theApp.GetSettings()->Display.bChannelState ? MF_CHECKED
                                                               : MF_UNCHECKED);
}

void CMainFrame::OnUpdateDisplayRegisterState(CCmdUI *pCmdUI) // // //
{
  pCmdUI->SetCheck(theApp.GetSettings()->Display.bRegisterState ? MF_CHECKED
                                                                : MF_UNCHECKED);
}
...
void CMainFrame::OnModuleBookmarkSettings() // // //
{
  ...
  m_pBookmarkDlg->SetManager(static_cast<CFamiTrackerDoc *>(GetActiveDocument())
                                 ->GetBookmarkManager());
  m_pBookmarkDlg->LoadBookmarks(m_iTrack);
}
...
void CMainFrame::UpdateTrackBox() {
  ...
  CComboBox *pTrackBox =
      static_cast<CComboBox *>(m_wndDialogBar.GetDlgItem(IDC_SUBTUNE));
  CFamiTrackerDoc *pDoc = static_cast<CFamiTrackerDoc *>(GetActiveDocument());
  CString Text;
...
void CMainFrame::OnRemoveFocus() { GetActiveView()->SetFocus(); }
...
void CMainFrame::OnToggleFollow() {
  m_wndOctaveBar.CheckDlgButton(IDC_FOLLOW,
                                !m_wndOctaveBar.IsDlgButtonChecked(IDC_FOLLOW));
  OnClickedFollow();
}
...
{
  m_wndOctaveBar.CheckDlgButton(
      IDC_CHECK_COMPACT, !m_wndOctaveBar.IsDlgButtonChecked(IDC_CHECK_COMPACT));
  OnClickedCompact();
}
...
void CMainFrame::OnDeltaposHighlightSpin1(NMHDR *pNMHDR,
                                          LRESULT *pResult) // // //
{
  if (CFamiTrackerDoc *pDoc =
          static_cast<CFamiTrackerDoc *>(GetActiveDocument())) {
    stHighlight Hl = pDoc->GetHighlight();
    Hl.First = std::max(0, std::min(MAX_PATTERN_LENGTH,
                                    Hl.First - ((NMUPDOWN *)pNMHDR)->iDelta));
    AddAction(new CPActionHighlight{Hl});
    theApp.GetSoundGenerator()->SetHighlightRows(Hl.First); // // //
  }
}

void CMainFrame::OnDeltaposHighlightSpin2(NMHDR *pNMHDR,
                                          LRESULT *pResult) // // //
{
  if (CFamiTrackerDoc *pDoc =
          static_cast<CFamiTrackerDoc *>(GetActiveDocument())) {
    stHighlight Hl = pDoc->GetHighlight();
    Hl.Second = std::max(0, std::min(MAX_PATTERN_LENGTH,
                                     Hl.Second - ((NMUPDOWN *)pNMHDR)->iDelta));
    AddAction(new CPActionHighlight{Hl});
  }
}

void CMainFrame::OnSelectPatternEditor() { GetActiveView()->SetFocus(); }

void CMainFrame::OnSelectFrameEditor() { m_pFrameEditor->EnableInput(); }
...
void CMainFrame::OnUpdateEditCopy(CCmdUI *pCmdUI) {
  CFamiTrackerView *pView = static_cast<CFamiTrackerView *>(GetActiveView());
  pCmdUI->Enable((pView->IsSelecting() || GetFocus() == m_pFrameEditor) ? 1
                                                                        : 0);
}
...
{
  CFamiTrackerView *pView = static_cast<CFamiTrackerView *>(GetActiveView());
  pCmdUI->Enable((pView->IsSelecting() && GetFocus() == GetActiveView()) ? 1
                                                                         : 0);
}
...
void CMainFrame::OnUpdateEditPaste(CCmdUI *pCmdUI) {
  if (GetFocus() == GetActiveView())
    pCmdUI->Enable(
        static_cast<CFamiTrackerView *>(GetActiveView())->IsClipboardAvailable()
            ? 1
            : 0);
  else if (GetFocus() == GetFrameEditor())
...
CFrameEditor *CMainFrame::GetFrameEditor() const { return m_pFrameEditor; }

CVisualizerWnd *CMainFrame::GetVisualizerWindow() const {
  return m_pVisualizerWnd;
}

void CMainFrame::OnEditEnableMIDI() { theApp.GetMIDI()->ToggleInput(); }
...
void CMainFrame::OnDestroy() {
  ...
  if (pSoundGen && pSoundGen->IsRunning()) {
    ...
    pSoundGen->PostThreadMessage(WM_USER_CLOSE_SOUND, (WPARAM)pSoundEvent,
                                 NULL);
    // Wait for sound to close
    DWORD dwResult = ::WaitForSingleObject(pSoundEvent->m_hObject,
                                           CSoundGen::AUDIO_TIMEOUT + 1000);

    if (dwResult != WAIT_OBJECT_0) {
      // The CEvent object will leak if this happens, but the program won't
      // crash
...
int CMainFrame::GetSelectedTrack() const { return m_iTrack; }
...
void CMainFrame::SelectTrack(unsigned int Track) {
  ...
  if (theApp.IsPlaying() &&
      Track != theApp.GetSoundGenerator()->GetPlayerTrack()) // // // 050B
    theApp.ResetPlayer();
...
{
  static_cast<CFamiTrackerView *>(GetActiveView())
      ->AdjustOctave(Octave - GetSelectedOctave());
  m_iOctave = Octave;
  DisplayOctave();
}
...
void CMainFrame::OnNewInstrumentMenu(NMHDR *pNotifyStruct, LRESULT *result) {
  ...
  int Chip = pDoc->GetExpansionChip();
  int SelectedChip = pDoc->GetChannel(pView->GetSelectedChannel())
                         ->GetChip(); // where the cursor is located

  menu.AppendMenu(MF_STRING, ID_INSTRUMENT_ADD_2A03, _T("New 2A03 instrument"));

  if (Chip & SNDCHIP_VRC6)
    menu.AppendMenu(MF_STRING, ID_INSTRUMENT_ADD_VRC6,
                    _T("New VRC6 instrument"));
  if (Chip & SNDCHIP_VRC7)
    menu.AppendMenu(MF_STRING, ID_INSTRUMENT_ADD_VRC7,
                    _T("New VRC7 instrument"));
  if (Chip & SNDCHIP_FDS)
    menu.AppendMenu(MF_STRING, ID_INSTRUMENT_ADD_FDS, _T("New FDS instrument"));
  if (Chip & SNDCHIP_MMC5)
    menu.AppendMenu(MF_STRING, ID_INSTRUMENT_ADD_MMC5,
                    _T("New MMC5 instrument"));
  if (Chip & SNDCHIP_N163)
    menu.AppendMenu(MF_STRING, ID_INSTRUMENT_ADD_N163,
                    _T("New Namco instrument"));
  if (Chip & SNDCHIP_S5B)
    menu.AppendMenu(MF_STRING, ID_INSTRUMENT_ADD_S5B,
                    _T("New Sunsoft instrument"));
  ...
  menu.TrackPopupMenu(TPM_LEFTALIGN | TPM_RIGHTBUTTON, rect.left, rect.bottom,
                      this);
}
...
void CMainFrame::OnLoadInstrumentMenu(NMHDR *pNotifyStruct, LRESULT *result) {
  ...
  if (!m_pInstrumentFileTree) {
    ...
    m_pInstrumentFileTree->BuildMenuTree(
        theApp.GetSettings()->InstrumentMenuPath);
  ...
  UINT retValue = m_pInstrumentFileTree->GetMenu()->TrackPopupMenu(
      TPM_LEFTALIGN | TPM_RIGHTBUTTON | TPM_RETURNCMD | TPM_NONOTIFY, rect.left,
      rect.bottom, this);
...
void CMainFrame::SelectInstrumentFolder() {
  ...
  //	Browse.lpszTitle	= _T("Choose a folder where you have instrument
  // files"); 	Browse.hwndOwner	= m_hWnd; 	Browse.pidlRoot = NULL;
  // Browse.lpfn			= NULL; 	Browse.ulFlags =
  // BIF_RETURNONLYFSDIRS | BIF_NEWDIALOGSTYLE; 	Browse.pszDisplayName =
  // Path;
...
void CMainFrame::UpdateMenu(CMenu *pMenu) {
  ...
  for (UINT i = 0; i < static_cast<unsigned int>(pMenu->GetMenuItemCount());
       ++i) { // // //
...
void CMainFrame::OnEditSelectother() // // //
{
  ...
  if (GetFocus() == pView) {
    ...
    if (m_pFrameEditor->IsSelecting()) {
      const CFrameSelection Sel =
          m_pFrameEditor->GetSelection().GetNormalized();
      m_pFrameEditor->CancelSelection();
      ...
      NewSel.m_cpEnd.m_iRow =
          pDoc->GetCurrentPatternLength(m_iTrack, Sel.m_cpStart.m_iFrame) - 1;
      NewSel.m_cpStart.m_iChannel = Sel.m_cpStart.m_iChannel;
      NewSel.m_cpEnd.m_iChannel = Sel.m_cpEnd.m_iChannel;
      NewSel.m_cpStart.m_iColumn = C_NOTE;
      NewSel.m_cpEnd.m_iColumn =
          pEditor->GetChannelColumns(Sel.m_cpEnd.m_iChannel);
...
void CMainFrame::OnDecayFast() {
  theApp.GetSoundGenerator()->SetMeterDecayRate(
      theApp.GetSettings()->MeterDecayRate = DECAY_FAST); // // // 050B
}

void CMainFrame::OnDecaySlow() {
  theApp.GetSoundGenerator()->SetMeterDecayRate(
      theApp.GetSettings()->MeterDecayRate = DECAY_SLOW); // // // 050B
}

void CMainFrame::OnUpdateDecayFast(CCmdUI *pCmdUI) // // // 050B
{
  pCmdUI->SetCheck(theApp.GetSoundGenerator()->GetMeterDecayRate() == DECAY_FAST
                       ? MF_CHECKED
                       : MF_UNCHECKED);
}
...
{
  pCmdUI->SetCheck(theApp.GetSoundGenerator()->GetMeterDecayRate() == DECAY_SLOW
                       ? MF_CHECKED
                       : MF_UNCHECKED);
}
...
void CMainFrame::OnEditClearPatterns() {
  ...
  if (AfxMessageBox(IDS_CLEARPATTERN,
...
void CMainFrame::OnEditRemoveUnusedInstruments() {
  ...
  if (AfxMessageBox(IDS_REMOVE_INSTRUMENTS,
                    MB_YESNO | MB_ICONWARNING | MB_DEFBUTTON2) == IDNO)
    return;
...
void CMainFrame::OnEditRemoveUnusedPatterns() {
  ...
  if (AfxMessageBox(IDS_REMOVE_PATTERNS,
                    MB_YESNO | MB_ICONWARNING | MB_DEFBUTTON2) == IDNO)
    return;
...
void CMainFrame::SetFrameEditorPosition(int Position) {
  ...
  ResizeFrameWindow(); // This must be called twice or the editor disappears, I
                       // don't know why
...
void CMainFrame::SetControlPanelPosition(
    control_panel_pos_t Position) // // // 050B
{
  ...
  case CONTROL_PANEL_POS_TOP:
          m_wndToolBar.SetBarStyle(CBRS_ALIGN_TOP | CBRS_BORDER_BOTTOM |
  CBRS_TOOLTIPS | CBRS_FLYBY); m_wndToolBar.CalcFixedLayout(TRUE, FALSE); break;
...
void CMainFrame::OnUpdateFrameTitle(BOOL bAddToTitle) {
  ...
  // Add name of subtune
  title.AppendFormat(_T(" [#%i %s]"), m_iTrack + 1,
                     pDoc->GetTrackTitle(GetSelectedTrack()).GetString());
...
void CMainFrame::OnEasterEggKraid5() {
  if (m_iKraidCounter == 4) {
    if (AfxMessageBox(IDS_KRAID, MB_YESNO | MB_ICONWARNING | MB_DEFBUTTON2) ==
        IDNO) {
      m_iKraidCounter = 0;
      return;
    }
    ...
    ResetUndo();
    ResizeFrameWindow();
    SetStatusText(_T("Famitracker - Metroid - Kraid's Lair (Uploaded on Jun ")
                  _T("9, 2010 http://www.youtube.com/watch?v=9yzCLy-fZVs) The ")
                  _T("FTM straight from the tutorial. - 8BitDanooct1"));
  }
  m_iKraidCounter = 0;
}

void CMainFrame::OnFindNext() { m_pFindDlg->OnBnClickedButtonFindNext(); }
...
void CMainFrame::ResetFind() { m_pFindDlg->Reset(); }

void CMainFrame::OnEditRemoveUnusedSamples() {
  CFamiTrackerDoc *pDoc = static_cast<CFamiTrackerDoc *>(GetActiveDocument());

  if (AfxMessageBox(IDS_REMOVE_SAMPLES,
                    MB_YESNO | MB_ICONWARNING | MB_DEFBUTTON2) == IDNO)
    return;
...
void CMainFrame::OnEditPopulateUniquePatterns() // // //
{
  ...
  if (AfxMessageBox(IDS_POPULATE_PATTERNS,
                    MB_YESNO | MB_ICONWARNING | MB_DEFBUTTON2) == IDNO)
    return;
...
void CMainFrame::OnTrackerPal() {
  ...
  theApp.GetSoundGenerator()->DocumentPropertiesChanged(pDoc); // // //
  m_wndInstEdit.SetRefreshRate(
      static_cast<float>(pDoc->GetFrameRate())); // // //
}
...
void CMainFrame::OnUpdateTrackerPal(CCmdUI *pCmdUI) {
  ...
  if (pCmdUI->m_pMenu != NULL)
    pCmdUI->m_pMenu->CheckMenuRadioItem(ID_TRACKER_NTSC, ID_TRACKER_PAL, item,
                                        MF_BYCOMMAND);
}
