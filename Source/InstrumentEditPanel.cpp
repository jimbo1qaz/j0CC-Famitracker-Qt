...
CInstrumentEditPanel::CInstrumentEditPanel(UINT nIDTemplate, CWnd *pParent)
...
BOOL CInstrumentEditPanel::OnEraseBkgnd(CDC *pDC) { return FALSE; }
...
void CInstrumentEditPanel::SetInstrumentManager(
    CInstrumentManager *pManager) // // //
{
...
CSequenceInstrumentEditPanel::CSequenceInstrumentEditPanel(UINT nIDTemplate,
                                                           CWnd *pParent)
    : CInstrumentEditPanel(nIDTemplate, pParent), m_pSequenceEditor(NULL),
      m_pSequence(NULL), m_pParentWin(pParent),
      m_iSelectedSetting(0), m_pParser{new CSequenceParser{}} // // //
{}
...
void CSequenceInstrumentEditPanel::SetupDialog(LPCTSTR *pListItems) {
  ...
  pList->InsertColumn(2, _T("Effect name"), LVCFMT_LEFT,
                      static_cast<int>(.6 * Width));
  ...
  CSpinButtonCtrl *pSequenceSpin =
      static_cast<CSpinButtonCtrl *>(GetDlgItem(IDC_SEQUENCE_SPIN));
...
void CSequenceInstrumentEditPanel::OnRClickInstSettings(NMHDR *pNMHDR,
                                                        LRESULT *pResult) {
  POINT oPoint;
  GetCursorPos(&oPoint);
  ...
  pMenu->EnableMenuItem(ID_CLONE_SEQUENCE, (m_pSequence->GetItemCount() != 0)
                                               ? MF_ENABLED
                                               : MF_DISABLED);
  pMenu->TrackPopupMenu(TPM_LEFTALIGN | TPM_RIGHTBUTTON, oPoint.x, oPoint.y,
                        this);
}
