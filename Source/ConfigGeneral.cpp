...
const CString CConfigGeneral::CONFIG_DESC[] = {
    // // //
    _T("Wrap around the cursor when reaching top or bottom in the pattern ")
    _T("editor."),
    _T("Move to next or previous frame when reaching top or bottom in the ")
    _T("pattern editor."),
    _T("Unlock the cursor from the center of the pattern editor."),
    _T("Preview wave and DPCM files in the open file dialog when loading ")
    _T("samples to the module."),
    _T("Enable key repetition in the pattern editor."),
    _T("Display row numbers and the frame count on the status bar in ")
    _T("hexadecimal."),
    ...
    _T("Ignore the pattern step setting when moving the cursor, only use it ")
    _T("when inserting notes."),
    _T("Make delete key pull up rows rather than only deleting the value, as ")
    _T("if by Shift+Delete."),
    _T("Create backup copy of unmodified file, when you open and save a ")
    _T("module."),
    ...
    _T("Do not select the whole channel when double-clicking in the pattern ")
    _T("editor."),
    _T("When using Shift + Mouse Wheel to modify a pattern value, allow the ")
    _T("parameter to wrap around its limit values."),
    _T("Always silent volume values below 1 due to Axy or 7xy effects."),
    _T("Use the existing volume table for the FDS channel which has higher ")
    _T("precision than in exported NSFs."),
    _T("Reconstruct the current channel's state from previous frames upon ")
    _T("playing (except when playing one row)."),
    _T("Move pasted pattern data outside the rows of the current frame to ")
    _T("subsequent frames."),
    _T("Display rows that are truncated by Bxx, Cxx, or Dxx effects."),
    _T("Use the extra keys on the keypad as hexadecimal digits in the pattern ")
    _T("editor."),
    _T("Allow pattern selections to span across multiple frames."),
    _T("Check for new 0CC-FamiTracker versions on startup if an internet ")
    _T("connection could be established."),
};
...
BOOL CConfigGeneral::OnSetActive() {
  ...
  static_cast<CComboBox *>(GetDlgItem(IDC_COMBO_STYLE))
      ->SetCurSel(m_iEditStyle); // // //

  return CPropertyPage::OnSetActive();
}

void CConfigGeneral::OnOK() { CPropertyPage::OnOK(); }
...
BOOL CConfigGeneral::OnInitDialog() {
  ...
  GetKeyNameText(MapVirtualKey(m_iKeyNoteRelease, MAPVK_VK_TO_VSC) << 16, Text,
                 64);
  ...
  GetKeyNameText(MapVirtualKey(m_iKeyEchoBuffer, MAPVK_VK_TO_VSC) << 16, Text,
                 64); // // //
  ...
  const bool CONFIG_BOOL[SETTINGS_BOOL_COUNT] = {
      // // //
      m_bWrapCursor,     m_bWrapFrames,      m_bFreeCursorEdit,
      m_bPreviewWAV,     m_bKeyRepeat,       m_bRowInHex,
      m_bFramePreview,   m_bNoDPCMReset,     m_bNoStepMove,
      m_bPullUpDelete,   m_bBackups,         m_bSingleInstance,
      m_bPreviewFullRow, m_bDisableDblClick, m_bWrapPatternValue,
      m_bCutVolume,      m_bFDSOldVolume,    m_bRetrieveChanState,
      m_bOverflowPaste,  m_bShowSkippedRows, m_bHexKeypad,
      m_bMultiFrameSel,  m_bCheckVersion,
  };
  ...
  pList->InsertColumn(1, _T("Option"), LVCFMT_LEFT,
                      r.Width() - 20 - ::GetSystemMetrics(SM_CXHSCROLL));
...
void CConfigGeneral::OnCbnEditupdatePagelength() { SetModified(); }

void CConfigGeneral::OnCbnSelendokPagelength() { SetModified(); }

void CConfigGeneral::OnCbnSelchangeComboStyle() // // //
{
  m_iEditStyle =
      static_cast<CComboBox *>(GetDlgItem(IDC_COMBO_STYLE))->GetCurSel();
  SetModified();
}

void CConfigGeneral::OnLvnItemchangedConfigList(NMHDR *pNMHDR,
                                                LRESULT *pResult) // // //
{
  ...
  if (pNMLV->uChanged & LVIF_STATE) {
    if (pNMLV->uNewState & LVNI_SELECTED || pNMLV->uNewState & 0x3000) {
      ...
      if (pNMLV->iItem >= 0 && pNMLV->iItem < SETTINGS_BOOL_COUNT)
        pList->SetItemState(pNMLV->iItem, LVIS_SELECTED | LVIS_FOCUSED,
                            LVIS_SELECTED | LVIS_FOCUSED);
    }
