...
CGraphEditor::CGraphEditor(CSequence *pSequence)
    : m_pBackDC(NULL), m_pBitmap(NULL), m_pSmallFont(NULL),
      m_iHighlightedItem(-1), m_iHighlightedValue(0), m_bButtonState(false),
      m_pSequence(pSequence), m_iLastPlayPos(0) {
...
BOOL CGraphEditor::OnEraseBkgnd(CDC *pDC) { return FALSE; }

BOOL CGraphEditor::CreateEx(DWORD dwExStyle, LPCTSTR lpszClassName,
                            LPCTSTR lpszWindowName, DWORD dwStyle,
                            const RECT &rect, CWnd *pParentWnd, UINT nID,
                            LPVOID lpParam) {
  if (CWnd::CreateEx(dwExStyle, lpszClassName, lpszWindowName, dwStyle, rect,
                     pParentWnd, nID, lpParam) == -1)
    return -1;
  ...
  m_pBitmap->CreateCompatibleBitmap(pDC, m_ClientRect.Width(),
                                    m_ClientRect.Height());
...
void CGraphEditor::OnPaint() { CPaintDC dc(this); }
...
void CGraphEditor::PaintBuffer(CDC *pBackDC, CDC *pFrontDC) {
  ...
  pFrontDC->BitBlt(0, 0, m_ClientRect.Width(), m_ClientRect.Height(), pBackDC,
                   0, 0, SRCCOPY);
}

void CGraphEditor::DrawBackground(CDC *pDC, int Lines, bool DrawMarks,
                                  int MarkOffset) const {
...
void CGraphEditor::DrawLoopPoint(CDC *pDC, int StepWidth) const {
  ...
  if (LoopPoint > -1) {
    ...
    GradientBar(pDC, x + 1, m_BottomRect.top, m_BottomRect.right - x,
                m_BottomRect.Height(), 0x808000, 0x202000);
...
void CGraphEditor::DrawReleasePoint(CDC *pDC, int StepWidth) const {
  ...
  if (ReleasePoint > -1) {
    ...
    GradientBar(pDC, x + 1, m_BottomRect.top, m_BottomRect.right - x,
                m_BottomRect.Height(), 0x800080, 0x200020);
...
void CGraphEditor::DrawLoopRelease(CDC *pDC, int StepWidth) const // // //
{
  ...
  if (ReleasePoint > LoopPoint) {
    ...
    GradientBar(pDC, x + 1, m_BottomRect.top, m_BottomRect.right - x,
                m_BottomRect.Height(), 0x008080, 0x002020);
...
template <COLORREF COL_BG1, COLORREF COL_BG2, COLORREF COL_EDGE1,
          COLORREF COL_EDGE2>
void CGraphEditor::DrawRect(CDC *pDC, int x, int y, int w, int h,
                            bool flat) const {
  if (h == 0)
    h = 1;
...
void CGraphEditor::DrawPlayRect(CDC *pDC, int x, int y, int w, int h) const {
  DrawRect<0xA0F0A0, 0x86DC86, RGB(198, 242, 198), RGB(106, 223, 106)>(
      pDC, x, y, w, h, false);
}

void CGraphEditor::DrawCursorRect(CDC *pDC, int x, int y, int w, int h) const {
  DrawRect<0xFFE0C0, 0xE5C699, RGB(215, 235, 253), RGB(120, 182, 226)>(
      pDC, x, y, w, h, m_bButtonState);
}
  ...
  if (nFlags & MK_LBUTTON) {
    ...
    case EDIT_LINE: {
      ...
      if (m_iEditing == EDIT_LINE) {
        ...
        int StartX =
            (m_ptLineStart.x < m_ptLineEnd.x ? m_ptLineStart.x : m_ptLineEnd.x);
        int StartY =
            (m_ptLineStart.x < m_ptLineEnd.x ? m_ptLineStart.y : m_ptLineEnd.y);
        int EndX =
            (m_ptLineStart.x > m_ptLineEnd.x ? m_ptLineStart.x : m_ptLineEnd.x);
        int EndY =
            (m_ptLineStart.x > m_ptLineEnd.x ? m_ptLineStart.y : m_ptLineEnd.y);
...
bool CGraphEditor::IsEditLine() const { return m_iEditing == EDIT_LINE; }
...
void CBarGraphEditor::OnPaint() {
  ...
  if (m_iHighlightedValue > 0 && m_iHighlightedItem >= 0 &&
      m_iHighlightedItem < Count) {
  ...
  for (int i = 0; i < Count; i++) {
    ...
    else if ((m_iHighlightedItem == i) &&
             (m_pSequence->GetItem(i) >= m_iHighlightedValue) && !IsEditLine())
...
void CBarGraphEditor::HighlightItem(CPoint point) {
  ...
  int ItemValue = m_iLevels - (((point.y - GetItemTop()) + (ItemHeight / 2)) /
                               ItemHeight); // // //
  ...
  if (m_GraphRect.PtInRect(point) == 0 ||
      unsigned(ItemIndex) >= m_pSequence->GetItemCount()) {
...
void CBarGraphEditor::ModifyItem(CPoint point, bool Redraw) {
  ...
  int ItemIndex = (point.x - GRAPH_LEFT) / ItemWidth;
  int ItemValue =
      m_iLevels - (((point.y - GetItemTop()) + (ItemHeight / 2)) / ItemHeight);
...
CArpeggioGraphEditor::CArpeggioGraphEditor(CSequence *pSequence)
    : CGraphEditor(pSequence), m_iScrollOffset(0), m_pScrollBar(NULL) {}

CArpeggioGraphEditor::~CArpeggioGraphEditor() { SAFE_RELEASE(m_pScrollBar); }
...
void CArpeggioGraphEditor::Initialize() {
  ...
  m_pScrollBar->Create(SBS_VERT | SBS_LEFTALIGN | WS_CHILD | WS_VISIBLE,
                       CRect(m_GraphRect.right, m_GraphRect.top,
                             m_GraphRect.right + SCROLLBAR_WIDTH,
                             m_GraphRect.bottom),
                       this, 0);
...
void CArpeggioGraphEditor::DrawRange(CDC *pDC, int Max, int Min) {
  const char NOTES_A[] = {'C', 'C', 'D', 'D', 'E', 'F',
                          'F', 'G', 'G', 'A', 'A', 'B'};
  const char NOTES_B[] = {'-', '#', '-', '#', '-', '-',
                          '#', '-', '#', '-', '#', '-'};
  ...
  if (m_pSequence->GetSetting() != SETTING_ARP_FIXED) {
    ...
    // Fixed
    pDC->FillSolidRect(m_ClientRect.left, m_GraphRect.top, m_GraphRect.left,
                       m_GraphRect.bottom, 0x00);
    ...
    line.Format(_T("%s%d"), stChanNote::NOTE_NAME[GET_NOTE(NoteValue) - 1],
                GET_OCTAVE(NoteValue)); // // //
...
void CArpeggioGraphEditor::OnPaint() {
  ...
  DrawBackground(m_pBackDC, ITEMS, true, // // //
                 m_pSequence->GetSetting() == SETTING_ARP_FIXED
                     ? 2 - m_iScrollOffset
                     : -m_iScrollOffset);
  ...
  // One last line
  m_pBackDC->FillSolidRect(m_GraphRect.left + 1, Top + ITEMS * StepHeight,
                           m_GraphRect.Width() - 2, 1, COLOR_LINES);

  if (m_iHighlightedItem >= 0 && m_iHighlightedItem < Count) {
    int item; // // //
    if (m_pSequence->GetSetting() == SETTING_ARP_SCHEME) {
      int value = (m_iHighlightedValue + 0x100) % 0x40;
      if (value > ARPSCHEME_MAX)
        value -= 0x40;
    ...
    if (item >= 0 && item <= ITEMS) {
      ...
      if (m_iCurrentPlayPos == i)
        DrawPlayRect(m_pBackDC, x, y, w, h);
               ...
               (m_pSequence->GetItem(i) == m_iHighlightedValue) &&
               !IsEditLine())
        DrawCursorRect(m_pBackDC, x, y, w, h);
      else
        DrawRect(m_pBackDC, x, y, w, h);
      ...
      if (m_pSequence->GetSetting() == SETTING_ARP_SCHEME) {
        ...
        m_pBackDC->TextOut(x + w / 2, y - 2 * h,
                           HEAD[(m_pSequence->GetItem(i) & 0xFF) >> 6]);
      }
    }
  }
...
void CArpeggioGraphEditor::OnVScroll(UINT nSBCode, UINT nPos,
  ...
  case SB_BOTTOM:
    m_iScrollOffset =
        (m_pSequence->GetSetting() != SETTING_ARP_FIXED) ? -m_iScrollMax : 0;
    break;
...
void CPitchGraphEditor::OnPaint() {
  ...
  m_pBackDC->FillSolidRect(m_GraphRect.left,
                           m_GraphRect.top + m_GraphRect.Height() / 2,
                           m_GraphRect.Width(), 1, COLOR_LINES);
  ...
  // One last line
  // m_pBackDC->FillSolidRect(m_GraphRect.left + 1, m_GraphRect.top + 20 *
  // StepHeight, m_GraphRect.Width() - 2, 1, COLOR_LINES);
...
void CPitchGraphEditor::HighlightItem(CPoint point) {
  ...
  if (m_GraphRect.PtInRect(point) == 0 || ItemIndex < 0 ||
      unsigned(ItemIndex) >= m_pSequence->GetItemCount()) {
    m_iHighlightedItem = -1;
    m_iHighlightedValue = 0;
  }
...
void CNoiseEditor::OnPaint() {
  ...
  // // // One last line
  m_pBackDC->FillSolidRect(m_GraphRect.left + 1,
                           Top + (m_iItems + 1) * StepHeight,
                           m_GraphRect.Width() - 2, 1, COLOR_LINES);
  ...
  for (int i = 0; i < Count; i++) {
    ...
    static const int BAR_MODE[] = {S5B_MODE_ENVELOPE, S5B_MODE_SQUARE,
                                   S5B_MODE_NOISE}; // // //
    ...
    for (int i = 0; i < sizeof(BAR_MODE) / sizeof(int); ++i) {
      ...
      m_pBackDC->Draw3dRect(x, y, w, h, BLEND(Color, 0xFFFFFF, 80),
                            BLEND(Color, 0x000000, 80));
    }
  }
...
void CNoiseEditor::ModifyItem(CPoint point, bool Redraw) {
  ...
  if (point.y >= Offset) {
    ...
    case 0:
      ItemValue = m_pSequence->GetItem(ItemIndex) ^ S5B_MODE_ENVELOPE;
      break;
    case 1:
...
void CNoiseEditor::ModifyReleased() { m_iLastIndex = -1; }
