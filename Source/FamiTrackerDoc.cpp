...
/*
 ...
 Ver 3.0
  - Sequences are stored in the way they are represented in the instrument
 editor
  - Added separate speed and tempo settings
  - Changed automatic portamento to 3xx and added 1xx & 2xx portamento

 Ver 2.1
  - Made some additions to support multiple effect columns and prepared for more
 channels
...
/*
         ...
         * 2.3 - Support for release points & extra setting in sequences, 2A03 &
   VRC6
...
std::pair<EffTable, EffTable> MakeEffectConversion(
    std::initializer_list<std::pair<effect_t, effect_t>> List) {
...
CFamiTrackerDoc::CFamiTrackerDoc()
    : m_bFileLoaded(false), m_bFileLoadFailed(false), m_iRegisteredChannels(0),
      m_iNamcoChannels(0), // // //
...
// Synchronization
BOOL CFamiTrackerDoc::LockDocument() const { return m_csDocumentLock.Lock(); }
...
BOOL CFamiTrackerDoc::OnNewDocument() {
  ...
  // CFamiTrackerDoc::OnNewDocument calls CDocument::OnNewDocument() and
  // CreateEmpty(). Both call DeleteContents. Opening files doesn't call
  // CDocument::OnNewDocument() but calls CreateEmpty(). Only the latter calls
  // DeleteContents.
  CreateEmpty();
...
BOOL CFamiTrackerDoc::OnOpenDocument(LPCTSTR lpszPathName) {
  ...
  if (!OpenDocument(lpszPathName)) {
    ...
    DeleteContents();
    CreateEmpty();
    for (int i = UPDATE_TRACK; i <= UPDATE_COLUMNS; ++i)		// // //
    test UpdateAllViews(NULL, i);
    */
...
BOOL CFamiTrackerDoc::OnSaveDocument(LPCTSTR lpszPathName) {
  ...
  if ((m_bForceBackup || theApp.GetSettings()->General.bBackups) &&
      !m_bBackupDone) {
    CString BakName;
...
void CFamiTrackerDoc::CreateEmpty() {
  // CFamiTrackerDoc::OnNewDocument calls CDocument::OnNewDocument() and
  // CreateEmpty(). Both call DeleteContents. OpenDocument() doesn't call
  // CDocument::OnNewDocument() but calls CreateEmpty(). Only the latter calls
  // DeleteContents.
...
#ifdef _DEBUG
void CFamiTrackerDoc::AssertValid() const { CDocument::AssertValid(); }

void CFamiTrackerDoc::Dump(CDumpContext &dc) const { CDocument::Dump(dc); }
#endif //_DEBUG
...
void CFamiTrackerDoc::ReorderSequences() {
  ...
  for (int i = 0; i < MAX_INSTRUMENTS; ++i) {
    if (auto pInst = std::dynamic_pointer_cast<CInstrument2A03>(
            m_pInstrumentManager->GetInstrument(i))) { // // //
      for (int j = 0; j < SEQ_COUNT; ++j) {
        if (pInst->GetSeqEnable(j)) {
          ...
          if (Indices[Index][j] >= 0 && Indices[Index][j] != -1) {
            ...
            m_pInstrumentManager->SetSequence(INST_2A03, j, Slots[j]++,
                                              Seq.Convert(j));
          }
        } else
...
void CFamiTrackerDoc::AssertFileData(bool Cond, std::string Msg) const {
  if (l <= theApp.GetSettings()->Version.iErrorLevel && !Cond) {
    CModuleException *e = m_pCurrentDocument
                              ? m_pCurrentDocument->GetException()
                              : new CModuleException();
    e->AppendError(Msg);
    e->Raise();
  }
}
...
000x: Version int, version number 000x: Start of blocks

 {FILE_BLOCK_PARAMS, 2}
  Expansion chip							char
  Channels int Machine type
...
BOOL CFamiTrackerDoc::SaveDocument(LPCTSTR lpszPathName) const {
  ...
  DWORD err = 0;
  if (!ReplaceFile(lpszPathName, TempFile, NULL,
                   REPLACEFILE_IGNORE_MERGE_ERRORS, NULL, NULL)) {
    err = GetLastError();
    if (err == ERROR_FILE_NOT_FOUND) {
      err = 0;
      if (!MoveFileEx(TempFile, lpszPathName,
...
bool CFamiTrackerDoc::WriteBlocks(CDocumentFile *pDocFile) const {
  ...
  static bool (CFamiTrackerDoc::*FTM_WRITE_FUNC[])(CDocumentFile *, const int)
      const = {// // //
  ...
  for (size_t i = 0; i < sizeof(FTM_WRITE_FUNC) / sizeof(*FTM_WRITE_FUNC);
       ++i) {
    if (!CALL_MEMBER_FN(this, FTM_WRITE_FUNC[i])(pDocFile,
                                                 DEFAULT_BLOCK_VERSION[i]))
      return false;
  }
  return true;
}

bool CFamiTrackerDoc::WriteBlock_Parameters(CDocumentFile *pDocFile,
...
bool CFamiTrackerDoc::WriteBlock_SongInfo(CDocumentFile *pDocFile,
                                          const int Version) const {
  // Song info
...
bool CFamiTrackerDoc::WriteBlock_Header(CDocumentFile *pDocFile,
                                        const int Version) const {
  /*
   *  Header data
   *
...
bool CFamiTrackerDoc::WriteBlock_Instruments(CDocumentFile *pDocFile,
                                             const int Version) const {
  // A bug in v0.3.0 causes a crash if this is not 2, so change only when that
  // ver is obsolete!
  //
  // Log:
  ...
  /*	if (m_iExpansionChip & SNDCHIP_FDS)
          ...
          for (int i = 0; i < MAX_INSTRUMENTS; ++i) {
                  if (m_pInstrumentManager->GetInstrument(i)->GetType() ==
     INST_FDS) Version = 4;
          }
  */
...
bool CFamiTrackerDoc::WriteBlock_Sequences(CDocumentFile *pDocFile,
                                           const int Version) const {
  /*
...
bool CFamiTrackerDoc::WriteBlock_SequencesVRC6(CDocumentFile *pDocFile,
                                               const int Version) const {
  /*
...
bool CFamiTrackerDoc::WriteBlock_SequencesN163(CDocumentFile *pDocFile,
                                               const int Version) const {
  /*
...
bool CFamiTrackerDoc::WriteBlock_SequencesS5B(CDocumentFile *pDocFile,
                                              const int Version) const {
  /*
   * Store 5B sequences
   */
...
bool CFamiTrackerDoc::WriteBlock_Frames(CDocumentFile *pDocFile,
                                        const int Version) const {
  /* Store frame count
   *
...
bool CFamiTrackerDoc::WriteBlock_Patterns(CDocumentFile *pDocFile,
                                          const int Version) const {
  /*
   * Version changes:
   *
          ...
          for (unsigned y = 0; y < PatternLen; y++) {
            if (!m_pTracks[t]->IsCellFree(i, x, y)) {
              ...
              for (int n = 0; n < EffColumns; n++) {
                pDocFile->WriteBlockChar(
                    EFF_CONVERSION_050
                        .second[Note->EffNumber[n]]); // // // 050B
...
bool CFamiTrackerDoc::WriteBlock_DSamples(CDocumentFile *pDocFile,
                                          const int Version) const {
...
bool CFamiTrackerDoc::WriteBlock_Comments(CDocumentFile *pDocFile,
                                          const int Version) const {
...
bool CFamiTrackerDoc::WriteBlock_ChannelLayout(CDocumentFile *pDocFile,
                                               const int Version) const {
...
BOOL CFamiTrackerDoc::OpenDocument(LPCTSTR lpszPathName) {
  ...
  // Open file
  if (!OpenFile.Open(lpszPathName, CFile::modeRead | CFile::shareDenyWrite,
                     &ex)) {
    TCHAR szCause[1024]; // // //
    CString strFormatted;
  ...
  while (FileBlock != FB_EOF) {
    ...
    case FB_SEQUENCES:
      ...
      for (i = 0; i < ReadCount; i++) {
        ...
        if (ImportedSequence.Count > 0 &&
            ImportedSequence.Count < MAX_SEQUENCE_ITEMS)
      ...
      for (unsigned int x = 0; x < m_iChannelsAvailable; x++) {
        for (c = 0; c < ReadCount; c++) {
          for (i = 0; i < PatternLength; i++) {
            ...
            Note->EffNumber[0] =
                static_cast<effect_t>(ImportedNote.ExtraStuff1);
    ...
    case FB_DSAMPLES:
      ...
      for (i = 0; i < ReadCount; i++) {
        ...
        if (ImportedDSample.SampleSize != 0 &&
            ImportedDSample.SampleSize < 0x4000) {
          ImportedDSample.SampleData = new char[ImportedDSample.SampleSize];
          pOpenFile->Read(ImportedDSample.SampleData,
                          ImportedDSample.SampleSize);
        } else
...
BOOL CFamiTrackerDoc::OpenDocumentNew(CDocumentFile &DocumentFile) {
  static std::unordered_map<std::string, void (CFamiTrackerDoc::*)(
                                             CDocumentFile *, const int)>
      FTM_READ_FUNC;
  ...
  FTM_READ_FUNC[FILE_BLOCK_INSTRUMENTS] =
      &CFamiTrackerDoc::ReadBlock_Instruments;
  ...
  FTM_READ_FUNC[FILE_BLOCK_SEQUENCES_VRC6] =
      &CFamiTrackerDoc::ReadBlock_SequencesVRC6;
  FTM_READ_FUNC[FILE_BLOCK_SEQUENCES_N163] =
      &CFamiTrackerDoc::ReadBlock_SequencesN163;
  FTM_READ_FUNC[FILE_BLOCK_SEQUENCES_N106] =
      &CFamiTrackerDoc::ReadBlock_SequencesN163; // Backward compatibility
  FTM_READ_FUNC[FILE_BLOCK_SEQUENCES_S5B] =
      &CFamiTrackerDoc::ReadBlock_SequencesS5B; // // //
  FTM_READ_FUNC[FILE_BLOCK_DETUNETABLES] =
      &CFamiTrackerDoc::ReadBlock_DetuneTables; // // //
  FTM_READ_FUNC[FILE_BLOCK_GROOVES] =
      &CFamiTrackerDoc::ReadBlock_Grooves; // // //
  FTM_READ_FUNC[FILE_BLOCK_BOOKMARKS] =
      &CFamiTrackerDoc::ReadBlock_Bookmarks; // // //
  FTM_READ_FUNC[FILE_BLOCK_PARAMS_EXTRA] =
      &CFamiTrackerDoc::ReadBlock_ParamsExtra;                       // // //
  ...
  while (!DocumentFile.Finished() && !ErrorFlag) {
    ...
    try {
      CALL_MEMBER_FN(this, FTM_READ_FUNC.at(BlockID))
      (&DocumentFile, DocumentFile.GetBlockVersion()); // // //
  ...
  if (m_bAdjustFDSArpeggio) {
    ...
    for (int i = 0; i < MAX_INSTRUMENTS; ++i) {
      if (GetInstrumentType(i) == INST_FDS) {
        CSequence *pSeq =
            std::static_pointer_cast<CSeqInstrument>(GetInstrument(i))
                ->GetSequence(SEQ_ARPEGGIO);
        if (pSeq != nullptr && pSeq->GetItemCount() > 0 &&
            pSeq->GetSetting() == SETTING_ARP_FIXED)
...
void CFamiTrackerDoc::ReadBlock_Parameters(CDocumentFile *pDocFile,
                                           const int Version) {
  ...
  m_iChannelsAvailable = AssertRange(pDocFile->GetBlockInt(), 1, MAX_CHANNELS,
                                     "Channel count"); // // //
  AssertRange<MODULE_ERROR_OFFICIAL>(static_cast<int>(m_iChannelsAvailable), 1,
                                     MAX_CHANNELS - 1, "Channel count");
  ...
  if (Version >= 7) { // // // 050B
    ...
    case 1:
      m_iEngineSpeed =
          static_cast<int>(1000000. / pDocFile->GetBlockInt() + .5);
      break;
    case 0:
    case 2:
    default:
  ...
  if (Version == 1) {
    if (pTrack->GetSongSpeed() > 19) {
      ...
      pTrack->SetSongTempo(m_iMachine == NTSC ? DEFAULT_TEMPO_NTSC
                                              : DEFAULT_TEMPO_PAL);
    }
  }
  ...
  if (Version >= 5 && (m_iExpansionChip & SNDCHIP_N163))
    m_iNamcoChannels =
        AssertRange(pDocFile->GetBlockInt(), 1, 8, "N163 channel count");
  else // // //
    m_iNamcoChannels = 0;
  ...
  AssertRange<MODULE_ERROR_STRICT>(m_iExpansionChip, 0, 0x3F,
                                   "Expansion chip flag");
...
void CFamiTrackerDoc::ReadBlock_SongInfo(CDocumentFile *pDocFile,
                                         const int Version) // // //
{
...
void CFamiTrackerDoc::ReadBlock_Header(CDocumentFile *pDocFile,
                                       const int Version) {
  if (Version == 1) {
    // Single track
    m_iTrackCount = 1;
    ...
    for (unsigned int i = 0; i < m_iChannelsAvailable; ++i)
      try {
        // Channel type (unused)
        AssertRange<MODULE_ERROR_STRICT>(pDocFile->GetBlockChar(), 0,
                                         CHANNELS - 1, "Channel type index");
        // Effect columns
        pTrack->SetEffectColumnCount(
            i, AssertRange<MODULE_ERROR_STRICT>(pDocFile->GetBlockChar(), 0,
                                                MAX_EFFECT_COLUMNS - 1,
                                                "Effect column count"));
      } catch (CModuleException *e) {
    ...
    // Multiple tracks
    m_iTrackCount = AssertRange(pDocFile->GetBlockChar() + 1, 1,
                                static_cast<int>(MAX_TRACKS),
                                "Track count"); // 0 means one track
    ...
    for (unsigned i = 0; i < m_iChannelsAvailable; ++i)
      try {
        AssertRange<MODULE_ERROR_STRICT>(
            pDocFile->GetBlockChar(), 0, CHANNELS - 1,
            "Channel type index"); // Channel type (unused)
        for (unsigned j = 0; j < m_iTrackCount; ++j)
          try {
            GetTrack(j)->SetEffectColumnCount(
                i, AssertRange<MODULE_ERROR_STRICT>(pDocFile->GetBlockChar(), 0,
                                                    MAX_EFFECT_COLUMNS - 1,
                                                    "Effect column count"));
          } catch (CModuleException *e) {
...
void CFamiTrackerDoc::ReadBlock_Comments(CDocumentFile *pDocFile,
                                         const int Version) {
  m_bDisplayComment = (pDocFile->GetBlockInt() == 1) ? true : false;
  m_strComment = pDocFile->ReadString();
}

void CFamiTrackerDoc::ReadBlock_ChannelLayout(CDocumentFile *pDocFile,
                                              const int Version) {
  // Todo
}

void CFamiTrackerDoc::ReadBlock_Instruments(CDocumentFile *pDocFile,
                                            const int Version) {
  /*
   * Version changes
   *
  ...
  const int Count =
      AssertRange(pDocFile->GetBlockInt(), 0,
                  CInstrumentManager::MAX_INSTRUMENTS, "Instrument count");

  for (int i = 0; i < Count; ++i) {
    // Instrument index
    int index = AssertRange(pDocFile->GetBlockInt(), 0,
                            CInstrumentManager::MAX_INSTRUMENTS - 1,
                            "Instrument index");
    ...
    m_pInstrumentManager->InsertInstrument(
        index, pInstrument); // this registers the instrument content provider

    try {
      // Load the instrument
      AssertFileData(pInstrument.get() != nullptr,
                     "Failed to create instrument");
      pInstrument->Load(pDocFile);
      // Read name
      int size =
          AssertRange(pDocFile->GetBlockInt(), 0, CInstrument::INST_NAME_MAX,
...
void CFamiTrackerDoc::ReadBlock_Sequences(CDocumentFile *pDocFile,
                                          const int Version) {
  unsigned int Count =
      AssertRange(pDocFile->GetBlockInt(), 0, MAX_SEQUENCES * SEQ_COUNT,
                  "2A03 sequence count");
  AssertRange<MODULE_ERROR_OFFICIAL>(
      Count, 0U, static_cast<unsigned>(MAX_SEQUENCES * SEQ_COUNT - 1),
      "2A03 sequence count"); // // //

  if (Version == 1) {
    for (unsigned int i = 0; i < Count; ++i) {
      COldSequence Seq;
      unsigned int Index = AssertRange(pDocFile->GetBlockInt(), 0,
                                       MAX_SEQUENCES - 1, "Sequence index");
      unsigned int SeqCount =
          static_cast<unsigned char>(pDocFile->GetBlockChar());
      AssertRange(SeqCount, 0U, static_cast<unsigned>(MAX_SEQUENCE_ITEMS - 1),
                  "Sequence item count");
    ...
    for (unsigned int i = 0; i < Count; ++i) {
      ...
      unsigned int Index = AssertRange(pDocFile->GetBlockInt(), 0,
                                       MAX_SEQUENCES - 1, "Sequence index");
      unsigned int Type = AssertRange(pDocFile->GetBlockInt(), 0, SEQ_COUNT - 1,
                                      "Sequence type");
      unsigned int SeqCount =
          static_cast<unsigned char>(pDocFile->GetBlockChar());
      AssertRange(SeqCount, 0U, static_cast<unsigned>(MAX_SEQUENCE_ITEMS - 1),
      ...
      m_pInstrumentManager->SetSequence(INST_2A03, Type, Index,
                                        Seq.Convert(Type)); // // //
    }
  } else if (Version >= 3) {
    CSequenceManager *pManager = GetSequenceManager(INST_2A03); // // //
    ...
    for (unsigned int i = 0; i < Count; ++i) {
      unsigned int Index = Indices[i] = AssertRange(
          pDocFile->GetBlockInt(), 0, MAX_SEQUENCES - 1, "Sequence index");
      unsigned int Type = Types[i] = AssertRange(
          pDocFile->GetBlockInt(), 0, SEQ_COUNT - 1, "Sequence type");
      try {
        unsigned char SeqCount = pDocFile->GetBlockChar();
        CSequence *pSeq = pManager->GetCollection(Type)->GetSequence(Index);
        pSeq->Clear();
        pSeq->SetItemCount(SeqCount < MAX_SEQUENCE_ITEMS ? SeqCount
                                                         : MAX_SEQUENCE_ITEMS);

        unsigned int LoopPoint = AssertRange<MODULE_ERROR_STRICT>(
            pDocFile->GetBlockInt(), -1, static_cast<int>(SeqCount),
            "Sequence loop point");
        ...
        if (Version == 4) {
          ...
          pSeq->SetReleasePoint(AssertRange<MODULE_ERROR_STRICT>(
              ReleasePoint, -1, static_cast<int>(SeqCount) - 1,
        ...
        e->AppendError("At 2A03 %s sequence %d,",
                       CInstrument2A03::SEQUENCE_NAME[Type], Index);
        throw;
      }
    }
    ...
    if (Version == 5) {
      ...
      for (unsigned int i = 0; i < MAX_SEQUENCES; ++i) {
        for (int j = 0; j < SEQ_COUNT; ++j)
          try {
            ...
            e->AppendError("At 2A03 %s sequence %d,",
                           CInstrument2A03::SEQUENCE_NAME[j], i);
            throw;
          }
      }
    } else if (Version >= 6) {
      ...
      for (unsigned int i = 0; i < Count; ++i)
        try {
          CSequence *pSeq =
              pManager->GetCollection(Types[i])->GetSequence(Indices[i]);
          pSeq->SetReleasePoint(AssertRange<MODULE_ERROR_STRICT>(
              pDocFile->GetBlockInt(), -1,
              static_cast<int>(pSeq->GetItemCount()) - 1,
              "Sequence release point"));
          pSeq->SetSetting(
              static_cast<seq_setting_t>(pDocFile->GetBlockInt())); // // //
        } catch (CModuleException *e) {
          e->AppendError("At 2A03 %s sequence %d,",
                         CInstrument2A03::SEQUENCE_NAME[Types[i]], Indices[i]);
          throw;
        }
    }
  }
...
void CFamiTrackerDoc::ReadBlock_SequencesVRC6(CDocumentFile *pDocFile,
                                              const int Version) {
  unsigned int Count =
      AssertRange(pDocFile->GetBlockInt(), 0, MAX_SEQUENCES * SEQ_COUNT,
                  "VRC6 sequence count");
  AssertRange<MODULE_ERROR_OFFICIAL>(Count, 0U,
                                     static_cast<unsigned>(MAX_SEQUENCES),
                                     "VRC6 sequence count"); // // //
  ...
  for (unsigned int i = 0; i < Count; ++i) {
    ...
    unsigned int Type = Types[i] =
        AssertRange(pDocFile->GetBlockInt(), 0, SEQ_COUNT - 1, "Sequence type");
    try {
      unsigned char SeqCount = pDocFile->GetBlockChar();
      CSequence *pSeq = pManager->GetCollection(Type)->GetSequence(Index);
      pSeq->Clear();
      ...
      pSeq->SetLoopPoint(AssertRange<MODULE_ERROR_STRICT>(
          pDocFile->GetBlockInt(), -1, static_cast<int>(SeqCount) - 1,
          "Sequence loop point"));
      ...
      e->AppendError("At VRC6 %s sequence %d,",
                     CInstrumentVRC6::SEQUENCE_NAME[Type], Index);
      throw;
    }
  }
  ...
  if (Version == 5) {
    ...
    for (int i = 0; i < MAX_SEQUENCES; ++i) {
      for (int j = 0; j < SEQ_COUNT; ++j)
        try {
          ...
          e->AppendError("At VRC6 %s sequence %d,",
                         CInstrumentVRC6::SEQUENCE_NAME[j], i);
          throw;
        }
    }
  } else if (Version >= 6) {
    for (unsigned int i = 0; i < Count; ++i)
      try {
        ...
        e->AppendError("At VRC6 %s sequence %d,",
                       CInstrumentVRC6::SEQUENCE_NAME[Types[i]], Indices[i]);
        throw;
      }
  }
}

void CFamiTrackerDoc::ReadBlock_SequencesN163(CDocumentFile *pDocFile,
                                              const int Version) {
  unsigned int Count =
      AssertRange(pDocFile->GetBlockInt(), 0, MAX_SEQUENCES * SEQ_COUNT,
                  "N163 sequence count");
  ...
  for (unsigned int i = 0; i < Count; i++) {
    ...
    try {
      ...
      e->AppendError("At N163 %s sequence %d,",
                     CInstrumentN163::SEQUENCE_NAME[Type], Index);
      throw;
    }
  }
}

void CFamiTrackerDoc::ReadBlock_SequencesS5B(CDocumentFile *pDocFile,
                                             const int Version) {
  unsigned int Count =
      AssertRange(pDocFile->GetBlockInt(), 0, MAX_SEQUENCES * SEQ_COUNT,
                  "5B sequence count");
  ...
  for (unsigned int i = 0; i < Count; i++) {
    ...
    try {
      ...
      e->AppendError("At 5B %s sequence %d,",
                     CInstrumentS5B::SEQUENCE_NAME[Type], Index);
      throw;
    }
  }
}

void CFamiTrackerDoc::ReadBlock_Frames(CDocumentFile *pDocFile,
                                       const int Version) {
  if (Version == 1) {
    unsigned int FrameCount = AssertRange(pDocFile->GetBlockInt(), 1,
                                          MAX_FRAMES, "Track frame count");
    m_iChannelsAvailable =
        AssertRange(pDocFile->GetBlockInt(), 0, MAX_CHANNELS, "Channel count");
    CPatternData *pTrack = GetTrack(0);
    pTrack->SetFrameCount(FrameCount);
    for (unsigned i = 0; i < FrameCount; ++i) {
      for (unsigned j = 0; j < m_iChannelsAvailable; ++j) {
        unsigned Pattern = static_cast<unsigned char>(pDocFile->GetBlockChar());
        AssertRange(Pattern, 0U, static_cast<unsigned>(MAX_PATTERN - 1),
                    "Pattern index");
    ...
    for (unsigned y = 0; y < m_iTrackCount; ++y) {
      unsigned int FrameCount = AssertRange(pDocFile->GetBlockInt(), 1,
                                            MAX_FRAMES, "Track frame count");
      unsigned int Speed = AssertRange<MODULE_ERROR_STRICT>(
          pDocFile->GetBlockInt(), 0, MAX_TEMPO, "Track default speed");

      CPatternData *pTrack = GetTrack(y);
      pTrack->SetFrameCount(FrameCount);

      if (Version >= 3) {
        unsigned int Tempo = AssertRange<MODULE_ERROR_STRICT>(
            pDocFile->GetBlockInt(), 0, MAX_TEMPO, "Track default tempo");
        // Speed
        // <= MAX_SPEED);
      ...
      unsigned PatternLength =
          AssertRange(pDocFile->GetBlockInt(), 1, MAX_PATTERN_LENGTH,
                      "Track default row count");
      pTrack->SetPatternLength(PatternLength);
      ...
      for (unsigned i = 0; i < FrameCount; ++i) {
        for (unsigned j = 0; j < m_iChannelsAvailable; ++j) {
          ...
          pTrack->SetFramePattern(
              i, j, AssertRange(Pattern, 0, MAX_PATTERN - 1, "Pattern index"));
        }
      }
    }
  }
...
void CFamiTrackerDoc::ReadBlock_Patterns(CDocumentFile *pDocFile,
                                         const int Version) {
#ifdef TRANSPOSE_FDS
  ...
  if (Version == 1) {
    int PatternLen = AssertRange(pDocFile->GetBlockInt(), 0, MAX_PATTERN_LENGTH,
                                 "Pattern data count");
    CPatternData *pTrack = GetTrack(0);
  ...
  while (!pDocFile->BlockDone()) {
    ...
    if (Version > 1)
      Track =
          AssertRange(pDocFile->GetBlockInt(), 0,
                      static_cast<int>(MAX_TRACKS) - 1, "Pattern track index");
    else if (Version == 1)
      Track = 0;

    unsigned Channel = AssertRange(pDocFile->GetBlockInt(), 0, MAX_CHANNELS - 1,
                                   "Pattern channel index");
    unsigned Pattern = AssertRange(pDocFile->GetBlockInt(), 0, MAX_PATTERN - 1,
                                   "Pattern index");
    unsigned Items = AssertRange(pDocFile->GetBlockInt(), 0, MAX_PATTERN_LENGTH,
                                 "Pattern data count");
    ...
    for (unsigned i = 0; i < Items; ++i)
      try {
        ...
        if (m_iFileVersion == 0x0200 || Version >= 6)
          ...
          Row = AssertRange(pDocFile->GetBlockInt(), 0, 0xFF,
                            "Row index"); // // //
        ...
        try {
          ...
          *Note = stChanNote{}; // // //
          ...
          if (ExpansionEnabled(SNDCHIP_N163) &&
              GetChipType(Channel) == SNDCHIP_N163) { // // //
          ...
          if (Version == 3) {
            ...
            else if (ExpansionEnabled(SNDCHIP_FDS) &&
                     GetChannelType(Channel) == CHANID_FDS) {
              for (int n = 0; n < MAX_EFFECT_COLUMNS; ++n) {
                switch (Note->EffNumber[n]) {
                case EF_PITCH:
                  ...
                  if (GetChannelType(Channel) == CHANID_NOISE) {
                          for (int n = 0; n < MAX_EFFECT_COLUMNS; ++n) {
                                  switch (Note->EffNumber[n]) {
                                          case EF_PORTA_DOWN:
                                                  Note->EffNumber[n] =
          EF_PORTA_UP; Note->EffParam[n] = Note->EffParam[n] << 4; break; case
          EF_PORTA_UP: Note->EffNumber[n] = EF_PORTA_DOWN; Note->EffParam[n] =
          Note->EffParam[n] << 4; break; case EF_PORTAMENTO: Note->EffParam[n] =
          Note->EffParam[n] << 4; break; case EF_SLIDE_UP: Note->EffParam[n] =
          Note->EffParam[n] + 0x70; break; case EF_SLIDE_DOWN: Note->EffParam[n]
          = Note->EffParam[n] + 0x70; break;
                                  }
                          }
                  }
          }
        ...
        e->AppendError("At pattern %02X, channel %d, track %d,", Pattern,
                       Channel, Track + 1);
        throw;
      }
  }
...
void CFamiTrackerDoc::ReadBlock_DSamples(CDocumentFile *pDocFile,
                                         const int Version) {
  unsigned int Count =
      AssertRange(static_cast<unsigned char>(pDocFile->GetBlockChar()), 0U,
                  CDSampleManager::MAX_DSAMPLES, "DPCM sample count");

  for (unsigned int i = 0; i < Count; ++i) {
    unsigned int Index =
        AssertRange(static_cast<unsigned char>(pDocFile->GetBlockChar()), 0U,
                    CDSampleManager::MAX_DSAMPLES - 1, "DPCM sample index");
    CDSample *pSample = nullptr;
    try {
      pSample = new CDSample(); // // //
      unsigned int Len =
          AssertRange(pDocFile->GetBlockInt(), 0, CDSample::MAX_NAME_SIZE - 1,
                      "DPCM sample name length");
      char Name[CDSample::MAX_NAME_SIZE] = {};
      pDocFile->GetBlock(Name, Len);
      pSample->SetName(Name);
      int Size =
          AssertRange(pDocFile->GetBlockInt(), 0, 0x7FFF, "DPCM sample size");
      AssertFileData<MODULE_ERROR_STRICT>(Size <= 0xFF1 && Size % 0x10 == 1,
                                          "Bad DPCM sample size");
...
void CFamiTrackerDoc::ReadBlock_DetuneTables(CDocumentFile *pDocFile,
                                             const int Version) {
  int Count = AssertRange(pDocFile->GetBlockChar(), 0, 6, "Detune table count");
  for (int i = 0; i < Count; i++) {
    int Chip =
        AssertRange(pDocFile->GetBlockChar(), 0, 5, "Detune table index");
    try {
      int Item = AssertRange(pDocFile->GetBlockChar(), 0, NOTE_COUNT,
                             "Detune table note count");
      for (int j = 0; j < Item; j++) {
        int Note = AssertRange(pDocFile->GetBlockChar(), 0, NOTE_COUNT - 1,
...
bool CFamiTrackerDoc::WriteBlock_DetuneTables(CDocumentFile *pDocFile,
                                              const int Version) const {
...
void CFamiTrackerDoc::ReadBlock_Grooves(CDocumentFile *pDocFile,
                                        const int Version) {
  const int Count =
      AssertRange(pDocFile->GetBlockChar(), 0, MAX_GROOVE, "Groove count");

  for (int i = 0; i < Count; i++) {
    int Index = AssertRange(pDocFile->GetBlockChar(), 0, MAX_GROOVE - 1,
                            "Groove index");
    try {
      int Size = AssertRange(pDocFile->GetBlockChar(), 1, MAX_GROOVE_SIZE,
                             "Groove size");
      ...
      for (int j = 0; j < Size; j++)
        try {
          m_pGrooveTable[Index]->SetEntry(
              j,
              AssertRange(static_cast<unsigned char>(pDocFile->GetBlockChar()),
                          1U, 0xFFU, "Groove item"));
        } catch (CModuleException *e) {
  ...
  AssertFileData<MODULE_ERROR_STRICT>(
      Tracks == m_iTrackCount,
      "Use-groove flag count does not match track count");
...
bool CFamiTrackerDoc::WriteBlock_Grooves(CDocumentFile *pDocFile,
                                         const int Version) const {
  int Count = 0;
  for (int i = 0; i < MAX_GROOVE; i++)
    if (m_pGrooveTable[i] != NULL)
      Count++;
  if (!Count)
    return true;
...
void CFamiTrackerDoc::ReadBlock_Bookmarks(CDocumentFile *pDocFile,
                                          const int Version) {
  ...
  for (int i = 0; i < Count; i++) {
    ...
    unsigned int Track =
        AssertRange(static_cast<unsigned char>(pDocFile->GetBlockChar()), 0,
                    m_iTrackCount - 1, "Bookmark track index");
    int Frame = static_cast<unsigned char>(pDocFile->GetBlockChar());
    int Row = static_cast<unsigned char>(pDocFile->GetBlockChar());
    pMark->m_iFrame = AssertRange(
        Frame, 0, static_cast<int>(m_pTracks[Track]->GetFrameCount()) - 1,
        "Bookmark frame index");
    pMark->m_iRow = AssertRange(
        Row, 0, static_cast<int>(m_pTracks[Track]->GetPatternLength()) - 1,
        "Bookmark row index");
...
bool CFamiTrackerDoc::WriteBlock_Bookmarks(CDocumentFile *pDocFile,
                                           const int Version) const {
...
const json DEFAULT = {{N163_OFFSET, 0}};

void CFamiTrackerDoc::ReadBlock_JSON(CDocumentFile *pDocFile,
                                     const int Version) {
  json out(DEFAULT);
  ...
  for (auto it : in.items()) {
    auto key = it.key(); // std::string is bad? If exists non-string, we can't
                         // handle it. So pass verbatim into unknowns.
  ...
  if (!unknowns.empty()) {
    auto err =
        "Warning: unknown JSON data (will be discarded):\n" + unknowns.dump();
...
bool CFamiTrackerDoc::WriteBlock_JSON(CDocumentFile *pDocFile,
                                      const int Version) const {
  const json j = {{N163_OFFSET, GetN163LevelOffset()}};
  if (j == DEFAULT) {
    return true;
  }
...
void CFamiTrackerDoc::ReadBlock_ParamsExtra(CDocumentFile *pDocFile,
                                            const int Version) {
  m_bLinearPitch = pDocFile->GetBlockInt() != 0;
  if (Version >= 2) {
    m_iDetuneSemitone = AssertRange(pDocFile->GetBlockChar(), -12, 12,
                                    "Global semitone tuning");
    m_iDetuneCent =
        AssertRange(pDocFile->GetBlockChar(), -100, 100, "Global cent tuning");
  }
}

bool CFamiTrackerDoc::WriteBlock_ParamsExtra(CDocumentFile *pDocFile,
                                             const int Version) const {
  if (!m_bLinearPitch && !m_iDetuneSemitone && !m_iDetuneCent)
    return true;
...
bool CFamiTrackerDoc::ImportInstruments(CFamiTrackerDoc *pImported,
                                        int *pInstTable) {
  ...
  if (GetInstrumentCount() + pImported->GetInstrumentCount() >
      MAX_INSTRUMENTS) {
  ...
  static const inst_type_t inst[] = {INST_2A03, INST_VRC6, INST_N163,
                                     INST_S5B}; // // //
  static const uint8_t chip[] = {SNDCHIP_NONE, SNDCHIP_VRC6, SNDCHIP_N163,
                                 SNDCHIP_S5B};
  int(*seqTable[])[SEQ_COUNT] = {SequenceTable2A03, SequenceTableVRC6,
                                 SequenceTableN163, SequenceTableS5B};

  // Copy sequences
  for (size_t i = 0; i < sizeof(chip); i++)
    for (int t = 0; t < SEQ_COUNT; ++t) {
      if (GetSequenceCount(inst[i], t) +
              pImported->GetSequenceCount(inst[i], t) >
          MAX_SEQUENCES) { // // //
        ...
        for (int t = 0; t < SEQ_COUNT; ++t)
          if (pInstrument->GetSeqEnable(t)) {
            for (size_t j = 0; j < sizeof(chip); j++)
              if (inst[j] == Type) {
                pInstrument->SetSeqIndex(
                    t, seqTable[j][pInstrument->GetSeqIndex(t)][t]);
                break;
              }
          }
      } break;
...
bool CFamiTrackerDoc::ImportGrooves(CFamiTrackerDoc *pImported,
                                    int *pGrooveMap) // // //
{
  int Index = 0;
  for (int i = 0; i < MAX_GROOVE; i++) {
    if (pImported->GetGroove(i) != NULL) {
      while (GetGroove(Index) != NULL)
        Index++;
      if (Index >= MAX_GROOVE) {
...
bool CFamiTrackerDoc::ImportTrack(int Track, CFamiTrackerDoc *pImported,
                                  int *pInstTable, int *pGrooveMap) // // //
{
  ...
  for (unsigned int f = 0; f < pImported->GetFrameCount(Track); ++f) {
    for (unsigned int c = 0; c < GetAvailableChannels(); ++c) {
      SetPatternAtFrame(NewTrack, f, c,
                        pImported->GetPatternAtFrame(Track, f, c));
    }
  }

  // // // Copy bookmarks
  m_pBookmarkManager->SetCollection(
      NewTrack, pImported->GetBookmarkManager()->PopCollection(Track));
...
bool CFamiTrackerDoc::IsSampleUsed(unsigned int Index) const {
  ASSERT(Index < MAX_DSAMPLES);
  return m_pInstrumentManager->GetDSampleManager()->IsSampleUsed(
      Index); // // //
}
...
CSequence *CFamiTrackerDoc::GetSequence(inst_type_t InstType,
                                        unsigned int Index,
                                        int Type) const // // //
{
...
unsigned int CFamiTrackerDoc::GetSequenceItemCount(inst_type_t InstType,
                                                   unsigned int Index,
                                                   int Type) const // // //
{
...
int CFamiTrackerDoc::GetFreeSequence(inst_type_t InstType, int Type,
                                     CSeqInstrument *pInst) const // // //
{
  ASSERT(Type >= 0 && Type < SEQ_COUNT);
  return m_pInstrumentManager->GetFreeSequenceIndex(InstType, Type, pInst);
}

int CFamiTrackerDoc::GetSequenceCount(inst_type_t InstType,
                                      int Type) const // // //
{
  ...
  int Count = 0;
  for (int i = 0; i < MAX_SEQUENCES; ++i) {
    if (GetSequenceItemCount(InstType, i, Type) >
        0) // TODO: fix this and the instrument interface
      ++Count;
  }
  return Count;
}
...
const char *CFamiTrackerDoc::GetSongName() const { return m_strName; }

const char *CFamiTrackerDoc::GetSongArtist() const { return m_strArtist; }

const char *CFamiTrackerDoc::GetSongCopyright() const { return m_strCopyright; }
...
std::shared_ptr<CInstrument>
CFamiTrackerDoc::GetInstrument(unsigned int Index) const {
...
void CFamiTrackerDoc::AddInstrument(CInstrument *pInstrument,
                                    unsigned int Slot) {
...
int CFamiTrackerDoc::AddInstrument(const char *pName, int ChipType) {
  ...
  if (!pInst) {
#ifdef _DEBUG
    MessageBox(NULL, _T("(TODO) add instrument definitions for this chip"),
               _T("Stop"), MB_OK);
#endif
    return INVALID_INSTRUMENT;
  }
...
int CFamiTrackerDoc::CloneInstrument(unsigned int Index) {
  ...
  if (Slot != INVALID_INSTRUMENT) {
    m_pInstrumentManager->InsertInstrument(
        Slot, m_pInstrumentManager->GetInstrument(Index)->Clone());
    SetModifiedFlag();
    SetExceededFlag(); // // //
  }
...
int CFamiTrackerDoc::DeepCloneInstrument(unsigned int Index) {
  ...
  if (Slot != INVALID_INSTRUMENT) {
    ...
    if (auto pInstrument = std::dynamic_pointer_cast<CSeqInstrument>(newInst)) {
      for (int i = 0; i < SEQ_COUNT; i++) {
        int freeSeq = m_pInstrumentManager->GetFreeSequenceIndex(
            it, i, pInstrument.get());
        if (freeSeq != -1) {
          if (pInstrument->GetSeqEnable(i))
            GetSequence(it, unsigned(freeSeq), i)
                ->Copy(pInstrument->GetSequence(i));
...
void CFamiTrackerDoc::SaveInstrument(unsigned int Index,
...
int CFamiTrackerDoc::LoadInstrument(CString FileName) {
  // Loads an instrument from file, return allocated slot or INVALID_INSTRUMENT
  // if failed
  //
  ...
  try {
    ...
    unsigned int NameLen =
        AssertRange(static_cast<int>(file.ReadInt()), 0,
                    CInstrument::INST_NAME_MAX, "Instrument name length");
    ...
    m_csDocumentLock.Unlock();
    return Slot;
  } catch (int ID) { // // // TODO: put all error messages into string table
                     // then add exception ctor
    m_csDocumentLock.Unlock();
...
void CFamiTrackerDoc::SetFrameCount(unsigned int Track, unsigned int Count) {
  ...
  if (Old != Count) {
    ...
    if (Count < Old)
      m_pBookmarkManager->GetCollection(Track)->RemoveFrames(Count,
                                                             Old - Count);
    SetModifiedFlag();
...
void CFamiTrackerDoc::SetPatternLength(unsigned int Track,
                                       unsigned int Length) {
  ASSERT(Track < MAX_TRACKS);
...
unsigned int CFamiTrackerDoc::GetCurrentPatternLength(unsigned int Track,
                                                      int Frame) const // // //
{
...
unsigned int CFamiTrackerDoc::GetEffColumns(unsigned int Track,
...
void CFamiTrackerDoc::SetEffColumns(unsigned int Track, unsigned int Channel,
...
unsigned int CFamiTrackerDoc::GetPatternAtFrame(unsigned int Track,
                                                unsigned int Frame,
                                                unsigned int Channel) const {
  ASSERT(Track < MAX_TRACKS);
...
void CFamiTrackerDoc::SetPatternAtFrame(unsigned int Track, unsigned int Frame,
...
void CFamiTrackerDoc::SetNoteData(unsigned int Track, unsigned int Frame,
                                  unsigned int Channel, unsigned int Row,
  ...
  memcpy(pTrack->GetPatternData(Channel, Pattern, Row), pData,
         sizeof(stChanNote));
  SetModifiedFlag();
}

void CFamiTrackerDoc::GetNoteData(unsigned int Track, unsigned int Frame,
                                  unsigned int Channel, unsigned int Row,
  ...
  CPatternData *pTrack = GetTrack(Track);
  int Pattern = pTrack->GetFramePattern(Frame, Channel);
  memcpy(pData, pTrack->GetPatternData(Channel, Pattern, Row),
         sizeof(stChanNote));
}

void CFamiTrackerDoc::SetDataAtPattern(unsigned int Track, unsigned int Pattern,
                                       unsigned int Channel, unsigned int Row,
                                       const stChanNote *pData) {
  ASSERT(Track < MAX_TRACKS);
  ASSERT(Pattern < MAX_PATTERN);
...
void CFamiTrackerDoc::GetDataAtPattern(unsigned int Track, unsigned int Pattern,
                                       unsigned int Channel, unsigned int Row,
                                       stChanNote *pData) const {
  ASSERT(Track < MAX_TRACKS);
  ASSERT(Pattern < MAX_PATTERN);
...
bool CFamiTrackerDoc::InsertRow(unsigned int Track, unsigned int Frame,
                                unsigned int Channel, unsigned int Row) {
  ASSERT(Track < MAX_TRACKS);
  ASSERT(Frame < MAX_FRAMES);
  ASSERT(Channel < MAX_CHANNELS);
  ...
  for (unsigned int i = PatternLen - 1; i > Row; i--) {
    memcpy(pTrack->GetPatternData(Channel, Pattern, i),
           pTrack->GetPatternData(Channel, Pattern, i - 1), sizeof(stChanNote));
  }
...
void CFamiTrackerDoc::ClearPattern(unsigned int Track, unsigned int Frame,
                                   unsigned int Channel) {
...
bool CFamiTrackerDoc::ClearRow(unsigned int Track, unsigned int Frame,
                               unsigned int Channel, unsigned int Row) {
  ASSERT(Track < MAX_TRACKS);
  ASSERT(Frame < MAX_FRAMES);
  ASSERT(Channel < MAX_CHANNELS);
...
bool CFamiTrackerDoc::ClearRowField(unsigned int Track, unsigned int Frame,
                                    unsigned int Channel, unsigned int Row,
                                    cursor_column_t Column) {
  ASSERT(Track < MAX_TRACKS);
  ASSERT(Frame < MAX_FRAMES);
  ASSERT(Channel < MAX_CHANNELS);
  ASSERT(Row < MAX_PATTERN_LENGTH);
...
bool CFamiTrackerDoc::RemoveNote(unsigned int Track, unsigned int Frame,
                                 unsigned int Channel, unsigned int Row) {
  ASSERT(Track < MAX_TRACKS);
  ASSERT(Frame < MAX_FRAMES);
  ASSERT(Channel < MAX_CHANNELS);
...
bool CFamiTrackerDoc::PullUp(unsigned int Track, unsigned int Frame,
                             unsigned int Channel, unsigned int Row) {
  // Todo: Use Track
  ASSERT(Track < MAX_TRACKS);
...
void CFamiTrackerDoc::CopyPattern(unsigned int Track, int Target, int Source,
                                  int Channel) {
...
void CFamiTrackerDoc::SwapChannels(unsigned int Track, unsigned int First,
                                   unsigned int Second) // // //
{
  ASSERT(Track < MAX_TRACKS);
...
bool CFamiTrackerDoc::DuplicateFrame(unsigned int Track, unsigned int Frame) {
  ...
  m_pBookmarkManager->GetCollection(Track)->InsertFrames(Frame + 1,
                                                         1U); // // //

  return true;
}

bool CFamiTrackerDoc::CloneFrame(unsigned int Track,
                                 unsigned int Frame) // // // renamed
{
  ASSERT(Track < MAX_TRACKS);
...
bool CFamiTrackerDoc::MoveFrameDown(unsigned int Track, unsigned int Frame) {
  ...
  m_pBookmarkManager->GetCollection(Track)->SwapFrames(Frame,
                                                       Frame + 1); // // //
...
bool CFamiTrackerDoc::AddFrames(unsigned int Track, unsigned int Frame,
                                int Count) // // //
{
  ASSERT(Track < MAX_TRACKS);
  ASSERT(Frame < MAX_FRAMES);
...
bool CFamiTrackerDoc::DeleteFrames(unsigned int Track, unsigned int Frame,
                                   int Count) {
  ASSERT(Track < MAX_TRACKS);
  ASSERT(Frame < MAX_FRAMES);
...
unsigned int CFamiTrackerDoc::GetTrackCount() const { return m_iTrackCount; }
    ...
    for (unsigned int i = 0; i < m_iTrackCount; ++i) {
      ...
      for (int j = 0; j < CHANNELS; j++) {
        if (oldIndex[j] != -1 && newIndex[j] != -1) {
          pNew->SetEffectColumnCount(newIndex[j],
                                     pTrack->GetEffectColumnCount(oldIndex[j]));
          for (int f = 0; f < MAX_FRAMES; f++)
            pNew->SetFramePattern(f, newIndex[j],
                                  pTrack->GetFramePattern(f, oldIndex[j]));
          for (int p = 0; p < MAX_PATTERN; p++)
            memcpy(pNew->GetPatternData(newIndex[j], p, 0),
                   pTrack->GetPatternData(oldIndex[j], p, 0),
                   MAX_PATTERN_LENGTH * sizeof(stChanNote));
        }
      }
      SAFE_RELEASE(pTrack);
      m_pTracks[i] = pNew;
...
CSequenceManager *const
CFamiTrackerDoc::GetSequenceManager(int InstType) const {
...
void CFamiTrackerDoc::ConvertSequence(stSequence *pOldSequence,
                                      CSequence *pNewSequence, int Type) {
  // This function is used to convert old version sequences (used by older file
  ...
  for (int i = 0; i < Count; ++i) {
    ...
    if (Length < 0) {
      iLoopPoint = 0;
      for (int l = signed(pOldSequence->Count) + Length - 1;
           l < signed(pOldSequence->Count) - 1; l++)
...
unsigned int CFamiTrackerDoc::GetFirstFreePattern(unsigned int Track,
                                                  unsigned int Channel) const {
  CPatternData *pTrack = GetTrack(Track);

  for (int i = 0; i < MAX_PATTERN; ++i) {
    if (!pTrack->IsPatternInUse(Channel, i) &&
        pTrack->IsPatternEmpty(Channel, i))
      return i;
  }
...
bool CFamiTrackerDoc::IsPatternEmpty(unsigned int Track, unsigned int Channel,
...
int CFamiTrackerDoc::GetChannelCount() const { return m_iRegisteredChannels; }

int CFamiTrackerDoc::GetChannelPosition(int Channel,
                                        unsigned char Chip) // // //
{
  // TODO: use information from the current channel map instead
  unsigned int pos = Channel;
  if (pos == CHANID_MMC5_VOICE)
    return -1;

  if (!(Chip & SNDCHIP_S5B)) {
    if (pos > CHANID_S5B_CH3)
      pos -= 3;
    else if (pos >= CHANID_S5B_CH1)
      return -1;
  }
  if (!(Chip & SNDCHIP_VRC7)) {
    if (pos > CHANID_VRC7_CH6)
      pos -= 6;
    else if (pos >= CHANID_VRC7_CH1)
      return -1;
  }
  if (!(Chip & SNDCHIP_FDS)) {
    if (pos > CHANID_FDS)
      pos -= 1;
    else if (pos >= CHANID_FDS)
      return -1;
  }
  if (pos > CHANID_N163_CH8)
    pos -= 8 - (!(Chip & SNDCHIP_N163) ? 0 : m_iNamcoChannels);
  else if (pos >
           CHANID_MMC5_VOICE + (!(Chip & SNDCHIP_N163) ? 0 : m_iNamcoChannels))
    return -1;
  if (pos > CHANID_MMC5_VOICE)
    pos -= 1;
  if (!(Chip & SNDCHIP_MMC5)) {
    if (pos > CHANID_MMC5_SQUARE2)
      pos -= 2;
    else if (pos >= CHANID_MMC5_SQUARE1)
      return -1;
  }
  if (!(Chip & SNDCHIP_VRC6)) {
    if (pos > CHANID_VRC6_SAWTOOTH)
      pos -= 3;
    else if (pos >= CHANID_VRC6_PULSE1)
      return -1;
  }
...
void CFamiTrackerDoc::RegisterChannel(CTrackerChannel *pChannel,
                                      int ChannelType, int ChipType) {
...
vibrato_t CFamiTrackerDoc::GetVibratoStyle() const { return m_iVibratoStyle; }
...
bool CFamiTrackerDoc::GetLinearPitch() const { return m_bLinearPitch; }
...
// N163 Volume Offset
int CFamiTrackerDoc::GetN163LevelOffset() const { return _N163LevelOffset; }

// DocumentPropertiesChanged calls GetN163LevelOffset and updates synth if
// modified.
...
CString CFamiTrackerDoc::GetFileTitle() const {
  ...
  static const LPCSTR EXT[] = {_T(".ftm"), _T(".0cc"), _T(".ftm.bak"),
                               _T(".0cc.bak")}; // // //
  // Remove extension
...
bool CFamiTrackerDoc::IsFileLoaded() const { return m_bFileLoaded; }

bool CFamiTrackerDoc::HasLastLoadFailed() const { return m_bFileLoadFailed; }
...
void CFamiTrackerDoc::SetupAutoSave() {
  ...
  if (file.Open(TempFile, CFile::modeRead)) {
    file.Close();
    if (AfxMessageBox(_T("It might be possible to recover last document, do ")
                      _T("you want to try?"),
                      MB_YESNO) == IDYES) {
...
CString CFamiTrackerDoc::GetComment() const { return m_strComment; }

bool CFamiTrackerDoc::ShowCommentOnOpen() const { return m_bDisplayComment; }

void CFamiTrackerDoc::SetSpeedSplitPoint(int SplitPoint) {
  m_iSpeedSplitPoint = SplitPoint;
}

int CFamiTrackerDoc::GetSpeedSplitPoint() const { return m_iSpeedSplitPoint; }

void CFamiTrackerDoc::SetHighlight(unsigned int Track,
                                   const stHighlight Hl) // // //
{
  CPatternData *pTrack = GetTrack(Track);
...
stHighlight CFamiTrackerDoc::GetHighlightAt(unsigned int Track,
                                            unsigned int Frame,
    ...
    for (unsigned i = 0; i < Count; ++i) {
      ...
      if (Dist <= Min) {
        ...
        if (pMark->m_Highlight.First != -1 &&
            (pMark->m_bPersist || pMark->m_iFrame == Frame))
...
unsigned int CFamiTrackerDoc::GetHighlightState(unsigned int Track,
                                                unsigned int Frame,
                                                unsigned int Row) const // // //
{
...
CBookmark *CFamiTrackerDoc::GetBookmarkAt(unsigned int Track,
                                          unsigned int Frame,
                                          unsigned int Row) const // // //
{
...
unsigned int
CFamiTrackerDoc::ScanActualLength(unsigned int Track,
                                  unsigned int Count) const // // //
{
  ...
  while (bScanning) {
    ...
    for (int j = 0; j < GetChannelCount(); ++j) {
      stChanNote *Note;
      Note = m_pTracks[Track]->GetPatternData(
          j, m_pTracks[Track]->GetFramePattern(f, j), r);
    ...
    if (SkipTo > -1) {
      r = std::min(static_cast<unsigned int>(SkipTo),
                   GetPatternLength(Track) - 1);
      SkipTo = -1;
    }
...
double
CFamiTrackerDoc::GetStandardLength(int Track,
                                   unsigned int ExtraLoops) const // // //
{
  char RowVisited[MAX_FRAMES][MAX_PATTERN_LENGTH];
  int JumpTo = -1;
  int SkipTo = -1;
  ...
  int GrooveIndex =
          GetSongSpeed(Track) * (m_pGrooveTable[GetSongSpeed(Track)] != NULL),
      GroovePointer = 0;
  bool bScanning = true;
  unsigned int FrameCount = GetFrameCount(Track);
    ...
    for (int j = 0; j < GetChannelCount(); ++j) {
      ...
      for (unsigned l = 0; l < GetEffColumns(Track, j) + 1; ++l) {
        ...
        case EF_SPEED:
          ...
          break;
        case EF_GROOVE:
          if (m_pGrooveTable[Note->EffParam[l]] == NULL)
            break;
          IsGroove = true;
...
void CFamiTrackerDoc::RemoveUnusedInstruments() {
  for (int i = 0; i < MAX_INSTRUMENTS; ++i) {
    if (IsInstrumentUsed(i)) {
      bool Used = false;
      for (unsigned int j = 0; j < m_iTrackCount; ++j) {
        for (unsigned int Channel = 0; Channel < m_iChannelsAvailable;
             ++Channel) {
          for (unsigned int Frame = 0; Frame < m_pTracks[j]->GetFrameCount();
               ++Frame) {
            unsigned int Pattern =
                m_pTracks[j]->GetFramePattern(Frame, Channel);
            for (unsigned int Row = 0; Row < m_pTracks[j]->GetPatternLength();
                 ++Row) {
              stChanNote *pNote =
                  m_pTracks[j]->GetPatternData(Channel, Pattern, Row);
    ...
    for (unsigned int c = 0; c < m_iChannelsAvailable; ++c) {
      for (unsigned int p = 0; p < MAX_PATTERN; ++p) {
        ...
        for (unsigned int f = 0; f < FrameCount; ++f) {
          bRemove =
              (m_pTracks[i]->GetFramePattern(f, c) == p) ? false : bRemove;
        }
        if (bRemove)
...
void CFamiTrackerDoc::RemoveUnusedSamples() // // //
{
    ...
    if (IsSampleUsed(i)) {
      ...
      for (unsigned int j = 0; j < m_iTrackCount; ++j) {
        for (unsigned int Frame = 0; Frame < m_pTracks[j]->GetFrameCount();
             ++Frame) {
          unsigned int Pattern =
              m_pTracks[j]->GetFramePattern(Frame, CHANID_DPCM);
          for (unsigned int Row = 0; Row < m_pTracks[j]->GetPatternLength();
               ++Row) {
            stChanNote *pNote =
                m_pTracks[j]->GetPatternData(CHANID_DPCM, Pattern, Row);
            int Index = pNote->Instrument;
            if (pNote->Note < NOTE_C || pNote->Note > NOTE_B ||
                Index == MAX_INSTRUMENTS)
              continue; // // //
            if (GetInstrumentType(Index) != INST_2A03)
              continue;
            AssignUsed[Index][pNote->Octave][pNote->Note - 1] = true;
            auto pInst =
                std::static_pointer_cast<CInstrument2A03>(GetInstrument(Index));
  ...
  for (int i = 0; i < MAX_INSTRUMENTS; i++)
    if (IsInstrumentUsed(i))
      if (auto pInst =
              std::dynamic_pointer_cast<CInstrument2A03>(GetInstrument(i)))
        for (int o = 0; o < OCTAVE_RANGE; o++)
          for (int n = 0; n < NOTE_RANGE; n++)
            if (!AssignUsed[i][o][n])
...
bool CFamiTrackerDoc::ArePatternsSame(unsigned int Track, unsigned int Channel,
                                      unsigned int Pattern1,
                                      unsigned int Pattern2) const // // //
{
  for (unsigned int r = 0, Count = m_pTracks[Track]->GetPatternLength();
       r < Count; ++r)
...
static void UpdateEchoTranspose(stChanNote *Note, int *Value,
                                unsigned int EffColumns) {
  for (int j = EffColumns; j >= 0; j--) {
    ...
    case EF_SLIDE_DOWN:
      *Value -= Param;
      return;
    case EF_TRANSPOSE: // Sometimes there are not enough ticks for the transpose
                       // to take place
...
stFullState *CFamiTrackerDoc::RetrieveSoundState(unsigned int Track,
                                                 unsigned int Frame,
                                                 unsigned int Row,
                                                 int Channel) {
  ...
  int(*Transpose)[ECHO_BUFFER_LENGTH + 1] =
      new int[m_iRegisteredChannels][ECHO_BUFFER_LENGTH + 1]();
  memset(BufferPos, -1, m_iRegisteredChannels * sizeof(int));
  bool maskFDS =
      false; // no need to create per-channel array since only one FDS channel
             // exists may not be the case in future additions
  ...
  while (true) {
    for (int c = m_iRegisteredChannels - 1; c >= 0; c--) {
      ...
      if (Note.Note != NONE && Note.Note != RELEASE) {
        for (int i = 0; i < std::min(BufferPos[c], ECHO_BUFFER_LENGTH + 1);
             i++) {
          if (State->Echo[i] == ECHO_BUFFER_ECHO) {
            UpdateEchoTranspose(&Note, &Transpose[c][i], EffColumns);
            switch (Note.Note) {
            case HALT:
              State->Echo[i] = ECHO_BUFFER_HALT;
              break;
            case ECHO:
              State->Echo[i] = ECHO_BUFFER_ECHO + Note.Octave;
              break;
            default:
          ...
          } else if (State->Echo[i] > ECHO_BUFFER_ECHO &&
                     State->Echo[i] <= ECHO_BUFFER_ECHO + ECHO_BUFFER_LENGTH)
            State->Echo[i]--;
        }
        if (BufferPos[c] >= 0 && BufferPos[c] <= ECHO_BUFFER_LENGTH) {
          ...
          switch (Note.Note) {
          case HALT:
            Value = ECHO_BUFFER_HALT;
            break;
          case ECHO:
            Value = ECHO_BUFFER_ECHO + Note.Octave;
            break;
          default:
      ...
      for (int k = EffColumns; k >= 0; k--) {
        ...
        case EF_TRANSPOSE:
        case EF_JUMP:
        case EF_SKIP: // no true backward iterator
          continue;
        case EF_HALT:
          Row = Frame = 0;
          goto outer;
        case EF_SPEED:
          if (S->Speed == -1 &&
              (xy < GetSpeedSplitPoint() || GetSongTempo(Track) == 0)) {
            S->Speed = xy;
            if (S->Speed < 1)
              S->Speed = 1;
            S->GroovePos = -2;
          } else if (S->Tempo == -1 && xy >= GetSpeedSplitPoint())
            S->Tempo = xy;
          continue;
        case EF_GROOVE:
          if (S->GroovePos == -1 && xy < MAX_GROOVE &&
              m_pGrooveTable[xy] != nullptr) {
        ...
        case EF_VOLUME:
          ...
          if (State->Effect_LengthCounter == -1 && xy >= 0xE0 && xy <= 0xE3)
            ...
            if (State->Effect_LengthCounter == -1)
              State->Effect_LengthCounter =
                  ch->GetID() == CHANID_TRIANGLE ? 0xE1 : 0xE2;
          }
          continue;
        case EF_NOTE_CUT:
          if (!ch->IsEffectCompatible(fx, xy))
            continue;
          if (ch->GetID() != CHANID_TRIANGLE)
            continue;
          if (State->Effect[fx] == -1) {
            ...
            if (State->Effect_LengthCounter != 0xE0) {
              State->Effect[fx] = xy;
              if (State->Effect_LengthCounter == -1)
                State->Effect_LengthCounter = 0xE1;
            }
          }
          continue;
        ...
        case EF_FDS_MOD_SPEED_HI:
          ...
          if (xy <= 0x0F)
            ...
            State->Effect[fx] = xy;
            if (State->Effect_AutoFMMult == -1)
              State->Effect_AutoFMMult = -2;
          }
          continue;
        ...
        case EF_PORTA_UP:
        case EF_PORTA_DOWN:
          if (State->Effect[EF_PORTAMENTO] ==
              -1) { // anything else within can be used here
  ...
  outer:
    if (Row)
      ...
      Row = GetFrameLength(Track, --Frame) - 1;
    else
      break;
    totalRows++;
  }
...
void CFamiTrackerDoc::SetExceededFlag(bool Exceed) { m_bExceeded = Exceed; }

int CFamiTrackerDoc::GetFrameLength(unsigned int Track,
  ...
  for (int j = 0; j < PatternLength; ++j) {
    for (int i = 0; i < GetChannelCount(); ++i) {
      ...
      // First look for pattern data, allow this to cancel earlier pattern
      // lengths
      /*
      if (Note.Note != NONE || Note.Instrument != MAX_INSTRUMENTS || Note.Vol !=
      0x10) HaltPoint = PatternLength; for (int k = 0; k < Columns; ++k) { if
      (Note.EffNumber[k] != EF_NONE) HaltPoint = PatternLength;
      }
      */
...
void CFamiTrackerDoc::MakeKraid() // // // Easter Egg
{
  ...
  // Patterns
  SetPatternAtFrame(0, 0, 0, 0);
  SetPatternAtFrame(0, 0, 1, 0);
  SetPatternAtFrame(0, 0, 2, 0);
  SetPatternAtFrame(0, 1, 0, 0);
  SetPatternAtFrame(0, 1, 1, 0);
  SetPatternAtFrame(0, 1, 2, 0);
  SetPatternAtFrame(0, 2, 0, 0);
  SetPatternAtFrame(0, 2, 1, 0);
  SetPatternAtFrame(0, 2, 2, 0);
  SetPatternAtFrame(0, 3, 0, 0);
  SetPatternAtFrame(0, 3, 1, 0);
  SetPatternAtFrame(0, 3, 2, 0);
  SetPatternAtFrame(0, 4, 0, 1);
  SetPatternAtFrame(0, 4, 1, 1);
  SetPatternAtFrame(0, 4, 2, 1);
  SetPatternAtFrame(0, 5, 0, 1);
  SetPatternAtFrame(0, 5, 1, 1);
  SetPatternAtFrame(0, 5, 2, 1);
  SetPatternAtFrame(0, 6, 0, 2);
  SetPatternAtFrame(0, 6, 1, 2);
  SetPatternAtFrame(0, 6, 2, 2);
  SetPatternAtFrame(0, 7, 0, 3);
  SetPatternAtFrame(0, 7, 1, 2);
  SetPatternAtFrame(0, 7, 2, 2);
  SetPatternAtFrame(0, 8, 0, 3);
  SetPatternAtFrame(0, 8, 1, 2);
  SetPatternAtFrame(0, 8, 2, 2);
  SetPatternAtFrame(0, 9, 0, 3);
  SetPatternAtFrame(0, 9, 1, 2);
  SetPatternAtFrame(0, 9, 2, 2);
  SetPatternAtFrame(0, 10, 0, 4);
  SetPatternAtFrame(0, 10, 1, 3);
  SetPatternAtFrame(0, 10, 2, 3);
  SetPatternAtFrame(0, 11, 0, 5);
  SetPatternAtFrame(0, 11, 1, 3);
  SetPatternAtFrame(0, 11, 2, 3);
  SetPatternAtFrame(0, 12, 0, 6);
  SetPatternAtFrame(0, 12, 1, 4);
  SetPatternAtFrame(0, 12, 2, 4);
  SetPatternAtFrame(0, 13, 0, 6);
  SetPatternAtFrame(0, 13, 1, 4);
  SetPatternAtFrame(0, 13, 2, 4);
  for (int i = 0; i < 14; i++) {
  ...
  for (int i = 0; i < MAX_INSTRUMENTS; i++)
    RemoveInstrument(i);
  auto kraidInst = std::static_pointer_cast<CInstrument2A03>(
      CInstrumentManager::CreateNew(INST_2A03));
  ...
  kraidInst = std::static_pointer_cast<CInstrument2A03>(
      CInstrumentManager::CreateNew(INST_2A03));
  kraidInst->SetSeqEnable(SEQ_VOLUME, 1);
  ...
  for (int i = 0; i < 24; i += 6) {
    kraidRow->Note = NOTE_E;
    kraidRow->Octave = 2;
    SetDataAtPattern(0, 0, 2, i, kraidRow);
    kraidRow->Octave = 3;
    SetDataAtPattern(0, 0, 2, i + 2, kraidRow);
  }
  for (int i = 0; i < 12; i += 6) {
    kraidRow->Note = NOTE_C;
    kraidRow->Octave = 2;
    SetDataAtPattern(0, 1, 2, i, kraidRow);
    kraidRow->Octave = 3;
    SetDataAtPattern(0, 1, 2, i + 2, kraidRow);
    kraidRow->Note = NOTE_D;
    kraidRow->Octave = 2;
    SetDataAtPattern(0, 1, 2, i + 12, kraidRow);
    kraidRow->Octave = 3;
    SetDataAtPattern(0, 1, 2, i + 14, kraidRow);
  }
  for (int i = 0; i < 6; i += 2) {
    kraidRow->Note = NOTE_E;
    kraidRow->Octave = 2 + i / 2;
    SetDataAtPattern(0, 2, 2, i, kraidRow);
    kraidRow->Note = NOTE_Fs;
    SetDataAtPattern(0, 2, 2, i + 6, kraidRow);
    kraidRow->Note = NOTE_F;
    SetDataAtPattern(0, 2, 2, i + 12, kraidRow);
    kraidRow->Note = NOTE_B;
    kraidRow->Octave = 1 + i / 2;
    SetDataAtPattern(0, 2, 2, i + 18, kraidRow);
  }
  kraidRow->Note = NOTE_E;
  kraidRow->Octave = 1;
  SetDataAtPattern(0, 4, 2, 0, kraidRow);
  kraidRow->Note = NOTE_C;
  kraidRow->Octave = 2;
  SetDataAtPattern(0, 3, 2, 12, kraidRow);
  kraidRow->Note = NOTE_E;
  SetDataAtPattern(0, 3, 2, 0, kraidRow);
  kraidRow->Note = NOTE_G;
  SetDataAtPattern(0, 3, 2, 16, kraidRow);
  kraidRow->Note = NOTE_A;
  SetDataAtPattern(0, 3, 2, 18, kraidRow);
  kraidRow->Note = NOTE_B;
  SetDataAtPattern(0, 3, 2, 4, kraidRow);
  SetDataAtPattern(0, 3, 2, 10, kraidRow);
  SetDataAtPattern(0, 3, 2, 22, kraidRow);
  kraidRow->Note = NOTE_C;
  kraidRow->Octave = 3;
  SetDataAtPattern(0, 3, 2, 6, kraidRow);

  // Pulse 2
  kraidRow->Instrument = 0;
  kraidRow->Note = NOTE_As;
  kraidRow->Octave = 2;
  SetDataAtPattern(0, 0, 1, 22, kraidRow);
  kraidRow->Note = NOTE_B;
  SetDataAtPattern(0, 0, 1, 4, kraidRow);
  ...
  kraidRow->Note = NOTE_Fs;
  SetDataAtPattern(0, 0, 1, 12, kraidRow);
  ...
  kraidRow->Note = NOTE_G;
  SetDataAtPattern(0, 1, 1, 0, kraidRow);
  ...
  kraidRow->Note = NOTE_A;
  SetDataAtPattern(0, 0, 1, 6, kraidRow);
  ...
  GetDataAtPattern(0, 0, 1, 0, kraidRow);
  kraidRow->EffNumber[0] = EF_DUTY_CYCLE;
  kraidRow->EffParam[0] = 2;
  SetDataAtPattern(0, 0, 1, 0, kraidRow);
  GetDataAtPattern(0, 1, 1, 0, kraidRow);
  kraidRow->EffNumber[0] = EF_DUTY_CYCLE;
  kraidRow->EffParam[0] = 2;
  SetDataAtPattern(0, 1, 1, 0, kraidRow);
  GetDataAtPattern(0, 2, 1, 0, kraidRow);
  kraidRow->EffNumber[0] = EF_DUTY_CYCLE;
  kraidRow->EffParam[0] = 2;
  SetDataAtPattern(0, 2, 1, 0, kraidRow);
  ...
  for (int i = 0; i < 23; i++) {
    ...
    if (kraidRow->Note != NONE) {
      kraidRow->Instrument = 1;
      kraidRow->EffNumber[1] = EF_DELAY;
      kraidRow->EffParam[1] = 3;
