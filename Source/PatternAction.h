...
/*
        \brief A structure responsible for recording the cursor and selection
   state of the pattern editor for use by pattern actions.
*/
struct CPatternEditorState // // // TODO: might be moved to PatternEditor.h
{
  /*!	\brief Constructor of the pattern editor state.
          \details On construction, the object retrieves the current state of
     the pattern editor immediately. Once created, a state object remains
     constant and can be applied back to the pattern editor as many times as
     desired. \param pEditor Pointer to the pattern editor. \param Track The
     track number. */
  ...
  /*!	\brief The current selection position at the time of the state's
   * creation. */
  CSelection Selection;

  /*!	\brief Whether a selection is active at the time of the state's
   * creation. */
  bool IsSelecting;
...
public:
  ...
  void SetDragAndDrop(const CPatternClipData *pClipData, bool bDelete,
                      bool bMix, const CSelection *pDragTarget);
...
protected:
  bool SetTargetSelection(CPatternEditor *pPatternEditor,
                          CSelection &Sel); // // //
  void DeleteSelection(CMainFrame *pMainFrm,
  ...
  GetIterators(const CMainFrame *pMainFrm) const; // // //
...
/*!
        \brief Specialization of the pattern action class for actions operating
   on a selection without modifying its span.
*/
...
public:
  CPActionDragDrop(const CPatternClipData *pClipData, bool bDelete, bool bMix,
