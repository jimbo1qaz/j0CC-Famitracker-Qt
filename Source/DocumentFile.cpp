...
//
// This class is based on CFile and has some simple extensions to create and
// read FTM files
//
...
// Class constants
const unsigned int CDocumentFile::FILE_VER =
    0x0440; // Current file version (4.40)
const unsigned int CDocumentFile::COMPATIBLE_VER =
    0x0100; // Compatible file version (1.0)
...
CDocumentFile::CDocumentFile() : m_pBlockData(NULL), m_cBlockID(new char[16]) {}
...
bool CDocumentFile::Finished() const { return m_bFileDone; }
...
void CDocumentFile::WriteBlockInt(int Value) { WriteBlockData(Value); }

void CDocumentFile::WriteBlockChar(char Value) { WriteBlockData(Value); }
...
void CDocumentFile::ValidateFile() {
  ...
  if (GetFileVersion() < COMPATIBLE_VER) {
    e->AppendError(
        "FamiTracker module version too old (0x%X), expected 0x%X or above",
  ...
  if (GetFileVersion() > 0x450U /*FILE_VER*/) { // // // 050B
    e->AppendError(
        "FamiTracker module version too new (0x%X), expected 0x%X or below",
...
char *CDocumentFile::GetBlockHeaderID() const { return m_cBlockID; }

int CDocumentFile::GetBlockVersion() const { return m_iBlockVersion; }
...
int CDocumentFile::GetBlockPos() const { return m_iBlockPointer; }

int CDocumentFile::GetBlockSize() const { return m_iBlockSize; }

bool CDocumentFile::IsFileIncomplete() const { return m_bIncomplete; }
