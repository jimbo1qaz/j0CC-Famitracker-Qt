...
BOOL CInstrumentEditorVRC7::OnInitDialog() {
  ...
  const TCHAR *const PATCH_NAME[16] = {
      _T("(custom patch)"), _T("Bell"),        _T("Guitar"),
      _T("Piano"),          _T("Flute"),       _T("Clarinet"),
      _T("Rattling Bell"),  _T("Trumpet"),     _T("Reed Organ"),
      _T("Soft Bell"),      _T("Xylophone"),   _T("Vibraphone"),
      _T("Brass"),          _T("Bass Guitar"), _T("Synthesizer"),
      _T("Chorus")};
...
void CInstrumentEditorVRC7::EnableControls(bool bEnable) {
  const int SLIDER_IDS[] = {
      IDC_M_AM,   IDC_M_AR,  IDC_M_DM, IDC_M_DR, IDC_M_EG, IDC_M_KSL,
      IDC_M_KSR2, IDC_M_MUL, IDC_M_RR, IDC_M_SL, IDC_M_SL, IDC_M_VIB,
      IDC_C_AM,   IDC_C_AR,  IDC_C_DM, IDC_C_DR, IDC_C_EG, IDC_C_KSL,
      IDC_C_KSR,  IDC_C_MUL, IDC_C_RR, IDC_C_SL, IDC_C_SL, IDC_C_VIB,
      IDC_TL,     IDC_FB};
...
void CInstrumentEditorVRC7::SelectInstrument(
...
void CInstrumentEditorVRC7::OnVScroll(UINT nSBCode, UINT nPos,
...
void CInstrumentEditorVRC7::OnHScroll(UINT nSBCode, UINT nPos,
                                      CScrollBar *pScrollBar) {
  SaveCustomPatch();
  SetFocus();
...
void CInstrumentEditorVRC7::OnContextMenu(CWnd * /*pWnd*/, CPoint point) {
  ...
  switch (menu.TrackPopupMenu(TPM_LEFTALIGN | TPM_RIGHTBUTTON | TPM_RETURNCMD,
...
void CInstrumentEditorVRC7::CopyAsPlainText() // // //
{
  ...
  unsigned char reg[8] = {};
  for (int i = 0; i < 8; ++i)
    reg[i] = patch == 0 ? m_pInstrument->GetCustomReg(i)
                        : default_inst[patch * 16 + i];
  ...
  for (int i = 0; i <= 1; i++)
    MML.AppendFormat(
        _T(" %2d,%2d,%2d,%2d,%2d,%2d,%2d,%2d,%2d,%2d,%2d,\r\n"),
        (reg[4 + i] >> 4) & 0x0F, reg[4 + i] & 0x0F, (reg[6 + i] >> 4) & 0x0F,
        reg[6 + i] & 0x0F, (reg[2 + i] >> 6) & 0x03, reg[i] & 0x0F,
        (reg[i] >> 7) & 0x01, (reg[i] >> 6) & 0x01, (reg[i] >> 5) & 0x01,
        (reg[i] >> 4) & 0x01, (reg[3] >> (4 - i)) & 0x01);
...
void CInstrumentEditorVRC7::PasteSettings(LPCTSTR pString) {
  ...
  for (int i = 0; (i < 8) && (begin != end); ++i) {
    int value =
        CSequenceInstrumentEditPanel::ReadStringValue(*begin++, false); // // //
    if (value < 0)
      value = 0;
    if (value > 0xFF)
      value = 0xFF;
