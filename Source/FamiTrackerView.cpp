...
// Clipboard ID
const TCHAR CFamiTrackerView::CLIPBOARD_ID[] =
    _T("application/x-qt-windows-mime;value=\"FamiTracker Pattern\"");
...
CFamiTrackerView::CFamiTrackerView()
    : m_iClipboard(0), m_iMoveKeyStepping(1), m_iInsertKeyStepping(1),
      m_bEditEnable(false), m_bMaskInstrument(false), m_bMaskVolume(true),
...
void CFamiTrackerView::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) {
  ...
  if ((nChar == VK_ADD || nChar == VK_SUBTRACT) &&
    ...
    HandleKeyboardInput(nChar);
           ...
           !(nChar == VK_RETURN && !(nFlags & KF_EXTENDED)))
    switch (nChar) {
    case VK_UP:
      OnKeyDirUp();
      break;
...
bool CFamiTrackerView::EditEffNumberColumn(stChanNote &Note,
                                           unsigned char nChar, int EffectIndex,
                                           bool &bStepDown) {
  int EditStyle = theApp.GetSettings()->General.iEditStyle;
...
bool CFamiTrackerView::EditEffParamColumn(stChanNote &Note, int Key,
...
void CFamiTrackerView::HandleKeyboardInput(unsigned char nChar) // // //
{
  ...
  case C_EFF1_PARAM1:
  case C_EFF1_PARAM2:
    if (!EditEffParamColumn(Note, nChar, Index, bStepDown, bMoveRight,
                            bMoveLeft))
      return;
    break;
  }
