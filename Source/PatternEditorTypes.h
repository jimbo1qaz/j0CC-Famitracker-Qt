...
inline column_t GetSelectColumn(cursor_column_t Column) {
  ...
  static const column_t COLUMNS[] = {
      COLUMN_NOTE, COLUMN_INSTRUMENT, COLUMN_INSTRUMENT, COLUMN_VOLUME,
      COLUMN_EFF1, COLUMN_EFF1,       COLUMN_EFF1,       COLUMN_EFF2,
      COLUMN_EFF2, COLUMN_EFF2,       COLUMN_EFF3,       COLUMN_EFF3,
      COLUMN_EFF3, COLUMN_EFF4,       COLUMN_EFF4,       COLUMN_EFF4};
...
inline cursor_column_t GetCursorStartColumn(column_t Column) {
  static const cursor_column_t COL_START[] = {
      C_NOTE,     C_INSTRUMENT1, C_VOLUME,  C_EFF1_NUM,
      C_EFF2_NUM, C_EFF3_NUM,    C_EFF4_NUM};
...
inline cursor_column_t GetCursorEndColumn(column_t Column) {
  static const cursor_column_t COL_END[] = {
      C_NOTE,        C_INSTRUMENT2, C_VOLUME,     C_EFF1_PARAM2,
      C_EFF2_PARAM2, C_EFF3_PARAM2, C_EFF4_PARAM2};
...
public:
  ...
  CPatternIterator(CFamiTrackerDoc *const pDoc, int Track,
                   const CCursorPos &Pos);
  CPatternIterator(const CFamiTrackerDoc *const pDoc, int Track,
                   const CCursorPos &Pos);

  static std::pair<CPatternIterator, CPatternIterator>
  FromCursor(const CCursorPos &Pos, CFamiTrackerDoc *const pDoc, int Track);
  static std::pair<CPatternIterator, CPatternIterator>
  FromSelection(const CSelection &Sel, CFamiTrackerDoc *const pDoc, int Track);
...
private:
        int		m_iPatternWidth;				// Width
of channels in pattern area
        int		m_iPatternHeight;				//
Height of channels in pattern area int		m_iLinesVisible;
// Number of lines visible on screen (may include one incomplete line) int
m_iLinesFullVisible;			// Number of lines full visible on
screen
        int		m_iChannelsVisible;				//
Number of channels visible on screen (may include one incomplete channel) int
m_iChannelsFullVisible;			// Number of channels full visible on
screen
        int		m_iFirstChannel;				// First
drawn channel
        int		m_iRowHeight;
// Height of each row in pixels

        int		m_iChannelWidths[MAX_CHANNELS];	// Cached width in
pixels of each channel
        int		m_iChannelOffsets[MAX_CHANNELS];// Cached x position of
channels int		m_iColumns[MAX_CHANNELS];		// Cached number
of columns in each channel
};
