...
#define LIMIT(v, max, min)                                                     \
  v = ((v > max)                                                               \
           ? max                                                               \
           : ((v < min)                                                        \
                  ? min                                                        \
                  : v)); //  if (v > max) v = max; else if (v < min) v = min;
...
/*!
        \brief A MFC document template supporting both .0cc and .ftm file
   extensions.
*/
class CDocTemplate0CC : public CSingleDocTemplate {
public:
  CDocTemplate0CC(UINT nIDResource, CRuntimeClass *pDocClass,
...
/*!
        \brief A MFC document manager allowing both .0cc and .ftm file
   extensions to be displayed while opening or saving documents. \details This
   class also saves the FTM path of the last load/save operation to the
   registry.
*/
