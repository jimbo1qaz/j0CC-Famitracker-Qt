...
OPLL_toggleMask. Added OPLL_writeIO. 2001 09-28 : Version 0.51 -- Removed the
noise table. 2002 01-28 : Version 0.52 -- Added Stereo mode. 2002 02-07 :
Version 0.53 -- Fixed some drum bugs. 2002 02-20 : Version 0.54 -- Added the
best quality mode. 2002 03-02 : Version 0.55 -- Removed OPLL_init & OPLL_close.
...
static void makeAdjustTable(void) {
  ...
  for (i = 1; i < (1 << EG_BITS); i++)
    AR_ADJUST_TABLE[i] = (uint16_t)((double)(1 << EG_BITS) - 1 -
...
static int32_t lin2db(double d) {
  if (d == 0)
    ...
    return Min(-(int32_t)(20.0 * log10(d) / DB_STEP),
               DB_MUTE - 1); /* 0 -- 127 */
}
...
static void makeSinTable(void) {
  ...
  for (i = 0; i < PG_WIDTH / 2; i++) {
    fullsintable[PG_WIDTH / 2 + i] =
        (uint32_t)(DB_MUTE + DB_MUTE + fullsintable[i]);
  }
...
static void makePmTable(void) {
  ...
  for (i = 0; i < PM_PG_WIDTH; i++)
    /* pmtable[i] = (int32_t) ((double) PM_AMP * pow (2, (double) PM_DEPTH * sin
     * (2.0 * PI * i / PM_PG_WIDTH) / 1200)); */
    pmtable[i] = (int32_t)(
        (double)PM_AMP *
        pow(2, (double)PM_DEPTH * saw(2.0 * PI * i / PM_PG_WIDTH) / 1200));
}
...
static void makeAmTable(void) {
  ...
  for (i = 0; i < AM_PG_WIDTH; i++)
    /* amtable[i] = (int32_t) ((double) AM_DEPTH / 2 / DB_STEP * (1.0 + sin (2.0
     * * PI * i / PM_PG_WIDTH))); */
    amtable[i] = (int32_t)((double)AM_DEPTH / 2 / DB_STEP *
...
static void makeDphaseTable(void) {
  ...
  uint32_t mltable[16] = {1,      1 * 2,  2 * 2,  3 * 2, 4 * 2,  5 * 2,
          ...
          if (KL == 0) {
            ...
            if (tmp <= 0)
              ...
              tllTable[fnum][block][TL][KL] =
                  (uint32_t)((tmp >> (3 - KL)) / EG_STEP) + TL2EG(TL);
          }
        }
}

#ifdef USE_SPEC_ENV_SPEED
static double attacktime[16][4] = {{0, 0, 0, 0},
...
static double decaytime[16][4] = {{0, 0, 0, 0},
  ...
  for (RM = 0; RM < 16; RM++)
    for (RL = 0; RL < 4; RL++) {
      if (RM == 0)
        ...
        attacktable[RM][RL] = (uint32_t)(
            (double)(1 << EG_DP_BITS) / (attacktime[RM][RL] * 3579545 / 72000));
    }
#endif
...
void OPLL_patch2dump(const OPLL_PATCH *patch, uint8_t *dump) {
  dump[0] = (uint8_t)((patch[0].AM << 7) + (patch[0].PM << 6) +
                      (patch[0].EG << 5) + (patch[0].KR << 4) + patch[0].ML);
  dump[1] = (uint8_t)((patch[1].AM << 7) + (patch[1].PM << 6) +
                      (patch[1].EG << 5) + (patch[1].KR << 4) + patch[1].ML);
  dump[2] = (uint8_t)((patch[0].KL << 6) + patch[0].TL);
  dump[3] = (uint8_t)((patch[1].KL << 6) + (patch[1].WF << 4) +
                      (patch[0].WF << 3) + patch[0].FB);
...
#define UPDATE_TLL(S)                                                          \
  (((S)->type == 0) ? ((S)->tll = tllTable[((S)->fnum) >> 5][(S)->block]       \
                                          [(S)->patch->TL][(S)->patch->KL])    \
                    : ((S)->tll = tllTable[((S)->fnum) >> 5][(S)->block]       \
                                          [(S)->volume][(S)->patch->KL]))
#define UPDATE_RKS(S)                                                          \
...
INLINE static void keyOn_SD(OPLL *opll) {
  if (!opll->slot_on_flag[SLOT_SD])
    slotOn(CAR(opll, 7));
}
INLINE static void keyOn_TOM(OPLL *opll) {
  if (!opll->slot_on_flag[SLOT_TOM])
    slotOn(MOD(opll, 8));
}
INLINE static void keyOn_HH(OPLL *opll) {
  if (!opll->slot_on_flag[SLOT_HH])
    slotOn2(MOD(opll, 7));
}
INLINE static void keyOn_CYM(OPLL *opll) {
...
INLINE static void keyOff_SD(OPLL *opll) {
  if (opll->slot_on_flag[SLOT_SD])
    slotOff(CAR(opll, 7));
}
INLINE static void keyOff_TOM(OPLL *opll) {
  if (opll->slot_on_flag[SLOT_TOM])
    slotOff(MOD(opll, 8));
}
INLINE static void keyOff_HH(OPLL *opll) {
  if (opll->slot_on_flag[SLOT_HH])
    slotOff(MOD(opll, 7));
}
INLINE static void keyOff_CYM(OPLL *opll) {
  if (opll->slot_on_flag[SLOT_CYM])
    slotOff(CAR(opll, 8));
}

/* Change a voice */
INLINE static void setPatch(OPLL *opll, int32_t i, int32_t num) {
...
/* Change Rhythm Mode */
INLINE static void update_rhythm_mode(OPLL *opll) {
...
#define S2E(x) (SL2EG((int32_t)(x / SL_STEP)) << (EG_DP_BITS - EG_BITS))
  ...
  static uint32_t SL[16] = {S2E(0.0),  S2E(3.0),  S2E(6.0),  S2E(9.0),
...
/* SNARE */
INLINE static int32_t calc_slot_snare(OPLL_SLOT *slot, uint32_t noise) {
  if (slot->egout >= (DB_MUTE - 1))
    return 0;
...
/*
  TOP-CYM
 */
INLINE static int32_t calc_slot_cym(OPLL_SLOT *slot, uint32_t pgout_hh) {
  uint32_t dbout;
...
/*
  HI-HAT
*/
INLINE static int32_t calc_slot_hat(OPLL_SLOT *slot, int32_t pgout_cym,
                                    uint32_t noise) {
  uint32_t dbout;
  ...
  if (slot->egout >= (DB_MUTE - 1))
    return 0;
      ...
      /* the same as fmopl.c */
      ((BIT(slot->pgout, PG_BITS - 8) ^ BIT(slot->pgout, PG_BITS - 1)) |
       BIT(slot->pgout, PG_BITS - 7)) ^
      /* different from fmopl.c */
...
static int16_t calc(OPLL *opll) {
  ...
  for (i = 0; i < 6; i++)
    if (!(opll->mask & OPLL_MASK_CH(i)) && (CAR(opll, i)->eg_mode != FINISH)) {
      int32_t absval,
          val = calc_slot_car(CAR(opll, i), calc_slot_mod(MOD(opll, i)));
      inst += val;
      absval = abs(val);
...
#ifdef EMU2413_COMPACTION
int16_t OPLL_calc(OPLL *opll) { return calc(opll); }
#else
int16_t OPLL_calc(OPLL *opll) {
  ...
  opll->out =
      (int16_t)(((double)opll->next * (opll->opllstep - opll->oplltime) +
                 (double)opll->prev * opll->oplltime) /
                opll->opllstep);
...
#ifndef EMU2413_COMPACTION
/* STEREO MODE (OPT) */
void OPLL_set_pan(OPLL *opll, uint32_t ch, uint32_t pan) {
  opll->pan[ch & 15] = pan & 3;
}

static void calc_stereo(OPLL *opll, int32_t out[2]) {
  ...
  for (i = 0; i < 6; i++)
    if (!(opll->mask & OPLL_MASK_CH(i)) && (CAR(opll, i)->eg_mode != FINISH))
      b[opll->pan[i]] +=
          calc_slot_car(CAR(opll, i), calc_slot_mod(MOD(opll, i)));

  if (opll->patch_number[6] <= 15) {
    if (!(opll->mask & OPLL_MASK_CH(6)) && (CAR(opll, 6)->eg_mode != FINISH))
      b[opll->pan[6]] +=
          calc_slot_car(CAR(opll, 6), calc_slot_mod(MOD(opll, 6)));
  } else {
    if (!(opll->mask & OPLL_MASK_BD) && (CAR(opll, 6)->eg_mode != FINISH))
      r[opll->pan[9]] +=
          calc_slot_car(CAR(opll, 6), calc_slot_mod(MOD(opll, 6)));
  ...
  if (opll->patch_number[7] <= 15) {
    if (!(opll->mask & OPLL_MASK_CH(7)) && (CAR(opll, 7)->eg_mode != FINISH))
      b[opll->pan[7]] +=
          calc_slot_car(CAR(opll, 7), calc_slot_mod(MOD(opll, 7)));
  } else {
    if (!(opll->mask & OPLL_MASK_HH) && (MOD(opll, 7)->eg_mode != FINISH))
      r[opll->pan[10]] += calc_slot_hat(MOD(opll, 7), CAR(opll, 8)->pgout,
                                        opll->noise_seed & 1);
    if (!(opll->mask & OPLL_MASK_SD) && (CAR(opll, 7)->eg_mode != FINISH))
  ...
  if (opll->patch_number[8] <= 15) {
    if (!(opll->mask & OPLL_MASK_CH(8)) && (CAR(opll, 8)->eg_mode != FINISH))
      b[opll->pan[8]] +=
          calc_slot_car(CAR(opll, 8), calc_slot_mod(MOD(opll, 8)));
  } else {
    if (!(opll->mask & OPLL_MASK_TOM) && (MOD(opll, 8)->eg_mode != FINISH))
...
void OPLL_calc_stereo(OPLL *opll, int32_t out[2]) {
  ...
  opll->oplltime -= opll->realstep;
  out[0] =
      (int16_t)(((double)opll->snext[0] * (opll->opllstep - opll->oplltime) +
                 (double)opll->sprev[0] * opll->oplltime) /
                opll->opllstep);
  out[1] =
      (int16_t)(((double)opll->snext[1] * (opll->opllstep - opll->oplltime) +
                 (double)opll->sprev[1] * opll->oplltime) /
                opll->opllstep);
}
