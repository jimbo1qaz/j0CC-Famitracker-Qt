...
void LogTableInitialize(void) {
  ...
  for (i = 1; i < (1 << LIN_BITS) + 1; i++) {
    ...
    ua = (uint32_t)((LOG_LIN_BITS - (double(log(a)) / double(log(2.0)))) *
                    (1 << LOG_BITS));
    lineartbl[i] = ua << 1;
  }
}
...
int32_t __fastcall FDSSoundRender(void) {
  ...
  // EDIT not using FDSSoundWGStep for modulator (op[1]), need to adjust bias
  // when sample changes
  ...
  if (fdssound.op[1].wg.disable)
    ...
    // advance the mod table wave and adjust the bias when/if next table entry
    // is reached
    const uint32_t ENTRY_WIDTH = 1 << (PGCPS_BITS + 16);
    uint32_t spd = fdssound.op[1].pg.spd; // phase to add
    while (spd) {
      uint32_t left =
          ENTRY_WIDTH - (fdssound.op[1].wg.phase & (ENTRY_WIDTH - 1));
      ...
      if (spd >= left) // advancing to the next entry
      {
        ...
        fdssound.op[1].wg.output =
            fdssound.op[1]
                .wg.wave[(fdssound.op[1].wg.phase >> (PGCPS_BITS + 16)) & 0x3f];
        ...
        while (fdssound.op[1].bias > 63)
          fdssound.op[1].bias -= 128;
        while (fdssound.op[1].bias < -64)
          fdssound.op[1].bias += 128;
  ...
  output = (fdssound.op[0].wg.output * output *
            fdssound.mastervolumel[fdssound.lvl]) >>
           (VOL_BITS - 4);
...
void __fastcall FDSSoundVolume(unsigned int volume) {
  ...
  fdssound.mastervolumel[0] =
      LogToLinear(fdssound.mastervolume, LOG_LIN_BITS - LIN_BITS - VOL_BITS) *
      2;
  fdssound.mastervolumel[1] =
      LogToLinear(fdssound.mastervolume, LOG_LIN_BITS - LIN_BITS - VOL_BITS) *
      4 / 3;
  fdssound.mastervolumel[2] =
      LogToLinear(fdssound.mastervolume, LOG_LIN_BITS - LIN_BITS - VOL_BITS) *
      2 / 2;
  fdssound.mastervolumel[3] =
      LogToLinear(fdssound.mastervolume, LOG_LIN_BITS - LIN_BITS - VOL_BITS) *
      8 / 10;
}
...
void __fastcall FDSSoundWrite(uint16_t address, uint8_t value) {
  if (0x4040 <= address && address <= 0x407F) {
    ...
    case 5:
      ...
      if (value & 0x40)
        fdssound.op[1].bias -= 0x40; // extend sign bit
...
void __fastcall FDSSoundReset(void) {
  ...
  fdssound.envcps =
      DivFix(NES_BASECYCLES, 12 * fdssound.srate, EGCPS_BITS + 5 - 9 + 1);
...
void FDSSoundInstall3(void) { LogTableInitialize(); }
