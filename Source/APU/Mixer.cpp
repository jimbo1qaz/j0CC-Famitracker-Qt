...
/*
 ...
 MMC5 (just breed):
        2A03 square @ v = $0F: 760mV (the cart attenuates internal channels a
 little) MMC5 square @ v = $0F: 900mV

 VRC7:
        2A03 Square  @ v = $0F: 300mV (the cart attenuates internal channels a
 lot) VRC7 Patch 5 @ v = $0F: 900mV Did some more tests and found patch 14 @
 v=15 to be 13.77dB stronger than a 50% square @ v=15
...
inline double CMixer::CalcPin2(double Val1, double Val2, double Val3) {
  ...
  if ((Val1 + Val2 + Val3) > 0)
    return 159.79 /
           ((1.0 / ((Val1 / 8227.0) + (Val2 / 12241.0) + (Val3 / 22638.0))) +
            100.0);
...
bool CMixer::AllocateBuffer(unsigned int BufferLength, uint32_t SampleRate,
                            uint8_t NrChannels) {
  m_iSampleRate = SampleRate;
  BlipBuffer.set_sample_rate(SampleRate,
                             (BufferLength * 1000 * 2) / SampleRate);
  return true;
}
...
int CMixer::SamplesAvail() const { return (int)BlipBuffer.samples_avail(); }
...
#ifdef LINEAR_MIXING
  SumL =
      ((m_iChannels[CHANID_SQUARE1].Left + m_iChannels[CHANID_SQUARE2].Left) *
       0.00752) *
      InternalVol;
  SumR =
      ((m_iChannels[CHANID_SQUARE1].Right + m_iChannels[CHANID_SQUARE2].Right) *
       0.00752) *
      InternalVol;
#else
  double Sum =
      CalcPin1(m_iChannels[CHANID_SQUARE1], m_iChannels[CHANID_SQUARE2]);
#endif
...
#ifdef LINEAR_MIXING
  SumL = ((0.00851 * m_iChannels[CHANID_TRIANGLE].Left +
           0.00494 * m_iChannels[CHANID_NOISE].Left +
           0.00335 * m_iChannels[CHANID_DPCM].Left)) *
         InternalVol;
  SumR = ((0.00851 * m_iChannels[CHANID_TRIANGLE].Right +
           0.00494 * m_iChannels[CHANID_NOISE].Right +
           0.00335 * m_iChannels[CHANID_DPCM].Right)) *
         InternalVol;
#else
  double Sum = CalcPin2(m_iChannels[CHANID_TRIANGLE], m_iChannels[CHANID_NOISE],
                        m_iChannels[CHANID_DPCM]);
#endif
...
void CMixer::AddValue(int ChanID, int Chip, int Value, int AbsValue,
                      int FrameCycles) {
  // Add sound to mixer
  //
