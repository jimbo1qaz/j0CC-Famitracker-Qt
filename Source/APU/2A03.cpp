...
void C2A03::Write(uint16_t Address, uint8_t Value) {
  if (Address < 0x4000U || Address > 0x401FU)
    return;
  switch (Address) {
  case 0x4015:
  ...
  switch (Address & 0x1C) {
  case 0x00:
    m_pSquare1->Write(Address & 0x03, Value);
    break;
  case 0x04:
    m_pSquare2->Write(Address & 0x03, Value);
    break;
  case 0x08:
    m_pTriangle->Write(Address & 0x03, Value);
    break;
  case 0x0C:
    m_pNoise->Write(Address & 0x03, Value);
    break;
  case 0x10:
    m_pDPCM->Write(Address & 0x03, Value);
    break;
  }
}
...
double C2A03::GetFreq(int Channel) const // // //
{
  switch (Channel) {
  case 0:
    return m_pSquare1->GetFrequency();
  case 1:
    return m_pSquare2->GetFrequency();
  case 2:
    return m_pTriangle->GetFrequency();
  case 3:
    return m_pNoise->GetFrequency();
  case 4:
    return m_pDPCM->GetFrequency();
  }
  return 0.0;
}
...
inline void C2A03::RunAPU1(uint32_t Time) {
  ...
  while (Time > 0) {
    uint32_t Period =
        std::min(m_pSquare1->GetPeriod(), m_pSquare2->GetPeriod());
...
uint8_t C2A03::GetSamplePos() const { return m_pDPCM->GetSamplePos(); }

uint8_t C2A03::GetDeltaCounter() const { return m_pDPCM->GetDeltaCounter(); }

bool C2A03::DPCMPlaying() const { return m_pDPCM->IsPlaying(); }
