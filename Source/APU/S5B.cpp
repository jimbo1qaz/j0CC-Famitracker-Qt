...
const int32_t EXP_VOLUME[32] = {
    0,  1,  1,  2,  2,  3,  3,  4,  5,  6,   7,   9,   11,  13,  15,  18,
    22, 26, 31, 37, 45, 53, 63, 76, 90, 106, 127, 151, 180, 212, 255, 255};

CS5BChannel::CS5BChannel(CMixer *pMixer, uint8_t ID)
    : CChannel(pMixer, SNDCHIP_S5B, ID), m_iVolume(0), m_iPeriod(0),
      m_iPeriodClock(0), m_bSquareHigh(false), m_bSquareDisable(false),
...
CS5B::CS5B(CMixer *pMixer) : CSoundChip(pMixer), m_cPort(0), m_iCounter(0) {
...
void CS5B::Process(uint32_t Time) {
  while (Time > 0U) {
    ...
    if (m_iEnvelopeClock < m_iEnvelopePeriod)
      TimeToRun =
          std::min<uint32_t>(m_iEnvelopePeriod - m_iEnvelopeClock, TimeToRun);
...
double CS5B::GetFreq(int Channel) const // // //
{
  ...
  case 3:
    ...
    if (!(m_iEnvelopeShape & 0x08) || (m_iEnvelopeShape & 0x01))
      return 0.;
    return CAPU::BASE_FREQ_NTSC / ((m_iEnvelopeShape & 0x02) ? 64. : 32.) /
           m_iEnvelopePeriod;
...
void CS5B::Log(uint16_t Address, uint8_t Value) // // //
{
  switch (Address) {
  case 0xC000:
    m_pRegisterLogger->SetPort(Value);
    break;
  case 0xE000:
  ...
  if (m_iEnvelopeClock >= m_iEnvelopePeriod && m_iEnvelopePeriod) {
    ...
    if (m_iEnvelopeLevel & 0x20) {
      if (m_iEnvelopeShape & 0x08) {
        if ((m_iEnvelopeShape & 0x03) == 0x01 ||
            (m_iEnvelopeShape & 0x03) == 0x02)
