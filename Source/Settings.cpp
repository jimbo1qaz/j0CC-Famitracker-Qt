...
void CSettings::SetupSettings() {
  //
  // This function defines all settings in the program that are stored in
  // registry All settings are loaded on program start and saved when closing
  // the program
  //
  ...
  //  2. Registry key name
  //  3. Default value
  //  4. A variable that contains the setting, loaded on program startup and
  //  saved on shutdown
  //

  const auto SETTING_INT = [&](LPCTSTR Section, LPCTSTR Entry, int Default,
                               int *Variable) { // // //
    return AddSetting<int>(_T(Section), _T(Entry), Default, Variable);
  };
  const auto SETTING_BOOL = [&](LPCTSTR Section, LPCTSTR Entry, bool Default,
                                bool *Variable) {
    return AddSetting<bool>(_T(Section), _T(Entry), Default, Variable);
  };
  const auto SETTING_STRING = [&](LPCTSTR Section, LPCTSTR Entry,
                                  CString Default, CString *Variable) {
  ...
  */
  ...
  SETTING_BOOL("General", "Double click selection", false,
               &General.bDblClickSelect);
  // // //
  SETTING_BOOL("General", "Wrap pattern values", false,
               &General.bWrapPatternValue);
  SETTING_BOOL("General", "Cut sub-volume", false, &General.bCutVolume);
  SETTING_BOOL("General", "Use old FDS volume table", false,
               &General.bFDSOldVolume);
  SETTING_BOOL("General", "Retrieve channel state", true,
               &General.bRetrieveChanState);
  SETTING_BOOL("General", "Overflow paste mode", false,
               &General.bOverflowPaste);
  SETTING_BOOL("General", "Show skipped rows", false,
               &General.bShowSkippedRows);
  SETTING_BOOL("General", "Hexadecimal keypad", false, &General.bHexKeypad);
  SETTING_BOOL("General", "Multi-frame selection", false,
               &General.bMultiFrameSel);
  SETTING_BOOL("General", "Check for new versions", true,
               &General.bCheckVersion);

  // // // Version / Compatibility info
  SETTING_INT("Version", "Module error level", MODULE_ERROR_DEFAULT,
              &Version.iErrorLevel);
  ...
  // Appearance
  SETTING_INT("Appearance", "Background", DEFAULT_COLOR_SCHEME.BACKGROUND,
              &Appearance.iColBackground);
  SETTING_INT("Appearance", "Background highlighted",
              DEFAULT_COLOR_SCHEME.BACKGROUND_HILITE,
              &Appearance.iColBackgroundHilite);
  SETTING_INT("Appearance", "Background highlighted 2",
              DEFAULT_COLOR_SCHEME.BACKGROUND_HILITE2,
              &Appearance.iColBackgroundHilite2);
  SETTING_INT("Appearance", "Pattern text", DEFAULT_COLOR_SCHEME.TEXT_NORMAL,
              &Appearance.iColPatternText);
  SETTING_INT("Appearance", "Pattern text highlighted",
              DEFAULT_COLOR_SCHEME.TEXT_HILITE,
              &Appearance.iColPatternTextHilite);
  SETTING_INT("Appearance", "Pattern text highlighted 2",
              DEFAULT_COLOR_SCHEME.TEXT_HILITE2,
              &Appearance.iColPatternTextHilite2);
  SETTING_INT("Appearance", "Pattern instrument",
              DEFAULT_COLOR_SCHEME.TEXT_INSTRUMENT,
              &Appearance.iColPatternInstrument);
  SETTING_INT("Appearance", "Pattern volume", DEFAULT_COLOR_SCHEME.TEXT_VOLUME,
              &Appearance.iColPatternVolume);
  SETTING_INT("Appearance", "Pattern effect", DEFAULT_COLOR_SCHEME.TEXT_EFFECT,
              &Appearance.iColPatternEffect);
  SETTING_INT("Appearance", "Selection", DEFAULT_COLOR_SCHEME.SELECTION,
              &Appearance.iColSelection);
  SETTING_INT("Appearance", "Cursor", DEFAULT_COLOR_SCHEME.CURSOR,
              &Appearance.iColCursor);
  // // //
  SETTING_INT("Appearance", "Current row (normal mode)",
              DEFAULT_COLOR_SCHEME.ROW_NORMAL,
              &Appearance.iColCurrentRowNormal);
  SETTING_INT("Appearance", "Current row (edit mode)",
              DEFAULT_COLOR_SCHEME.ROW_EDIT, &Appearance.iColCurrentRowEdit);
  SETTING_INT("Appearance", "Current row (playing)",
              DEFAULT_COLOR_SCHEME.ROW_PLAYING,
              &Appearance.iColCurrentRowPlaying);
  SETTING_STRING("Appearance", "Pattern font", FONT_FACE, &Appearance.strFont)
      ->UpdateDefault("General", "Pattern font");
  SETTING_INT("Appearance", "Pattern font size", FONT_SIZE,
              &Appearance.iFontSize)
      ->UpdateDefault("General", "Pattern font size");
  SETTING_STRING("Appearance", "Frame font", FONT_FACE,
                 &Appearance.strFrameFont)
  ...
  // Display
  SETTING_BOOL("Display", "Average BPM", false,
               &Display.bAverageBPM); // // // 050B
  SETTING_BOOL("Display", "Channel state", false,
               &Display.bChannelState); // // // 050B todo
  SETTING_BOOL("Display", "Register state", false,
               &Display.bRegisterState); // // // 050B

  // Other
  SETTING_INT("Other", "Sample window state", 0, &SampleWinState);
  SETTING_INT("Other", "Frame editor position", 0, &FrameEditPos);
  SETTING_INT("Other", "Control panel position", 0,
              &ControlPanelPos); // // // 050B todo
  SETTING_BOOL("Other", "Follow mode", true, &FollowMode);
  SETTING_BOOL("Other", "Meter decay rate", false,
               &MeterDecayRate); // // // 050B
...
template <class T>
CSettingBase *CSettings::AddSetting(LPCTSTR pSection, LPCTSTR pEntry,
                                    T tDefault, T *pVariable) {
  ASSERT(m_iAddedSettings < MAX_SETTINGS);
  return m_pSettings[m_iAddedSettings++] = new CSettingType<T>(
             pSection, pEntry, tDefault, pVariable); // // //
}
...
void CSettings::SetWindowPos(int Left, int Top, int Right, int Bottom,
                             int State) {
...
template <class T> void CSettingType<T>::Load() {
...
template <> void CSettingType<bool>::Load() {
  bool Value = m_tDefaultValue; // // //
  {
    stOldSettingContext s;
    if (m_pSectionSecond)
      Value =
          theApp.GetProfileInt(m_pSectionSecond, m_pEntrySecond, Value) != 0;
...
template <> void CSettingType<CString>::Load() {
...
template <class T> void CSettingType<T>::Save() {
  theApp.WriteProfileInt(m_pSection, m_pEntry, *m_pVariable);
}

template <> void CSettingType<CString>::Save() {
  theApp.WriteProfileString(m_pSection, m_pEntry, *m_pVariable);
}

template <class T> void CSettingType<T>::Default() {
