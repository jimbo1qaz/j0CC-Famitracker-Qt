...
public:
  CFileSoundDialog(BOOL bOpenFileDialog, LPCTSTR lpszDefExt = NULL,
...
CFileSoundDialog::CFileSoundDialog(BOOL bOpenFileDialog, LPCTSTR lpszDefExt,
...
void CFileSoundDialog::OnFileNameChange() {
  ...
  qDebug("PlaySound?");
  //	if (!GetFileExt().CompareNoCase(_T("wav")) &&
  // theApp.GetSettings()->General.bWavePreview)
  // PlaySound(GetPathName(), NULL, SND_FILENAME | SND_NODEFAULT | SND_ASYNC |
  // SND_NOWAIT);
...
CPCMImport::CPCMImport(CWnd *pParent /*=NULL*/)
    : CDialog(CPCMImport::IDD, pParent), m_pCachedSample(NULL),
      m_iCachedQuality(0), m_iCachedVolume(0),
...
CDSample *CPCMImport::ShowDialog() {
  ...
  OpenFileDialog.m_pOFN->lpstrInitialDir =
      theApp.GetSettings()->GetPath(PATH_WAV);
...
BOOL CPCMImport::OnInitDialog() {
  ...
  CSliderCtrl *pQualitySlider =
      static_cast<CSliderCtrl *>(GetDlgItem(IDC_QUALITY));
  CSliderCtrl *pVolumeSlider =
      static_cast<CSliderCtrl *>(GetDlgItem(IDC_VOLUME));
...
void CPCMImport::UpdateText() {
  CString Text;
  AfxFormatString1(Text, IDS_DPCM_IMPORT_QUALITY_FORMAT,
                   MakeIntString(m_iQuality));
  SetDlgItemText(IDC_QUALITY_FRM, Text);
  AfxFormatString1(Text, IDS_DPCM_IMPORT_GAIN_FORMAT,
                   MakeFloatString(float(m_iVolume), _T("%+.0f")));
...
void CPCMImport::OnBnClickedPreview() {
  ...
  CString text;
  AfxFormatString1(text, IDS_DPCM_IMPORT_SIZE_FORMAT,
                   MakeIntString(pSample->GetSize()));
  SetDlgItemText(IDC_SAMPLESIZE, text);
...
void CPCMImport::UpdateFileInfo() {
  ...
  float base_freq =
      (float)CAPU::BASE_FREQ_NTSC / (float)CDPCM::DMC_PERIODS_NTSC[m_iQuality];

  CString Resampling;
  AfxFormatString1(Resampling, IDS_DPCM_IMPORT_TARGET_FORMAT,
                   MakeFloatString(base_freq));
  SetDlgItemText(IDC_RESAMPLING, Resampling);
}

CDSample *CPCMImport::GetSample() {
  if (m_pCachedSample == NULL || m_iCachedQuality != m_iQuality ||
...
CDSample *CPCMImport::ConvertFile() {
  ...
  resampler resmpler(*m_psinc, resample_factor, m_iChannels, m_iSampleSize,
                     m_iWaveSize, m_fSampleFile);
  float val;
  // Conversion
  while (resmpler.get(val) && (iSamples < CDSample::MAX_SIZE)) { // // //
    ...
    val =
        (std::max<float>(std::min<float>(val, (float)MAX_AMP), (float)MIN_AMP));
    ...
    if (!memcmp(Header, "WAVE", 4)) {
      ...
      if (!memcmp(Header, "fmt ", 4)) {
        ...
        if (WaveFormat.wf.wFormatTag != WAVE_FORMAT_PCM) {
          ...
          Scanning = false;
          ValidWave = false;
          //					TRACE(_T("DPCM import:
          // Unrecognized wave format
        ...
        // Unrecognized block
        //				TRACE(_T("DPCM import: Unrecognized
