...
const TCHAR *ROOT_ITEMS[] = {_T("2A03/2A07"),     _T("Konami VRC6"),
                             _T("Konami VRC7"),   _T("Nintendo FDS"),
                             _T("Nintendo MMC5"), _T("Namco 106"),
                             _T("Sunsoft 5B")};
...
const int CHILD_ITEMS_ID[ROOT_ITEM_COUNT][9] = {
    ...
    // VRC 7
    {CHANID_VRC7_CH1, CHANID_VRC7_CH2, CHANID_VRC7_CH3, CHANID_VRC7_CH4,
     CHANID_VRC7_CH5, CHANID_VRC7_CH6},
    // FDS
    {CHANID_FDS},
    // MMC5
    {CHANID_MMC5_SQUARE1, CHANID_MMC5_SQUARE2},
    // N163
    {CHANID_N163_CH1, CHANID_N163_CH2, CHANID_N163_CH3, CHANID_N163_CH4,
...
const TCHAR *CHILD_ITEMS[ROOT_ITEM_COUNT][9] = {
    ...
    // VRC 7
    {_T("Channel 1"), _T("Channel 2"), _T("Channel 3"), _T("Channel 4"),
     _T("Channel 5"), _T("Channel 6")},
    // FDS
    {_T("FDS")},
    // MMC5
    ...
    // N163
    {_T("Channel 1"), _T("Channel 2"), _T("Channel 3"), _T("Channel 4"),
     _T("Channel 5"), _T("Channel 6"), _T("Channel 7"), _T("Channel 8")},
    // S5B
...
// ON_NOTIFY(NM_RCLICK, IDC_AVAILABLE_TREE,
// &CChannelsDlg::OnNMRClickAvailableTree) ON_BN_CLICKED(IDC_MOVE_UP,
// &CChannelsDlg::OnBnClickedMoveUp)
    ...
    for (int i = 0; i < ROOT_ITEM_COUNT; ++i) {
      ...
      for (int j = 0; CHILD_ITEMS[i][j] != NULL; ++j) {
        if (CHILD_ITEMS_ID[i][j] == ChanID) {
          ...
          str.Format(_T("%i: %s"), j + 1, CHILD_ITEMS[i][j]);
          HTREEITEM hChild =
              m_pAvailableTree->InsertItem(str, hParent, hParent);
          m_pAvailableTree->SetItemData(hChild, CHILD_ITEMS_ID[i][j]);
...
void CChannelsDlg::AddChannel(int ChanID) {
  for (int i = 0; i < ROOT_ITEM_COUNT; ++i) {
    HTREEITEM hItem =
        m_pAvailableTree->GetNextItem(m_hRootItems[i], TVGN_CHILD);
...
void CChannelsDlg::InsertChannel(HTREEITEM hItem) {
  ...
  if (hParentItem != NULL) {
    ...
    CString AddStr =
        ChipName + _T(" :: ") + ChanName.Right(ChanName.GetLength() - 3);
...
void CChannelsDlg::OnBnClickedMoveDown() {
  ...
  m_pAddedChannels->SetItemText(Index, 0,
                                m_pAddedChannels->GetItemText(Index + 1, 0));
  m_pAddedChannels->SetItemData(Index,
                                m_pAddedChannels->GetItemData(Index + 1));
  ...
  m_pAddedChannels->SetItemState(Index + 1, LVIS_SELECTED | LVIS_FOCUSED,
