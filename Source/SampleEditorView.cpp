...
CSampleEditorView::CSampleEditorView()
    : m_iSelStart(-1), m_iSelEnd(-1), m_iStartCursor(0), m_pSamples(NULL),
      m_bClicked(false), m_pScrollBar(NULL), m_fZoom(1.0f), m_iViewStart(0),
      m_iViewEnd(0) {
...
void CSampleEditorView::OnPaint() {
  ...
  m_bmpCopy.CreateCompatibleBitmap(&dc, m_clientRect.Width(),
                                   m_clientRect.Height());
  ...
  if (m_iSize == 0) {
    ...
    dc.BitBlt(0, 0, m_clientRect.Width(), m_clientRect.Height(), &m_dcCopy, 0,
              0, SRCCOPY);
    return;
  }
  ...
  if (m_iSelStart != m_iSelEnd) {
    ...
    m_dcCopy.FillSolidRect(StartPixel, 0, EndPixel - StartPixel, Height,
                           SEL_COLOR);
  }
  ...
  dc.BitBlt(0, 0, m_clientRect.Width(), m_clientRect.Height(), &m_dcCopy, 0, 0,
            SRCCOPY);
}

BOOL CSampleEditorView::OnEraseBkgnd(CDC *pDC) { return FALSE; }
...
void CSampleEditorView::OnMouseMove(UINT nFlags, CPoint point) {
  ...
  if (nFlags & MK_LBUTTON && m_iSelStart != -1) {
    int Block = GetBlock(point.x + (m_iBlockSize / 2) +
                         int(m_iViewStart / m_dSampleStep));
    m_iSelEnd = Block;
    Invalidate();
    RedrawWindow();
...
void CSampleEditorView::OnLButtonUp(UINT nFlags, CPoint point) {
  ...
  double Sample =
      double(point.x + int(m_iViewStart / m_dSampleStep)) * m_dSampleStep;
  int Offset = int(Sample / (8.0 * 64.0));
...
void CSampleEditorView::DrawPlayCursor(int Pos) {
  CDC *pDC = GetDC();
  int x = int((double(Pos + m_iStartCursor) * 8.0 * 64.0 - m_iViewStart) /
              m_dSampleStep);

  pDC->BitBlt(0, 0, m_clientRect.Width(), m_clientRect.Height(), &m_dcCopy, 0,
              0, SRCCOPY);
...
void CSampleEditorView::DrawStartCursor() {
  CDC *pDC = GetDC();
  int x =
      int((double(m_iStartCursor) * 8.0 * 64.0 - m_iViewStart) / m_dSampleStep);
...
bool CSampleEditorView::HasSelection() const { return m_iSelStart != -1; }
...
void CSampleEditorView::PreSubclassWindow() {
  ...
  if (m_pScrollBar->m_hWnd == NULL) {
    CRect rect;
    GetClientRect(&rect);
    m_pScrollBar->Create(SBS_HORZ | SBS_BOTTOMALIGN | WS_CHILD | WS_VISIBLE,
                         rect, this, 1);
  }
...
void CSampleEditorView::SetZoom(float Factor) {
  ...
  m_fZoom =
      ((MAX_FACTOR - MIN_FACTOR) * (Factor * Factor)) + MIN_FACTOR; // // //
  ...
  if (Factor == 1.0f) {
    ...
    m_pScrollBar->SetScrollPos((m_iViewStart * m_iScrollMax) /
                               (m_iSize - NewViewSize));
...
void CSampleEditorView::OnHScroll(UINT nSBCode, UINT nPos,
