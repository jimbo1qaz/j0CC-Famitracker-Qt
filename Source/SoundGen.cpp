...
const double CSoundGen::OLD_VIBRATO_DEPTH[] = {
    1.0,  1.0,  2.0,  3.0,  4.0,  7.0,   8.0,   15.0,
    16.0, 31.0, 32.0, 63.0, 64.0, 127.0, 128.0, 255.0};
...
CSoundGen::CSoundGen()
    : m_pAPU(NULL), m_pDSound(NULL), m_pDSoundChannel(NULL),
      m_pAccumBuffer(NULL), m_iGraphBuffer(NULL), m_pDocument(NULL),
      m_pTrackerView(NULL), m_bRendering(false), m_bPlaying(false),
      m_bHaltRequest(false), m_bDoHalt(false), // // //
      m_pPreviewSample(NULL), m_pVisualizerWnd(NULL), m_iSpeed(0), m_iTempo(0),
      m_iGrooveIndex(-1),                             // // //
      m_iGroovePosition(0),                           // // //
      m_pInstRecorder(new CInstrumentRecorder(this)), // // //
      m_bWaveChanged(0), m_iMachineType(NTSC), m_bRunning(false),
      m_hInterruptEvent(NULL), m_bBufferTimeout(false), m_bDirty(false),
      m_iQueuedFrame(-1), m_iPlayFrame(0), m_iPlayRow(0), m_iPlayTrack(0),
      m_iPlayTicks(0), m_iRegisterStream(), // // //
      m_bBufferUnderrun(false), m_bAudioClipping(false), m_iClipCounter(0),
      m_pSequencePlayPos(NULL), m_iSequencePlayPos(0), m_iSequenceTimeout(0),
...
void CSoundGen::CreateChannels() {
  ...
  AssignChannel(new CTrackerChannel(_T("Pulse 1"), _T("PU1"), SNDCHIP_NONE,
                                    CHANID_SQUARE1));
  AssignChannel(new CTrackerChannel(_T("Pulse 1 SFX"), _T("PU1*"), SNDCHIP_NONE,
                                    CHANID_SQUARE1));
#else
  AssignChannel(new CTrackerChannel(_T("Pulse 1"), _T("PU1"), SNDCHIP_NONE,
                                    CHANID_SQUARE1));
  AssignChannel(new CTrackerChannel(_T("Pulse 2"), _T("PU2"), SNDCHIP_NONE,
                                    CHANID_SQUARE2));
#endif
  AssignChannel(new CTrackerChannel(_T("Triangle"), _T("TRI"), SNDCHIP_NONE,
                                    CHANID_TRIANGLE));
  AssignChannel(
      new CTrackerChannel(_T("Noise"), _T("NOI"), SNDCHIP_NONE, CHANID_NOISE));
  AssignChannel(
      new CTrackerChannel(_T("DPCM"), _T("DMC"), SNDCHIP_NONE, CHANID_DPCM));

  // Konami VRC6
  AssignChannel(new CTrackerChannel(_T("VRC6 Pulse 1"), _T("V1"), SNDCHIP_VRC6,
                                    CHANID_VRC6_PULSE1));
  AssignChannel(new CTrackerChannel(_T("VRC6 Pulse 2"), _T("V2"), SNDCHIP_VRC6,
                                    CHANID_VRC6_PULSE2));
  AssignChannel(new CTrackerChannel(_T("Sawtooth"), _T("SAW"), SNDCHIP_VRC6,
                                    CHANID_VRC6_SAWTOOTH));

  // // // Nintendo MMC5
  AssignChannel(new CTrackerChannel(_T("MMC5 Pulse 1"), _T("PU3"), SNDCHIP_MMC5,
                                    CHANID_MMC5_SQUARE1));
  AssignChannel(new CTrackerChannel(_T("MMC5 Pulse 2"), _T("PU4"), SNDCHIP_MMC5,
                                    CHANID_MMC5_SQUARE2));
  AssignChannel(new CTrackerChannel(_T("MMC5 PCM"), _T("PCM"), SNDCHIP_MMC5,
                                    CHANID_MMC5_VOICE)); // null channel handler

  // Namco N163
  AssignChannel(new CTrackerChannel(_T("Namco 1"), _T("N1"), SNDCHIP_N163,
                                    CHANID_N163_CH1));
  AssignChannel(new CTrackerChannel(_T("Namco 2"), _T("N2"), SNDCHIP_N163,
                                    CHANID_N163_CH2));
  AssignChannel(new CTrackerChannel(_T("Namco 3"), _T("N3"), SNDCHIP_N163,
                                    CHANID_N163_CH3));
  AssignChannel(new CTrackerChannel(_T("Namco 4"), _T("N4"), SNDCHIP_N163,
                                    CHANID_N163_CH4));
  AssignChannel(new CTrackerChannel(_T("Namco 5"), _T("N5"), SNDCHIP_N163,
                                    CHANID_N163_CH5));
  AssignChannel(new CTrackerChannel(_T("Namco 6"), _T("N6"), SNDCHIP_N163,
                                    CHANID_N163_CH6));
  AssignChannel(new CTrackerChannel(_T("Namco 7"), _T("N7"), SNDCHIP_N163,
                                    CHANID_N163_CH7));
  AssignChannel(new CTrackerChannel(_T("Namco 8"), _T("N8"), SNDCHIP_N163,
                                    CHANID_N163_CH8));

  // Nintendo FDS
  AssignChannel(
      new CTrackerChannel(_T("FDS"), _T("FDS"), SNDCHIP_FDS, CHANID_FDS));

  // Konami VRC7
  AssignChannel(new CTrackerChannel(_T("FM Channel 1"), _T("FM1"), SNDCHIP_VRC7,
                                    CHANID_VRC7_CH1));
  AssignChannel(new CTrackerChannel(_T("FM Channel 2"), _T("FM2"), SNDCHIP_VRC7,
                                    CHANID_VRC7_CH2));
  AssignChannel(new CTrackerChannel(_T("FM Channel 3"), _T("FM3"), SNDCHIP_VRC7,
                                    CHANID_VRC7_CH3));
  AssignChannel(new CTrackerChannel(_T("FM Channel 4"), _T("FM4"), SNDCHIP_VRC7,
                                    CHANID_VRC7_CH4));
  AssignChannel(new CTrackerChannel(_T("FM Channel 5"), _T("FM5"), SNDCHIP_VRC7,
                                    CHANID_VRC7_CH5));
  AssignChannel(new CTrackerChannel(_T("FM Channel 6"), _T("FM6"), SNDCHIP_VRC7,
                                    CHANID_VRC7_CH6));

  // // // Sunsoft 5B
  AssignChannel(new CTrackerChannel(_T("5B Square 1"), _T("5B1"), SNDCHIP_S5B,
                                    CHANID_S5B_CH1));
  AssignChannel(new CTrackerChannel(_T("5B Square 2"), _T("5B2"), SNDCHIP_S5B,
                                    CHANID_S5B_CH2));
  AssignChannel(new CTrackerChannel(_T("5B Square 3"), _T("5B3"), SNDCHIP_S5B,
                                    CHANID_S5B_CH3));
}
...
void CSoundGen::AssignDocument(CFamiTrackerDoc *pDoc) {
  ...
  // Ignore all but the first document (as new documents are used to import
  // files)
  if (m_pDocument != NULL)
    return;
...
void CSoundGen::RegisterChannels(int Chip, CFamiTrackerDoc *pDoc) {
  // This method will add channels to the document object, depending on the
  // expansion chip used. Called from the document object (from the main thread)
  ...
  for (int i = 0; i < sizeof(m_pTrackerChannels) / sizeof(CTrackerChannel *);
       ++i) {
    int ID = m_pTrackerChannels[i]->GetID(); // // //
    if (m_pChannels[i] &&
        ((m_pTrackerChannels[i]->GetChip() & Chip) || (i < 5)) // // //
        &&
        (i >= CHANID_FDS || i < CHANID_N163_CH1 + pDoc->GetNamcoChannels())) {
      pDoc->RegisterChannel(m_pTrackerChannels[i], ID,
                            m_pTrackerChannels[i]->GetChip());
    }
  }
...
void CSoundGen::DocumentPropertiesChanged(CFamiTrackerDoc *pDocument) {
  ...
  const double A440_NOTE =
      45. - pDocument->GetTuningSemitone() - pDocument->GetTuningCent() / 100.;
  ...
  for (int i = 0; i < NOTE_COUNT; ++i) {
    ...
    m_iNoteLookupTablePAL[i] =
        (unsigned int)(Pitch - pDocument->GetDetuneOffset(1, i)); // // //

    // 2A03 / MMC5 / VRC6
    Pitch = (clock_ntsc / Freq) - 0.5;
    m_iNoteLookupTableNTSC[i] =
        (unsigned int)(Pitch - pDocument->GetDetuneOffset(0, i)); // // //
    m_iNoteLookupTableS5B[i] = m_iNoteLookupTableNTSC[i] + 1;     // correction

    // VRC6 Saw
    Pitch = ((clock_ntsc * 16.0) / (Freq * 14.0)) - 0.5;
    m_iNoteLookupTableSaw[i] =
        (unsigned int)(Pitch - pDocument->GetDetuneOffset(2, i)); // // //
...
#endif
    m_iNoteLookupTableFDS[i] =
        (unsigned int)(Pitch + pDocument->GetDetuneOffset(4, i)); // // //

    // N163
    Pitch =
        ((Freq * pDocument->GetNamcoChannels() * 983040.0) / clock_ntsc + 0.5) /
        4; // // //
    m_iNoteLookupTableN163[i] =
        (unsigned int)(Pitch + pDocument->GetDetuneOffset(5, i)); // // //
    ...
    if (i < NOTE_RANGE) {
      ...
      m_iNoteLookupTableVRC7[i] =
          (unsigned int)(Pitch + pDocument->GetDetuneOffset(3, i)); // // //
    }
  }
  ...
  for (int i = 0; i < CHANNELS; ++i) {
    ...
    case CHANID_VRC7_CH6:
      Table = m_iNoteLookupTableVRC7;
      break;
    case CHANID_FDS:
      Table = m_iNoteLookupTableFDS;
      break;
    ...
    case CHANID_N163_CH8:
      Table = m_iNoteLookupTableN163;
      break;
    case CHANID_S5B_CH1:
    case CHANID_S5B_CH2:
    case CHANID_S5B_CH3:
      Table = m_iNoteLookupTableS5B;
      break;
    default:
      continue;
    }
  ...
  // One possible optimization is to store the PAL table as the difference from
  // the NTSC table
  ...
  period_file.WriteString(
      "; Fnum table, multiplied by 4 for higher resolution\n");
  period_file.WriteString(".define ft_vrc7_table ");

  for (int i = 0; i <= NOTE_RANGE;
       ++i) { // // // include last item for linear pitch code optimization
    CString str;
    if (i == NOTE_RANGE)
...
void CSoundGen::PreviewSample(const CDSample *pSample, int Offset,
                              int Pitch) // // //
{
  if (!m_hThread)
    return;
  ...
  PostThreadMessage(WM_USER_PREVIEW_SAMPLE, (WPARAM)pSample,
                    MAKELPARAM(Offset, Pitch));
}
...
bool CSoundGen::IsRunning() const { return (m_hThread != NULL) && m_bRunning; }
...
bool CSoundGen::ResetAudioDevice() {
  ...
  // Create more blocks if a bigger buffer than 100ms is used to enhance program
  // response
  //	if (BufferLen > 100)
  ...
  // Create channel
  m_pDSoundChannel =
      m_pDSound->OpenChannel(SampleRate, SampleSize, 1, BufferLen, iBlocks);
  ...
  if (!m_pAPU->SetupSound(
          SampleRate, 1, (m_iMachineType == NTSC) ? MACHINE_NTSC : MACHINE_PAL))
    return false;

  currN163LevelOffset = m_pDocument->GetN163LevelOffset();

  m_pAPU->SetChipLevel(CHIP_LEVEL_APU1,
                       float(pSettings->ChipLevels.iLevelAPU1 / 10.0f));
  m_pAPU->SetChipLevel(CHIP_LEVEL_APU2,
                       float(pSettings->ChipLevels.iLevelAPU2 / 10.0f));
  m_pAPU->SetChipLevel(CHIP_LEVEL_VRC6,
                       float(pSettings->ChipLevels.iLevelVRC6 / 10.0f));
  m_pAPU->SetChipLevel(CHIP_LEVEL_VRC7,
                       float(pSettings->ChipLevels.iLevelVRC7 / 10.0f));
  m_pAPU->SetChipLevel(CHIP_LEVEL_MMC5,
                       float(pSettings->ChipLevels.iLevelMMC5 / 10.0f));
  m_pAPU->SetChipLevel(CHIP_LEVEL_FDS,
                       float(pSettings->ChipLevels.iLevelFDS / 10.0f));
  m_pAPU->SetChipLevel(
      CHIP_LEVEL_N163,
      float((pSettings->ChipLevels.iLevelN163 + currN163LevelOffset) / 10.0f));
  m_pAPU->SetChipLevel(CHIP_LEVEL_S5B,
                       float(pSettings->ChipLevels.iLevelS5B / 10.0f));
  /*
          m_pAPU->SetChipLevel(SNDCHIP_NONE,
  0);//pSettings->ChipLevels.iLevel2A03); m_pAPU->SetChipLevel(SNDCHIP_VRC6,
  0);//pSettings->ChipLevels.iLevelVRC6); m_pAPU->SetChipLevel(SNDCHIP_VRC7,
  0);//pSettings->ChipLevels.iLevelVRC7); m_pAPU->SetChipLevel(SNDCHIP_MMC5,
  0);//pSettings->ChipLevels.iLevelMMC5); m_pAPU->SetChipLevel(SNDCHIP_FDS,
  0);//pSettings->ChipLevels.iLevelFDS);
  ...
  */
  ...
  m_pAPU->SetupMixer(
      pSettings->Sound.iBassFilter, pSettings->Sound.iTrebleFilter,
  ...
  //	TRACE("SoundGen: Created sound channel with params: %i Hz, %i bits, %i
...
bool CSoundGen::PlayBuffer() {
  if (m_bRendering) {
    ...
    while ((dwEvent = m_pDSoundChannel->WaitForSyncEvent(AUDIO_TIMEOUT)) !=
           BUFFER_IN_SYNC) {
      //			switch (dwEvent) {
      //				case BUFFER_TIMEOUT:
      //					// Buffer timeout
...
unsigned int CSoundGen::GetUnderruns() const { return m_iAudioUnderruns; }
...
int CSoundGen::ReadPeriodTable(int Index, int Table) const // // //
{
  ...
  case CDetuneTable::DETUNE_PAL:
    return m_iNoteLookupTablePAL[Index];
    break;
  case CDetuneTable::DETUNE_SAW:
    return m_iNoteLookupTableSaw[Index];
    break;
  case CDetuneTable::DETUNE_VRC7:
    return m_iNoteLookupTableVRC7[Index];
    break;
  case CDetuneTable::DETUNE_FDS:
    return m_iNoteLookupTableFDS[Index];
    break;
  case CDetuneTable::DETUNE_N163:
    return m_iNoteLookupTableN163[Index];
    break;
  case CDetuneTable::DETUNE_S5B:
    return m_iNoteLookupTableNTSC[Index] + 1;
    break;
  default:
    AfxDebugBreak();
    return m_iNoteLookupTableNTSC[Index];
...
{
  int Frame =
      IsPlaying() ? GetPlayerFrame() : m_pTrackerView->GetSelectedFrame();
  int Row = IsPlaying() ? GetPlayerRow() : m_pTrackerView->GetSelectedRow();
  if (stFullState *State =
          m_pDocument->RetrieveSoundState(GetPlayerTrack(), Frame, Row, -1)) {
    if (State->Tempo != -1)
    ...
    if (State->GroovePos >= 0) {
      ...
      if (m_pDocument->GetGroove(m_iGrooveIndex) != NULL)
        m_iSpeed =
            m_pDocument->GetGroove(m_iGrooveIndex)->GetEntry(m_iGroovePosition);
    } else {
      if (State->Speed >= 0)
    ...
    m_iLastHighlight =
        m_pDocument->GetHighlightAt(GetPlayerTrack(), Frame, Row).First;
    SetupSpeed();
    for (int i = 0; i < m_pDocument->GetChannelCount(); i++) {
      for (int j = 0;
           j < sizeof(m_pTrackerChannels) / sizeof(CTrackerChannel *);
           ++j) // // // pick this out later
        if (m_pChannels[j] &&
            m_pTrackerChannels[j]->GetID() == State->State[i].ChannelIndex) {
          m_pChannels[j]->ApplyChannelState(&State->State[i]);
          break;
        }
    }
    delete State;
...
/*!	\brief Obtains a human-readable form of a channel state object.
        \warning The output of this method is neither guaranteed nor required to
   match that of CChannelHandler::GetStateString. \param State A reference to
   the channel state object. \return A string representing the channel's state.
...
static CString GetStateString(const stChannelState &State) {
  ...
  log.AppendFormat(_T("        Vol.: %X        Active effects:"),
                   State.Volume >= MAX_VOLUME ? 0xF : State.Volume);

  CString effStr = _T("");

  const effect_t SLIDE_EFFECT = State.Effect[EF_ARPEGGIO] >= 0
                                    ? EF_ARPEGGIO
                                    : State.Effect[EF_PORTA_UP] >= 0
                                          ? EF_PORTA_UP
                                          : State.Effect[EF_PORTA_DOWN] >= 0
                                                ? EF_PORTA_DOWN
                                                : EF_PORTAMENTO;
  for (const auto &x : {SLIDE_EFFECT, EF_VIBRATO, EF_TREMOLO, EF_VOLUME_SLIDE,
                        EF_PITCH, EF_DUTY_CYCLE}) {
    int p = State.Effect[x];
    if (p < 0)
      continue;
    if (p == 0 && x != EF_PITCH)
      continue;
    if (p == 0x80 && x == EF_PITCH)
      continue;
    effStr.AppendFormat(_T(" %c%02X"), EFF_CHAR[x - 1], p);
  }

  if ((State.ChannelIndex >= CHANID_SQUARE1 &&
       State.ChannelIndex <= CHANID_SQUARE2) ||
      State.ChannelIndex == CHANID_NOISE ||
      (State.ChannelIndex >= CHANID_MMC5_SQUARE1 &&
       State.ChannelIndex <= CHANID_MMC5_SQUARE2))
  ...
  else if (State.ChannelIndex >= CHANID_VRC7_CH1 &&
           State.ChannelIndex <= CHANID_VRC7_CH6)
    ...
    for (const auto &x : FDS_EFFECTS) {
      int p = State.Effect[x];
      if (p < 0 || (x == EF_FDS_MOD_BIAS && p == 0x80))
        continue;
      effStr.AppendFormat(_T(" %c%02X"), EFF_CHAR[x - 1], p);
    }
  else if (State.ChannelIndex >= CHANID_S5B_CH1 &&
           State.ChannelIndex <= CHANID_S5B_CH3)
  ...
  else if (State.ChannelIndex >= CHANID_N163_CH1 &&
           State.ChannelIndex <= CHANID_N163_CH8)
    for (const auto &x : N163_EFFECTS) {
      int p = State.Effect[x];
      if (p < 0 || (x == EF_N163_WAVE_BUFFER && p == 0x7F))
        continue;
      effStr.AppendFormat(_T(" %c%02X"), EFF_CHAR[x - 1], p);
    }
  if (State.Effect_LengthCounter >= 0)
    effStr.AppendFormat(_T(" %c%02X"), EFF_CHAR[EF_VOLUME - 1],
                        State.Effect_LengthCounter);
  if (State.Effect_AutoFMMult >= 0)
    effStr.AppendFormat(_T(" %c%02X"), EFF_CHAR[EF_FDS_MOD_DEPTH - 1],
                        State.Effect_AutoFMMult);
...
CString CSoundGen::RecallChannelState(int Channel) const // // //
{
  ...
  CString str = _T("");
  if (stFullState *State = m_pDocument->RetrieveSoundState(
          GetPlayerTrack(), Frame, Row, Channel)) {
      ...
      if (State->GroovePos >= 0) {
        ...
        for (unsigned char i = 0; i < Size; i++)
          str.AppendFormat(_T(" %d"),
                           Groove->GetEntry((i + State->GroovePos) % Size));
      } else
...
void CSoundGen::HaltPlayer() {
  ...
  while (!m_iRegisterStream.empty()) { // TODO: move this to a separate thread
    ...
    if (Val == 0x62) {
      ...
      if (Delay) {
        ...
        if (Samples ==
            static_cast<int>(VGM_SAMPLE_RATE / CAPU::FRAME_RATE_NTSC)) { // 735
          c = 0x62;
          vgm.Write(&c, 1);
        } else if (Samples == static_cast<int>(VGM_SAMPLE_RATE /
                                               CAPU::FRAME_RATE_PAL)) { // 882
          c = 0x63;
          vgm.Write(&c, 1);
        } else {
          c = 0x61 | (Samples << 8);
  ...
  *reinterpret_cast<int *>(Header + 0x04) =
      static_cast<int>(vgm.GetLength()) - 4;
  *reinterpret_cast<int *>(Header + 0x08) = 0x161;
  *reinterpret_cast<int *>(Header + 0x18) =
      static_cast<int>(VGM_SAMPLE_RATE * DelayTotal / m_iFrameRate);
...
CRegisterState *CSoundGen::GetRegState(unsigned Chip,
                                       unsigned Reg) const // // //
{
...
void CSoundGen::ResetTempo() {
  ...
  if (m_pDocument->GetSongGroove(m_iPlayTrack) &&
      m_pDocument->GetGroove(m_iSpeed) != NULL) { // // //
...
float CSoundGen::GetTempo() const {
  ...
  if (!m_iTempo)
    Tempo = static_cast<float>(2.5 * m_iFrameRate);
  ...
  if (m_iGrooveIndex != -1) {
    if (m_pDocument->GetGroove(m_iGrooveIndex) == NULL)
      ...
      Speed = m_pDocument->GetGroove(m_iGrooveIndex)->GetAverage();
  } else
    Speed = static_cast<float>(m_iSpeed);
...
float CSoundGen::GetCurrentBPM() const // // //
{
  ...
  float BPM = std::min(IsPlaying() && theApp.GetSettings()->Display.bAverageBPM
                           ? GetAverageBPM()
                           : GetTempo(),
                       EngineSpeed * 15); // // // 050B
  return static_cast<float>(BPM * 4. /
                            (m_iLastHighlight ? m_iLastHighlight : 4));
}
...
void CSoundGen::RunFrame() {
  ...
  if (IsPlaying()) {
    ...
    if (m_iTempoAccum <= 0) {
      ...
      if (m_iGrooveIndex != -1 &&
          m_pDocument->GetGroove(m_iGrooveIndex) != NULL) { // // //
        m_iSpeed =
            m_pDocument->GetGroove(m_iGrooveIndex)->GetEntry(m_iGroovePosition);
        SetupSpeed();
        m_iGroovePosition++;
      ...
      if (auto pMark = m_pDocument->GetBookmarkAt(m_iPlayTrack, m_iPlayFrame,
                                                  m_iPlayRow)) // // //
...
void CSoundGen::LoadMachineSettings() // // //
{
  ...
  int BaseFreq =
      (m_iMachineType == NTSC) ? CAPU::BASE_FREQ_NTSC : CAPU::BASE_FREQ_PAL;
  int DefaultRate =
      (m_iMachineType == NTSC) ? CAPU::FRAME_RATE_NTSC : CAPU::FRAME_RATE_PAL;
...

  {
    ...
    m_pAPU->ChangeMachineRate(
        m_iMachineType == NTSC ? MACHINE_NTSC : MACHINE_PAL, Rate); // // //
  }
...
void CSoundGen::SetSkipRow(int Row) { m_iSkipToRow = Row; }

void CSoundGen::SetJumpPattern(int Pattern) { m_iJumpToPattern = Pattern; }
...
void CSoundGen::EvaluateGlobalEffects(stChanNote *NoteData, int EffColumns) {
  ...
  for (int i = 0; i < EffColumns; ++i) {
    ...
    case EF_GROOVE: // // //
      if (m_pDocument->GetGroove(EffParam % MAX_GROOVE) == NULL)
        break;
...
bool CSoundGen::RenderToFile(LPTSTR pFile, render_end_t SongEndType,
                             int SongEndParam, int Track) {
  // Called from main thread
  //	ASSERT(GetCurrentThreadId() == theApp.m_nThreadID);
  ASSERT(m_pDocument != NULL);
  ...
  if (m_iRenderEndWhen == SONG_TIME_LIMIT) {
    ...
    m_iRenderEndParam =
        m_pDocument->ScanActualLength(Track, m_iRenderEndParam); // // //
  ...
  if (!m_pWaveFile ||
      !m_pWaveFile->OpenFile(pFile, theApp.GetSettings()->Sound.iSampleRate,
                             theApp.GetSettings()->Sound.iSampleSize, 1)) {
...
bool CSoundGen::IsRendering() const { return m_bRendering; }

bool CSoundGen::IsBackgroundTask() const { return m_bRendering; }
...
bool CSoundGen::PreviewDone() const { return (m_pAPU->DPCMPlaying() == false); }
...
BOOL CSoundGen::InitInstance() {
  ...
  m_iSpeed = DEFAULT_SPEED;
  m_iTempo =
      (DEFAULT_MACHINE_TYPE == NTSC) ? DEFAULT_TEMPO_NTSC : DEFAULT_TEMPO_PAL;
...
BOOL CSoundGen::OnIdle(LONG lCount) {
  ...
  // Access the document object, skip if access wasn't granted to avoid gaps in
  // audio playback
...
void CSoundGen::PlayChannelNotes() {
  ...
  for (int i = 0; i < CHANNELS; ++i) { // // //
    ...
    int Channel =
        m_pDocument->GetChannelIndex(m_pTrackerChannels[Index]->GetID());
    if (Channel == -1)
      continue;
    ...
    if (m_pTrackerChannels[Index]->NewNoteData()) {
      ...
      PlayNote(Index, &Note,
               m_pDocument->GetEffColumns(m_iPlayTrack, Channel) + 1);
    }
    ...
    m_pTrackerChannels[Index]->SetVolumeMeter(
        m_pAPU->GetVol(m_pTrackerChannels[Index]->GetID()));
  }
...
void CSoundGen::UpdatePlayer() {
  ...
  if (m_bPlaying) {
    if (m_iTempoAccum <= 0) {
      ...
      m_iTempoAccum +=
          (m_iTempo ? 60 * TicksPerSec : m_iSpeed) - m_iTempoRemainder; // // //
    }
...
void CSoundGen::OnSilentAll(WPARAM wParam, LPARAM lParam) { MakeSilent(); }
...
void CSoundGen::OnStopPlayer(WPARAM wParam, LPARAM lParam) { HaltPlayer(); }
...
void CSoundGen::OnStopRender(WPARAM wParam, LPARAM lParam) { StopRendering(); }

void CSoundGen::OnPreviewSample(WPARAM wParam, LPARAM lParam) {
  PlaySample(reinterpret_cast<CDSample *>(wParam), LOWORD(lParam),
             HIWORD(lParam));
}
...
bool CSoundGen::HasWaveChanged() const { return m_bInternalWaveChanged; }
...
void CSoundGen::ReadPatternRow() {
  ...
  for (int i = 0; i < Channels; ++i) {
    if (m_pTrackerView->PlayerGetNote(m_iPlayTrack, m_iPlayFrame, i, m_iPlayRow,
                                      NoteData))
...
void CSoundGen::QueueNote(int Channel, stChanNote &NoteData,
                          note_prio_t Priority) const {
  if (m_pDocument == NULL)
    return;

  // Queue a note for play
  m_pDocument->GetChannel(Channel)->SetNote(NoteData, Priority);
  theApp.GetMIDI()->WriteNote(Channel, NoteData.Note, NoteData.Octave,
                              NoteData.Vol);
}
...
void CSoundGen::ForceReloadInstrument(int Channel) // // //
{
  if (m_pDocument == NULL)
    return;
  m_pChannels[m_pDocument->GetChannel(Channel)->GetID()]
      ->ForceReloadInstrument();
}

int CSoundGen::GetPlayerRow() const { return m_iPlayRow; }

int CSoundGen::GetPlayerFrame() const { return m_iPlayFrame; }

int CSoundGen::GetPlayerTrack() const { return m_iPlayTrack; }

int CSoundGen::GetPlayerTicks() const { return m_iPlayTicks; }
...
void CSoundGen::SetQueueFrame(int Frame) { m_iQueuedFrame = Frame; }

int CSoundGen::GetQueueFrame() const { return m_iQueuedFrame; }
...
int CSoundGen::GetMeterDecayRate() const { return m_pAPU->GetMeterDecayRate(); }
