...
const COLOR_SCHEME *CConfigAppearance::COLOR_SCHEMES[] = {
    &DEFAULT_COLOR_SCHEME, &MONOCHROME_COLOR_SCHEME, &RENOISE_COLOR_SCHEME,
    &WHITE_COLOR_SCHEME,
    &SATURDAY_COLOR_SCHEME // // //
};

const int CConfigAppearance::NUM_COLOR_SCHEMES =
    sizeof(COLOR_SCHEMES) / sizeof(COLOR_SCHEME *);
...
int CALLBACK CConfigAppearance::EnumFontFamExProc(ENUMLOGFONTEX *lpelfe,
  ...
  if (lpelfe->elfLogFont.lfCharSet == ANSI_CHARSET &&
    ...
    reinterpret_cast<CConfigAppearance *>(lParam)->AddFontName(
...
// ON_BN_CLICKED(IDC_PATTERNCOLORS,
// &CConfigAppearance::OnBnClickedPatterncolors)
// ON_BN_CLICKED(IDC_DISPLAYFLATS,
//&CConfigAppearance::OnBnClickedDisplayFlats)
...
void CConfigAppearance::OnPaint() {
  ...
  for (int i = 0; i < iRows; ++i) {
    ...
    if ((i & 3) == 0) {
      if ((i & 6) == 0)
        GradientBar(&dc, Rect.left, OffsetTop, Rect.right - Rect.left, iRowSize,
                    Hilight2BgCol, BgCol);
      else
        GradientBar(&dc, Rect.left, OffsetTop, Rect.right - Rect.left, iRowSize,
                    HilightBgCol, BgCol);
      ...
      if (i == 0) {
        ...
        GradientBar(&dc, Rect.left + 5, OffsetTop, 40, iRowSize, CursorCol,
                    GetColor(COL_BACKGROUND));
        dc.Draw3dRect(Rect.left + 5, OffsetTop, 40, iRowSize, CursorCol,
                      CursorShadedCol);
      } else
...
BOOL CConfigAppearance::OnInitDialog() {
  ...
  if (pDC != NULL) {
    ...
    EnumFontFamiliesEx(pDC->m_hDC, &LogFont, (FONTENUMPROC)EnumFontFamExProc,
                       (LPARAM)this, 0);
    ReleaseDC(pDC);
  }

  CComboBox *pFontList = static_cast<CComboBox *>(GetDlgItem(IDC_FONT));
  CComboBox *pFontSizeList =
      static_cast<CComboBox *>(GetDlgItem(IDC_FONT_SIZE));
  ...
  m_iColors[COL_BACKGROUND_HILITE2] =
      pSettings->Appearance.iColBackgroundHilite2;
  m_iColors[COL_PATTERN_TEXT] = pSettings->Appearance.iColPatternText;
  m_iColors[COL_PATTERN_TEXT_HILITE] =
      pSettings->Appearance.iColPatternTextHilite;
  m_iColors[COL_PATTERN_TEXT_HILITE2] =
      pSettings->Appearance.iColPatternTextHilite2;
  m_iColors[COL_PATTERN_INSTRUMENT] =
      pSettings->Appearance.iColPatternInstrument;
  ...
  m_iColors[COL_CURRENT_ROW_NORMAL] =
      pSettings->Appearance.iColCurrentRowNormal; // // //
  m_iColors[COL_CURRENT_ROW_EDIT] = pSettings->Appearance.iColCurrentRowEdit;
  m_iColors[COL_CURRENT_ROW_PLAYING] =
      pSettings->Appearance.iColCurrentRowPlaying;
...
int CConfigAppearance::GetColor(int Index) const { return m_iColors[Index]; }
...
void CConfigAppearance::OnBnClickedButtonAppearanceSave() // // // 050B
{
  ...
  CFileDialog fileDialog{FALSE, NULL, _T("Theme.txt"),
...
void CConfigAppearance::ExportSettings(const char *Path) const // // // 050B
{
  if (auto file = std::fstream{Path, std::ios_base::out}) {
    ...
    for (size_t i = 0; i < std::size(m_iColors); ++i)
      file << COLOR_ITEMS[i] << SETTING_SEPARATOR << HEX_PREFIX
           << conv::from_uint_hex(m_iColors[i], 6) << std::endl;
    file << "Pattern colors" << SETTING_SEPARATOR << m_bPatternColors
         << std::endl;
...
void CConfigAppearance::ImportSettings(const char *Path) // // // 050B
{
  ...
  while (true) {
    ...
    if (Pos == std::string::npos)
      continue;
    auto sv = std::string_view(Line).substr(Pos + std::size(SETTING_SEPARATOR) -
                                            1); // // //
    ...
    for (size_t i = 0; i < sizeof(m_iColors) / sizeof(*m_iColors); ++i) {
      ...
      if (n == std::string_view::npos)
        continue;
      if (auto c =
              conv::to_uint32(sv.substr(n + std::size(HEX_PREFIX) - 1), 16))
        m_iColors[i] = *c;
    }
