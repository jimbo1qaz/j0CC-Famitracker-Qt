...
// TODO move general tracker state variables to the mainframe instead of the
// view, such as selected octave, instrument etc
...
public:
  ...
  void PlayerTick();
  bool PlayerGetNote(int Track, int Frame, int Channel, int Row,
                     stChanNote &NoteData);
...
private:
  ...
  bool EditInstrumentColumn(stChanNote &Note, int Value, bool &StepDown,
                            bool &MoveRight, bool &MoveLeft);
  bool EditVolumeColumn(stChanNote &Note, int Value, bool &bStepDown);
  bool EditEffNumberColumn(stChanNote &Note, unsigned char nChar,
                           int EffectIndex, bool &bStepDown);
  bool EditEffParamColumn(stChanNote &Note, int Value, int EffectIndex,
                          bool &bStepDown, bool &bMoveRight, bool &bMoveLeft);
  ...
  void SplitAdjustChannel(unsigned int &Channel,
                          const stChanNote &Note) const; // // //

  // MIDI note functions
  void TriggerMIDINote(unsigned int Channel, unsigned int MidiNote,
  ...
  // Note handling
  void PlayNote(unsigned int Channel, unsigned int Note, unsigned int Octave,
                unsigned int Velocity) const;
  void ReleaseNote(unsigned int Channel, unsigned int Note,
                   unsigned int Octave) const; // // //
  void HaltNote(unsigned int Channel, unsigned int Note,
                unsigned int Octave) const;        // // //
...
private:
  ...
  unsigned int
      m_iInsertKeyStepping;   // Number of rows to move when inserting notes
  ...
  std::unordered_map<unsigned char, int>
      m_iNoteCorrection;    // // // correction from changing octaves
