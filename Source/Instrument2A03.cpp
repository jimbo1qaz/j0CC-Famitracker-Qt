...
LPCTSTR CInstrument2A03::SEQUENCE_NAME[] = {_T("Volume"), _T("Arpeggio"),
                                            _T("Pitch"), _T("Hi-pitch"),
                                            _T("Duty / Noise")};

CInstrument2A03::CInstrument2A03()
    : CSeqInstrument(INST_2A03), // // //
      m_cSamples(), m_cSamplePitch(), m_cSampleLoopOffset() {
  ...
  const auto ReadAssignment = [&](int Octave, int Note) {
    try {
      int Index = CModuleException::AssertRangeFmt<MODULE_ERROR_STRICT>(
          pDocFile->GetBlockChar(), 0, MAX_DSAMPLES,
          "DPCM sample assignment index", "%i");
      if (Index > MAX_DSAMPLES)
        Index = 0;
      SetSampleIndex(Octave, Note, Index);
      char Pitch = pDocFile->GetBlockChar();
      CModuleException::AssertRangeFmt<MODULE_ERROR_STRICT>(
          Pitch & 0x7F, 0, 0xF, "DPCM sample pitch", "%i");
  ...
  if (Version >= 7) { // // // 050B
    ...
    for (int i = 0; i < Count; ++i) {
      int Note = CModuleException::AssertRangeFmt<MODULE_ERROR_STRICT>(
          pDocFile->GetBlockChar(), 0, NOTE_COUNT - 1,
          "DPCM sample assignment note index", "%i");
...
bool CInstrument2A03::LoadFile(CInstrumentFile *pFile, int iVersion) {
  ...
  CModuleException::AssertRangeFmt(Count, 0U, static_cast<unsigned>(NOTE_COUNT),
                                   "DPCM assignment count", "%u");
  ...
  for (unsigned int i = 0; i < Count; ++i) {
    ...
    try {
      unsigned char Sample = CModuleException::AssertRangeFmt(
          pFile->ReadChar(), 0U, 0x7FU, "DPCM sample assignment index", "%u");
      if (Sample > MAX_DSAMPLES)
        Sample = 0;
      unsigned char Pitch = pFile->ReadChar();
      CModuleException::AssertRangeFmt(Pitch & 0x7FU, 0U, 0xFU,
                                       "DPCM sample pitch", "%u");
      SetSamplePitch(Octave, Note, Pitch);
      SetSampleIndex(Octave, Note, Sample);
      SetSampleDeltaValue(
          Octave, Note,
          CModuleException::AssertRangeFmt(
              static_cast<char>(iVersion >= 24 ? pFile->ReadChar() : -1), -1,
              0x7F, "DPCM sample delta value", "%i"));
    } catch (CModuleException *e) {
      e->AppendError("At note %i, octave %i,", Note + 1, Octave);
      throw;
    }
  ...
  for (unsigned int i = 0; i < SampleCount; ++i) {
    int Index = CModuleException::AssertRangeFmt(
        pFile->ReadInt(), 0U, static_cast<unsigned>(MAX_DSAMPLES - 1),
        "DPCM sample index", "%u");
    int Len = CModuleException::AssertRangeFmt(
        pFile->ReadInt(), 0U,
        static_cast<unsigned>(CDSample::MAX_NAME_SIZE - 1),
        "DPCM sample name length", "%u");
    ...
    for (int j = 0; j < MAX_DSAMPLES; ++j)
      if (const CDSample *pSample = m_pInstManager->GetDSample(j)) { // // //
        // Compare size and name to see if identical sample exists
        if (pSample->GetSize() == Size &&
            !memcmp(pSample->GetData(), SampleData, Size) && // // //
    ...
    if (TotalSize + Size > MAX_SAMPLE_SPACE) {
      ...
      e->AppendError("Insufficient DPCM sample space (maximum %d KB)",
                     MAX_SAMPLE_SPACE / 1024);
      e->Raise();
    }
...
void CInstrument2A03::SetSampleLoop(int Octave, int Note, bool Loop) {
  m_cSamplePitch[Octave][Note] =
      (m_cSamplePitch[Octave][Note] & 0x7F) | (Loop ? 0x80 : 0);
  InstrumentChanged();
}
