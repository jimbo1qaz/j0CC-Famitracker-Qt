...
#define DEBUG_OUT(...)                                                         \
  {                                                                            \
    CString s__;                                                               \
    s__.Format(__VA_ARGS__);                                                   \
    OutputDebugString(s__);                                                    \
  }
...
enum {
  ...
  // macros
  CT_MACRO, // uint (type) uint (index) int (loop) int (release) int (setting) :
            // int_list
  CT_MACROVRC6, // uint (type) uint (index) int (loop) int (release) int
                // (setting) : int_list
  CT_MACRON163, // uint (type) uint (index) int (loop) int (release) int
                // (setting) : int_list
  CT_MACROS5B,  // uint (type) uint (index) int (loop) int (release) int
                // (setting) : int_list
  // dpcm samples
  ...
  CT_INSTVRC7, // uint (index) int (patch) hex hex hex hex hex hex hex hex
               // string (name)
  CT_INSTFDS,  // uint (index) int (mod enable) int (m speed) int (m depth) int
               // (m delay) string (name)
  CT_INSTN163, // uint (index) int int int int int uint (w size) uint (w pos)
               // uint (w count) string (name)
  CT_INSTS5B,  // uint (index) int int int int int  string (name)
  // instrument data
  CT_KEYDPCM,  // uint (inst) uint (oct) uint (note) uint (sample) uint (pitch)
               // uint (loop) uint (loop_point)
  CT_FDSWAVE,  // uint (inst) : uint_list x 64
  CT_FDSMOD,   // uint (inst) : uint_list x 32
  CT_FDSMACRO, // uint (inst) uint (type) int (loop) int (release) int (setting)
               // : int_list
...
public:
  ...
  int GetColumn() const { return 1 + pos - linestart; }
  ...
  CString ReadToken() {
    ...
    do {
      ...
      if (c == TCHAR('\"')) {
        if (!inQuote &&
            t.GetLength() == 0) // first quote begins a quoted string
        {
          inQuote = true;
        } else {
  ...
  bool ReadInt(int &i, int range_min, int range_max, CString *err) {
    ...
    if (t.GetLength() < 1) {
      if (err)
        err->Format(_T("Line %d column %d: expected integer, no token found."),
                    line, c);
      return false;
    }
    ...
    if (result == EOF || result == 0) {
      if (err)
        err->Format(_T("Line %d column %d: expected integer, '%s' found."),
                    line, c, (LPCTSTR)t);
      return false;
    }
    ...
    if (i < range_min || i > range_max) {
      if (err)
        err->Format(_T("Line %d column %d: expected integer in range [%d,%d], ")
                    _T("%d found."),
                    line, c, range_min, range_max, i);
      return false;
    }
  ...
  bool ReadHex(int &i, int range_min, int range_max, CString *err) {
    ...
    if (t.GetLength() < 1) {
      if (err)
        err->Format(
            _T("Line %d column %d: expected hexadecimal, no token found."),
            line, c);
      return false;
    }
    ...
    if (result == EOF || result == 0) {
      if (err)
        err->Format(_T("Line %d column %d: expected hexadecimal, '%s' found."),
                    line, c, (LPCTSTR)t);
      return false;
    }
    ...
    if (i < range_min || i > range_max) {
      if (err)
        err->Format(_T("Line %d column %d: expected hexidecmal in range ")
                    _T("[%X,%X], %X found."),
                    line, c, range_min, range_max, i);
      return false;
    }
  ...
  bool ReadEOL(CString *err) {
    ...
    if (s.GetLength() > 0) {
      if (err)
        err->Format(_T("Line %d column %d: expected end of line, '%s' found."),
                    line, c, (LPCTSTR)s);
      return false;
    }
    ...
    if (eol != TCHAR('\r') && eol != TCHAR('\n')) {
      if (err)
        err->Format(_T("Line %d column %d: expected end of line, '%c' found."),
                    line, c, eol);
      return false;
    }
...
bool CTextExport::ImportHex(CString &sToken, int &i, int line, int column,
                            CString &sResult) {
  i = 0;
  for (int d = 0; d < sToken.GetLength(); ++d) {
    ...
    int h = 0;
    for (h = 0; h < 16; ++h)
      if (0 == t.CompareNoCase(HEX_TEXT[h]))
        break;
    if (h >= 16) {
      sResult.Format(
          _T("Line %d column %d: hexadecimal number expected, '%s' found."),
          line, column, (LPCTSTR)sToken);
      return false;
    }
    i += h;
  }
...
bool CTextExport::ImportCellText( // // //
    CFamiTrackerDoc *pDoc, Tokenizer &t, unsigned int track,
    unsigned int pattern, unsigned int channel, unsigned int row,
    CString &sResult) {
  stChanNote Cell{}; // // //
  ...
  if (sNote == _T("...")) {
    ...
    if (sNote.GetLength() != 3) {
      sResult.Format(_T("Line %d column %d: note column should be 3 ")
                     _T("characters wide, '%s' found."),
                     t.line, t.GetColumn(), (LPCTSTR)sNote);
      return false;
    }
    ...
    if (channel == 3) // noise
    {
    ...
    } else if (sNote.GetAt(0) == TCHAR('^') &&
               sNote.GetAt(1) == TCHAR('-')) { // // //
      int o = sNote.GetAt(2) - TCHAR('0');
      if (o < 0 || o > ECHO_BUFFER_LENGTH) {
        sResult.Format(
            _T("Line %d column %d: out-of-bound echo buffer accessed."), t.line,
            t.GetColumn());
        return false;
      }
      Cell.Note = ECHO;
      ...
      case TCHAR('B'):
        ...
        break;
      default:
        sResult.Format(_T("Line %d column %d: unrecognized note '%s'."), t.line,
                       t.GetColumn(), (LPCTSTR)sNote);
        return false;
      }
      ...
      if (o < 0 || o >= OCTAVE_RANGE) {
        sResult.Format(_T("Line %d column %d: unrecognized octave '%s'."),
                       t.line, t.GetColumn(), (LPCTSTR)sNote);
        return false;
      }
      Cell.Octave = o;
  ...
  if (sInst == _T("..")) {
    ...
    if (sInst.GetLength() != 2) {
      sResult.Format(_T("Line %d column %d: instrument column should be 2 ")
                     _T("characters wide, '%s' found."),
                     t.line, t.GetColumn(), (LPCTSTR)sInst);
      return false;
    }
    int h;
    ...
    if (h >= MAX_INSTRUMENTS) {
      sResult.Format(_T("Line %d column %d: instrument '%s' is out of bounds."),
                     t.line, t.GetColumn(), (LPCTSTR)sInst);
      return false;
    }
    Cell.Instrument = h;
  ...
  const TCHAR *VOL_TEXT[17] = {_T("0"), _T("1"), _T("2"), _T("3"), _T("4"),
                               _T("5"), _T("6"), _T("7"), _T("8"), _T("9"),
                               _T("A"), _T("B"), _T("C"), _T("D"), _T("E"),
                               _T("F"), _T(".")};
  int v = 0;
  for (; v <= 17; ++v)
    if (0 == sVol.CompareNoCase(VOL_TEXT[v]))
      break;
  if (v > 17) {
    sResult.Format(_T("Line %d column %d: unrecognized volume token '%s'."),
                   t.line, t.GetColumn(), (LPCTSTR)sVol);
    return false;
  }
  Cell.Vol = v;
  ...
  for (unsigned int e = 0; e <= pDoc->GetEffColumns(track, channel); ++e) {
    ...
    if (sEff != _T("...")) {
      if (sEff.GetLength() != 3) {
        sResult.Format(_T("Line %d column %d: effect column should be 3 ")
                       _T("characters wide, '%s' found."),
                       t.line, t.GetColumn(), (LPCTSTR)sEff);
        return false;
      }

      TCHAR pC = sEff.GetAt(0);
      if (pC >= TCHAR('a') && pC <= TCHAR('z'))
        pC += TCHAR('A') - TCHAR('a');

      bool Valid; // // //
      effect_t Eff = GetEffectFromChar(pC, pDoc->GetChipType(channel), &Valid);
      if (!Valid) {
        sResult.Format(_T("Line %d column %d: unrecognized effect '%s'."),
                       t.line, t.GetColumn(), (LPCTSTR)sEff);
        return false;
      }
...
const CString &CTextExport::ExportCellText(const stChanNote &stCell,
                                           ...
                                           bool bNoise) // // //
{
  ...
  static const char *TEXT_NOTE[ECHO + 1] = {
      // // //
      _T("..."), _T("C-?"), _T("C#?"), _T("D-?"), _T("D#?"), _T("E-?"),
      _T("F-?"), _T("F#?"), _T("G-?"), _T("G#?"), _T("A-?"), _T("A#?"),
      _T("B-?"), _T("==="), _T("---"), _T("^-?")};

  s = (stCell.Note <= ECHO) ? TEXT_NOTE[stCell.Note] : "..."; // // //
  if (stCell.Note >= NOTE_C && stCell.Note <= NOTE_B ||
      stCell.Note == ECHO) // // //
  {
    if (bNoise) {
  ...
  s += (stCell.Instrument == MAX_INSTRUMENTS)
           ? _T(" ..")
           : (stCell.Instrument == HOLD_INSTRUMENT) ? _T(" &&")
                                                    : tmp; // // // 050B
  ...
  for (unsigned int e = 0; e < nEffects; ++e) {
    if (stCell.EffNumber[e] == 0) {
      ...
      tmp.Format(_T(" %c%02X"), EFF_CHAR[stCell.EffNumber[e] - 1],
                 stCell.EffParam[e]);
      s += tmp;
    }
  }
...
#define CHECK_SYMBOL(x)                                                        \
  {                                                                            \
    ...
    if (symbol_ != _T(x)) {                                                    \
      sResult.Format(_T("Line %d column %d: expected '%s', '%s' found."),      \
                     t.line, t.GetColumn(), _T(x), (LPCTSTR)symbol_);          \
      return sResult;                                                          \
    }                                                                          \
  }
...
const CString &CTextExport::ImportFile(LPCTSTR FileName,
  ...
  while (!t.Finished()) {
    ...
    int c = 0;
    for (; c < CT_COUNT; ++c)
      if (0 == command.CompareNoCase(CT[c]))
        break;
    ...
    case CT_MACRON163:
    case CT_MACROS5B: {
      static const inst_type_t CHIP_MACRO[4] = {INST_2A03, INST_VRC6, INST_N163,
                                                INST_S5B}; // // //
      int chip = c - CT_MACRO;
      ...
      while (!t.IsEOL()) {
        ...
        if (count >= MAX_SEQUENCE_ITEMS) {
          sResult.Format(_T("Line %d column %d: macro overflow, max size: %d."),
                         t.line, t.GetColumn(), MAX_SEQUENCE_ITEMS);
          return sResult;
        }
    ...
    case CT_DPCM: {
      ...
      while (!t.IsEOL()) {
        ...
        if (dpcm_pos >= dpcm_sample->GetSize()) {
          sResult.Format(_T("Line %d column %d: DPCM sample %d overflow, ")
                         _T("increase size used in %s."),
                         t.line, t.GetColumn(), dpcm_index, CT[CT_DPCMDEF]);
          return sResult;
        }
    ...
    case CT_INSTS5B: {
      ...
      auto seqInst = dynamic_cast<CSeqInstrument *>(
          CInstrumentFactory::CreateNew(Type)); // // //
    ...
    case CT_KEYDPCM: {
      ...
      if (pDoc->GetInstrumentType(i) != INST_2A03) {
        sResult.Format(_T("Line %d column %d: instrument %d is not defined as ")
                       _T("a 2A03 instrument."),
                       t.line, t.GetColumn(), i);
        return sResult;
      }
      auto pInst =
          std::static_pointer_cast<CInstrument2A03>(pDoc->GetInstrument(i));
    ...
    case CT_FDSWAVE: {
      ...
      if (pDoc->GetInstrumentType(i) != INST_FDS) {
        sResult.Format(_T("Line %d column %d: instrument %d is not defined as ")
                       _T("an FDS instrument."),
                       t.line, t.GetColumn(), i);
        return sResult;
      }
      auto pInst =
          std::static_pointer_cast<CInstrumentFDS>(pDoc->GetInstrument(i));
      CHECK_COLON();
    ...
    case CT_N163WAVE: {
      ...
      if (pDoc->GetInstrumentType(i) != INST_N163) {
        sResult.Format(_T("Line %d column %d: instrument %d is not defined as ")
                       _T("an N163 instrument."),
                       t.line, t.GetColumn(), i);
        return sResult;
      }
      auto pInst =
          std::static_pointer_cast<CInstrumentN163>(pDoc->GetInstrument(i));
    ...
    case CT_TRACK: {
      if (track != 0) {
        if (pDoc->AddTrack() == -1) {
          sResult.Format(_T("Line %d column %d: unable to add new track."),
                         t.line, t.GetColumn());
          return sResult;
        }
      }
    ...
    case CT_ROW: {
      if (track == 0) {
        sResult.Format(
            _T("Line %d column %d: no TRACK defined, cannot add ROW data."),
            t.line, t.GetColumn());
        return sResult;
      }
    ...
    case CT_COUNT:
    default:
      sResult.Format(_T("Unrecognized command at line %d: '%s'."), t.line,
                     (LPCTSTR)command);
      return sResult;
    }
  }
  ...
  if (N163count != -1) { // // //
    ...
    pDoc->SelectExpansionChip(
        pDoc->GetExpansionChip()); // calls ApplyExpansionChip()
  }
  return sResult;
}
...
const CString &CTextExport::ExportRows(LPCTSTR FileName,
                                       CFamiTrackerDoc *pDoc) {
  static CString sResult;
  sResult = _T("");
  ...
  f.WriteString(_T("ID,TRACK,CHANNEL,PATTERN,ROW,NOTE,OCTAVE,INST,VOLUME,FX1,")
                _T("FX1PARAM,FX2,FX2PARAM,FX3,FX3PARAM,FX4,FX4PARAM\n"));
    ...
    for (int c = 0; c < pDoc->GetChannelCount(); c++)
      for (int p = 0; p < MAX_PATTERN; p++)
        for (unsigned int r = 0; r < pDoc->GetPatternLength(t); r++) {
          ...
          bool isEmpty = true;
          if (stCell.Note != NONE || stCell.Instrument != MAX_INSTRUMENTS ||
              stCell.Vol != MAX_VOLUME)
            isEmpty = false;
          for (int fx = 0; fx < MAX_EFFECT_COLUMNS; fx++)
            if (stCell.EffNumber[fx] != EF_NONE)
              isEmpty = false;
          if (isEmpty)
            continue;
          l.Format(s, id++, t, c, p, r, stCell.Note, stCell.Octave,
                   stCell.Instrument, stCell.Vol, stCell.EffNumber[0],
                   stCell.EffParam[0], stCell.EffNumber[1], stCell.EffParam[1],
                   stCell.EffNumber[2], stCell.EffParam[2], stCell.EffNumber[3],
                   stCell.EffParam[3]);
          f.WriteString(l);
        }
  return sResult;
...
const CString &CTextExport::ExportFile(LPCTSTR FileName,
                                       CFamiTrackerDoc *pDoc) {
  static CString sResult;
  sResult = _T("");
  ...
  s.Format(_T("# Global settings\n"
           ...
           CT[CT_MACHINE], pDoc->GetMachine(), CT[CT_FRAMERATE],
           pDoc->GetEngineSpeed(), CT[CT_EXPANSION], pDoc->GetExpansionChip(),
           CT[CT_VIBRATO], pDoc->GetVibratoStyle(), CT[CT_SPLIT],
           pDoc->GetSpeedSplitPoint()
           //				,CT[CT_PLAYBACKRATE], pDoc->, pDoc->
  );
  if (pDoc->GetTuningSemitone() || pDoc->GetTuningCent()) // // // 050B
    s.AppendFormat(_T("%-15s %d %d\n"), CT[CT_TUNING],
                   pDoc->GetTuningSemitone(), pDoc->GetTuningCent());
  f.WriteString(s);
  f.WriteString(_T("\n"));
  ...
  for (int c = 0; c < 4; ++c) {
    const inst_type_t CHIP_MACRO[4] = {INST_2A03, INST_VRC6, INST_N163,
                                       INST_S5B};
    ...
    for (int st = 0; st < SEQ_COUNT; ++st)
      for (int seq = 0; seq < MAX_SEQUENCES; ++seq) {
        ...
        if (pSequence && pSequence->GetItemCount() > 0) {
          s.Format(_T("%-9s %3d %3d %3d %3d %3d :"), CT[CT_MACRO + c], st, seq,
                   pSequence->GetLoopPoint(), pSequence->GetReleasePoint(),
    ...
    if (const CDSample *pSample = pDoc->GetSample(smp)) // // //
    {
      ...
      for (unsigned int i = 0; i < size; i += 32) {
        ...
        for (unsigned int j = 0; j < 32 && (i + j) < size; ++j) {
          s.Format(_T(" %02X"),
                   (unsigned char)(*(pSample->GetData() + (i + j))));
          f.WriteString(s);
        }
        f.WriteString(_T("\n"));
      }
  ...
  for (unsigned int i = 0; i < MAX_INSTRUMENTS; ++i) {
    ...
    case INST_2A03:
      CTstr = CT[CT_INST2A03];
      break;
    case INST_VRC6:
      CTstr = CT[CT_INSTVRC6];
      break;
    case INST_VRC7:
      CTstr = CT[CT_INSTVRC7];
      break;
    case INST_FDS:
      CTstr = CT[CT_INSTFDS];
      break;
    case INST_N163:
      CTstr = CT[CT_INSTN163];
      break;
    case INST_S5B:
      CTstr = CT[CT_INSTS5B];
      break;
    ...
    if (seqInst && pInst->GetType() != INST_FDS) {
      ...
      for (int j = 0; j < SEQ_COUNT; j++)
        s.AppendFormat(_T("%3d "),
                       seqInst->GetSeqEnable(j) ? seqInst->GetSeqIndex(j) : -1);
      f.WriteString(s);
    }
    ...
    case INST_N163: {
      ...
      s.Format(_T("%3d %3d %3d "), pDI->GetWaveSize(), pDI->GetWavePos(),
               pDI->GetWaveCount());
      f.WriteString(s);
    } break;
    case INST_VRC7: {
    ...
    case INST_FDS: {
      ...
      s.Format(_T("%3d %3d %3d %3d "), pDI->GetModulationEnable(),
               pDI->GetModulationSpeed(), pDI->GetModulationDepth(),
    ...
    case INST_N163: {
      ...
      for (int w = 0; w < pDI->GetWaveCount(); ++w) {
        s.Format(_T("%s %3d %3d :"), CT[CT_N163WAVE], i, w);
        f.WriteString(s);
    ...
    case INST_FDS: {
      ...
      for (int seq = 0; seq < 3; ++seq) {
        ...
        s.Format(_T("%-8s %3d %3d %3d %3d %3d :"), CT[CT_FDSMACRO], i, seq,
                 pSequence->GetLoopPoint(), pSequence->GetReleasePoint(),
                 pSequence->GetSetting());
        f.WriteString(s);
        for (unsigned int i = 0; i < pSequence->GetItemCount(); ++i) {
  ...
  for (unsigned int t = 0; t < pDoc->GetTrackCount(); ++t) {
    ...
    s.Format(_T("%s %3d %3d %3d %s\n"), CT[CT_TRACK], pDoc->GetPatternLength(t),
             pDoc->GetSongSpeed(t), pDoc->GetSongTempo(t),
    ...
    for (int p = 0; p < MAX_PATTERN; ++p) {
      ...
      for (unsigned int r = 0; r < pDoc->GetPatternLength(t); ++r) {
        ...
        for (int c = 0; c < pDoc->GetChannelCount(); ++c) {
          ...
          pDoc->GetDataAtPattern(t, p, c, r, &stCell);
          f.WriteString(
              ExportCellText(stCell, pDoc->GetEffColumns(t, c) + 1, c == 3));
        }
        f.WriteString(_T("\n"));
      }
      f.WriteString(_T("\n"));
