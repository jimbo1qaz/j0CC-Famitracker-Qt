...
void CVisualizerStatic::Draw() {
  ...
  const COLORREF Color =
      FixRGB(theApp.GetSettings()->Appearance.iColPatternText);
  ...
  for (size_t i = 0; i < COUNT; ++i) {
    ...
    DrawChar(STR[i], static_cast<int>(x) + 1,
             static_cast<int>(m_iHeight / 2. - 3.5 - y) + 1, Shadow);
    DrawChar(STR[i], static_cast<int>(x),
             static_cast<int>(m_iHeight / 2. - 3.5 - y), Color);
  }

  ++t;
}

void CVisualizerStatic::DrawChar(char n, int xPos, int yPos,
                                 const COLORREF &Color) // // //
{
  ...
  switch (n) {
  case '0':
    return drawFunc(LOGO_FONT[0]);
  case 'C':
  case 'c':
    return drawFunc(LOGO_FONT[1]);
  case '-':
    return drawFunc(LOGO_FONT[2]);
  case 'F':
  case 'f':
    return drawFunc(LOGO_FONT[3]);
  case 'A':
  case 'a':
    return drawFunc(LOGO_FONT[4]);
  case 'M':
  case 'm':
    return drawFunc(LOGO_FONT[5]);
  case 'I':
  case 'i':
    return drawFunc(LOGO_FONT[6]);
  case 'T':
  case 't':
    return drawFunc(LOGO_FONT[7]);
  case 'R':
  case 'r':
    return drawFunc(LOGO_FONT[8]);
  case 'K':
  case 'k':
    return drawFunc(LOGO_FONT[9]);
  case 'E':
  case 'e':
    return drawFunc(LOGO_FONT[10]);
  }
}
