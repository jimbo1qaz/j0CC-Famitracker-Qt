...
/*
LPCTSTR CInstrumentEditorSeq::INST_SETTINGS[] = {_T(""), _T(""), _T(""), _T(""),
_T("")}; const int CInstrumentEditorSeq::MAX_VOLUME = 0; const int
CInstrumentEditorSeq::MAX_DUTY = 0; const inst_type_t
CInstrumentEditorSeq::INST_TYPE = INST_NONE;
*/

IMPLEMENT_DYNAMIC(CInstrumentEditorSeq, CSequenceInstrumentEditPanel)
CInstrumentEditorSeq::CInstrumentEditorSeq(CWnd *pParent, TCHAR *Title,
                                           LPCTSTR *SeqName, int Vol, int Duty,
                                           inst_type_t Type)
    : CSequenceInstrumentEditPanel(CInstrumentEditorSeq::IDD, pParent),
      m_pTitle(Title), m_pSequenceName(SeqName), m_iMaxVolume(Vol),
      m_iMaxDuty(Duty), m_iInstType(Type) {}

void CInstrumentEditorSeq::SelectInstrument(
  ...
  if (CListCtrl *pList =
          static_cast<CListCtrl *>(GetDlgItem(IDC_INSTSETTINGS))) { // // //
  ...
  // Setting text box
  SetDlgItemInt(IDC_SEQ_INDEX,
                m_pInstrument->GetSeqIndex(m_iSelectedSetting = Sel));

  SelectSequence(m_pInstrument->GetSeqIndex(m_iSelectedSetting),
                 m_iSelectedSetting);
...
void CInstrumentEditorSeq::SelectSequence(int Sequence, int Type) {
  ...
  if (CListCtrl *pList = static_cast<CListCtrl *>(GetDlgItem(IDC_INSTSETTINGS)))
    pList->SetItemState(Type, LVIS_SELECTED | LVIS_FOCUSED,
...
void CInstrumentEditorSeq::SetupParser() const // // //
{
  ...
  case SEQ_VOLUME:
    Max =
        m_pSequence->GetSetting() == SETTING_VOL_64_STEPS ? 0x3F : m_iMaxVolume;
    Min = 0;
    break;
  case SEQ_ARPEGGIO:
    switch (m_pSequence->GetSetting()) {
    case SETTING_ARP_SCHEME: // // //
      pConv = new CSeqConversionArpScheme{ARPSCHEME_MIN};
      break;
    case SETTING_ARP_FIXED:
      pConv = new CSeqConversionArpFixed{};
      break; // // //
    default:
      Max = NOTE_COUNT;
      Min = -NOTE_COUNT;
...
void CInstrumentEditorSeq::UpdateSequenceString(bool Changed) // // //
{
  ...
  SetupParser(); // // //
  SetDlgItemText(IDC_SEQUENCE_STRING,
                 m_pParser->PrintSequence().c_str()); // // //
  // If the sequence was changed, assume the user wants to enable it
  if (Changed) {
    static_cast<CListCtrl *>(GetDlgItem(IDC_INSTSETTINGS))
        ->SetCheck(m_iSelectedSetting, 1);
  }
}
...
void CInstrumentEditorSeq::OnLvnItemchangedInstsettings(NMHDR *pNMHDR,
                                                        LRESULT *pResult) {
  ...
  if (pNMLV->uChanged & LVIF_STATE && m_pInstrument != NULL) {
    ...
    if (pNMLV->uNewState & LVIS_SELECTED ||
        pNMLV->uNewState & LCTRL_CHECKBOX_STATE) {
      ...
      pList->SetItemState(m_iSelectedSetting, LVIS_SELECTED | LVIS_FOCUSED,
                          LVIS_SELECTED | LVIS_FOCUSED);
    }
...
void CInstrumentEditorSeq::OnBnClickedFreeSeq() {
  int FreeIndex = m_pInstManager->AddSequence(
      m_iInstType, m_iSelectedSetting, nullptr, m_pInstrument.get()); // // //
  if (FreeIndex == -1)
    FreeIndex = 0;
  SetDlgItemInt(IDC_SEQ_INDEX, FreeIndex,
                FALSE); // Things will update automatically by changing this
}
...
void CInstrumentEditorSeq::OnCloneSequence() {
  ...
  int FreeIndex = m_pInstManager->AddSequence(m_iInstType, m_iSelectedSetting,
                                              pSeq, m_pInstrument.get());
  if (FreeIndex != -1)
