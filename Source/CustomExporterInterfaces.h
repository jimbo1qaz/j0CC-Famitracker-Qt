...
/*
** This class is a pure virtual interface to CSequence, which can be used by
*custom exporters
*/
...
//
// Pure virtual interface to famitracker document, to make it simple for custom
// exporter DLLs to interact with the document. Also, all the getters are const
// so that custom exporters cannot screw anything up =)
//

class CFamiTrackerDocInterface {
public:
  virtual void GetNoteData(unsigned int Frame, unsigned int Channel,
                           unsigned int Row, stChanNote *Data) const = 0;
  virtual unsigned int GetFrameCount() const = 0;
  virtual unsigned int GetPatternLength() const = 0;
  virtual unsigned int GetSongSpeed() const = 0;
  virtual CSequenceInterface const *GetSequence(unsigned int Index,
                                                int Type) const = 0;
  virtual int GetSequenceCount(int Type) const = 0;
  virtual int GetInstrumentCount() const = 0;
  virtual CInstrument2A03Interface const *
  Get2A03Instrument(int Instrument) const = 0;
  virtual unsigned int GetNoteEffectType(unsigned int Frame,
                                         unsigned int Channel, unsigned int Row,
                                         int Index) const = 0;
  virtual unsigned int GetNoteEffectParam(unsigned int Frame,
  ...
  virtual char GetSampleData(unsigned int Sample,
                             unsigned int Offset) const = 0;

  virtual CSeqInstrumentInterface const *
  GetSeqInstrument(int Instrument) const = 0; // // //
};

//
// Struct containing a set of C function pointers pointing to functions which
// forward calls to the currently loaded Famitracker document. This was added to
// allow exporters to be written with any win32 capable compiler. The functions
// which retrieve sequences and instruments now retrieve a void pointer. This is
// so exporters don't try to make any calls that might use the object's vtable.
// Instead, several function pointers are provided which expect these handles to
// be passed in in order to forward appropriate calls to c++ objects inside
// Famitracker.
//
struct FamitrackerDocInterface {
  // overall document functions
  void(__cdecl *GetNoteData)(unsigned int Frame, unsigned int Channel,
                             unsigned int Row, stChanNote *Data);
  ...
  // effect functions
  unsigned int(__cdecl *GetNoteEffectType)(unsigned int Frame,
                                           unsigned int Channel,
                                           unsigned int Row, int Index);
  unsigned int(__cdecl *GetNoteEffectParam)(unsigned int Frame,
                                            unsigned int Channel,
                                            unsigned int Row, int Index);
  ...
  char(__cdecl *GetSample)(Instrument2A03Handle instrument, int Octave,
                           int Note);
  char(__cdecl *GetSamplePitch)(Instrument2A03Handle instrument, int Octave,
                                int Note);
  char(__cdecl *GetSampleLoopOffset)(Instrument2A03Handle instrument,
                                     int Octave, int Note);
