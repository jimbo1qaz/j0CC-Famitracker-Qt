...
CChannelHandlerN163::CChannelHandlerN163()
    : CChannelHandlerInverted(0xFFFF, 0x0F), m_bDisableLoad(false), // // //
      m_bResetPhase(false), m_iWaveLen(4),                          // // //
      m_iWaveCount(0) {
  m_iDutyPeriod = 0;
}
...
bool CChannelHandlerN163::HandleEffect(effect_t EffNum,
  ...
  case EF_DUTY_CYCLE:
    ...
    m_bLoadWave = true;
    if (auto pHandler =
            dynamic_cast<CSeqInstHandlerN163 *>(m_pInstHandler.get()))
...
bool CChannelHandlerN163::HandleInstrument(bool Trigger, bool NewInstrument) {
  ...
  if (!m_bDisableLoad) {
    m_iWavePos =
        /*pInstrument->GetAutoWavePos() ? GetIndex() * 16 :*/ m_iWavePosOld;
  }
...
void CChannelHandlerN163::RefreshChannel() {
  ...
  if (Channel + m_iChannels >= 8) { // // //
    ...
    WriteData(ChannelAddrBase + 4,
              (WaveSize << 2) | ((Frequency >> 16) & 0x03));
...
{
  ASSERT(Length >= 4 && Length <= CInstrumentN163::MAX_WAVE_SIZE &&
         !(Length % 4));
  m_iWaveLen = Length;
}
...
CString CChannelHandlerN163::GetSlideEffectString() const // // //
{
  ...
  case EF_PORTA_DOWN:
    if (m_iPortaSpeed)
      str.AppendFormat(_T(" %c%02X"), EFF_CHAR[EF_PORTA_UP - 1],
                       m_iPortaSpeed >> N163_PITCH_SLIDE_SHIFT);
    break;
  case EF_PORTAMENTO:
    if (m_iPortaSpeed)
      str.AppendFormat(_T(" %c%02X"), EFF_CHAR[m_iEffect - 1],
                       m_iPortaSpeed >> N163_PITCH_SLIDE_SHIFT);
    break;
  }
...
void CChannelHandlerN163::WriteData(char Data) { WriteRegister(0x4800, Data); }
