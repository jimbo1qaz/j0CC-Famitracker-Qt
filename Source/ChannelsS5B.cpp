...
CChannelHandlerS5B::CChannelHandlerS5B()
    : CChannelHandler(0xFFF, 0x0F), m_bEnvelopeEnabled(false), // // // 050B
...
bool CChannelHandlerS5B::HandleEffect(effect_t EffNum, unsigned char EffParam) {
  ...
  case EF_SUNSOFT_ENV_TYPE: // H
    ...
    break;
  case EF_DUTY_CYCLE:
    m_iDefaultDuty = m_iDutyPeriod =
        (EffParam << 6) | ((EffParam & 0x04) << 3); // // // 050B
  ...
  case INST_FDS:
    ...
    default:
      m_pInstHandler.reset(
          new CSeqInstHandlerS5B(this, 0x0F, Type == INST_S5B ? 0x40 : 0));
      return true;
    }
  }
  return false;
...
{
  return LimitVolume((m_iVolume >> VOL_COLUMN_SHIFT) + m_iInstVolume - 15 -
                     GetTremolo());
}
...
void CChannelHandlerS5B::RefreshChannel() {
  ...
  unsigned char Envelope = (m_bGate && (m_iDutyPeriod & S5B_MODE_ENVELOPE))
                               ? 0x10
                               : 0; // m_bEnvelopeEnabled ? 0x10 : 0;
