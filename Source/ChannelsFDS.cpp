...
CChannelHandlerFDS::CChannelHandlerFDS() : CChannelHandlerInverted(0xFFF, 32) {
...
void CChannelHandlerFDS::HandleCut() { CutNote(); }
...
int CChannelHandlerFDS::CalculateVolume() const // // //
{
  if (!theApp.GetSettings()->General.bFDSOldVolume) // // // match NSF setting
    return LimitVolume(
        ((m_iInstVolume + 1) * ((m_iVolume >> VOL_COLUMN_SHIFT) + 1) - 1) / 16 -
        GetTremolo());
...
void CChannelHandlerFDS::RefreshChannel() {
  ...
  if (m_bAutoModulation) { // // //
    int newFreq =
        Frequency * m_iEffModSpeedHi / m_iEffModSpeedLo + m_iModulationOffset;
    if (newFreq < 0)
      newFreq = 0;
    if (newFreq > 0xFFF)
      newFreq = 0xFFF;
...
CString CChannelHandlerFDS::GetCustomEffectString() const // // //
{
  ...
  if (m_bAutoModulation) {
    str.AppendFormat(_T(" I%X%X"),
                     m_iEffModSpeedHi > 0xF ? 1 : m_iEffModSpeedHi,
                     m_iEffModSpeedLo - 1);
