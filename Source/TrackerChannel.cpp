...
/*
 * This class serves as the interface between the UI and the sound player for
 * each channel Thread synchronization should be done here
 *
 */

CTrackerChannel::CTrackerChannel(LPCTSTR pName, LPCTSTR pShort, const int iChip,
                                 chan_id_t iID)
    :                       // // //
      m_pShortName(pShort), // // //
      m_pChannelName(pName), m_iChip(iChip), m_iChannelID(iID),
      m_iColumnCount(0), m_bNewNote(false), m_iPitch(0),
      m_iNotePriority(NOTE_PRIO_0), m_iVolumeMeter(0) {}

CTrackerChannel::~CTrackerChannel(void) {}

LPCTSTR CTrackerChannel::GetChannelName() const { return m_pChannelName; }

LPCTSTR CTrackerChannel::GetShortName() const { return m_pShortName; }

const char CTrackerChannel::GetChip() const { return m_iChip; }
...
const int CTrackerChannel::GetColumnCount() const { return m_iColumnCount; }

void CTrackerChannel::SetColumnCount(int Count) { m_iColumnCount = Count; }
...
bool CTrackerChannel::NewNoteData() const { return m_bNewNote; }
...
void CTrackerChannel::SetVolumeMeter(int Value) { m_iVolumeMeter = Value; }

int CTrackerChannel::GetVolumeMeter() const { return m_iVolumeMeter; }

void CTrackerChannel::SetPitch(int Pitch) { m_iPitch = Pitch; }

int CTrackerChannel::GetPitch() const { return m_iPitch; }

bool CTrackerChannel::IsInstrumentCompatible(int Instrument,
                                             inst_type_t Type) const {
  switch (m_iChip) {
  case SNDCHIP_NONE:
  case SNDCHIP_MMC5:
  case SNDCHIP_N163: // // //
...
bool CTrackerChannel::IsEffectCompatible(int EffNumber,
                                         int EffParam) const // // //
{
  switch (EffNumber) {
  case EF_NONE:
  ...
  case EF_VOLUME:
    return ((m_iChip == SNDCHIP_NONE && m_iChannelID != CHANID_DPCM) ||
