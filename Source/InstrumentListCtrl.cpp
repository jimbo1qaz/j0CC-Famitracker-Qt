...
//   &CInstrumentList::OnLvnBeginlabeledit) ON_NOTIFY_REFLECT(NM_CLICK,
//   &CInstrumentList::OnNMClick) ON_NOTIFY_REFLECT(LVN_KEYDOWN,
//   &CInstrumentList::OnLvnKeydown) ON_NOTIFY_REFLECT(LVN_ENDLABELEDIT,
//   &CInstrumentList::OnLvnEndlabeledit) ON_NOTIFY_REFLECT(LVN_ITEMCHANGED,
//   &CInstrumentList::OnLvnItemchanged) ON_NOTIFY_REFLECT(NM_DBLCLK,
//   &CInstrumentList::OnNMDblclk) ON_NOTIFY_REFLECT(LVN_BEGINDRAG,
//   &CInstrumentList::OnLvnBegindrag)
...
CInstrumentList::CInstrumentList(CMainFrame *pMainFrame)
    : m_pMainFrame(pMainFrame), m_pDragImage(NULL), m_nDragIndex(-1),
      m_nDropIndex(-1), m_bDragging(false) {}
...
void CInstrumentList::SelectInstrument(int Index) {
  ...
  SetItemState(ListIndex, LVIS_SELECTED | LVIS_FOCUSED,
...
void CInstrumentList::OnLvnBegindrag(NMHDR *pNMHDR, LRESULT *pResult) {
  ...
  //    m_pDragImage = CreateDragImage(m_nDragIndex, &pt);	// Delete this
  //    later ASSERT(m_pDragImage);
...
void CInstrumentList::OnMouseMove(UINT nFlags, CPoint point) {
  ...
  //		if (m_nDropIndex != -1) {
  //			SetItemState(m_nDropIndex, LVIS_DROPHILITED,
  // LVIS_DROPHILITED); 			RedrawItems(m_nDropIndex,
  // m_nDropIndex); UpdateWindow();
  //		}
