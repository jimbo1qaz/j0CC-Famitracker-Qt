...
typedef void (*SDL_Callback_Function)(void *userdata, uint8_t *stream,
                                      int32_t len);
...
public:
  ...
  CDSoundChannel *OpenChannel(int SampleRate, int SampleSize, int Channels,
                              int BufferLength, int Blocks);
  void CloseChannel(CDSoundChannel *pChannel);

  int CalculateBufferLength(int BufferLen, int Samplerate, int Samplesize,
                            int Channels) const;

  // Enumeration
  void EnumerateDevices();
  void ClearEnumeration();
  BOOL EnumerateCallback(LPGUID lpGuid, LPCTSTR lpcstrDescription,
...
public:
  ...
  // protected:
  //	static BOOL CALLBACK DSEnumCallback(LPGUID lpGuid, LPCTSTR
