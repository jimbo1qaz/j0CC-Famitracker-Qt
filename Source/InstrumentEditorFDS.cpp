...
CInstrumentEditorFDS::CInstrumentEditorFDS(CWnd *pParent)
    : CInstrumentEditPanel(CInstrumentEditorFDS::IDD, pParent),
      m_pWaveEditor(NULL), m_pModSequenceEditor(NULL) {}
...
void CInstrumentEditorFDS::SelectInstrument(
  ...
  static_cast<CSpinButtonCtrl *>(GetDlgItem(IDC_MOD_RATE_SPIN))
      ->SetPos(m_pInstrument->GetModulationSpeed());
  static_cast<CSpinButtonCtrl *>(GetDlgItem(IDC_MOD_DEPTH_SPIN))
      ->SetPos(m_pInstrument->GetModulationDepth());
  static_cast<CSpinButtonCtrl *>(GetDlgItem(IDC_MOD_DELAY_SPIN))
...
//	ON_BN_CLICKED(IDC_ENABLE_FM, OnBnClickedEnableFm)
...
BOOL CInstrumentEditorFDS::OnInitDialog() {
  ...
  m_pModSequenceEditor->CreateEx(WS_EX_CLIENTEDGE, NULL, _T(""),
                                 WS_CHILD | WS_VISIBLE,
                                 DPI::Rect(10, 200, 0, 0), this); // // //
...
void CInstrumentEditorFDS::OnVScroll(UINT nSBCode, UINT nPos,
