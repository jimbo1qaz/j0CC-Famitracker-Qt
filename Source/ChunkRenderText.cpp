...
const char CChunkRenderText::LABEL_GROOVE[] =
    "ft_groove_%i"; // // // one argument
...
const char CChunkRenderText::LABEL_SEQ_2A03[] =
    "ft_seq_2a03_%i"; // one argument
const char CChunkRenderText::LABEL_SEQ_VRC6[] =
    "ft_seq_vrc6_%i";                                           // one argument
const char CChunkRenderText::LABEL_SEQ_FDS[] = "ft_seq_fds_%i"; // one argument
const char CChunkRenderText::LABEL_SEQ_N163[] =
    "ft_seq_n163_%i"; // one argument
const char CChunkRenderText::LABEL_SEQ_S5B[] =
    "ft_seq_s5b_%i"; // // // one argument
...
const char CChunkRenderText::LABEL_SONG_FRAMES[] =
    "ft_s%i_frames";                                           // one argument
const char CChunkRenderText::LABEL_SONG_FRAME[] = "ft_s%if%i"; // two arguments
const char CChunkRenderText::LABEL_PATTERN[] =
    "ft_s%ip%ic%i"; // three arguments
...
void CChunkRenderText::StoreChunks(const std::vector<CChunk *> &Chunks) {
  ...
  for (const auto pChunk : Chunks)
    for (int j = 0; j < sizeof(RENDER_FUNCTIONS) / sizeof(stChunkRenderFunc);
         ++j)
...
void CChunkRenderText::StoreSamples(
    const std::vector<const CDSample *> &Samples) {
  ...
  for (size_t i = 0; i < Samples.size(); ++i)
    if (const CDSample *pDSample = Samples[i]) { // // //
...
void CChunkRenderText::DumpStrings(const CStringA &preStr,
                                   const CStringA &postStr,
...
void CChunkRenderText::StoreSampleListChunk(CChunk *pChunk, CFile *pFile) {
  ...
  for (int i = 0; i < pChunk->GetLength(); i += 3) {
    str.AppendFormat("\t.byte %i, %i, %i\n", pChunk->GetData(i + 0),
                     pChunk->GetData(i + 1), pChunk->GetData(i + 2));
  }
...
void CChunkRenderText::StoreSamplePointersChunk(CChunk *pChunk, CFile *pFile) {
  ...
  if (len > 0) {
    ...
    int len = pChunk->GetLength();
    for (int i = 0; i < len; ++i) {
      str.AppendFormat("%i%s", pChunk->GetData(i),
                       (i < len - 1) && (i % 3 != 2) ? ", " : "");
      if (i % 3 == 2 && i < (len - 1))
        str.Append("\n\t.byte ");
    }
...
void CChunkRenderText::StoreGrooveListChunk(CChunk *pChunk,
                                            CFile *pFile) // // //
{
  CStringA str;
...
void CChunkRenderText::StoreFrameChunk(CChunk *pChunk, CFile *pFile) {
  ...
  for (int i = 0, j = 0; i < len; ++i) {
    if (pChunk->IsDataReference(i))
      str.AppendFormat("%s%s", (j++ > 0) ? _T(", ") : _T(""),
                       pChunk->GetDataRefName(i));
  }
  ...
  for (int i = 0, j = 0; i < len; ++i) {
    if (pChunk->IsDataBank(i)) {
      ...
      str.AppendFormat("%s$%02X", (j++ > 0) ? _T(", ") : _T(""),
                       pChunk->GetData(i));
    }
  }
...
void CChunkRenderText::WriteFileString(const CStringA &str,
                                       CFile *pFile) const {
  pFile->Write(const_cast<CStringA &>(str).GetBuffer(), str.GetLength());
}

void CChunkRenderText::StoreByteString(const char *pData, int Len,
                                       CStringA &str, int LineBreak) const {
  str.Append("\t.byte ");
