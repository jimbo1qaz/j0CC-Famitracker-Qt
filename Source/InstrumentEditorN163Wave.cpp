...
#define WAVE_SIZE_AVAILABLE                                                    \
  (256 - 16 * GetDocument()->GetNamcoChannels()) // // //

// CInstrumentEditorN163Wave dialog

IMPLEMENT_DYNAMIC(CInstrumentEditorN163Wave, CInstrumentEditPanel)

CInstrumentEditorN163Wave::CInstrumentEditorN163Wave(CWnd *pParent)
    : CInstrumentEditPanel(CInstrumentEditorN163Wave::IDD, pParent),
      m_pWaveEditor(NULL), m_iWaveIndex(0) {}
...
void CInstrumentEditorN163Wave::SelectInstrument(
  ...
  pSizeBox->SelectString(-1,
                         MakeIntString(m_pInstrument->GetWaveSize())); // // //
...
BOOL CInstrumentEditorN163Wave::OnInitDialog() {
  ...
  m_pWaveEditor->CreateEx(WS_EX_CLIENTEDGE, NULL, _T(""), WS_CHILD | WS_VISIBLE,
                          DPI::Rect(20, 30, 0, 0), this); // // //
  ...
  m_pWaveListCtrl->InsertColumn(0, _T("Wave"), LVCFMT_LEFT,
                                static_cast<int>(.83 * r.Width()));
  m_pWaveListCtrl->InsertColumn(1, _T("#"), LVCFMT_LEFT,
                                static_cast<int>(.17 * r.Width()));
  m_pWaveListCtrl->SetColumnOrderArray(2, order);

  m_WaveImage.Create(CInstrumentN163::MAX_WAVE_SIZE, 16, ILC_COLOR8, 0,
...
void CInstrumentEditorN163Wave::GenerateWaves(
    CWaveformGenerator *pWaveGen) // // // test
{
...
void CInstrumentEditorN163Wave::ParseManyStrings(string pStrings) {
  ...
  for (string &str : seglist) {
    ...
    if (curr >= nwave()) {
      ...
      if (!m_pInstrument->InsertNewWave(curr))
        break;
    }
...
void CInstrumentEditorN163Wave::ParseString(string pString, int waveIndex) {
  // The second parameter waveIndex is optional, and defaults to -1 in the
  // header file. The GUI MML setter calls without a wave index, and writes to
  // the wave selected in the GUI. The clipboard-paste code calls with a wave
  // index >= 0, and writes to each wave #N.
  ...
  static_cast<CComboBox *>(GetDlgItem(IDC_WAVE_SIZE))
      ->SelectString(0, MakeIntString(size));
...
void CInstrumentEditorN163Wave::FillPosBox(int size) {
  ...
  for (int i = 0; i <= WAVE_SIZE_AVAILABLE - size;
       i += size) { // // // prevent reading non-wave n163 registers
...
void CInstrumentEditorN163Wave::CreateWaveImage(char *const Pos,
                                                int Index) const // // //
{
...
void CInstrumentEditorN163Wave::OnLvnItemchangedN163Waves(
...
void CInstrumentEditorN163Wave::OnBnClickedN163Add() // // //
{
  if (m_pInstrument->InsertNewWave(m_iWaveIndex + 1)) {
    PopulateWaveBox();
    m_pWaveListCtrl->SetItemState(++m_iWaveIndex, LVIS_FOCUSED | LVIS_SELECTED,
