...
CFrameEditorState::CFrameEditorState(const CFamiTrackerView *pView, int Track)
    : // // //
      Track(Track), Cursor{static_cast<CMainFrame *>(pView->GetParentFrame())
                               ->GetFrameEditor()
                               ->GetEditFrame(),
                           pView->GetSelectedChannel()},
      OriginalSelection(static_cast<CMainFrame *>(pView->GetParentFrame())
                            ->GetFrameEditor()
                            ->GetSelection()),
      IsSelecting(static_cast<CMainFrame *>(pView->GetParentFrame())
                      ->GetFrameEditor()
                      ->IsSelecting()) {
...
void CFrameEditorState::ApplyState(CFamiTrackerView *pView) const {
  auto pEditor =
      static_cast<CMainFrame *>(pView->GetParentFrame())->GetFrameEditor();
...
void CFrameAction::SaveUndoState(const CMainFrame *pMainFrm) // // //
{
  ...
  m_pUndoState =
      new CFrameEditorState{pView, pMainFrm->GetSelectedTrack()}; // // //
  m_itFrames = make_int_range(m_pUndoState->GetFrameStart(),
                              m_pUndoState->GetFrameEnd());
  m_itChannels =
      make_int_range(m_pUndoState->GetChanStart(), m_pUndoState->GetChanEnd());
}
...
bool CFActionRemoveFrame::SaveState(const CMainFrame *pMainFrm) {
  ...
  if (pDoc->GetFrameCount(m_pUndoState->Track) <= 1)
    return false;
  m_pRowClipData =
      pMainFrm->GetFrameEditor()->CopyFrame(m_pUndoState->Cursor.m_iFrame);
  return true;
}
...
void CFActionRemoveFrame::Undo(CMainFrame *pMainFrm) const {
  ...
  pMainFrm->GetFrameEditor()->PasteInsert(
      m_pUndoState->Track, m_pUndoState->Cursor.m_iFrame, m_pRowClipData);
}
...
void CFActionCloneFrame::Undo(CMainFrame *pMainFrm) const {
  ...
  for (int i = 0; i < Count; ++i)
    pDoc->ClearPattern(m_pUndoState->Track, m_pUndoState->Cursor.m_iFrame + 1,
                       i);
  pDoc->RemoveFrame(m_pUndoState->Track, m_pUndoState->Cursor.m_iFrame + 1);
}
...
void CFActionSetPattern::Undo(CMainFrame *pMainFrm) const {
  pMainFrm->GetFrameEditor()->PasteAt(m_pUndoState->Track, m_pClipData,
...
bool CFActionSetPattern::Merge(const CAction *Other) // // //
{
  ...
  if (!pAction)
    return false;
  if (m_pUndoState->Track != pAction->m_pUndoState->Track ||
      m_itFrames != pAction->m_itFrames ||
      m_itChannels != pAction->m_itChannels)
    return false;
...
void CFActionSetPatternAll::Undo(CMainFrame *pMainFrm) const {
  pMainFrm->GetFrameEditor()->PasteAt(m_pUndoState->Track, m_pRowClipData,
                                      m_pUndoState->Cursor);
}
...
void CFActionSetPatternAll::Redo(CMainFrame *pMainFrm) const {
  ...
  for (int i = 0; i < m_pRowClipData->ClipInfo.Channels; ++i)
    pDoc->SetPatternAtFrame(m_pUndoState->Track, m_pUndoState->Cursor.m_iFrame,
                            i, m_iNewPattern);
}
...
void CFActionChangePattern::Redo(CMainFrame *pMainFrm) const {
  ...
  const int c0 = m_pUndoState->IsSelecting ? m_pUndoState->GetChanStart()
                                           : 0; // copies entire row by default
  for (int f : m_itFrames)
    for (int c : m_itChannels) {
...
bool CFActionChangePattern::Merge(const CAction *Other) // // //
{
  ...
  if (m_pUndoState->Track != pAction->m_pUndoState->Track ||
    ...
    return false;
  if (m_bOverflow &&
      m_iPatternOffset * pAction->m_iPatternOffset < 0) // different directions
    return false;
...
bool CFActionMoveDown::SaveState(const CMainFrame *pMainFrm) {
  const CFamiTrackerDoc *pDoc =
      static_cast<CFamiTrackerView *>(pMainFrm->GetActiveView())->GetDocument();
  return m_pUndoState->Cursor.m_iFrame <
         static_cast<int>(pDoc->GetFrameCount(m_pUndoState->Track)) - 1;
}
...
bool CFActionPaste::SaveState(const CMainFrame *pMainFrm) {
  ...
  return m_pClipData->ClipInfo.Channels <= pDoc->GetChannelCount() &&
         m_pClipData->ClipInfo.Frames +
                 pDoc->GetFrameCount(m_pUndoState->Track) <=
             MAX_FRAMES;
}
...
void CFActionPaste::Undo(CMainFrame *pMainFrm) const {
  CFamiTrackerDoc *pDoc =
      static_cast<CFamiTrackerView *>(pMainFrm->GetActiveView())->GetDocument();
  if (m_bClone)
    pMainFrm->GetFrameEditor()->ClearPatterns(m_pUndoState->Track,
                                              m_pRedoState->Selection); // // //
  pDoc->DeleteFrames(m_pUndoState->Track, m_iTargetFrame,
                     m_pClipData->ClipInfo.Frames);
  pMainFrm->UpdateControls();
}
...
bool CFActionPasteOverwrite::SaveState(const CMainFrame *pMainFrm) // // //
{
  ...
  m_TargetSelection.m_cpEnd.m_iFrame =
      m_TargetSelection.m_cpStart.m_iFrame + m_pClipData->ClipInfo.Frames - 1;
  m_TargetSelection.m_cpStart.m_iChannel = m_pClipData->ClipInfo.FirstChannel;
  m_TargetSelection.m_cpEnd.m_iChannel =
      m_TargetSelection.m_cpStart.m_iChannel + m_pClipData->ClipInfo.Channels -
      1;
...
{
  pMainFrm->GetFrameEditor()->PasteAt(m_pUndoState->Track, m_pOldClipData,
                                      m_pUndoState->Cursor);
}
...
bool CFActionDropMove::SaveState(const CMainFrame *pMainFrm) { return true; }
...
void CFActionDropMove::Undo(CMainFrame *pMainFrm) const {
  ...
  if (m_pRedoState->Selection.m_cpStart.m_iFrame < Orig.m_iFrame)
    Orig.m_iFrame += m_pRedoState->Selection.m_cpEnd.m_iFrame -
                     m_pRedoState->Selection.m_cpStart.m_iFrame + 1;
  pMainFrm->GetFrameEditor()->MoveSelection(m_pUndoState->Track,
                                            m_pRedoState->Selection, Orig);
}

void CFActionDropMove::Redo(CMainFrame *pMainFrm) const {
  pMainFrm->GetFrameEditor()->MoveSelection(
      m_pUndoState->Track, m_pUndoState->Selection,
...
bool CFActionClonePatterns::SaveState(const CMainFrame *pMainFrm) // // //
{
  ...
  if (pDoc->IsPatternEmpty(m_pUndoState->Track, m_pUndoState->Cursor.m_iChannel,
                           m_iOldPattern))
    return false;
  m_iNewPattern = pDoc->GetFirstFreePattern(m_pUndoState->Track,
...
void CFActionClonePatterns::Undo(CMainFrame *pMainFrm) const // // //
{
  if (m_pUndoState->IsSelecting) {
    ...
    pEditor->PasteAt(m_pUndoState->Track, m_pClipData,
                     m_pUndoState->Selection.m_cpStart);
  } else {
    CFamiTrackerDoc *pDoc =
        static_cast<CFamiTrackerView *>(pMainFrm->GetActiveView())
...
{
  if (m_pUndoState->IsSelecting)
    pMainFrm->GetFrameEditor()->ClonePatterns(m_pUndoState->Track,
                                              m_pUndoState->Selection);
  else {
    CFamiTrackerDoc *pDoc =
        static_cast<CFamiTrackerView *>(pMainFrm->GetActiveView())
            ->GetDocument();
    pDoc->SetPatternAtFrame(STATE_EXPAND(m_pUndoState), m_iNewPattern);
    pDoc->CopyPattern(m_pUndoState->Track, m_iNewPattern, m_iOldPattern,
                      m_pUndoState->Cursor.m_iChannel);
  }
}
...
void CFActionDeleteSel::Undo(CMainFrame *pMainFrm) const {
  CFrameEditor *pFrameEditor = pMainFrm->GetFrameEditor();
  pFrameEditor->PasteInsert(m_pUndoState->Track,
...
void CFActionDeleteSel::Redo(CMainFrame *pMainFrm) const {
  ...
  pView->GetDocument()->DeleteFrames(
      m_pUndoState->Track, m_pUndoState->Selection.m_cpStart.m_iFrame,
      m_pUndoState->Selection.m_cpEnd.m_iFrame -
          m_pUndoState->Selection.m_cpStart.m_iFrame + 1); // // //
  ...
  for (int c = 0; c < Channels; ++c) {
    ...
    for (int f = 0; f < Frames; ++f) {
      unsigned int uiPattern =
          pDoc->GetPatternAtFrame(m_pUndoState->Track, f, c);
    ...
    // apply mapping
    for (int f = 0; f < Frames; ++f)
      m_pClipData->SetFrame(
          f, c,
          uiPatternUsed[pDoc->GetPatternAtFrame(m_pUndoState->Track, f, c)]);
  }
