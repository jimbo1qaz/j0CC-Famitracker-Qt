...
// assumptions code makes about implementation-defined features
  ...
  //		// casting to smaller signed type truncates bits and extends
  // sign 		i = (SHRT_MAX + 1) * 5; 		assert( (short)
  // i
  // == SHRT_MIN
  // );
#endif
...
Blip_Buffer::~Blip_Buffer() { free(buffer_); }
...
Blip_Buffer::blargg_err_t Blip_Buffer::set_sample_rate(long new_rate,
                                                       int msec) {
...
blip_resampled_time_t Blip_Buffer::clock_rate_factor(long clock_rate) const {
  ...
  assert(factor > 0 ||
         !sample_rate_); // fails if clock/output ratio is too large
...
void Blip_Buffer::remove_silence(long count) {
  assert(count <=
         samples_avail()); // tried to remove more samples than available
...
static void gen_sinc(float *out, int count, double oversample, double treble,
                     double cutoff) {
  if (cutoff >= 0.999)
    cutoff = 0.999;
...
void Blip_Buffer::mix_samples(blip_sample_t const *in, long count) {
  buf_t_ *out =
      buffer_ + (offset_ >> BLIP_BUFFER_ACCURACY) + blip_widest_impulse_ / 2;
