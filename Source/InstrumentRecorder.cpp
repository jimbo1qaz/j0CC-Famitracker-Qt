...
CInstrumentRecorder::CInstrumentRecorder(CSoundGen *pSG)
    : m_pSoundGen(pSG), m_iRecordChannel(-1), m_iDumpCount(0),
...
void CInstrumentRecorder::RecordInstrument(const unsigned Tick,
                                           CView *pView) // // //
{
  unsigned int Intv = static_cast<unsigned>(m_stRecordSetting.Interval);
  if (m_iRecordChannel == -1 || Tick > Intv * m_stRecordSetting.InstCount + 1)
    return;
  ...
  char Chip =
      m_pDocument->GetChannel(m_pDocument->GetChannelIndex(m_iRecordChannel))
          ->GetChip();
  ...
  case SNDCHIP_NONE:
    ID -= CHANID_SQUARE1;
    PitchReg =
        m_iRecordChannel == CHANID_NOISE
            ? (0x0F & REG(0x400E))
            : (REG(0x4002 | (ID << 2)) | (0x07 & REG(0x4003 | (ID << 2))) << 8);
    break;
  case SNDCHIP_VRC6:
    ID -= CHANID_VRC6_PULSE1;
    PitchReg = REG(0x9001 + (ID << 12)) | (0x0F & REG(0x9002 | (ID << 12)))
                                              << 8;
    break;
  case SNDCHIP_FDS:
    ID -= CHANID_FDS; // ID = 0;
    PitchReg = REG(0x4082) | (0x0F & REG(0x4083)) << 8;
    break;
  case SNDCHIP_MMC5:
    ID -= CHANID_MMC5_SQUARE1;
    PitchReg =
        (REG(0x5002 | (ID << 2)) | (0x07 & REG(0x5003 | (ID << 2))) << 8);
    break;
  case SNDCHIP_N163:
    ID -= CHANID_N163_CH1;
    PitchReg = (REG(0x78 - (ID << 3)) | (REG(0x7A - (ID << 3))) << 8 |
                (0x03 & REG(0x7C - (ID << 3))) << 16) >>
               2; // N163_PITCH_SLIDE_SHIFT;
    break;
  case SNDCHIP_S5B:
    ID -= CHANID_S5B_CH1;
    PitchReg = (REG(ID << 1) | (0x0F & REG(1 + (ID << 1))) << 8);
    break;
  }
  ...
  switch (Chip) {
  case SNDCHIP_NONE:
    Table = m_pDocument->GetMachine() == PAL ? CDetuneTable::DETUNE_PAL
                                             : CDetuneTable::DETUNE_NTSC;
    break;
  case SNDCHIP_VRC6:
    Table = m_iRecordChannel == CHANID_VRC6_SAWTOOTH
                ? CDetuneTable::DETUNE_SAW
                : CDetuneTable::DETUNE_NTSC;
    break;
  case SNDCHIP_VRC7:
    Table = CDetuneTable::DETUNE_VRC7;
    break;
  case SNDCHIP_FDS:
    Table = CDetuneTable::DETUNE_FDS;
    break;
  case SNDCHIP_MMC5:
    Table = CDetuneTable::DETUNE_NTSC;
    break;
  case SNDCHIP_N163:
    Table = CDetuneTable::DETUNE_N163;
    break;
  case SNDCHIP_S5B:
    Table = CDetuneTable::DETUNE_S5B;
    break;
  }
  int Note = 0;
  ...
  case INST_S5B:
    for (int i = 0; i < SEQ_COUNT; i++) {
      ...
      case SEQ_VOLUME:
        switch (Chip) {
        case SNDCHIP_NONE:
          Val = m_iRecordChannel == CHANID_TRIANGLE
                    ? ((0x7F & REG(0x4008)) ? 15 : 0)
                    : (0x0F & REG(0x4000 | (ID << 2)));
          break;
        case SNDCHIP_VRC6:
          Val = m_iRecordChannel == CHANID_VRC6_SAWTOOTH
                    ? (0x0F & REG(0xB000) >> 1)
                    : (0x0F & REG(0x9000 | (ID << 12)));
          break;
        case SNDCHIP_MMC5:
          Val = 0x0F & REG(0x5000 | (ID << 2));
          break;
        case SNDCHIP_N163:
          Val = 0x0F & REG(0x7F - (ID << 3));
          break;
        case SNDCHIP_S5B:
          Val = 0x0F & REG(0x08 + ID);
          break;
        }
        break;
      case SEQ_ARPEGGIO:
        Val = static_cast<char>(Note);
        break;
      case SEQ_PITCH:
        Val = static_cast<char>(Detune % 16);
        break;
      case SEQ_HIPITCH:
        Val = static_cast<char>(Detune / 16);
        break;
      case SEQ_DUTYCYCLE:
        ...
        case SNDCHIP_NONE:
          Val = m_iRecordChannel == CHANID_TRIANGLE
                    ...
                    : m_iRecordChannel == CHANID_NOISE
                          ? (0x01 & REG(0x400E) >> 7)
                          : (0x03 & REG(0x4000 | (ID << 2)) >> 6);
          break;
        case SNDCHIP_VRC6:
          Val = m_iRecordChannel == CHANID_VRC6_SAWTOOTH
                    ? (0x01 & REG(0xB000) >> 5)
                    : (0x07 & REG(0x9000 | (ID << 12)) >> 4);
          break;
        case SNDCHIP_MMC5:
          Val = 0x03 & REG(0x5000 | (ID << 2)) >> 6;
          break;
        case SNDCHIP_N163:
          Val = 0;
          if (m_iRecordWaveCache == NULL) {
            ...
            if (Size <= CInstrumentN163::MAX_WAVE_SIZE) {
              ...
              m_iRecordWaveCache = new char[m_iRecordWaveSize *
                                            CInstrumentN163::MAX_WAVE_COUNT]();
        ...
        case SNDCHIP_S5B:
          Val = 0x1F & REG(0x06) | (0x10 & REG(0x08 + ID)) << 1 |
                (0x01 << ID & ~REG(0x07)) << (6 - ID) |
                (0x08 << ID & ~REG(0x07)) << (4 - ID);
          break;
        }
        break;
      }
...
CInstrument *CInstrumentRecorder::GetRecordInstrument(unsigned Tick) const {
  return m_pDumpCache[Tick / m_stRecordSetting.Interval -
                      (m_pSoundGen->IsPlaying() ? 1 : 0)];
}

int CInstrumentRecorder::GetRecordChannel() const { return m_iRecordChannel; }
...
void CInstrumentRecorder::FinalizeRecordInstrument() {
  ...
  CInstrumentN163 *N163Inst =
      dynamic_cast<CInstrumentN163 *>(*m_pDumpInstrument);
  if (Inst != NULL) {
    (*m_pDumpInstrument)->RegisterManager(m_pDocument->GetInstrumentManager());
    for (int i = 0; i < SEQ_COUNT; i++) {
      if (Inst->GetSeqEnable(i) != 0) {
        m_pSequenceCache[i]->SetLoopPoint(m_pSequenceCache[i]->GetItemCount() -
                                          1);
  ...
  case INST_FDS:
    ...
    for (int i = 0; i < CInstrumentFDS::SEQUENCE_COUNT; i++) {
            ...
            ASSERT(Seq != NULL);
            m_pSequenceCache[i]->SetLoopPoint(m_pSequenceCache[i]->GetItemCount()
    - 1); FDSInst->SetSequence(i, m_pSequenceCache[i]); m_pSequenceCache[i] =
    new CSequence();
    }
    */
    for (int i = 0; i < CInstrumentFDS::WAVE_SIZE; i++)
      FDSInst->SetSample(i,
                         0x3F & m_pSoundGen->GetReg(SNDCHIP_FDS, 0x4040 + i));
    FDSInst->SetModulationDepth(0x3F &
                                m_pSoundGen->GetReg(SNDCHIP_FDS, 0x4084));
    FDSInst->SetModulationSpeed(
        m_pSoundGen->GetReg(SNDCHIP_FDS, 0x4086) |
        (0x0F & m_pSoundGen->GetReg(SNDCHIP_FDS, 0x4087)) << 8);
    break;
  case INST_N163:
    ...
    if (m_iRecordWaveCache != NULL) {
      for (int i = 0; i <= m_iRecordWaveCount; i++)
        for (int j = 0; j < m_iRecordWaveSize; j++)
          N163Inst->SetSample(i, j,
                              m_iRecordWaveCache[m_iRecordWaveSize * i + j]);
      SAFE_RELEASE(m_iRecordWaveCache);
    } else
      for (int j = 0; j < m_iRecordWaveSize;
           j++) // fallback for blank recording
