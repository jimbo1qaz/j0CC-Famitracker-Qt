...
bool CChannelHandlerVRC6::HandleEffect(effect_t EffNum,
...
void CChannelHandlerVRC6::HandleCut() { CutNote(); }
...
int CVRC6Square::ConvertDuty(int Duty) const // // //
{
  switch (m_iInstTypeCurrent) {
  case INST_2A03:
    return DUTY_VRC6_FROM_2A03[Duty & 0x03];
  case INST_S5B:
    return 0x07;
  default:
    return Duty;
...
bool CVRC6Sawtooth::CreateInstHandler(inst_type_t Type) // // //
{
  ...
  case INST_FDS:
    ...
    default:
      m_pInstHandler.reset(
          new CSeqInstHandlerSawtooth(this, 0x0F, Type == INST_S5B ? 0x40 : 0));
      return true;
    }
  }
  return false;
...
int CVRC6Sawtooth::CalculateVolume() const // // //
{
  bool _64_step = false;
  if (auto pHandler =
          dynamic_cast<CSeqInstHandlerSawtooth *>(m_pInstHandler.get()))
    _64_step = pHandler->IsDutyIgnored();

  if (_64_step) {
    if (!theApp.GetSettings()->General.bFDSOldVolume) // // // match NSF setting
      return LimitVolume(
          ((m_iInstVolume + 1) * ((m_iVolume >> VOL_COLUMN_SHIFT) + 1) - 1) /
              16 -
          GetTremolo());
    return CChannelHandler::CalculateVolume();
  }

  return (CChannelHandler::CalculateVolume() << 1) |
         ((m_iDutyPeriod & 0x01) << 5);
}
