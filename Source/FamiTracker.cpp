...
CFamiTrackerApp::CFamiTrackerApp()
    : //   m_bThemeActive(false),
      m_pMIDI(NULL), m_pAccel(NULL), m_pSettings(NULL), m_pSoundGenerator(NULL),
      m_pChannelMap(NULL), m_customExporters(NULL),
//	m_hWndMapFile(NULL),
...
//	InitCommonControls();
  ...
  //		HKEY HKCU;
  //		long res_reg = ::RegOpenKey(HKEY_CURRENT_USER,
  //_T("Software\\Classes"), &HKCU); 		if(res_reg == ERROR_SUCCESS)
  ...
  //		strTemp.Format(_T("%s\\shell\\play\\%s"),
  //(LPCTSTR)strFileTypeId, _T("command"));
  ...
  // CP: Showing the window here causes a blip of a not-yet-initialized window
  // because Qt hasn't yet actually "taken over" for MFC, so I'm hacking this
  // out and letting the Qt subsystem show the window when it's done
  // "initializing" it.
...
void CFamiTrackerApp::CheckAppThemed() {
  ...
  //		pIsAppThemed = (ISAPPTHEMEDPROC) ::GetProcAddress(hinstDll,
  //"IsAppThemed");
...
void CFamiTrackerApp::LoadLocalization() {
  ...
  //		if (Major != VERSION_MAJ || Minor != VERSION_MIN || Revision !=
  // VERSION_REV) 			return;
...
void CFamiTrackerApp::RegisterSingleInstance() {
  ...
  //	m_hWndMapFile = CreateFileMapping(INVALID_HANDLE_VALUE, NULL,
  // PAGE_READWRITE, 0, SHARED_MEM_SIZE, FT_SHARED_MEM_NAME);

  //	if (m_hWndMapFile != NULL) {
  //		LPTSTR pBuf = (LPTSTR) MapViewOfFile(m_hWndMapFile,
  // FILE_MAP_ALL_ACCESS, 0, 0, SHARED_MEM_SIZE); 		if (pBuf !=
  // NULL)
  // {
        ...
        for (const auto &i : j) {
          ...
          if (Ver[0] > VERSION_API ||
            ...
            m_pVersionMessage.Format(
                _T("A new version of 0CC-FamiTracker is now available:\n\n"
            ...
            m_pVersionURL.Format(
                _T("https://github.com/HertzDevil/0CC-FamiTracker/releases/")
                _T("tag/v%d.%d.%d.%d"),
...
bool CFamiTrackerApp::CheckSingleInstance(CFTCommandLineInfo &cmdInfo) {
  ...
  // FT_SHARED_MEM_NAME); 		if (hMapFile != NULL) {
  // LPCTSTR pBuf = (LPTSTR) MapViewOfFile(hMapFile, FILE_MAP_ALL_ACCESS, 0, 0,
  // SHARED_MEM_SIZE); 			if (pBuf != NULL) {
  //				// Get window handle
  //				HWND hWnd = (HWND)_ttoi(pBuf);
  //				if (hWnd != NULL) {
  //					// Get file name
  //					LPTSTR pFilePath =
  // cmdInfo.m_strFileName.GetBuffer();
  //					// We have the window handle & file,
  //send a message to
  // open the file 					COPYDATASTRUCT data;
  // data.dwData = cmdInfo.m_bPlay ? IPC_LOAD_PLAY : IPC_LOAD;
  // data.cbData = (DWORD)((_tcslen(pFilePath) + 1) * sizeof(TCHAR));
  // data.lpData = pFilePath; 					DWORD result;
  //					SendMessageTimeout(hWnd, WM_COPYDATA,
  // NULL, (LPARAM)&data, SMTO_NORMAL, 100, &result);
  // UnmapViewOfFile(pBuf);
  // CloseHandle(hMapFile); TRACE("App: Found another instance, shutting
  // down\n");
...
int CFamiTrackerApp::GetCPUUsage() const {
  ...
  //	static FILETIME KernelLastTime[THREAD_COUNT],
  // UserLastTime[THREAD_COUNT]; 	const HANDLE hThreads[THREAD_COUNT] =
  //{m_hThread, m_pSoundGenerator->m_hThread};
  unsigned int TotalTime = 0;

  //	for (int i = 0; i < THREAD_COUNT; ++i) {
  //		FILETIME CreationTime, ExitTime, KernelTime, UserTime;
  //		GetThreadTimes(hThreads[i], &CreationTime, &ExitTime,
  //&KernelTime, &UserTime); 		TotalTime += (KernelTime.dwLowDateTime -
  // KernelLastTime[i].dwLowDateTime) / 1000; 		TotalTime +=
...
BOOL CDocManager0CC::DoPromptFileName(CString &fileName, UINT nIDSTitle,
  ...
  CFileDialog OpenFileDlg(bOpenFileDialog, _T("0cc"), NULL,
