...
CCommentsDlg::CCommentsDlg(CWnd *pParent /*=NULL*/)
    : CDialog(CCommentsDlg::IDD, pParent), m_pFont(NULL), m_sComment(""),
      m_bShowOnLoad(false) // // //
{}

CCommentsDlg::~CCommentsDlg() { SAFE_RELEASE(m_pFont); }
...
void CCommentsDlg::SetComment(CString Str) { m_sComment = Str; }

bool CCommentsDlg::GetShowOnLoad() const { return m_bShowOnLoad; }

void CCommentsDlg::SetShowOnLoad(bool Enable) { m_bShowOnLoad = Enable; }

bool CCommentsDlg::IsChanged() const { return m_bChanged; }
...
BOOL CCommentsDlg::OnInitDialog() {
  ...
  if (WinRect.top == 0 && WinRect.bottom == 0)
    ...
    MoveWindow(WinRect.left, WinRect.top, WinRect.right - WinRect.left,
...
void CCommentsDlg::OnEnChangeComments() { m_bChanged = true; }

void CCommentsDlg::OnBnClickedShowonopen() { m_bChanged = true; }
