...
void CCreateWaveDlg::OnBnClickedBegin() {
  ...
  if (pDoc->GetTrackCount() > 1) {
    FileName.AppendFormat(_T(" - Track %02i (%s)"), Track + 1,
                          pDoc->GetTrackTitle(Track).GetBuffer());
  }

  CWavProgressDlg ProgressDlg;
  CString fileFilter = LoadDefaultFilter(IDS_FILTER_WAV, _T(".wav"));
  CFileDialog SaveDialog(FALSE, _T("wav"), FileName,
...
void CCreateWaveDlg::ShowDialog() { CDialog::DoModal(); }
