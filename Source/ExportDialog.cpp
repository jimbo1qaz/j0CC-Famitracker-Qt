...
const exportFunc_t CExportDialog::DEFAULT_EXPORT_FUNCS[] = {
    &CExportDialog::CreateNSF,  &CExportDialog::CreateNES,
    &CExportDialog::CreateBIN,  &CExportDialog::CreatePRG,
...
LPCTSTR CExportDialog::DPCMS_FILTER[] = {_T("DPCM sample bank (*.bin)"),
                                         _T(".bin")};
LPCTSTR CExportDialog::PRG_FILTER[] = {_T("NES program bank (*.prg)"),
                                       _T(".prg")};
LPCTSTR CExportDialog::ASM_FILTER[] = {_T("Assembly text (*.asm)"), _T(".asm")};
LPCTSTR CExportDialog::NSFE_FILTER[] = {_T("NSFe file (*.nsfe)"),
                                        _T(".nsfe")}; // // //
...
void CExportDialog::OnBnClickedClose() { EndDialog(0); }
...
BOOL CExportDialog::OnInitDialog() {
  ...
  CFamiTrackerDoc *pDoc =
      static_cast<CFamiTrackerDoc *>(pFrameWnd->GetActiveDocument());
...
void CExportDialog::CreateNSF() {
  ...
  CFileDialog FileDialog(FALSE, NSF_FILTER[1], DefFileName,
...
void CExportDialog::CreateNES() {
  ...
  CFileDialog FileDialog(FALSE, NES_FILTER[1], DefFileName,
                         OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, filter);
  ...
  Compiler.ExportNES(FileDialog.GetPathName(),
                     (IsDlgButtonChecked(IDC_PAL) != 0));
...
void CExportDialog::CreateBIN() {
  ...
  CFileDialog FileDialogMusic(FALSE, RAW_FILTER[1], DEFAULT_MUSIC_NAME,
                              OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT,
                              MusicFilter);
  CFileDialog FileDialogSamples(FALSE, DPCMS_FILTER[1], DEFAULT_SAMPLE_NAME,
                                OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT,
                                DPCMFilter);

  FileDialogMusic.m_pOFN->lpstrInitialDir =
...
void CExportDialog::CreatePRG() {
  ...
  CFileDialog FileDialog(FALSE, PRG_FILTER[1], _T("music.prg"),
                         OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, Filter);
  ...
  Compiler.ExportPRG(FileDialog.GetPathName(),
                     (IsDlgButtonChecked(IDC_PAL) != 0));
...
void CExportDialog::CreateASM() {
  ...
  CFileDialog FileDialogMusic(FALSE, ASM_FILTER[1], _T("music.asm"),
                              OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, Filter);
...
void CExportDialog::CreateCustom(CString name) {
  ...
  CString custom_exporter_extension =
      theApp.GetCustomExporters()->GetCurrentExporter().getExt();
  CString custom_filter_name =
      CString("Custom Song Data (*") + custom_exporter_extension + CString(")");
  CString default_custom_file_name =
      CString("music") + custom_exporter_extension;

  CString Filter =
      LoadDefaultFilter(custom_filter_name, custom_exporter_extension);
  CFileDialog FileDialogCustom(FALSE, custom_exporter_extension,
  ...
  if (theApp.GetCustomExporters()->GetCurrentExporter().Export(
          &documentWrapper, CStringA(fileName))) {
...
void CExportDialog::OnBnClickedPlay() {
  ...
  CFamiTrackerDoc *pDoc = CFamiTrackerDoc::GetDoc();
  CCompiler Compiler(
      pDoc, new CEditLog(static_cast<CEdit *>(GetDlgItem(IDC_OUTPUT))));
