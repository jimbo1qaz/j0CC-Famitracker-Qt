...
void CTransposeDlg::SetTrack(unsigned int Track) { m_iTrack = Track; }
    ...
    for (int p = 0; p < MAX_PATTERN; ++p)
      for (int r = 0; r < MAX_PATTERN_LENGTH; ++r) {
        ...
        if (Note.Note >= NOTE_C && Note.Note <= NOTE_B &&
            !s_bDisableInst[Note.Instrument]) {
          int MIDI = MIDI_NOTE(Note.Octave, Note.Note) + Trsp;
          if (MIDI < 0)
            MIDI = 0;
          else if (MIDI >= NOTE_COUNT)
            MIDI = NOTE_COUNT - 1;
...
ON_CONTROL_RANGE(BN_CLICKED, BUTTON_ID, BUTTON_ID + MAX_INSTRUMENTS - 1,
                 OnBnClickedInst)
...
BOOL CTransposeDlg::OnInitDialog() {
  ...
  for (int i = 0; i < MAX_INSTRUMENTS; ++i) {
    ...
    m_cInstButton[i]->Create(
        Name, WS_CHILD | WS_VISIBLE | WS_TABSTOP | BS_AUTOCHECKBOX,
  ...
  CSpinButtonCtrl *pSpin =
      static_cast<CSpinButtonCtrl *>(GetDlgItem(IDC_SPIN_TRSP_SEMITONE));
  pSpin->SetRange(0, 96);
  pSpin->SetPos(0);

  CheckRadioButton(IDC_RADIO_SEMITONE_INC, IDC_RADIO_SEMITONE_DEC,
                   IDC_RADIO_SEMITONE_INC);
...
void CTransposeDlg::OnBnClickedOk() {
  ...
  if (GetCheckedRadioButton(IDC_RADIO_SEMITONE_INC, IDC_RADIO_SEMITONE_DEC) ==
      IDC_RADIO_SEMITONE_DEC)
    Trsp = -Trsp;
...
void CTransposeDlg::OnBnClickedButtonTrspReverse() {
  for (int i = 0; i < MAX_INSTRUMENTS; ++i)
    CheckDlgButton(BUTTON_ID + i,
                   IsDlgButtonChecked(BUTTON_ID + i) == BST_UNCHECKED);
}
