...
//
// This is the new NSF data compiler, music is compiled to an object list
// instead of a binary chunk
//
 ...
 *
 * - $8000 - $AFFF: Music driver and song data (instruments, frames & patterns,
 * unpaged)
...
const int CCompiler::DPCM_PAGE_WINDOW =
    3; // Number of switchable pages in the DPCM area
const int CCompiler::DPCM_SWITCH_ADDRESS =
    0xF000; // Switch to new banks when reaching this address
...
CCompiler *CCompiler::GetCompiler() { return pCompiler; }
...
CCompiler::CCompiler(CFamiTrackerDoc *pDoc, CCompilerLog *pLogger)
    : m_pDocument(pDoc), m_pLogger(pLogger), m_iWaveTables(0),
      m_pSamplePointersChunk(NULL), m_pHeaderChunk(NULL), m_pDriverData(NULL),
      m_iLastBank(0), m_iHashCollisions(0) {
...
void CCompiler::ExportNSF(LPCTSTR lpszFileName, int MachineType) {
  ...
  if ((PAGE_SAMPLES - m_iDriverSize - m_iMusicDataSize) < 0x8000 ||
      m_bBankSwitched ||
      m_iActualChip != m_pDocument->GetExpansionChip()) // // //
  ...
  // Load driver
  std::unique_ptr<char[]> pDriverPtr(
      LoadDriver(m_pDriverData, m_iDriverAddress)); // // //
  ...
  if (m_bBankSwitched) {
    int Percent =
        (100 * m_iMusicDataSize) / (0x80000 - m_iDriverSize - m_iSamplesSize);
    ...
    int Percent =
        (100 * m_iMusicDataSize) / (0x8000 - m_iDriverSize - m_iSamplesSize);
    Print(_T(" * Song data size: %i bytes (%i%%)\n"), m_iMusicDataSize,
          Percent);
...
void CCompiler::ExportNSFE(LPCTSTR lpszFileName, int MachineType) // // //
{
  ...
  iAuthSize = strlen(m_pDocument->GetSongName()) +
              strlen(m_pDocument->GetSongArtist()) +
  ...
  OutputFile.Write(m_pDocument->GetSongName(),
                   strlen(m_pDocument->GetSongName()) + 1);
  OutputFile.Write(m_pDocument->GetSongArtist(),
                   strlen(m_pDocument->GetSongArtist()) + 1);
  OutputFile.Write(m_pDocument->GetSongCopyright(),
                   strlen(m_pDocument->GetSongCopyright()) + 1);
  ...
  for (unsigned int i = 0; i < m_pDocument->GetTrackCount(); i++) {
    int t =
        static_cast<int>(m_pDocument->GetStandardLength(i, 1) * 1000.0 + 0.5);
  ...
  for (unsigned int i = 0; i < m_pDocument->GetTrackCount(); i++) {
    OutputFile.Write(m_pDocument->GetTrackTitle(i),
                     strlen(m_pDocument->GetTrackTitle(i)) + 1);
  }
  ...
  const unsigned char NEndIdent[] = {'\0', '\0', '\0', '\0',
                                     'N',  'E',  'N',  'D'}; // // //
...
void CCompiler::ExportNES(LPCTSTR lpszFileName, bool EnablePAL) {
  ...
  if (m_pDocument->GetExpansionChip() != SNDCHIP_NONE) {
    ...
    AfxMessageBox(_T("Expansion chips are currently not supported when ")
                  _T("exporting to .NES!"),
                  0, 0);
    return;
  }
...
void CCompiler::ExportPRG(LPCTSTR lpszFileName, bool EnablePAL) {
  ...
  if (m_pDocument->GetExpansionChip() != SNDCHIP_NONE) {
    ...
    AfxMessageBox(_T("Error: Expansion chips is currently not supported when ")
                  _T("exporting to PRG!"),
                  0, 0);
    return;
  }
...
void CCompiler::ExportASM(LPCTSTR lpszFileName) {
  ...
  UpdateSamplePointers(
      PAGE_SAMPLES); // Always start at C000 when exporting to ASM
...
char *CCompiler::LoadDriver(const driver_t *pDriver,
  ...
  for (size_t i = 0; i < pDriver->word_reloc_size; ++i) {
    // Words
    unsigned short value = pData[pDriver->word_reloc[i]] +
                           (pData[pDriver->word_reloc[i] + 1] << 8);
    value += Origin;
  ...
  for (size_t i = 0; i < pDriver->adr_reloc_size; i += 2) { // // //
    unsigned short value =
        pData[pDriver->adr_reloc[i]] + (pData[pDriver->adr_reloc[i + 1]] << 8);
    value += Origin;
  ...
  if (m_iActualChip == SNDCHIP_N163) {
    pData[m_iDriverSize - 2 - 0x100 - 0xC0 * 2 - 8 - 1 - 8 +
          m_iActualNamcoChannels] = 3;
  }

  if (m_iActualChip &
      (m_iActualChip - 1)) { // // // special processing for multichip
    ...
    const int CH_MAP[] = {
        0,  1,  2,  3,  27, 6,  7,  8,  4,  5,  -1, 9,  10, 11, 12,
        13, 14, 15, 16, 17, 21, 22, 23, 24, 25, 26, 18, 19, 20,
    };
...
void CCompiler::SetDriverSongAddress(char *pDriver,
...
void CCompiler::PatchVibratoTable(char *pDriver) const {
  ...
  for (int i = 0; i < 256; ++i) {
    *(pDriver + m_iVibratoTableLocation + i) =
        (char)pSoundGen->ReadVibratoTable(i);
  }
}
...
void CCompiler::CreateHeader(stNSFHeader *pHeader, int MachineType) const {
  ...
  if (m_bBankSwitched) {
    for (int i = 0; i < 4; ++i) {
      ...
      pHeader->BankValues[i + 4] =
          (SampleBank < m_iLastBank) ? SampleBank : m_iLastBank;
    }
    if (LAST_BANK_FIXED) {
...
void CCompiler::CreateNSFeHeader(stNSFeHeader *pHeader,
                                 int MachineType) // // //
{
  memset(pHeader, 0, 40);
...
void CCompiler::UpdateSamplePointers(unsigned int Origin) {
  ...
  //
  // TODO: rewrite this to utilize the CChunkDataBank to resolve bank numbers
  // automatically
  //
...
void CCompiler::UpdateFrameBanks() {
  // Write bank numbers to frame lists (can only be used when bankswitching is
  // used)
  ...
  for (CChunk *pChunk : m_vFrameChunks) {
    if (pChunk->GetType() == CHUNK_FRAME) {
      ...
      for (int j = 0; j < Channels; ++j) {
        unsigned char bank =
            GetObjectByRef(pChunk->GetDataRefName(j))->GetBank();
...
void CCompiler::UpdateSongBanks() {
  // Write bank numbers to song lists (can only be used when bankswitching is
  // used)
...
void CCompiler::EnableBankswitching() {
  ...
  unsigned char flags =
      (unsigned char)m_pHeaderChunk->GetData(m_iHeaderFlagOffset);
...
void CCompiler::CollectLabels(
    CMap<CStringA, LPCSTR, int, int> &labelMap) const {
...
bool CCompiler::CollectLabelsBankswitched(
    CMap<CStringA, LPCSTR, int, int> &labelMap) {
  int Offset = 0;
  ...
  for (CChunk *pChunk : m_vChunks) {
    ...
    switch (pChunk->GetType()) {
    case CHUNK_FRAME_LIST:
      // Make sure the entire frame list will fit, if not then allocate a new
      // bank
  ...
  if (m_bBankSwitched)
    m_iFirstSampleBank =
        ((Bank < 4) ? ((Offset + m_iDriverSize) >> 12) : Bank) + 1;
...
bool CCompiler::CompileData() {
  ...
  const int Chip =
      m_pDocument
          ->GetExpansionChip(); // 0CC: use m_iActualChip once cc65 is embedded
  int Channel = 0;
  for (int i = 0; i < 4; i++) {
  ...
  if (Chip & SNDCHIP_N163) {
    ...
    for (int i = 0; i < lim;
         i++) { // 0CC: use m_iActualNamcoChannels once cc65 is embedded
...
void CCompiler::AddBankswitching() {
  ...
  for (int i = 0; i < TrackCount; ++i) {
    m_iTrackFrameSize[i] +=
        m_pDocument->GetChannelCount() * m_pDocument->GetFrameCount(i);
  }
...
void CCompiler::ScanSong() {
  ...
  memset(m_bSequencesUsedS5B, false,
         sizeof(bool) * MAX_SEQUENCES * SEQ_COUNT); // // //

  static const inst_type_t inst[] = {INST_2A03, INST_VRC6, INST_N163,
                                     INST_S5B}; // // //
  bool *used[] = {*m_bSequencesUsed2A03, *m_bSequencesUsedVRC6,
                  *m_bSequencesUsedN163, *m_bSequencesUsedS5B};
  ...
  for (int i = 0; i < MAX_INSTRUMENTS; ++i) {
    if (m_pDocument->IsInstrumentUsed(i) && IsInstrumentInPattern(i)) {
      ...
      for (size_t z = 0; z < sizeof(used) / sizeof(bool *); z++)
        if (it == inst[z]) {
          auto pInstrument = std::static_pointer_cast<CSeqInstrument>(
              m_pDocument->GetInstrument(i));
  ...
  memset(m_bSamplesAccessed, 0,
         MAX_INSTRUMENTS * OCTAVE_RANGE * NOTE_RANGE * sizeof(bool));
...
void CCompiler::CreateMainHeader() {
  ...
  if (m_pDocument->ExpansionEnabled(SNDCHIP_N163) || bMultichip) {
    ...
    pChunk->StoreByte(std::max(m_iActualNamcoChannels, 1));
  }
...
void CCompiler::CreateSequenceList() {
  ...
  const bool *used[] = {*m_bSequencesUsed2A03, *m_bSequencesUsedVRC6,
                        *m_bSequencesUsedN163, *m_bSequencesUsedS5B};
  ...
  for (int i = 0; i < MAX_INSTRUMENTS; ++i) {
    if (auto pInstrument = std::dynamic_pointer_cast<CInstrumentFDS>(
            m_pDocument->GetInstrument(i))) {
...
void CCompiler::CreateInstrumentList() {
  ...
  CChunk *pInstListChunk = CreateChunk(CHUNK_INSTRUMENT_LIST,
                                       CChunkRenderText::LABEL_INSTRUMENT_LIST);

  if (m_pDocument->ExpansionEnabled(SNDCHIP_FDS)) {
    pWavetableChunk =
        CreateChunk(CHUNK_WAVETABLE, CChunkRenderText::LABEL_WAVETABLE);
  }
  ...
  for (unsigned int i = 0; i < m_iInstruments; ++i) {
    ...
    if (m_pDocument->GetInstrumentType(iIndex) == INST_N163 &&
        m_iWaveBanks[i] == -1) {
      auto pInstrument = std::static_pointer_cast<CInstrumentN163>(
          m_pDocument->GetInstrument(iIndex));
      for (unsigned int j = i + 1; j < m_iInstruments; ++j) {
        int inst = m_iAssignedInstruments[j];
        if (m_pDocument->GetInstrumentType(inst) == INST_N163 &&
            m_iWaveBanks[j] == -1) {
          auto pNewInst = std::static_pointer_cast<CInstrumentN163>(
              m_pDocument->GetInstrument(inst));
  ...
  for (unsigned int i = 0; i < m_iInstruments; ++i) {
    ...
    /*
                    if (pInstrument->GetType() == INST_N163) {
                            ...
                            // Store waves
                            iWaveSize +=
       ((CInstrumentN163*)pInstrument)->StoreWave(pWavesChunk);
                    }
    */
    ...
    if (pInstrument->GetType() == INST_FDS && pWavetableChunk != NULL) {
      // Store wave
      AddWavetable(std::static_pointer_cast<CInstrumentFDS>(pInstrument).get(),
                   pWavetableChunk);
...
void CCompiler::CreateSampleList() {
   ...
   *
   * Each item is stored as a pair of the sample pitch and pointer to the sample
   * table
   *
   */
  ...
  CChunk *pChunk =
      CreateChunk(CHUNK_SAMPLE_LIST, CChunkRenderText::LABEL_SAMPLES_LIST);

  // Store sample instruments
  unsigned int Item = 0;
  for (int i = 0; i < MAX_INSTRUMENTS; ++i) {
    if (m_pDocument->IsInstrumentUsed(i) &&
        m_pDocument->GetInstrumentType(i) == INST_2A03) {
      auto pInstrument = std::static_pointer_cast<CInstrument2A03>(
          m_pDocument->GetInstrument(i));
      ...
      for (int j = 0; j < OCTAVE_RANGE; ++j) {
        for (int k = 0; k < NOTE_RANGE; ++k) {
          ...
          if ((iSample > 0) && m_bSamplesAccessed[i][j][k] &&
              m_pDocument->IsSampleUsed(iSample - 1)) {
...
void CCompiler::StoreSamples() {
  ...
  CChunk *pChunk =
      CreateChunk(CHUNK_SAMPLE_POINTERS, CChunkRenderText::LABEL_SAMPLES);
...
void CCompiler::StoreGrooves() {
  ...
  CChunk *pGrooveListChunk =
      CreateChunk(CHUNK_GROOVE_LIST, CChunkRenderText::LABEL_GROOVE_LIST);
...
void CCompiler::StoreSongs() {
  ...
  CChunk *pSongListChunk =
      CreateChunk(CHUNK_SONG_LIST, CChunkRenderText::LABEL_SONG_LIST);
  ...
  for (int i = 0; i < TrackCount; ++i) {
    ...
    if (m_pDocument->GetSongGroove(i)) // // //
      if (m_pDocument->GetGroove(m_pDocument->GetSongSpeed(i)) != NULL)
        ...
        pChunk->StoreByte(DEFAULT_SPEED);
    else
    ...
    if (m_pDocument->GetSongGroove(i) &&
        m_pDocument->GetGroove(m_pDocument->GetSongSpeed(i)) != NULL) { // // //
      int Pos = 1;
  ...
  m_iSongBankReference = m_vSongChunks[0]->GetLength() -
                         1; // Save bank value position (all songs are equal)
...
void CCompiler::CreateFrameList(unsigned int Track) {
   ...
   * and the frame list holds the offset addresses for the patterns for all
   * channels
   *
   * ---------------------
...
void CCompiler::StorePatterns(unsigned int Track) {
  ...
  CPatternCompiler PatternCompiler(m_pDocument, m_iAssignedInstruments,
                                   (DPCM_List_t *)&m_iSamplesLookUp, m_pLogger);
    ...
    for (int j = 0; j < iChannels; ++j) {
      ...
      if (IsPatternAddressed(Track, i, j)) {
        ...
        if (pDuplicate != NULL) {
          ...
          if (PatternCompiler.CompareData(
                  pDuplicate->GetStringData(PATTERN_CHUNK_INDEX))) {
...
bool CCompiler::IsPatternAddressed(unsigned int Track, int Pattern,
                                   int Channel) const {
