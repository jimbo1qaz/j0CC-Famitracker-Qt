...
// BOOL CALLBACK CDSound::DSEnumCallback(LPGUID lpGuid, LPCTSTR
// lpcstrDescription, LPCTSTR lpcstrModule, LPVOID lpContext)
//{
//	return pThisObject->EnumerateCallback(lpGuid, lpcstrDescription,
// lpcstrModule, lpContext);
//}
...
extern "C" void SDL_FamiTracker(void *userdata, uint8_t *stream, int32_t len) {
  ...
  if (invisibleFamiTracker) {
    memset(stream, 0, len);
    if (m_pSoundBuffer && m_iTotalSamples)
...
void CDSound::CloseDevice() { SDL_Quit(); }

void CDSound::ClearEnumeration() { m_iDevices = 0; }

BOOL CDSound::EnumerateCallback(LPGUID lpGuid, LPCTSTR lpcstrDescription,
...
unsigned int CDSound::GetDeviceCount() const { return m_iDevices; }
...
int CDSound::CalculateBufferLength(int BufferLen, int Samplerate,
                                   int Samplesize, int Channels) const {
  // Calculate size of the buffer, in bytes
  return ((Samplerate * BufferLen) / 1000) * (Samplesize / 8) * Channels;
}

CDSoundChannel *CDSound::OpenChannel(int SampleRate, int SampleSize,
                                     int Channels, int BufferLength,
                                     int Blocks) {
  ...
  while ((SampleRate * BufferLength / (Blocks * 1000) !=
          (double)SampleRate * BufferLength / (Blocks * 1000)))
    ++BufferLength;
  ...
  int SoundBufferSize =
      CalculateBufferLength(BufferLength, SampleRate, SampleSize, Channels);
  ...
  if (!invisibleFamiTracker) {
    ...
    //   qDebug("Adjusting audio:
    //   %d",memcmp(&sdlAudioSpecIn,&sdlAudioSpecOut,sizeof(sdlAudioSpecIn)));
    SoundBufferSize = sdlAudioSpecOut.samples * sdlAudioSpecOut.channels *
                      ((sdlAudioSpecOut.format == AUDIO_U8 ? 8 : 16) >> 3);
...
bool CDSoundChannel::IsPlaying() const { return m_bPaused; }
