...
void CSeqInstHandlerFDS::LoadInstrument(
    std::shared_ptr<CInstrument> pInst) // // //
{
  CSeqInstHandler::LoadInstrument(pInst);

  if (auto pFDSInst =
          std::dynamic_pointer_cast<const CInstrumentFDS>(m_pInstrument))
...
void CSeqInstHandlerFDS::TriggerInstrument() {
  ...
  CChannelHandlerInterfaceFDS *pInterface =
      dynamic_cast<CChannelHandlerInterfaceFDS *>(m_pInterface);
