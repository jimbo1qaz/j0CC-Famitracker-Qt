...
CModuleImportDlg::CModuleImportDlg(CFamiTrackerDoc *pDoc)
    : CDialog(CModuleImportDlg::IDD, NULL), m_pImportedDoc(NULL),
      m_pDocument(pDoc) {}

CModuleImportDlg::~CModuleImportDlg() { SAFE_RELEASE(m_pImportedDoc); }
...
BOOL CModuleImportDlg::OnInitDialog() {
  ...
  for (int i = 0; i < TrackCount; ++i) {
    CString str;
    str.Format(_T("#%02i %s"), i + 1,
               m_pImportedDoc->GetTrackTitle(i).GetString());
...
void CModuleImportDlg::OnBnClickedOk() {
  if (!(ImportInstruments() && ImportGrooves() && ImportDetune() &&
        ImportTracks())) // // //
...
void CModuleImportDlg::OnBnClickedCancel() { OnCancel(); }
...
bool CModuleImportDlg::LoadFile(CString Path, CFamiTrackerDoc *pDoc) {
  ...
  if (m_pImportedDoc->GetNamcoChannels() != m_pDocument->GetNamcoChannels()) {
    ...
    if (m_pDocument->GetNamcoChannels() > Max)
      Max = m_pDocument->GetNamcoChannels();
    m_pImportedDoc->SetNamcoChannels(Max, true);
    m_pDocument->SetNamcoChannels(Max, true);
    unsigned char Chip =
        m_pImportedDoc->GetExpansionChip() | m_pDocument->GetExpansionChip();
